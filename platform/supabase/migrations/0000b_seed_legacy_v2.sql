-- 0000b_seed_legacy_v2.sql
-- Supplements 0000_seed_legacy_applied.sql (which only seeded 0001_brahma_baseline.sql).
-- Seeds the remaining 86 migration files that were applied to prod before migrate.ts
-- tracking started (at 118_build_events.sql, 2026-06-04).
--
-- Files are skipped by migrate.ts once their filename appears in _migrations_applied.
-- ON CONFLICT DO NOTHING is safe — any already-tracked filename is a no-op.

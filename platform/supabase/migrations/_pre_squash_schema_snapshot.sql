...
\restrict EPmua133s5L3PBhQnzV5i9MXzwWPHPLlZtMhAX3nquQfJOLHH9bS15qQQyuxYRp
...
CREATE TABLE public.bodha_domain_links (
...
CREATE TABLE public.bodha_graph (
...
CREATE TABLE public.bodha_graph_edges (
    edge_id text NOT NULL,
    chart_id uuid NOT NULL,
    from_signal_id text NOT NULL,
    to_signal_id text NOT NULL,
...
CREATE TABLE public.bodha_graph_staging (
    edge_id text NOT NULL,
    chart_id uuid,
    from_signal_id text NOT NULL,
    to_signal_id text NOT NULL,
...
CREATE TABLE public.bodha_remediation (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
...
CREATE TABLE public.bodha_remediation_staging (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    contradiction_hub_id text NOT NULL,
    hub_label text,
...
CREATE TABLE public.bodha_resonance (
    chart_id uuid NOT NULL,
...
CREATE TABLE public.bodha_signals (
    signal_id text NOT NULL,
    chart_id uuid NOT NULL,
...
CREATE TABLE public.build_engine_versions (
...
CREATE TABLE public.builds_staging (
    build_id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    triggered_by_uid text NOT NULL,
    triggered_by_role text NOT NULL,
...
CREATE TABLE public.capability_asset_tool_bindings (
...
CREATE TABLE public.capability_tool_registry (
    tool_name text NOT NULL,
...
CREATE TABLE public.context_assembly_item_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    query_id uuid NOT NULL,
...
CREATE TABLE public.conversation_folder_members (
    conversation_id uuid NOT NULL,
...
CREATE TABLE public.conversation_message_embeddings (
...
CREATE TABLE public.event_chart_state_index (
...
CREATE TABLE public.ganita_dashas (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    build_id text NOT NULL,
...
CREATE TABLE public.ganita_positions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    build_id text NOT NULL,
    ayanamsha_id text NOT NULL,
...
CREATE TABLE public.kala_convergence (
...
CREATE SEQUENCE public.kala_convergence_convergence_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.kala_convergence_staging (
...
CREATE TABLE public.kala_obstruction (
    id bigint NOT NULL,
    chart_id uuid,
    date date NOT NULL,
...
CREATE SEQUENCE public.kala_obstruction_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.kala_timeline (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    date date NOT NULL,
...
CREATE TABLE public.life_events_staging (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    event_id text NOT NULL,
    event_date date NOT NULL,
    category text NOT NULL,
...
CREATE TABLE public.llm_budget_rules (
...
CREATE TABLE public.llm_cost_reconciliation (
...
CREATE TABLE public.llm_pricing_versions (
...
CREATE TABLE public.llm_provider_cost_reports (
...
CREATE TABLE public.llm_stack_config (
    scope text NOT NULL,
...
CREATE TABLE public.mcp_alerts_config (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.mcp_disagreements (
...
CREATE TABLE public.mcp_prediction_outcomes (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.mcp_predictions (
    prediction_id text NOT NULL,
    logged_at timestamp with time zone DEFAULT now() NOT NULL,
    horizon text,
    domain text,
...
CREATE TABLE public.mimamsa_calibration (
    id bigint NOT NULL,
    chart_id uuid NOT NULL,
...
CREATE SEQUENCE public.mimamsa_calibration_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.mimamsa_export_log (
...
CREATE TABLE public.mimamsa_export_log_staging (
    export_id uuid DEFAULT gen_random_uuid() NOT NULL,
    export_at timestamp with time zone DEFAULT now() NOT NULL,
    table_name text NOT NULL,
    row_count integer DEFAULT 0 NOT NULL,
...
CREATE TABLE public.mimamsa_multipliers (
    technique text NOT NULL,
    ayanamsha_id text NOT NULL,
...
CREATE TABLE public.mimamsa_qa_eval (
...
CREATE TABLE public.pending_streams (
...
CREATE TABLE public.performance_judge_verdict (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE public.performance_queries (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    audit_event_id uuid,
    eval_run_id uuid,
...
CREATE SEQUENCE public.phala_anchors_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.phala_mitigation (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    anchor_id text NOT NULL,
    theme text,
...
CREATE TABLE public.phala_mitigation_staging (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    chart_id uuid NOT NULL,
    anchor_id text NOT NULL,
    theme text,
...
CREATE TABLE public.phala_muhurta (
...
CREATE SEQUENCE public.phala_rectification_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.plan_alternatives_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    query_id uuid NOT NULL,
...
CREATE TABLE public.query_plan_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    query_id uuid NOT NULL,
    conversation_id uuid,
    chart_id text,
...
CREATE TABLE public.query_trace_steps (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    query_id uuid NOT NULL,
    conversation_id uuid,
...
CREATE TABLE public.synthesis_quality_scorecard (
    query_id uuid NOT NULL,
...
CREATE UNIQUE INDEX access_requests_pending_email_unique ON public.access_requests USING btree (lower(email)) WHERE (status = 'pending'::text);
...
CREATE INDEX access_requests_status_requested_at_idx ON public.access_requests USING btree (status, requested_at DESC);
...
CREATE INDEX bev_version_idx ON public.build_engine_versions USING btree (version_id);
...
CREATE INDEX bodha_domain_links_chart_id_idx ON public.bodha_domain_links USING btree (chart_id);
...
CREATE INDEX bodha_domain_links_domains_idx ON public.bodha_domain_links USING btree (chart_id, from_domain, to_domain);
...
CREATE INDEX bodha_graph_edges_chart_id_idx ON public.bodha_graph_edges USING btree (chart_id);
...
CREATE INDEX bodha_graph_edges_from_signal_idx ON public.bodha_graph_edges USING btree (chart_id, from_signal_id);
...
CREATE INDEX bodha_graph_edges_to_signal_idx ON public.bodha_graph_edges USING btree (chart_id, to_signal_id);
...
CREATE INDEX bodha_graph_staging_chart_id_edge_type_idx ON public.bodha_graph_staging USING btree (chart_id, edge_type);
...
CREATE INDEX bodha_graph_staging_chart_id_from_signal_id_idx ON public.bodha_graph_staging USING btree (chart_id, from_signal_id);
...
CREATE INDEX bodha_graph_staging_chart_id_idx ON public.bodha_graph_staging USING btree (chart_id);
...
CREATE INDEX bodha_graph_staging_chart_id_to_signal_id_idx ON public.bodha_graph_staging USING btree (chart_id, to_signal_id);
...
CREATE INDEX bodha_remediation_staging_chart_id_domain_idx ON public.bodha_remediation_staging USING btree (chart_id, domain);
...
CREATE INDEX bodha_remediation_staging_chart_id_idx ON public.bodha_remediation_staging USING btree (chart_id);
...
CREATE INDEX bodha_remediation_staging_contradicting_signal_ids_idx ON public.bodha_remediation_staging USING gin (contradicting_signal_ids);
...
CREATE INDEX bodha_remediation_staging_contradiction_hub_id_idx ON public.bodha_remediation_staging USING btree (contradiction_hub_id);
...
CREATE INDEX bodha_remediation_staging_remedy_type_idx ON public.bodha_remediation_staging USING btree (remedy_type);
...
CREATE INDEX bodha_remediation_staging_source_l0_rule_id_idx ON public.bodha_remediation_staging USING btree (source_l0_rule_id);
...
CREATE INDEX bodha_resonance_chart_id_idx ON public.bodha_resonance USING btree (chart_id);
...
CREATE INDEX bodha_signals_chart_id_idx ON public.bodha_signals USING btree (chart_id);
...
CREATE INDEX bodha_signals_domain_chart_idx ON public.bodha_signals USING btree (chart_id, domain);
...
CREATE INDEX bodha_signals_salience_idx ON public.bodha_signals USING btree (chart_id, salience DESC NULLS LAST);
...
CREATE INDEX build_events_build_id_emitted_at_idx ON public.build_events USING btree (build_id, emitted_at DESC);
...
CREATE INDEX build_events_chart_id_emitted_at_idx ON public.build_events USING btree (chart_id, emitted_at DESC);
...
CREATE INDEX build_notif_build_idx ON public.build_notifications USING btree (build_id, created_at DESC);
...
CREATE INDEX build_notif_undelivered_idx ON public.build_notifications USING btree (build_id) WHERE (delivered_at IS NULL);
...
CREATE INDEX build_steps_build_idx ON public.build_steps USING btree (build_id, ayanamsha_id);
...
CREATE INDEX build_steps_category_idx ON public.build_steps USING btree (category);
...
CREATE INDEX build_steps_status_idx ON public.build_steps USING btree (status) WHERE (status = ANY (ARRAY['queued'::text, 'running'::text]));
...
CREATE INDEX builds_chart_idx ON public.builds USING btree (chart_id, queued_at DESC);
...
CREATE INDEX builds_staging_chart_id_queued_at_idx ON public.builds_staging USING btree (chart_id, queued_at DESC);
...
CREATE INDEX builds_staging_status_idx ON public.builds_staging USING btree (status) WHERE (status = ANY (ARRAY['queued'::text, 'running'::text, 'cancelling'::text]));
...
CREATE INDEX builds_status_idx ON public.builds USING btree (status) WHERE (status = ANY (ARRAY['queued'::text, 'running'::text, 'cancelling'::text]));
...
CREATE INDEX conversation_messages_conv_time ON public.conversation_messages USING btree (conversation_id, created_at);
...
CREATE INDEX conversation_messages_parent ON public.conversation_messages USING btree (parent_message_id) WHERE (parent_message_id IS NOT NULL);
...
CREATE INDEX conversations_active_ayanamshas_gin ON public.conversations USING gin (active_ayanamshas);
...
CREATE INDEX ecsi_event_id_idx ON public.event_chart_state_index USING btree (event_id);
...
CREATE INDEX eval_runs_created_idx ON public.eval_runs USING btree (created_at DESC);
...
CREATE INDEX idx_audit_events_query_class ON public.audit_events USING btree (query_class);
...
CREATE INDEX idx_audit_events_query_id ON public.audit_events USING btree (query_id);
...
CREATE INDEX idx_audit_log_query_id ON public.audit_log USING btree (query_id);
...
CREATE INDEX idx_bodha_graph_chart_id ON public.bodha_graph USING btree (chart_id);
...
CREATE INDEX idx_bodha_graph_edge_type ON public.bodha_graph USING btree (chart_id, edge_type);
...
CREATE INDEX idx_bodha_graph_from_signal ON public.bodha_graph USING btree (chart_id, from_signal_id);
...
CREATE INDEX idx_bodha_graph_to_signal ON public.bodha_graph USING btree (chart_id, to_signal_id);
...
CREATE INDEX idx_bodha_remediation_chart ON public.bodha_remediation USING btree (chart_id);
...
CREATE INDEX idx_bodha_remediation_domain ON public.bodha_remediation USING btree (chart_id, domain);
...
CREATE INDEX idx_bodha_remediation_hub_id ON public.bodha_remediation USING btree (contradiction_hub_id);
...
CREATE INDEX idx_bodha_remediation_l0_rule ON public.bodha_remediation USING btree (source_l0_rule_id);
...
CREATE INDEX idx_bodha_remediation_remedy_type ON public.bodha_remediation USING btree (remedy_type);
...
CREATE INDEX idx_bodha_remediation_signal_ids ON public.bodha_remediation USING gin (contradicting_signal_ids);
...
CREATE INDEX idx_bodha_resonance_chart_ayan ON public.bodha_resonance USING btree (chart_id, ayanamsha_id);
...
CREATE INDEX idx_bodha_resonance_constituents_gin ON public.bodha_resonance USING gin (constituents);
...
CREATE INDEX idx_bodha_resonance_domains_gin ON public.bodha_resonance USING gin (domains_primary);
...
CREATE INDEX idx_bodha_resonance_element_type ON public.bodha_resonance USING btree (chart_id, ayanamsha_id, element_type);
...
CREATE INDEX idx_bodha_resonance_strength ON public.bodha_resonance USING btree (chart_id, ayanamsha_id, strength DESC);
...
CREATE INDEX idx_bodha_signal_embeddings_embedded_at ON public.bodha_signal_embeddings USING btree (embedded_at DESC);
...
CREATE INDEX idx_bodha_signal_embeddings_hnsw ON public.bodha_signal_embeddings USING hnsw (embedding public.vector_cosine_ops) WITH (m='16', ef_construction='64');
...
CREATE INDEX idx_bodha_signal_embeddings_model ON public.bodha_signal_embeddings USING btree (model_name);
...
CREATE INDEX idx_capability_asset_tool_bindings_asset ON public.capability_asset_tool_bindings USING btree (asset_canonical_id);
...
CREATE INDEX idx_capability_asset_tool_bindings_tool ON public.capability_asset_tool_bindings USING btree (tool_name);
...
CREATE INDEX idx_chart_grants_chart_id ON public.chart_grants USING btree (chart_id);
...
CREATE INDEX idx_chart_grants_principal_id ON public.chart_grants USING btree (principal_id);
...
CREATE INDEX idx_cme_embedding ON public.conversation_message_embeddings USING ivfflat (embedding public.vector_cosine_ops) WITH (lists='100');
...
CREATE INDEX idx_context_assembly_item_log_query ON public.context_assembly_item_log USING btree (query_id, item_rank);
...
CREATE INDEX idx_conv_messages_body_trgm ON public.conversation_messages USING gin (((parts_json)::text) public.gin_trgm_ops);
...
CREATE INDEX idx_conversation_branches_conversation_id ON public.conversation_branches USING btree (conversation_id, created_at DESC);
...
CREATE INDEX idx_conversation_folders_user ON public.conversation_folders USING btree (user_id, created_at DESC);
...
CREATE INDEX idx_conversation_shares_conversation_id ON public.conversation_shares USING btree (conversation_id);
...
CREATE INDEX idx_conversations_chart_id ON public.conversations USING btree (chart_id);
...
CREATE INDEX idx_conversations_pinned ON public.conversations USING btree (user_id, pinned DESC, updated_at DESC);
...
CREATE INDEX idx_conversations_title_trgm ON public.conversations USING gin (title public.gin_trgm_ops) WHERE (title IS NOT NULL);
...
CREATE INDEX idx_folder_members_folder ON public.conversation_folder_members USING btree (folder_id, added_at DESC);
...
CREATE INDEX idx_ganita_dashas_chart ON public.ganita_dashas USING btree (chart_id, dasha_system, level);
...
CREATE INDEX idx_ganita_dashas_date ON public.ganita_dashas USING btree (chart_id, start_date, end_date);
...
CREATE INDEX idx_ganita_positions_chart ON public.ganita_positions USING btree (chart_id, ayanamsha_id);
...
CREATE INDEX idx_kala_convergence_chart_id ON public.kala_convergence USING btree (chart_id);
...
CREATE INDEX idx_kala_convergence_score ON public.kala_convergence USING btree (chart_id, convergence_score DESC);
...
CREATE INDEX idx_kala_convergence_window ON public.kala_convergence USING btree (chart_id, window_start, window_end);
...
CREATE INDEX idx_kala_obstruction_chart_id ON public.kala_obstruction USING btree (chart_id);
...
CREATE INDEX idx_kala_obstruction_date ON public.kala_obstruction USING btree (chart_id, date);
...
CREATE INDEX idx_kala_obstruction_factors ON public.kala_obstruction USING gin (factors);
...
CREATE INDEX idx_kala_obstruction_severity ON public.kala_obstruction USING btree (chart_id, severity DESC);
...
CREATE INDEX idx_kala_obstruction_type ON public.kala_obstruction USING btree (chart_id, obstruction_type);
...
CREATE INDEX idx_kala_timeline_chart_date ON public.kala_timeline USING btree (chart_id, date);
...
CREATE INDEX idx_kala_timeline_mahadasha ON public.kala_timeline USING btree (chart_id, active_mahadasha, date);
...
CREATE INDEX idx_kala_timeline_signal_activations ON public.kala_timeline USING gin (signal_activations);
...
CREATE INDEX idx_llm_call_log_call_stage ON public.llm_call_log USING btree (call_stage);
...
CREATE INDEX idx_llm_call_log_created_at ON public.llm_call_log USING btree (created_at DESC);
...
CREATE INDEX idx_llm_call_log_model_id ON public.llm_call_log USING btree (model_id);
...
CREATE INDEX idx_llm_call_log_query_id ON public.llm_call_log USING btree (query_id);
...
CREATE INDEX idx_llm_cost_reconciliation_date_provider ON public.llm_cost_reconciliation USING btree (reconciliation_date, provider);
...
CREATE INDEX idx_llm_pricing_versions_lookup ON public.llm_pricing_versions USING btree (provider, model, effective_from DESC);
...
CREATE INDEX idx_llm_provider_cost_reports_provider_bucket ON public.llm_provider_cost_reports USING btree (provider, time_bucket_start);
...
CREATE INDEX idx_llm_usage_events_conversation_id ON public.llm_usage_events USING btree (conversation_id);
...
CREATE INDEX idx_llm_usage_events_pipeline_stage ON public.llm_usage_events USING btree (pipeline_stage);
...
CREATE INDEX idx_llm_usage_events_provider_model ON public.llm_usage_events USING btree (provider, model);
...
CREATE INDEX idx_llm_usage_events_started_at ON public.llm_usage_events USING btree (started_at);
...
CREATE INDEX idx_llm_usage_events_user_id ON public.llm_usage_events USING btree (user_id);
...
CREATE INDEX idx_mimamsa_calibration_ayanamsha ON public.mimamsa_calibration USING btree (ayanamsha_id, computed_at DESC);
...
CREATE INDEX idx_mimamsa_calibration_chart_technique ON public.mimamsa_calibration USING btree (chart_id, technique);
...
CREATE INDEX idx_mimamsa_calibration_computed_at ON public.mimamsa_calibration USING btree (computed_at DESC);
...
CREATE INDEX idx_mimamsa_export_log_export_at ON public.mimamsa_export_log USING btree (export_at DESC);
...
CREATE INDEX idx_mimamsa_export_log_table_name ON public.mimamsa_export_log USING btree (table_name);
...
CREATE INDEX idx_mimamsa_multipliers_lookup ON public.mimamsa_multipliers USING btree (technique, ayanamsha_id);
...
CREATE INDEX idx_mimamsa_predictions_chart_id ON public.mimamsa_predictions USING btree (chart_id);
...
CREATE INDEX idx_mimamsa_predictions_domain ON public.mimamsa_predictions USING btree (chart_id, domain);
...
CREATE INDEX idx_mimamsa_predictions_horizon ON public.mimamsa_predictions USING btree (chart_id, horizon_date);
...
CREATE INDEX idx_mimamsa_predictions_open ON public.mimamsa_predictions USING btree (chart_id) WHERE (outcome_observed IS NULL);
...
CREATE INDEX idx_mimamsa_predictions_predicted_at ON public.mimamsa_predictions USING btree (predicted_at);
...
CREATE INDEX idx_mimamsa_qa_eval_b11 ON public.mimamsa_qa_eval USING btree (b11_compliance);
...
CREATE INDEX idx_mimamsa_qa_eval_created ON public.mimamsa_qa_eval USING btree (created_at DESC);
...
CREATE INDEX idx_mimamsa_qa_eval_grounding ON public.mimamsa_qa_eval USING btree (grounding_score DESC NULLS LAST);
...
CREATE INDEX idx_mimamsa_qa_eval_layer_coverage ON public.mimamsa_qa_eval USING btree (layer_coverage DESC NULLS LAST);
...
CREATE INDEX idx_pending_streams_expires_at ON public.pending_streams USING btree (expires_at);
...
CREATE UNIQUE INDEX idx_personas_user_default ON public.personas USING btree (user_id) WHERE (is_default = true);
...
CREATE INDEX idx_phala_anchors_chart_id ON public.phala_anchors USING btree (chart_id);
...
CREATE INDEX idx_phala_anchors_confidence ON public.phala_anchors USING btree (chart_id, confidence DESC);
...
CREATE INDEX idx_phala_anchors_contributing_signals ON public.phala_anchors USING gin (contributing_signals);
...
CREATE INDEX idx_phala_anchors_state ON public.phala_anchors USING btree (prediction_state);
...
CREATE INDEX idx_phala_anchors_window ON public.phala_anchors USING btree (chart_id, window_start, window_end);
...
CREATE INDEX idx_phala_mitigation_anchor ON public.phala_mitigation USING btree (chart_id, anchor_id);
...
CREATE INDEX idx_phala_mitigation_chart ON public.phala_mitigation USING btree (chart_id);
...
CREATE INDEX idx_phala_mitigation_l0_rule ON public.phala_mitigation USING btree (source_l0_rule_id);
...
CREATE INDEX idx_phala_mitigation_l2_hub ON public.phala_mitigation USING btree (l2_remediation_hub_id) WHERE (l2_remediation_hub_id IS NOT NULL);
...
CREATE INDEX idx_phala_mitigation_type ON public.phala_mitigation USING btree (mitigation_type);
...
CREATE INDEX idx_phala_muhurta_chart_action ON public.phala_muhurta USING btree (chart_id, action_type);
...
CREATE INDEX idx_phala_muhurta_chart_action_score ON public.phala_muhurta USING btree (chart_id, action_type, auspiciousness_score DESC);
...
CREATE INDEX idx_phala_muhurta_factors_gin ON public.phala_muhurta USING gin (factors);
...
CREATE INDEX idx_phala_muhurta_score ON public.phala_muhurta USING btree (auspiciousness_score DESC);
...
CREATE UNIQUE INDEX idx_phala_muhurta_unique_window ON public.phala_muhurta USING btree (chart_id, action_type, window_start);
...
CREATE INDEX idx_phala_muhurta_window_start ON public.phala_muhurta USING btree (window_start);
...
CREATE INDEX idx_plan_alternatives_log_query ON public.plan_alternatives_log USING btree (query_id);
...
CREATE INDEX idx_project_conv_conv ON public.project_conversations USING btree (conversation_id);
...
CREATE INDEX idx_project_conv_project ON public.project_conversations USING btree (project_id);
...
CREATE INDEX idx_qts_user_id ON public.query_trace_steps USING btree (user_id) WHERE (user_id IS NOT NULL);
...
CREATE INDEX idx_query_plan_log_conversation_id ON public.query_plan_log USING btree (conversation_id);
...
CREATE INDEX idx_query_plan_log_created_at ON public.query_plan_log USING btree (created_at DESC);
...
CREATE INDEX idx_query_plan_log_fallback ON public.query_plan_log USING btree (fallback_used) WHERE (fallback_used = true);
...
CREATE INDEX idx_query_plan_log_query_id ON public.query_plan_log USING btree (query_id);
...
CREATE INDEX idx_query_trace_steps_created_at ON public.query_trace_steps USING btree (created_at DESC);
...
CREATE INDEX idx_query_trace_steps_mcp_tool ON public.query_trace_steps USING btree (mcp_tool) WHERE (mcp_tool IS NOT NULL);
...
CREATE INDEX idx_query_trace_steps_query_id ON public.query_trace_steps USING btree (query_id, step_seq);
...
CREATE INDEX idx_tool_execution_log_query_id ON public.tool_execution_log USING btree (query_id) WHERE (query_id IS NOT NULL);
...
CREATE INDEX idx_tool_execution_log_status ON public.tool_execution_log USING btree (status);
...
CREATE INDEX idx_tool_execution_log_tool_name ON public.tool_execution_log USING btree (tool_name);
...
CREATE INDEX idx_tool_execution_log_zero_rows ON public.tool_execution_log USING btree (tool_name) WHERE (status = 'zero_rows'::text);
...
CREATE INDEX judge_verdict_pq_idx ON public.performance_judge_verdict USING btree (performance_query_id);
...
CREATE INDEX judge_verdict_run_idx ON public.performance_judge_verdict USING btree (judge_run_id);
...
CREATE INDEX kala_convergence_staging_chart_id_convergence_score_idx ON public.kala_convergence_staging USING btree (chart_id, convergence_score DESC);
...
CREATE INDEX kala_convergence_staging_chart_id_idx ON public.kala_convergence_staging USING btree (chart_id);
...
CREATE INDEX kala_convergence_staging_chart_id_window_start_window_end_idx ON public.kala_convergence_staging USING btree (chart_id, window_start, window_end);
...
CREATE INDEX life_events_staging_category_event_date_idx ON public.life_events_staging USING btree (category, event_date);
...
CREATE INDEX life_events_staging_event_date_idx ON public.life_events_staging USING btree (event_date);
...
CREATE INDEX mcp_alerts_config_enabled ON public.mcp_alerts_config USING btree (metric_name, scope, enabled) WHERE (enabled = true);
...
CREATE INDEX mcp_api_keys_auth_lookup_idx ON public.mcp_api_keys USING btree (key_id, revoked_at);
...
CREATE INDEX mcp_api_keys_user_uid_idx ON public.mcp_api_keys USING btree (user_uid);
...
CREATE INDEX mcp_disagreements_key_id_idx ON public.mcp_disagreements USING btree (key_id, logged_at DESC);
...
CREATE INDEX mcp_disagreements_status_idx ON public.mcp_disagreements USING btree (status, logged_at DESC);
...
CREATE INDEX mcp_prediction_outcomes_prediction_id ON public.mcp_prediction_outcomes USING btree (prediction_id);
...
CREATE INDEX mcp_predictions_chart_idx ON public.mcp_predictions USING btree (chart_id, predicted_at_iso DESC) WHERE (chart_id IS NOT NULL);
...
CREATE INDEX mcp_predictions_key_id_idx ON public.mcp_predictions USING btree (key_id, logged_at DESC) WHERE (key_id IS NOT NULL);
...
CREATE UNIQUE INDEX mcp_predictions_stamp_uq ON public.mcp_predictions USING btree (chart_id, ayanamsha_id, query_hash, salience_formula_version) WHERE ((chart_id IS NOT NULL) AND (ayanamsha_id IS NOT NULL) AND (query_hash IS NOT NULL) AND (salience_formula_version IS NOT NULL));
...
CREATE INDEX mcp_predictions_trace_id_idx ON public.mcp_predictions USING btree (trace_id) WHERE (trace_id IS NOT NULL);
...
CREATE INDEX mimamsa_export_log_staging_export_at_idx ON public.mimamsa_export_log_staging USING btree (export_at DESC);
...
CREATE INDEX mimamsa_export_log_staging_table_name_idx ON public.mimamsa_export_log_staging USING btree (table_name);
...
CREATE UNIQUE INDEX mv_session_summary_pk ON public.mv_session_summary USING btree (mcp_key_id, session_hour);
...
CREATE UNIQUE INDEX mv_tool_metrics_24h_pk ON public.mv_tool_metrics_24h USING btree (mcp_tool_name, source, audience_tier);
...
CREATE INDEX nv_build_idx ON public.notification_views USING btree (build_id);
...
CREATE INDEX nv_user_idx ON public.notification_views USING btree (user_id, viewed_at DESC);
...
CREATE INDEX perf_queries_audit_event_idx ON public.performance_queries USING btree (audit_event_id);
...
CREATE INDEX perf_queries_class_created_idx ON public.performance_queries USING btree (query_class, created_at DESC);
...
CREATE INDEX perf_queries_created_at_idx ON public.performance_queries USING btree (created_at DESC);
...
CREATE INDEX perf_queries_eval_run_idx ON public.performance_queries USING btree (eval_run_id);
...
CREATE INDEX perf_queries_source_created_idx ON public.performance_queries USING btree (source, created_at DESC);
...
CREATE INDEX phala_mitigation_staging_chart_id_anchor_id_idx ON public.phala_mitigation_staging USING btree (chart_id, anchor_id);
...
CREATE INDEX phala_mitigation_staging_chart_id_idx ON public.phala_mitigation_staging USING btree (chart_id);
...
CREATE INDEX phala_mitigation_staging_l2_remediation_hub_id_idx ON public.phala_mitigation_staging USING btree (l2_remediation_hub_id) WHERE (l2_remediation_hub_id IS NOT NULL);
...
CREATE INDEX phala_mitigation_staging_mitigation_type_idx ON public.phala_mitigation_staging USING btree (mitigation_type);
...
CREATE INDEX phala_mitigation_staging_source_l0_rule_id_idx ON public.phala_mitigation_staging USING btree (source_l0_rule_id);
...
CREATE INDEX phala_rectification_chart_id_idx ON public.phala_rectification USING btree (chart_id);
...
CREATE INDEX phala_rectification_chart_score_idx ON public.phala_rectification USING btree (chart_id, alignment_score DESC);
...
CREATE UNIQUE INDEX runtime_config_key_chart_uq ON public.runtime_config USING btree (key, COALESCE(chart_id, '__global__'::text));
...
CREATE INDEX tool_execution_log_audit_queue ON public.tool_execution_log USING btree (created_at, audit_flagged) WHERE (audit_flagged IS DISTINCT FROM true);
...
CREATE INDEX tool_execution_log_mcp_tool_idx ON public.tool_execution_log USING btree (mcp_tool_name, created_at) WHERE (mcp_tool_name IS NOT NULL);
...
CREATE INDEX tool_execution_log_source_idx ON public.tool_execution_log USING btree (source, created_at) WHERE (source IS NOT NULL);
...
CREATE INDEX tool_execution_log_trace_id ON public.tool_execution_log USING btree (trace_id) WHERE (trace_id IS NOT NULL);
...
CREATE UNIQUE INDEX uq_llm_cost_reconciliation_natural_key ON public.llm_cost_reconciliation USING btree (reconciliation_date, provider, COALESCE(model, ''::text));
...
CREATE UNIQUE INDEX uq_llm_pricing_versions_natural_key ON public.llm_pricing_versions USING btree (provider, model, token_class, effective_from);

-- Migration 911: bg_sarvatobhadra_grid asset_freshness sentinel row
-- =============================================================================
...
BEGIN
    ...
    IF _cnt = 0 THEN
        RAISE EXCEPTION
            'Migration 911 preflight failed: no asset_throughput(state=''lit'') row '
...
BEGIN
    ...
    IF _cnt != 0 THEN
        RAISE EXCEPTION
            'Migration 911 preflight failed: bg_sarvatobhadra_grid has % rows '
...
BEGIN
    ...
    IF _state IS NULL THEN
        RAISE EXCEPTION
            'Migration 911 post-condition failed: no asset_freshness row found '
            'for bg_sarvatobhadra_grid after INSERT. This should not happen.';
    END IF;

    IF _state != 'fresh' THEN
        RAISE EXCEPTION
            'Migration 911 post-condition failed: asset_freshness.freshness_state=% '

...
run_scan() {
  ...
  for pat in "${PATTERNS[@]}"; do
    ...
    while IFS= read -r line; do
      ...
      local body="${line#*:*:}"
      if [[ "${body}" =~ (process\.env|os\.environ|getenv|secrets\.|\$\{[A-Z_]+\}|:latest($|[^a-zA-Z0-9])) ]]; then
        # Cloud-Run-style "DB_PASSWORD=amjis-db-password:latest" — tolerated.
        # Likewise env reads. The :latest match anchors on a non-alphanumeric
        # boundary so it works in flowing prose ("...:latest`" or ":latest ").
        continue
      fi
  ...
  for f in "${fail_dir}"/*; do
    ...
    for pat in "${PATTERNS[@]}"; do
      if grep -EI -q "${pat}" "${f}" 2>/dev/null; then
        ...
        if grep -EI "${pat}" "${f}" 2>/dev/null | grep -Ev '(process\.env|os\.environ|getenv|secrets\.|\$\{[A-Z_]+\}|:latest$|:latest[[:space:]])' >/dev/null 2>&1; then
          hits=$((hits + 1))
          break
        fi
      fi

"""
rebuild_d1_5b_precascade.py — D-1.5b pre-lane cascade rebuild

D-1.5b's Binder found the deployed estate carrying D-1.5a's carried-forward
ka_vighnakara FK defect (23 error + 19 stale assets on 482012f1, everything
downstream of ka_sangam/bo_laksana). D-1.5b's brief mandates a full L1->L5
rebuild for its own Gate B — that cannot go green while this cascade sits
broken. Clears the full non-lit closure (topo-sorted via
asset_registry.depends_on, same Kahn's-algorithm approach D-1.5a's cascade
rebuild used) before any D-1.5b lane spawns, so the wave opens on a clean
estate rather than inheriting a stale one.
...
Usage (Cloud SQL Auth Proxy must be running on port 5433):
  ...
  python -m scripts.rebuild_d1_5b_precascade
"""
...
TRIGGERED_BY = "rebuild-script-d1-5b-precascade"

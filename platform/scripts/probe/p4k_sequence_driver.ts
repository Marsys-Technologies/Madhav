...
// `--service-url`/`--self-test` off `process.argv` at call time, so an importer whose OWN
// argv happened to carry `--service-url` would have this module spawn `ask.ts` six times
// against that URL — real network egress and real authenticated turns against whatever
// service was named — as a side effect of being imported. Its safe default is DO NOT RUN.

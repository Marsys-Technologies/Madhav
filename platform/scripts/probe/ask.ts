 ...
 * wrongly-true answer runs the probe as an import side effect: it acquires secrets from
 * Secret Manager, authenticates, and POSTs a question at the deployed service.
 *
...
//
// THE HAZARD IS RUNNING. `main()` shells out to `gcloud secrets versions access` and
// `firebase apps:sdkconfig` to obtain credentials when the matching env vars are unset,
// authenticates as `PROBE_UID`, POSTs a question to the deployed service (default
// `DEFAULT_SERVICE_URL`, a production Cloud Run URL), runs `UPDATE conversations SET title`
// against the database, and writes a transcript into `scripts/probe/out/`. Importing this file
// for any reason performed all of that, and its `process.exit` paths killed the importer.
//
// This guard PREVENTS a credential acquisition. It performs, reads, rotates, relocates and logs
// none — CHARTER §2 P4 is not engaged in the direction it protects (ruling D-74 part 2). No
// logging was added anywhere near the secret path, and this file was never executed to verify
// the repair.
//

...
describe('loadRenumberDisclosures', () => {
  ...
  it('the checked-in allowlist parses; three known disclosures: 484→543 bg_muhurta_lattice + 485→544 bg_parihara_rules (2026-08-07) + 806→813 mi_jivanaghatana output_digest_spec (2026-09-06)', () => {
    ...
    //   DB, renumbered 809->811->813 after main independently claimed each in turn, after colliding with an unrelated L1 806 migration that merged to
    ...
    const entry813 = map.get('813_nirmana_l5_mi_jivanaghatana_output_digest_spec.sql')
    expect(entry813).toBeDefined()
    expect(entry813!.applied_filename).toBe('806_nirmana_l5_mi_jivanaghatana_output_digest_spec.sql')
    expect(entry813!.sql_identity).toBe('2490ae2d69d3a7ed465a708e2d9564602e3fc5fe0ac6ba8690af7e1d0663f102')
    expect(entry813!.disposition).toBe('already-applied-under-old-name')
    expect(entry813!.disclosed_on).toBe('2026-09-06')
  })
})

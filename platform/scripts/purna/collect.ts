...
function asCase(value: typeof BEYOND_ACARYA_ACCEPTANCE_CASES[number]): AcceptanceCase { return { id: value.case_id, question: value.question, scope_tuple: value.scope_tuple as Record<string, unknown>, requiredDimensions: [], expected: 'supported_complete' } }
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  const suite = args.get('--suite'); const configPath = args.get('--config'); const artifactDir = args.get('--artifact-dir'); const caseId = args.get('--case'); if (suite !== 'beyond_acarya' || !configPath || !artifactDir || args.size < 3 || args.size > 4) throw new Error('PURNA_COLLECTION_CLI_REQUIRED_ARGUMENT_MISSING')
  const selected = caseId ? BEYOND_ACARYA_ACCEPTANCE_CASES.filter((item) => item.case_id === caseId) : BEYOND_ACARYA_ACCEPTANCE_CASES

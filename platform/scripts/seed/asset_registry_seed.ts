...
export const ASSETS: AssetDef[] = [
  ...
  },
  {
    ...
    expected_volume_formula: null,
    expected_volume_inputs: null,
    volume_explanation: 'Live-verified 2026-07-30 against a real throwaway Postgres (pg17 + pgvector): 1,826 agnivasa + 1,481 combination_yoga + 33,650 kalam + 54,780 ghati_muhurta = 91,737, over horizon 2026-07-30 -> 2031-07-30 (today+5y at verification time). A later build reads >= this count as the rolling forward horizon advances (never less; old rows are never deleted).',
    depends_on: [],
    scope: 'global', is_active: true, estimated_seconds: null,
    asset_kind: 'data',
  },
  {
    ...
    english_description: "Global chart-independent parihāra (doṣa-cancellation) graph, per-activity muhūrta factor-quality rules, and the Muhūrta Factor Census + corpus-gap register. Reuses/joins brahma_dosha_catalog, brahma_remedy_corpus, and panchang_engine.shastra_tables.EVENT_TABLES — never re-ingests corpus content. Corpus gaps (e.g. mṛtyu-yoga, dagdha-yoga day-quality tables, Śiva-vāsa) are honestly recorded as not_in_corpus, never fabricated. ṢAḌ-DARŚANA campaign item 36-substrate/41.",
    storage_type: 'postgres_table',

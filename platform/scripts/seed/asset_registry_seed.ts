...
export const ASSET_REGISTRY_UPSERT_SQL = `INSERT INTO asset_registry (
  ...
  -- those formulas are chart-partitioned SQL predicates that `parseFormula`
  -- above cannot parse: its grammar admits only ACTUAL(), FILE_COUNT() and
  -- arithmetic, and throws on lowercase identifiers, `>=`, commas or `$chart`.
  -- Mirroring the campaign's values back into the TS literals would therefore
  -- require a SECOND, incompatible grammar in a column that already executes
  -- one. Governing the columns from migrations instead keeps `parseFormula`
  -- with exactly one grammar over its own literals, and keeps six parallel
  -- sessions out of this shared file.
  --
  -- Note `estimated_seconds` is already treated this way by omission: it is in
  -- the INSERT column list but not in this DO UPDATE SET, so re-measured
  -- durations already survive a re-seed. This generalises that treatment from
  -- one measured field to the rest.

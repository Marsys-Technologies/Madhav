...
WHERE cf.chart_id = $1 AND fco.owning_asset_id = 'ga_structural'`,
  ...
  },
  {
    ...
    // Reads the complete shared bodha_msr_signals relation, so it follows all
    // six INSERT producers. Downstream rerank and Saṅgati then consume the
    // complete graph/contradiction result rather than an early partial pass.
    layer: 'bodha', sort_order: 27,
    catalog_status: 'CURRENT',
  ...
  {
    asset_id: 'bo_bimba',
    // The node builder scans the complete shared bodha_msr_signals relation.
    // It therefore follows every INSERT producer before Kāraṇajāla consumes
    // its node set.
    layer: 'bodha', sort_order: 24,
    catalog_status: 'CURRENT',
    sanskrit_name: 'Bimba',
  ...
  {
    asset_id: 'bo_sangati',
    // Runs after every bodha_msr_signals producer and the UPDATE-only rerank
    // pass because this writer reads the shared table without a class filter.
    // sort_order 29 follows Bimba (24), bo_grounding (25), Saṃskāra (26),
    // Kāraṇajāla (27), and bo_laksana_rerank (28).
    layer: 'bodha', sort_order: 29,
    catalog_status: 'CURRENT',
  ...
  },
  {
    ...
    // Migration 660 moved this to the first unused legacy slot; keep the seed
    // aligned so a re-seed cannot recreate the old collision at 20.

...
const ASSETS: AssetDef[] = [
  ...
  },
  {
    ...
    expected_volume_inputs: null,
    volume_explanation: 'Sum of 5 reference table row counts — established at seed; static thereafter',
    depends_on: [],
    scope: 'global', is_active: true, estimated_seconds: null,
  },
  {
  ...
  },
  {
    ...
    english_description: 'Embedded subset of classical_text_chunks for hybrid retrieval (vector + lexical + rerank)',
    storage_type: 'postgres_table',
    target_table: 'classical_text_chunks',
    count_sql: 'SELECT count(*) FROM classical_text_chunks WHERE embedding IS NOT NULL',
    size_sql: "SELECT pg_total_relation_size('classical_text_chunks')",
    target_floor: null,
    expected_volume_formula: null,
    expected_volume_inputs: null,
    volume_explanation: 'Subset of bg_texts where embedding column is populated — count grows with ingestion',
    depends_on: ['bg_texts'],
    scope: 'global', is_active: true, estimated_seconds: null,
  },
  {
  ...
  },
  {
    ...
    english_description: 'Cross-school agreement/divergence index (BPHS vs Jaimini vs Tajaka vs KP per topic) — DORMANT placeholder',
    storage_type: 'postgres_table',
    ...
    expected_volume_inputs: null,
    volume_explanation: 'Future build — registered as dormant. The classical_attributions table is a stub until cross-school divergence work begins.',
    depends_on: ['bg_rules'],
    scope: 'global', is_active: true, estimated_seconds: null,
  },

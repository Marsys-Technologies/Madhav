/**
 * F-S2: openai-tool-events.test.ts
 *
 * Verifies OpenAI adapter chat() emits tool_use_start, tool_use_input_delta,
    ...
    choices: [
      {
        delta: {
          tool_calls: [
            { index: 0, id: 'call_abc', function: { name: 'query_signals', arguments: '' } },
          ],
        },
        finish_reason: null,
      },
...
async function collectToolCallEvents() {
  ...
  for await (const e of adapter.chat({ messages: [{ role: 'user', content: 'test' }], model: 'gpt-4.1' })) {
...
describe('F-S2: OpenAIAdapter.chat() — tool_use event emission', () => {
  it('emits tool_use_start with correct id and name', async () => {
    ...
    expect(startEvent).toMatchObject({ type: 'tool_use_start', id: 'call_abc', name: 'query_signals' })
  })
...
describe('F-S2: OpenAIAdapter.chat() — regular stop (no tools) still works', () => {

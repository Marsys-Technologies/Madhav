/**
 * F-S2: deepseek-tool-events.test.ts
 *
 * Verifies DeepSeek adapter chat() emits tool_use_start, tool_use_input_delta,
 * and tool_use_complete ChatEvents when the model returns finish_reason=tool_calls.
 *
 * DeepSeek uses the openai SDK pointed at DeepSeek base URL (OpenAI-compat).
 * Stream format is identical to OpenAI, so the test fixture mirrors
 * openai-tool-events.test.ts exactly.
 *
...
async function collectToolCallEvents() {
  ...
  for await (const e of adapter.chat({ messages: [{ role: 'user', content: 'test' }], model: 'deepseek-chat' })) {
...
describe('F-S2: DeepSeekAdapter.chat() — tool_use event emission', () => {
  it('emits tool_use_start with correct id and name', async () => {
    ...
    expect(startEvent).toMatchObject({ type: 'tool_use_start', id: 'call_ds1', name: 'query_signals' })
  })
...
describe('F-S2: DeepSeekAdapter.chat() — regular stop (no tools) still works', () => {
  it('emits text_delta + message_stop(stop) for finish_reason=stop, no tool events', async () => {
    async function* normalStream() {
      ...
      yield {
        ...
        usage: { prompt_tokens: 5, completion_tokens: 2, prompt_cache_hit_tokens: 0 },
      }
    }
    ...
    const adapter = new DeepSeekAdapter()
    const events: Array<Record<string, unknown>> = []
    for await (const e of adapter.chat({ messages: [{ role: 'user', content: 'hi' }], model: 'deepseek-chat' })) {
      events.push(e as Record<string, unknown>)
    }

/**
 * e2e-loop-roundtrip.test.ts — R11F-B-S5 E2E integration test for NVIDIA NIM
 *
 * NVIDIA NIM — Case A: llama-3.1-70b-instruct and llama-3.3-70b-instruct support
 * OpenAI-compat function calling via finish_reason === 'tool_calls'.
 *
 * Proves that the NVIDIA adapter forwards tools to the NIM API and that the
 * agentic loop correctly processes tool calls, injects results, and terminates
 * on a final stop signal.
 *
 * Three test cases:
 ...
 * NIM uses the raw OpenAI SDK — parameters is plain JSON Schema, no jsonSchema() wrapper.
 *
 * Unsupported models (mistral-7b-instruct, mixtral-8x7b-instruct) are not tested here —
 * they would surface as API errors from the live NIM endpoint, not from the adapter logic.
 */
...
const BASE_REQUEST: ChatRequest = {
  ...
  messages: [
    ...
    {
      role: 'user',
      content: 'What does Jupiter indicate in the chart?',
    },
  ],
  tools: B11_TOOLS,
}
...
describe('R11F-B-S5: NVIDIA NIM round-trip (Case A) — tools forwarded + loop terminates on stop', () => {
  beforeEach(() => {
  ...
  it('forwards tools in OpenAI-compat function format when request.tools is provided', async () => {
    ...
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.007: Jupiter in 9H — dharmic wisdom')

    const events: Array<Record<string, unknown>> = []
    for await (const e of runAgenticLoop(adapter, BASE_REQUEST, toolExecutor, NVIDIA_LOOP_CONFIG)) {
      events.push(e as Record<string, unknown>)
    }

    // Verify tools were forwarded on the FIRST call to the NIM API
    ...
    // NIM uses plain JSON Schema — no jsonSchema() wrapper
  ...
  it('emits tool_use_start, tool_use_input_delta, tool_use_complete from first iteration', async () => {
    ...
    const adapter = new NVIDIAAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.007: Jupiter data')
  ...
  it('executes the tool executor with the parsed tool input', async () => {
    ...
    const adapter = new NVIDIAAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.007: Jupiter 9H')

    for await (const _e of runAgenticLoop(adapter, BASE_REQUEST, toolExecutor, NVIDIA_LOOP_CONFIG)) {
      // drain
    }

    expect(toolExecutor).toHaveBeenCalledOnce()
    expect(toolExecutor).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'call_nim1', name: 'query_signals', input: { signal_id: 'MSR.007' } }),
    )
  })
  ...
  it('injects tool result into second-iteration message history', async () => {
    ...
    const adapter = new NVIDIAAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.007: Jupiter 9H dharma')
  ...
  it('emits final text_delta from second iteration after tool result', async () => {
    ...
    const adapter = new NVIDIAAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('MSR.007 data')
    ...
    expect(fullText).toContain('Jupiter in 9H indicates dharmic wisdom.')
  })
...
describe('R11F-B-S5: iteration cap — AgenticLoopCapExceeded after MAX_ITERATIONS', () => {
  ...
  it('does not exceed MAX_ITERATIONS calls to the NIM API', async () => {
    const adapter = new NVIDIAAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('result')
...
describe('R11F-B-S5: B.11 floor — system prompt preserved across loop iterations', () => {
  ...
  it('NVIDIA_LOOP_CONFIG is Case A — finish_reason_tool_calls (model-dependent, llama-3.1+)', () => {
    // Structural invariant: NIM llama-3.1-70b-instruct supports OpenAI-compat tool calling.

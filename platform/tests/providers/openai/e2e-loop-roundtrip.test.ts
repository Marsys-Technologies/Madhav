/**
 * e2e-loop-roundtrip.test.ts — R11F-B-S3 E2E integration test
 *
 * Proves that the OpenAI adapter forwards tools to the OpenAI API and that
 * the agentic loop correctly processes tool calls, injects results, and
 ...
 * We spy on mockCreate to verify tools were forwarded in the correct OpenAI format.
 */
...
const BASE_REQUEST: ChatRequest = {
  ...
  messages: [
    ...
    {
      role: 'user',
      content: 'What does Saturn indicate in the chart?',
    },
  ],
  tools: B11_TOOLS,
}
...
describe('R11F-B-S3: OpenAI round-trip — tools forwarded + loop terminates on stop', () => {
  beforeEach(() => {
    mockCreate.mockReset()
  })

  it('forwards tools in OpenAI function format when request.tools is provided', async () => {
    ...
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.001: Saturn in 10H — career prominence')

    const events: Array<Record<string, unknown>> = []
    for await (const e of runAgenticLoop(adapter, BASE_REQUEST, toolExecutor, OPENAI_LOOP_CONFIG)) {
      events.push(e as Record<string, unknown>)
    }

    // Verify tools were forwarded on the FIRST call to the OpenAI API
  ...
  it('emits tool_use_start, tool_use_input_delta, tool_use_complete from first iteration', async () => {
    ...
    const adapter = new OpenAIAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.001: Saturn data')
  ...
  it('executes the tool executor with the parsed tool input', async () => {
    ...
    const adapter = new OpenAIAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.001: Saturn 10H')
    ...
    expect(toolExecutor).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'call_t1', name: 'query_signals', input: { signal_id: 'MSR.001' } }),
    )
  })
  ...
  it('injects tool result into second-iteration message history', async () => {
    ...
    const adapter = new OpenAIAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('Signal MSR.001: Saturn 10H career')
  ...
  it('emits final text_delta from second iteration after tool result', async () => {
    ...
    const adapter = new OpenAIAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('MSR.001 data')
    ...
    expect(fullText).toContain('Saturn is the dominant force.')
  })
...
describe('R11F-B-S3: iteration cap — AgenticLoopCapExceeded after MAX_ITERATIONS', () => {
  beforeEach(() => {
    mockCreate.mockReset()
  ...
  it(`throws AgenticLoopCapExceeded after ${MAX_ITERATIONS} iterations`, async () => {
    ...
    async function drainLoop() {
      for await (const _e of runAgenticLoop(adapter, BASE_REQUEST, toolExecutor, OPENAI_LOOP_CONFIG)) {
        // drain
      }
    }
  ...
  it('does not exceed MAX_ITERATIONS calls to the OpenAI API', async () => {
    const adapter = new OpenAIAdapter()
    const toolExecutor = vi.fn().mockResolvedValue('result')
    ...
    // The cap check fires before the (MAX_ITERATIONS + 1)th call
...
describe('R11F-B-S3: B.11 floor — system prompt preserved across loop iterations', () => {
  beforeEach(() => {
    mockCreate.mockReset()
  })
  ...
  it('no tools forwarded when request.tools is empty — clean path', async () => {
    ...
    // When tools is empty, createParams.tools should not be set

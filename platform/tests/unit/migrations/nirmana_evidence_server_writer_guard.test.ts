...
describe('migration 632: Nirmana server-reconstructed evidence writer guard', () => {
  it('rejects direct shared-principal inserts and provisions only a distinct ingress login boundary', () => {
    ...
    expect(sql).toContain('REVOKE nirmana_evidence_ingress_writer FROM %I')
    ...
    expect(sql).toContain('another principal can assume')
    expect(sql).toContain('owns a database')
    expect(sql).toContain('owns a schema')
    expect(sql).toContain('effective ingress relation privileges outside the evidence schema')
    expect(sql).toContain('refuses a pre-existing elevated nirmana_evidence_ingress_writer role')
    expect(sql).not.toContain('NOSUPERUSER')

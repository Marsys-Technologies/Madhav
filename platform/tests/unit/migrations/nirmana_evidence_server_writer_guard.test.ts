...
describe('migration 632: Nirmana server-reconstructed evidence writer guard', () => {
  it('rejects direct shared-principal inserts and provisions only a distinct ingress login boundary', () => {
    ...
    expect(sql).toContain("'bg_parihara_rules'")
    expect(sql).toContain("'bg_muhurta_activity_rules'")
    expect(sql).toContain("'bg_muhurta_factor_census'")
    expect(sql).toContain("'bg_sarvatobhadra_grid'")
    expect(sql).toContain('ALTER ROLE nirmana_evidence_ingress_writer')
    expect(sql).toContain('REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA')
    expect(sql).toContain('REVOKE %I FROM nirmana_evidence_ingress_writer')
    expect(sql).toContain('could not revoke stale ingress table privileges')
    expect(sql).toContain('could not revoke stale ingress column privileges')

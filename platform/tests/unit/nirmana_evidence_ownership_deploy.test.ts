...
describe('Nirmana ownership deployment attestation', () => {
  ...
  it('routes the successor lifecycle migration through the one-shot protected-owner runner', () => {
    ...
    expect(purnaSchemaCapability).toContain('GRANT USAGE ON SCHEMA public TO purna_inquiry_owner')
    expect(purnaSchemaCapability).toContain('GRANT USAGE ON SCHEMA public TO role_web_serve, purna_inquiry_owner')

...
describe('DP-SD-020 isolated validation bootstrap', () => {
  ...
  it.each([
    'postgresql://postgres:test@rogue.example:5432/amjis',
    'postgresql://postgres:test@127.0.0.1:6543/amjis',
    'postgresql://postgres:test@127.0.0.1:5432/other',
    'postgresql://postgres:test@127.0.0.1:5432/amjis?host=rogue.example',
  ])('rejects an ownership credential whose production route is not exact: %s', (url) => {
    expect(() => ownershipAdminProxyConfig(url)).toThrow(/production proxy route/)
  })

  it('rejects a validation-credential swap on the ownership route', () => {
    expect(() => ownershipAdminProxyConfig('postgresql://admin:test@127.0.0.1:5432/amjis'))
      .toThrow(/authenticate as postgres/)
  })
...
describe('DP-SD-018 lifecycle SQL contract', () => {
  ...
  it('normalizes the complete pre-existing table, sequence, and schema ACL surface', () => {
    ...
    expect(preflight).toContain('amjis_app, role_web_serve')
    expect(preflight).toContain("IF to_regrole('purna_inquiry_owner') IS NOT NULL")
    expect(preflight).toContain('GRANT USAGE ON SCHEMA public TO purna_inquiry_owner')
    expect(readFileSync(resolve(__dirname, '../../scripts/data-plane-ownership-status.ts'), 'utf8'))
      .toContain("('role_web_serve','USAGE')")
    expect(readFileSync(resolve(__dirname, '../../scripts/data-plane-ownership-status.ts'), 'utf8'))
      .toContain("SELECT 'purna_inquiry_owner','USAGE'")
...
describe('DP-SD-018 deployment ordering', () => {
  ...
  it('requires exact backup and successful-restore identifiers as one receipt', () => {
    ...
    const sourceSocketCarrierWithMetadata = `${sourceSocketCarrier}&sslmode=disable&application_name=restore-attestation`
    expect(assertValidationConnectorBinding(sourceSocketCarrierWithMetadata, {
      instance, validationInstance: receipt.validationInstance, connectionName: instance.connectionName,
      proxyPort: '5433', project: 'madhav-astrology',
    })).toMatchObject({ host: '127.0.0.1', port: 5433, user: 'validator', database: 'restored' })
    ...
    for (const credentialOverride of ['user=rogue', 'password=rogue']) {
      expect(() => assertValidationConnectorBinding(`${sourceSocketCarrier}&${credentialOverride}`, {
        instance, validationInstance: receipt.validationInstance, connectionName: instance.connectionName,
        proxyPort: '5433', project: 'madhav-astrology',
      })).toThrow(/authenticated isolated Cloud SQL proxy identity/)
    }
    ...
    expect(workflow).toContain('DATA_PLANE_RESTORE_VALIDATION_DATABASE_URL: ${{ secrets.DATA_PLANE_RESTORE_VALIDATION_DATABASE_URL }}')
    expect(workflow).not.toContain('DATA_PLANE_RESTORE_VALIDATION_DATABASE_URL: ${{ secrets.DATA_PLANE_VERIFIER_DATABASE_URL }}')

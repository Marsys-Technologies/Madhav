-- 802_nirmana_l1_ga_structural_integrity_contract_specialpointaspect.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-eighth follow-up F-A14 widening pass for
-- ga_structural, third in the 800-819 range (adjudication #2057, continuation 4). Migration 745
...
--
-- aspect_received_by_special_point (`_build_special_point_relationship_rows`) stores, for every
-- GA5-loaded special point (arudha lagnas, bhava arudhas, gulika-family points, etc.) and every
-- graha whose classical Parashari aspect (via the SAME `get_graha_aspects` canonical authority
-- already used by migration 798) lands on that special point's house, a row keyed
-- (special_point, aspected_by_{GRAHA}). Like migration 793's graha_effective_dignity_modified_
-- by_aspects, this category is FULLY SELF-CONTAINED: value_jsonb already stores
-- {special_point, aspecting_graha, graha_house, aspect_offset, target_house, target_sign,
-- strength}, everything needed to re-verify the row's own geometry and classical aspect
-- membership without any cross-category or cross-asset join.
--
--   (a10) fact_value_num domain: must equal 1.0 -- the classical Parashari scheme has no
--         fractional aspects (every entry in brahmagyan/aspects.py's
--         _GRAHA_PARASHARI_ASPECTS carries strength 1.0), the same domain already established
--         for migration 798's aspect_parashari_given/received. 0/1296 violations live.
...
--         stored graha_house/aspect_offset fields -- the writer's own target_house formula,
--         identical to migration 798's conjunct (c6)/(d6) geometry. 0/1296 violations live.
--   (d10) (aspecting_graha, aspect_offset) classical validity: must be a legitimate Parashari
--         aspect pair per the hardcoded classical offset table (Mars 4/7/8; Jupiter/Rahu/Ketu
--         5/7/9; Saturn 3/7/10; all others only the universal 7th) -- re-derives the writer's
--         own `get_graha_aspects(g_name)` lookup rather than restating it. 0/1296 violations
--         live.
...
-- 801's one-hundred-and-five original conjuncts (a) through (d9) forward VERBATIM, including
-- conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY exactly as
-- migration 801 already documented. Because (b), (e), (f), and (e7) are all red, the combined
-- 111-conjunct SELECT evaluates to false on live production today; the new conjuncts were
...
-- aspect_received_by_special_point, 43 of 57 owned categories). D-CND-03: chart-partitioned /
-- row-wise, attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none
-- of these forty-three categories' natural key needs re-asserting

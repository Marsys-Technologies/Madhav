-- 840_nirmana_l1_ga_structural_integrity_contract_argalanatalmatrix.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-sixth follow-up F-A14 widening pass for
-- ga_structural, first in the NEW 840-859 range (L1 continuation 5, granted by the Conductor's
-- ruling on #2101 -- the requested 820-839 was already granted to L5 by #2086; 840-859 is
-- confirmed clear of both L5's block and anything else on origin/main). Migration 819 added
-- sambandha_grade (60/64 by the corrected running tally -- see D-L1-106 for the 54-vs-55
-- off-by-one correction, and D-L1-105 for the underlying 64-not-57 denominator finding), the
-- last migration the exhausted 800-819 range could hold. This adds argala_natal_matrix.
--
-- argala_natal_matrix (the "Argala matrices" block inside `_build_argala_rows`) is a full
-- 12x12 sign-to-sign matrix per varga (144 atomic rows): for every (target_sign, source_sign)
-- pair, an argala score is stored -- 0.0 unless the classical offset (2nd/4th/5th/11th from the
-- target) qualifies, in which case the score is 1.0 reduced by 0.25 per natural malefic
-- (Sun/Mars/Saturn/Rahu/Ketu) occupying the source sign, floored at -1.0. Occupancy is derived
-- from the SAME (chart, ayanamsha, build, varga)'s sign data already re-verified as ground truth
-- via the sibling `graha_dignity_per_varga` category (reused since migration 806) -- no fresh
-- occupancy computation needed, and no Lagna dependency at all (pure sign-to-sign geometry, one
-- of the few remaining categories immune to the F-A24/F-A25 bug class by construction).
--
--   (a27) non-argala-offset domain: for every stored row whose offset is NOT one of the writer's
--         own four argala offsets (2, 4, 5, 11), the score must be EXACTLY 0.0 -- the writer's
--         own `else: net_argala = 0.0` branch, re-derived as a data check.
--   (b27) fact_key format: must equal "from_sign_{source_sign_num}_offset_{offset}" -- re-derived
--         from the row's own stored fact_key fields (parsed, not restated).
--   (c27) offset full re-derivation: must equal the classical 1-based signed offset from target
--         to source (safe-wraparound arithmetic, avoiding SQL's sign-following `%` operator per
--         D-L1-102) -- caught the SAME modulo-sign hazard migration 807 first caught, before
--         landing.
--   (e27) argala-offset full re-derivation: for every row whose offset IS one of the four argala
--         offsets, the stored score must equal `round(GREATEST(1.0 - 0.25 * malefic_count,
--         -1.0), 4)`, where malefic_count is re-derived by cross-referencing the sibling
--         `graha_dignity_per_varga` category for the SAME (chart, ayanamsha, build, varga) --
--         which grahas occupy the source sign, filtered to the classical natural-malefic set.
...
-- 817's/818's/819's two-hundred-and-forty-one original conjuncts (a) through (i26) forward
-- VERBATIM, including conjuncts (b), (e), (f), (e7), (j22), and (i25), which remain GENUINELY RED
-- TODAY exactly as migration 819 already documented (F-A15/F-A17/F-157/F-A18/F-A24/F-A25) --
-- note F-A24/F-A25 are now fixed at the writer level (PR #2105) but their tracked conjuncts stay
-- red until the affected chart (cb73cd3d) next rebuilds. Because these six are all red, the
-- combined 248-conjunct SELECT evaluates to false on live production today; the new conjuncts
-- were verified INDIVIDUALLY (their own subquery in isolation) rather than via the full combined
-- SELECT.
--
...
-- argala_natal_matrix,
-- 61 of 64 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these sixty-one categories' natural key needs re-asserting

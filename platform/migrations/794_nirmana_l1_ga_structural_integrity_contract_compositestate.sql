-- 794_nirmana_l1_ga_structural_integrity_contract_compositestate.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twentieth follow-up F-A14 widening pass for ga_structural,
-- fifteenth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- added the Group H avastha bundle; migration 789 added nakshatra_dispositor_chain; migration
-- 790 added chandra_bala_natal_baseline; migration 791 added the Group O tri-deva bundle;
-- migration 792 added graha_functional_class_per_ascendant; migration 793 added
-- graha_effective_dignity_modified_by_aspects (26/57). This adds
-- graha_composite_state_classification, taking coverage to 27/57.
--
-- graha_composite_state_classification (Group L/X, near `_build_special_state_rows`) assigns
-- each graha ONE of seven values from a decision tree: severely_afflicted (debilitated AND
-- combust) > debilitation_cancelled / debilitated (debilitated, split by the writer's own
-- neecha-bhanga-raja-yoga deferral to `ga_yoga_writer.evaluate_nbry`) > afflicted (combust,
-- not debilitated) > well_placed (exalted/own_sign) > weak (retrograde, none of the above) >
-- neutral (fallback). Rather than trusting the writer's own in-memory dignity_status /
-- is_combust / retrograde variables, this conjunct re-derives the ENTIRE decision tree from
-- first principles, reading three independently-verifiable sources:
--
--   1. Dignity (exalted/debilitated/own_sign/neutral) is re-derived purely from
--      `ga_positions`' own graha_position.sign (text) against the classical fixed
--      exaltation/debilitation/own-sign sign tables copied verbatim from
--      `pyjhora_adapter/dignities.py`'s own `_EXALT_SIGN`/`_DEBIL_SIGN`/`_OWN_SIGNS` dicts (the
--      writer's own single source of truth for this exact computation) -- NOT the mismatched
--      5-way `graha_dignity_per_varga` vocabulary (the documented vocabulary-mismatch dead-end,
--      migration-49-era finding, correctly avoided again here). Rahu/Ketu have no entries in
--      any of the three tables and so always resolve to 'neutral' dignity, matching the
--      writer's own behavior (confirmed live: both mean nodes classify 'neutral').
--   2. Combustion is re-derived from `ga_positions`' own graha_position.combustion_state
--      ('combust'/'none') -- the SAME stored fact the writer's own is_combust boolean is a
--      re-derivation of, read directly rather than recomputed from raw longitudes.
--   3. Retrograde is re-derived from `ga_positions`' own graha_position.retrograde_flag
--      ('direct'/'retrograde').
--   4. The debilitation_cancelled/debilitated split re-derives the writer's own §N.5 deferral
--      to `ga_yoga_writer.evaluate_nbry` by reading `ga_yoga`'s OWN authoritative
--      `ga_yoga_firings` table for the `neecha_bhanga_raja_yoga` canonical id
--      (`fired = true` AND the graha present in `constituent_planets`) for the SAME (chart,
--      ayanamsha) -- a genuine cross-ASSET §N.5 reference, not a restated local computation.
--
-- All three source fact_keys (sign/combustion_state/retrograde_flag) live under `ga_positions`'
-- own graha_position category -- the layer-root T0 asset, the canonical D1-position authority
-- every other L1 asset is expected to reference per §N.5, not a second independent PyJHora
-- invocation. The full re-derivation was verified against ALL 135 live rows before landing (not
-- a sample): 0/135 mismatches, including all 10 live debilitation_cancelled rows and all 9
-- afflicted/severely_afflicted rows and all 5 weak rows exercising their respective real
-- cross-reference branches (not vacuous coverage).
--
--   (a4) fact_value_text domain: must be one of the writer's own seven possible classification
--        values ('severely_afflicted', 'debilitated', 'debilitation_cancelled', 'afflicted',
--        'well_placed', 'weak', 'neutral') -- 'debilitated' (plain, uncancelled) has 0 live rows
--        today (every currently-debilitated graha across all charts/ayanamshas happens to be
--        NBRY-cancelled) but remains a real, legitimate value in the writer's own decision tree.
--        0/135 violations live.
--   (b4) full first-principles re-derivation of the entire seven-way decision tree from
--        graha_position's sign/combustion_state/retrograde_flag plus ga_yoga_firings' NBRY
--        authority, exactly matching the writer's own branch order and priority. 0/135
--        violations live.
--
...
-- 786's/787's/788's/789's/790's/791's/792's/793's sixty-eight original conjuncts (a) through
-- (ff3) forward VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED
-- TODAY exactly as migration 793 already documented (F-A15/F-A17/F-157 fixes landed, tracked
-- charts not yet rebuilt). Because (b), (e), (f) are all red, the combined 70-conjunct SELECT
-- evaluates to false on live production today; conjuncts (a4)/(b4) were verified INDIVIDUALLY
...
-- graha_effective_dignity_modified_by_aspects / graha_composite_state_classification, 27 of 57
-- owned categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare
-- count pin (C12). No distinctness conjunct -- none of these twenty-seven categories' natural
-- key needs re-asserting (chart_facts_unique_null_formula already covers it).
SELECT

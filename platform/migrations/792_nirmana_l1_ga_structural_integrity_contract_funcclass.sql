-- 792_nirmana_l1_ga_structural_integrity_contract_funcclass.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Eighteenth follow-up F-A14 widening pass for ga_structural,
-- thirteenth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- (24/57). This adds graha_functional_class_per_ascendant, taking coverage to 25/57.
--
-- graha_functional_class_per_ascendant stores two rows per classical graha (fact_key =
-- 'bphs_canonical' and 'raman_variant'): the writer's own `_build_functional_class_rows`
-- computes BOTH via the exact same call, `_get_functional_class_dynamic(g_name, lagna_sign)`,
-- with the SAME arguments -- the writer's own STAGE-2 HONEST-TIERS comment already documents
-- this is not two independently-implemented derivations. `_get_functional_class_dynamic` itself
-- is fully deterministic: for Aries lagna it returns a hardcoded 7-entry canonical table
-- (FUNCTIONAL_CLASS_BPHS, two-pass verified separately); for any other lagna it derives
-- dynamically via kendra/trikona/dusthana/upachaya classification of the houses each graha
-- rules from that lagna, using the classical _SIGN_LORDS_ORDERED table (the same 12-sign
-- structure already embedded in SQL by migration 757's conjunct (g)).
--
-- Confirmed live before designing the re-derivation conjunct that BOTH branches are genuinely
-- exercised in production, not just the Aries table: two of the three charts have Aries lagna,
-- but the third (cb73cd3d) has Cancer lagna across all five ayanamshas, producing real dynamic
-- classifications (e.g. Moon and Mars both resolve to "yogakaraka" from Cancer lagna) -- so the
-- conjunct below is not merely exercising dead code for the dynamic branch. Lagna sign is read
-- from ga_positions' own graha_position category (fact_subject LAGNA, fact_key sign) -- the
-- layer-root T0 asset, the canonical D1-position authority every other L1 asset is expected to
-- reference per §N.5, not a second independent PyJHora invocation of the D1 dual-source shape.
--
--   (zz2) fact_value_text domain: must be one of the 5 values either branch of
--         `_get_functional_class_dynamic` can ever produce ('functional_benefic', 'neutral',
--         'temporal_benefic', 'temporal_malefic', 'yogakaraka') -- confirmed against all 210
--         live rows; 'functional_malefic' (referenced only in a downstream bucket-classifier
--         constant elsewhere in the writer, never actually emitted by this function) is
--         correctly excluded from the domain. 0/210 violations live.
--   (aa3) bphs_canonical must equal raman_variant for the same (chart, ayanamsha, graha) -- the
--         writer's own STAGE-2 comment confirms both are computed by the literal same function
--         call with the same arguments, so this is a genuine, real self-consistency check (a
--         future code change that diverges the two calls would be caught), not a tautology.
--         0/105 violations live.
--   (bb3) full re-derivation: bphs_canonical must equal `_get_functional_class_dynamic`'s own
--         two-branch formula, re-derived directly from the graha's classical sign-lordship
--         (via _SIGN_LORDS_ORDERED) and the chart's own lagna sign (from graha_position) --
--         covering BOTH the Aries-table branch and the dynamic kendra/trikona/dusthana/
--         upachaya branch, not restated. 0/105 violations live.
--
...
-- 786's/787's/788's/789's/790's/791's sixty-one original conjuncts (a) through (yy2) forward
-- VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED TODAY exactly as
-- migration 791 already documented (F-A15/F-A17/F-157 fixes landed, tracked charts not yet
-- rebuilt). Because (b), (e), (f) are all red, the combined 64-conjunct SELECT evaluates to
-- false on live production today; conjuncts (zz2)/(aa3)/(bb3) were verified INDIVIDUALLY (their
-- own subquery in isolation) rather than via the full combined SELECT.
--
...
-- graha_tri_deva_role_strength / graha_functional_class_per_ascendant, 25 of 57 owned
-- categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count
-- pin (C12). No distinctness conjunct -- none of these twenty-five categories' natural key

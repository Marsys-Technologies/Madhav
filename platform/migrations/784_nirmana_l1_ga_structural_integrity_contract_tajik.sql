-- 784_nirmana_l1_ga_structural_integrity_contract_tajik.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Tenth follow-up F-A14 widening pass for ga_structural, fifth
-- in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- tara_bala_natal_baseline; migration 783 added conjunction_within_orb (11/57). This adds
-- aspect_tajik, taking coverage to 12/57.
--
-- aspect_tajik (M-12) classifies each classical-graha pair in a recognized whole-sign aspect
-- relation into one of four live Tajik yoga types (Nakta is a three-body yoga retained in the
-- writer's vocabulary but never fires from this pairwise loop) based on orb vs. mutual deeptamsa
-- and applying/separating motion. fact_subject = "{g1_subj}_{g2_subj}", fact_key = the taj_type
-- itself (yamaya/ithasala/eesarpha/manaau), fact_value_num = the orb, value_jsonb carries
-- orb_deg/deeptamsa_sum_deg/orb_strength/applying/salience/house_diff.
--
-- This is another D1-sourced category (from ga_structural's own in-memory chart_output), so per
-- the now-established D1 dual-independent-PyJHora-source discipline (four _per_varga occurrences
-- plus conjunction_within_orb's pure-D1 occurrence, migration 783) this pass avoids re-deriving
-- against an external authority (e.g. re-computing house placement from ga_positions) and instead
-- ships six self-consistency/domain/cross-field conjuncts against the row's own already-stored
-- fields -- all of which the writer's own branching logic (quoted in the writer's own inline
-- comments, lines ~1239-1348) guarantees but never states as one explicit claim:
--
--   (z) fact_key domain: must be one of the four live types ('yamaya', 'ithasala', 'eesarpha',
--       'manaau') -- Nakta never fires from this loop per the writer's own comment. 0/76
--       violations live.
--   (aa) fact_value_num must equal value_jsonb.orb_deg -- the same orb value stored twice. 0/76
--        violations live.
--   (bb) value_jsonb.orb_strength must equal the writer's own per-type formula: 1.0 for yamaya;
--        0.1 for manaau; round(greatest(0, 1 - orb_deg/deeptamsa_sum_deg), 4) for
--        ithasala/eesarpha -- a genuine cross-field re-derivation combining two already-stored
--        fields (orb_deg, deeptamsa_sum_deg) into a third, not a bare restatement. 0/76 violations
--        live.
--   (cc) value_jsonb.applying must match the writer's own per-type constraint: true for ithasala
--        and manaau (both only fire on the `applying` branch), false for eesarpha (fires only on
--        the separating branch); yamaya is unconstrained (fires regardless of motion). 0/76
--        violations live.
--   (dd) value_jsonb.salience must equal the writer's own fixed per-type mapping
--        ({"yamaya":"high","ithasala":"high","eesarpha":"medium","manaau":"low"}). 0/76 violations
--        live.
--   (ee) orb_deg must satisfy the writer's own per-type threshold: <= 1.0 for yamaya; in
--        (1.0, deeptamsa_sum_deg] for ithasala/eesarpha; > deeptamsa_sum_deg for manaau -- the
--        exact if/elif/elif branch structure that decides which taj_type a row gets, re-derived
--        as a domain check on what actually got stored. 0/76 violations live.
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries migrations 745's/755's/756's/757's/758's/759's/780's/781's/782's/783's twenty-five
-- original conjuncts (a) through (y) forward VERBATIM, including conjuncts (b), (e), and (f),
-- which remain GENUINELY RED TODAY exactly as migration 783 already documented (F-A15/F-A17/F-157
-- fixes landed, tracked charts not yet rebuilt). Because (b), (e), (f) are all red, the combined
-- 31-conjunct SELECT evaluates to false on live production today; conjuncts (z) through (ee) were
...
-- kala_sarpa_per_varga / tara_bala_natal_baseline / conjunction_within_orb / aspect_tajik, 12 of
-- 57 owned categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare
-- count pin (C12). No distinctness conjunct -- none of these twelve categories' natural key needs

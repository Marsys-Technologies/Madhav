-- 795_nirmana_l1_ga_structural_integrity_contract_karakaoverlap.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-first follow-up F-A14 widening pass for
-- ga_structural, sixteenth in the 780-799 range (adjudication #2012). Migration 745 covered
...
--
-- karaka_house_lord_overlap_flag.is_overlap (Group K Karakatva, `_build_karakatva_rows`) is
-- TRUE iff a significance's fixed classical natural karaka (NATURAL_KARAKAS, e.g. self→Sun,
-- wealth→Jupiter) is the SAME graha as the lord of that significance's fixed house
-- (significance_to_house, e.g. self→house 1, wealth→house 2) via `_get_house_lord`, which
-- itself resolves the sign occupying that house from Lagna, then looks up SIGN_LORDS. Only 12
-- of the writer's 30 KARAKATVA_SIGNIFICANCES have a `significance_to_house` entry (self through
-- losses, houses 1-12); the remaining 18 (dharma, artha, kama, ... liberation) get no
-- karaka_house_lord_overlap_flag row at all, matching the writer's own `if h is not None:`
-- guard -- confirmed live: exactly 12 distinct fact_subject values, 180 rows (12 × 3 charts × 5
-- ayanamshas).
--
-- Fully re-derived from first principles: lagna sign is read from `ga_positions`' own
-- graha_position.LAGNA.sign (the layer-root T0 asset, §N.5), the house-from-lagna arithmetic
-- reuses the exact `((lagna_idx + house_num - 1) % 12) + 1` formula already proven in migration
-- 792's conjunct (bb3), and the classical SIGN_LORDS table is the same 12-sign table already
-- embedded in SQL since migration 757's conjunct (g). NATURAL_KARAKAS and significance_to_house
-- are both fixed classical assignment dicts hardcoded directly from the writer's own source
-- (not re-derived from any further authority -- they ARE the authority, same status as
-- SIGN_LORDS itself). Verified against ALL 180 live rows (not a sample) -- 0 mismatches,
-- including all 50 live `true` rows (non-vacuous: both true and false branches genuinely
-- exercised).
--
--   (a5) fact_value_text domain: must be 'true' or 'false' -- no third value is ever legitimate
--        for this boolean flag. 0/180 violations live.
--   (b5) full re-derivation: is_overlap must equal (natural_karaka == house lord of the
--        significance's fixed house, re-derived from Lagna sign + classical SIGN_LORDS).
...
-- 786's/787's/788's/789's/790's/791's/792's/793's/794's seventy original conjuncts (a) through
-- (b4) forward VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED
-- TODAY exactly as migration 794 already documented (F-A15/F-A17/F-157 fixes landed, tracked
-- charts not yet rebuilt). Because (b), (e), (f) are all red, the combined 72-conjunct SELECT
-- evaluates to false on live production today; conjuncts (a5)/(b5) were verified INDIVIDUALLY
...
-- karaka_house_lord_overlap_flag, 28 of 57 owned categories). D-CND-03: chart-partitioned /
-- row-wise, attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these twenty-eight categories' natural key needs re-asserting

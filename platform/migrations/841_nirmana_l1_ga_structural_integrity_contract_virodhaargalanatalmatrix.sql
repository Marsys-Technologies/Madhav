-- 841_nirmana_l1_ga_structural_integrity_contract_virodhaargalanatalmatrix.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-seventh follow-up F-A14 widening pass for
-- ga_structural, second in the 840-859 range (adjudication #2101, L1 continuation 5). Migration
-- 840 added argala_natal_matrix (61/64 by the corrected running tally -- see D-L1-106 for the
-- 54-vs-55 off-by-one correction, and D-L1-105 for the underlying 64-not-57 denominator finding),
-- confirmed immune to F-A24/F-A25 by construction (the malefic-count filter never sees a "LAGNA"
-- occupant). This adds virodha_argala_natal_matrix -- the LAST real remaining ga_structural
-- category (eclipse_proximity_natal remains a documented, permanently-excluded B.10 placeholder,
-- not counted toward this tally).
--
-- virodha_argala_natal_matrix (the "Virodha argala matrix" block inside `_build_argala_rows`,
-- same function, same 144-cell-per-varga loop as argala_natal_matrix) stores, for every
-- (target_sign, source_sign) pair, a BINARY counter-intervention score: 1.0 if the offset is one
-- of the four classical virodha positions (12th/10th/9th/3rd from the target) AND the source sign
-- is occupied by ANY graha (no malefic filter, unlike argala's weighted score), else 0.0.
--
-- **F-A26 (GENUINELY RED, a NEW writer bug, distinct root cause from F-A24/F-A25/F-A24-class,
-- confirmed and profiled before shipping)**: the occupancy map this function actually receives is
-- NOT built from `chart_output.get("grahas")` directly (that internal fallback path is dead code
-- at this call site) -- it is built by the CALLER (`_build_varga_relationship_rows`, the per-varga
-- loop) at lines ~6210-6214, which iterates `varga_state.items()` and buckets every key by its
-- `sign` field with NO exclusion for the "LAGNA"/"Lagna" pseudo-entry `_extract_chart_state`
-- (D1) and `_load_varga_positions` (non-D1) both legitimately carry for lagna-sign-number
-- consumers (migration 815's F-A24, migration 818's F-A25). Because virodha's occupancy check is
-- unfiltered ("if occupants_in_source" -- ANY entry, not just malefics), a chart whose lagna sign
-- happens to be a virodha-offset SOURCE sign gets a spurious 1.0 even when NO real graha occupies
-- that sign. argala_natal_matrix's own malefic-restricted check (migration 840, conjunct (e27))
-- never surfaces this, since "LAGNA" is never a member of the malefics_set token list -- this is
-- why 840 could honestly claim immunity while 841 cannot. Confirmed on both Aries-lagna canonical
-- charts (`482012f1` across all 5 ayanamshas; `1c826d5a` in the one ayanamsha --
-- surya_siddhanta_classical -- where no real graha ALSO happens to occupy Aries): 24/62640 D1
-- rows wrong, cross-checked against the already-verified `graha_dignity_per_varga` category as
-- ground truth (which correctly shows Aries empty of any of the 9 tracked grahas in every one of
-- these combinations) and mutation-proven in both directions (fixing a false-positive row drops
-- the violation count by one; corrupting a genuinely-correct row raises it by one). `cb73cd3d`
-- (Cancer lagna) shows zero violations here, exactly as expected -- Cancer is never a virodha
-- SOURCE sign relative to any of its own targets under this chart's real graha placements. Shipped
-- honestly RED per the never-weaken-a-gate doctrine, joining (b)/(e)/(f)/(e7)/(j22)/(i25) as a
-- SEVENTH tracked-red conjunct; the writer fix (excluding the Lagna/LAGNA key from the occupancy
-- bucket at the `_build_varga_relationship_rows` call site) is left as a follow-up, not attempted
-- in this migration-authoring cycle -- mirroring exactly how F-A24/F-A25 were handled.
--
--   (a28) non-virodha-offset domain: for every stored row whose offset is NOT one of the four
--         classical virodha offsets (3, 9, 10, 12), the score must be EXACTLY 0.0 -- the writer's
--         own `else: virodha_score = 0.0` branch, re-derived as a data check.
--   (b28) fact_key format: must match `^from_sign_[0-9]+_offset_[0-9]+$` -- a genuine format
--         check (NOT a tautological self-reconstruction; an earlier draft that rebuilt fact_key
--         from its own substrings was caught during mutation-testing as unbreakable by
--         construction, the same tautology-conjunct defect class first caught at migration 814's
--         (e21) and again at 817's (e24), and replaced with this regex before landing).
--   (c28) offset full re-derivation: must equal the classical 1-based signed offset from target
--         to source, safe-wraparound arithmetic (D-L1-102).
--   (d28) **F-A26, GENUINELY RED TODAY** -- virodha-offset full re-derivation: for every row
--         whose offset IS one of the four virodha offsets, the stored score must equal 1.0 if
--         ANY graha occupies the source sign (cross-referenced via the sibling
--         `graha_dignity_per_varga` category, no token filter) else 0.0. 24/24960 virodha-offset
--         rows violate (all D1, all Aries-source, the Lagna-contamination pattern above).
...
--         exactly 144 rows (12 targets x 12 sources) -- the writer's own build-fatal assertion,
--         re-derived as a data check rather than trusted.
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's/810's/811's/812's/813's/814's/815's/816's/
-- 817's/818's/819's/840's two-hundred-and-forty-eight original conjuncts (a) through (h27)
-- forward VERBATIM, including conjuncts (b), (e), (f), (e7), (j22), and (i25), which remain
-- GENUINELY RED TODAY exactly as migration 840 already documented (F-A15/F-A17/F-157/F-A18/
-- F-A24/F-A25) -- note F-A24/F-A25 are fixed at the writer level (PR #2105) but their tracked
-- conjuncts stay red until the affected chart (cb73cd3d) next rebuilds. This migration's own
-- (d28)/F-A26 is a SEVENTH genuinely-red conjunct, newly discovered here, not carried forward.
-- Because these seven are all red, the combined 255-conjunct SELECT evaluates to false on live
-- production today; the new conjuncts were verified INDIVIDUALLY (their own subquery in
...
SELECT
  ...
  -- (j22) F-A24, GENUINELY RED TODAY -- bhava_sign must equal the D1-lagna-based house_sign
  -- (bhava_num) invariant (varga-independent, per migration 811), cross-referenced against
  -- lord_in_house_per_varga's own D1 H1 sign as ground truth. 0/12150 non-cb73cd3d-non-D1 rows
  -- violate; 4200/4200 of cb73cd3d's non-D1 rows violate (confirmed Aries-lagna-hardcode bug,
  ...
  -- (i25) F-A25, GENUINELY RED TODAY -- a lord_placed row's lord must equal the classical
  -- sign-lord of the D1-lagna-based house_sign(source_house) invariant (varga-independent, per
  -- migration 811), cross-referenced against lord_in_house_per_varga's own D1 H1 sign as ground
  -- truth. 1450/1740 of cb73cd3d's lord_placed rows violate (all 29 vargas, including D1); 0
  -- elsewhere.
  AND NOT EXISTS (
    WITH lagna AS (
      SELECT chart_id, ayanamsha_id, build_id, fact_value_jsonb->>'sign' AS lagna_sign
    ...
    WHERE cf.fact_category = 'bhava_significance_link' AND cf.fact_key = 'lord_placed'
  ...
  -- (d28) F-A26, GENUINELY RED TODAY -- virodha-offset full re-derivation: for every row whose
  -- offset IS one of the four virodha offsets, the stored score must equal 1.0 if ANY graha
  -- occupies the source sign (cross-referencing the sibling graha_dignity_per_varga category,
  -- no token filter -- unlike argala's malefic-restricted (e27)) else 0.0. 24/24960 violate.
  AND NOT EXISTS (
    WITH parsed AS (
      SELECT cf.chart_id, cf.ayanamsha_id, cf.build_id,
        split_part(cf.fact_subject, '_SIGN_', 1) AS varga,

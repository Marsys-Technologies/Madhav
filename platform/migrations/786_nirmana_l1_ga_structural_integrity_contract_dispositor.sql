-- 786_nirmana_l1_ga_structural_integrity_contract_dispositor.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twelfth follow-up F-A14 widening pass for ga_structural,
-- seventh in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- aspect_tajik; migration 785 added graha_yoga_karaka_flag (13/57). This adds
-- graha_dispositor_chain, taking coverage to 14/57.
--
-- graha_dispositor_chain stores one row per graha (9 grahas incl. Rahu/Ketu, fact_subject =
-- graha, fact_key = 'chain_jsonb_atomic'), whose value_jsonb walks the classical dispositor chain
-- (graha → SIGN_LORDS[graha's sign] → SIGN_LORDS[that lord's sign] → ...) until a repeat is
-- detected, storing {chain: [names], signs: [sign names], length: int, cycle_detected_at_step:
-- int}. Like graha_yoga_karaka_flag (migration 785), this walk resolves entirely from
-- chart_output's own already-computed sign assignments via the classical SIGN_LORDS lookup table
-- (the same 12-sign table already embedded directly in SQL by migration 757's conjunct (g)) --
-- not two independent PyJHora position computations -- so this is NOT the D1 dual-source shape
-- either, and no disclaimer is needed.
--
-- Six self-consistency/domain/cross-field conjuncts, several re-deriving the classical dispositor
-- rule directly rather than merely checking the writer's own bookkeeping was self-consistent:
--
--   (hh) chain[0] domain: must equal the row's own graha (the writer's own `chain: list[str] =
--        [g_name]` initialization) -- re-derived from fact_subject rather than restated. 0/135
--        violations live.
...
--        to both in lockstep (unless a dispositor's own sign can't be resolved, an edge case that
--        cannot occur here since every SIGN_LORDS value is one of the 7 classical grahas, all of
--        which chart_output always carries). 0/135 violations live.
--   (kk) when cycle_detected_at_step is not null, it must equal length exactly -- re-derives the
--        writer's own loop arithmetic (cycle_at = step+1 at the exact break point, and length is
--        the array size at that same point, so the two are structurally identical whenever a
--        cycle is detected -- confirmed no chart on file today hits the uncapped max_depth=10
--        case, so this conjunct's "when not null" guard is not currently vacuous camouflage; a
--        null case remains a legitimate, un-violated third state). 0/135 violations live.
--   (ll) full classical re-derivation: for every consecutive pair in the stored chain, the next
--        graha must equal SIGN_LORDS[the previous graha's stored sign] -- re-derives the actual
--        dispositor rule (not just the writer's own bookkeeping) using the same classical table
--        already used by migration 757's conjunct (g), walked step-by-step via
--        generate_series over the JSON array rather than restated. 0/318 pairs checked
--        (sum of length-1 across all 135 rows), 0 violations live.
--   (mm) terminal cycle-closure: the classical dispositor of the LAST stored sign must already be
--        a member of the stored chain -- confirms the writer's own claim that a cycle was
--        genuinely detected (not merely that the loop stopped for some other reason), re-derived
--        directly from the classical table rather than trusting cycle_detected_at_step alone.
--        0/135 violations live.
--
...
-- thirty-three original conjuncts (a) through (gg) forward VERBATIM, including conjuncts (b),
-- (e), and (f), which remain GENUINELY RED TODAY exactly as migration 785 already documented
-- (F-A15/F-A17/F-157 fixes landed, tracked charts not yet rebuilt). Because (b), (e), (f) are all
-- red, the combined 39-conjunct SELECT evaluates to false on live production today; conjuncts
-- (hh) through (mm) were verified INDIVIDUALLY (their own subquery in isolation) rather than via
...
-- graha_yoga_karaka_flag / graha_dispositor_chain, 14 of 57 owned categories). D-CND-03:
-- chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No distinctness
-- conjunct -- none of these fourteen categories' natural key needs re-asserting

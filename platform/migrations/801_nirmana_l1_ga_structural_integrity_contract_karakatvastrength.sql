-- 801_nirmana_l1_ga_structural_integrity_contract_karakatvastrength.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-seventh follow-up F-A14 widening pass for
-- ga_structural, second in the 800-819 range (adjudication #2057, continuation 4). Migration
-- 745 covered graha_vargottama_amplification_factor; migration 755 added bhadra_flag/
...
-- coverage to 42/57.
--
-- karakatva_strength_per_significance (Group K Karakatva, `_build_karakatva_rows` -- the SAME
-- function that emits karaka_house_lord_overlap_flag, covered by migration 795) stores two
-- fact_keys per significance across all 30 KARAKATVA_SIGNIFICANCES (unlike
-- karaka_house_lord_overlap_flag, which only covers the 12 house-mapped significances):
-- natural_karaka (the significance's fixed classical natural karaka, e.g. self->Sun) and
-- composite_strength (the average of that karaka's own dignity-based strength and house-based
-- strength).
--
-- Fully re-derived from first principles: natural_karaka is the writer's own NATURAL_KARAKAS
-- dict, hardcoded directly as the authority (they ARE the authority, same status as SIGN_LORDS
-- -- the same doctrine already applied to migration 795's NATURAL_KARAKAS/significance_to_house
-- reuse). composite_strength = round((karaka_strength + house_strength) / 2.0, 4), where
-- karaka_strength re-derives the natural karaka's OWN dignity via the SAME classical
-- exaltation/debilitation tables (from graha_position.sign) already embedded in SQL since
-- migration 794's conjunct (b4), and house_strength is a pure function of the natural karaka's
-- OWN house (from graha_position.house_d1): 1.0 for kendra (1,4,7,10), 0.75 for trikona (5,9),
-- 0.5 otherwise. Note Rahu (foreign_travel's natural karaka) has no entry in the classical
-- exaltation/debilitation tables and so always resolves to 'neutral' dignity (karaka_strength
-- 0.5), matching migration 794's own Rahu/Ketu handling -- confirmed live.
--
--   (a9) natural_karaka fact_value_text domain: must be one of the 8 grahas that ever appear as
--        a NATURAL_KARAKAS value (Sun/Moon/Mars/Mercury/Jupiter/Venus/Saturn/Rahu). 0/450
--        violations live.
...
--        formula's four dignity tiers x three house tiers (0.375, 0.5, 0.625, 0.75, 0.8125,
--        0.9375, 1.0). 0/450 violations live.
--   (d9) composite_strength full re-derivation: must equal round((karaka_strength +
--        house_strength) / 2.0, 4), re-derived directly from the SAME significance's own
--        natural karaka's graha_position.sign (via the classical dignity tables) and
--        graha_position.house_d1 -- a genuine two-source cross-field re-derivation, not a bare
--        restatement. 0/450 violations live (confirmed non-vacuous: 450/450 rows join-matched).
--
...
-- one-hundred-and-one original conjuncts (a) through (f8) forward VERBATIM, including
-- conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY exactly as
-- migration 800 already documented. Because (b), (e), (f), and (e7) are all red, the combined
-- 105-conjunct SELECT evaluates to false on live production today; the new conjuncts were
...
-- chart_center_of_gravity / karakatva_strength_per_significance, 42 of 57 owned categories).
-- D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No
-- distinctness conjunct -- none of these forty-two categories' natural key needs re-asserting

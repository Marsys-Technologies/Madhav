-- 818_nirmana_l1_ga_structural_integrity_contract_bhavasignificancelink.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-fourth follow-up F-A14 widening pass for
-- ga_structural, nineteenth in the 800-819 range (adjudication #2057, continuation 4). Migration
-- 817 added aspect_parashari_per_varga (58/64 by the corrected running tally -- see D-L1-106 for
-- the 54-vs-55 off-by-one correction, and D-L1-105 for the underlying 64-not-57 denominator
-- finding), confirmed immune to F-A24. This adds bhava_significance_link.
--
-- bhava_significance_link (`_build_bhava_web_per_varga_rows`) emits, for each of the 12 houses in
-- a varga, one "lord_placed" row (where that house's classical sign lord currently sits in this
-- varga) plus zero-or-more "lord_aspects" rows (every OTHER house that lord's own Parashari
-- aspects reach, via the same `_graha_aspects_house`/`get_graha_aspects` canonical authority
-- reused since migration 807).
--
-- **F-A25 (GENUINELY RED, a NEW writer bug, distinct root cause from F-A24, confirmed and
-- profiled before shipping)**: the caller computes this function's `lagna_sign_num` parameter as
-- `int(varga_state.get("Lagna", {}).get("sign_num", 1) ...)` -- checking ONLY the mixed-case key
-- `"Lagna"`. But `_extract_chart_state` (the D1 loader) only ever sets `state["LAGNA"]`
-- (ALL CAPS) -- so this lookup NEVER matches, for ANY varga, INCLUDING D1 (unlike F-A24's
-- `_build_karaka_bhava_concordance_per_varga_rows`, which defensively checks BOTH `"Lagna"` and
-- `"LAGNA"` and is therefore only wrong for non-D1 vargas). The result: `lagna_sign_num` is
-- ALWAYS 1 (Aries) here, for every varga on every chart, regardless of the chart's real lagna.
-- Invisible on two of three test charts whose own D1 lagna genuinely is Aries; confirmed WRONG on
-- chart `cb73cd3d` (D1 lagna = Cancer) across 1450 of 1740 lord_placed rows (spanning ALL 29
-- vargas INCLUDING D1 -- not confined to non-D1 the way F-A24 was), cross-checked against
-- `lord_in_house_per_varga`'s own (already-correct) D1 H1 sign as ground truth. The ~17% of rows
-- that happen to read correctly despite the wrong lagna are houses where the (wrong) Aries-based
-- sign and the (correct) Cancer-based sign are exactly 3 signs apart and share the same classical
-- lord (Mercury rules Gemini/Virgo, Jupiter rules Sagittarius/Pisces, both pairs 3 signs apart) --
-- a coincidental partial match, not evidence the bug is smaller than it is. Shipped honestly RED
-- per the never-weaken-a-gate doctrine, joining F-A15/F-A17/F-157/F-A18/F-A24 as a sixth
-- tracked-red conjunct; the writer fix (checking `"LAGNA"`, mirroring F-A24's own eventual fix
-- and `_build_karaka_bhava_concordance_per_varga_rows`'s existing defensive `or` check) is left
-- as a follow-up, not attempted in this migration-authoring cycle. The "lord_aspects" rows are
-- verified for INTERNAL self-consistency against whatever "lord_placed" already stores for the
-- same house (conjuncts (f25)/(g25)/(h25)) -- these hold regardless of whether lord_placed's own
-- lord is itself correct, so they are not separately red.
--
...
--         `_bhava_link_type` decision tree (kendra checked first, then trikona, then dusthana,
--         else neutral -- house 1 is kendra-classified despite also being trikona, since kendra
--         is checked first and the function short-circuits).
...
--         ayanamsha, build, varga, source_house)'s lord_placed row's own lord -- a genuine
--         cross-row check within the category, independent of whether that lord is itself
--         correct (see F-A25 above).
--   (g25) lord_aspects target-house exclusion: a lord_aspects row's target_house must never equal
--         the lord's own house (the sibling lord_placed row's target_house for the same
--         source_house) -- the writer's own `if tgt_h == lord_h: continue` guard.
--   (h25) lord_aspects full classical re-derivation: the offset from the lord's own house to the
--         aspected target_house must be one of the writer's own `get_graha_aspects` offsets for
--         that lord -- the same canonical authority already reused since migration 807.
--   (i25) **F-A25, GENUINELY RED TODAY** -- a lord_placed row's lord must equal the classical
--         sign-lord of the D1-lagna-based house_sign(source_house) invariant (varga-independent,
--         per migration 811), cross-referenced against `lord_in_house_per_varga`'s own D1 H1 sign
--         as ground truth. 0/290 non-cb73cd3d-lagna-coincidence rows violate elsewhere; 1450/1740
--         of cb73cd3d's lord_placed rows violate (spanning all 29 vargas, including D1).
--
...
-- 817's two-hundred-and-twenty-three original conjuncts (a) through (h24) forward VERBATIM,
-- including conjuncts (b), (e), (f), (e7), and (j22), which remain GENUINELY RED TODAY exactly as
-- migration 817 already documented (F-A15/F-A17/F-157/F-A18/F-A24). This migration's own (i25)/
-- F-A25 is a SIXTH genuinely-red conjunct, newly discovered here, not carried forward. Because
-- (b), (e), (f), (e7), (j22), and now (i25) are all red, the combined 232-conjunct SELECT
...
-- aspect_parashari_per_varga / bhava_significance_link,
-- 59 of 64 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these fifty-nine categories' natural key needs re-asserting

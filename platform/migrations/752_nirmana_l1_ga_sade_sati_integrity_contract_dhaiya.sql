-- 752_nirmana_l1_ga_sade_sati_integrity_contract_dhaiya.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Follow-up F-A14 widening pass for ga_sade_sati: migration
-- 748 shipped a bounded first pass (sade_sati_cycle, sade_sati_phase_quarter — 2/15 categories).
-- This pass adds the 4 Dhaiya-family categories (dhaiya_period, kantaka_shani_period,
-- ashtama_shani_period, ardha_ashtama_shani_period — Saturn's 4H/8H transits from natal Moon,
-- ga_sade_sati_writer.py's _emit_dhaiya_rows), taking coverage to 6/15 categories.
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries the FULL replacement value: migration 748's three original conjuncts (a)/(b)/(c),
-- verbatim, PLUS four new conjuncts (d)/(e)/(f)/(g) below. Applying this migration without the
-- prior conjuncts would silently regress migration 748's own coverage back to zero.
--
-- Every new conjunct below was EXECUTED against live production and MUTATION-PROVED before
-- landing: each was re-run against a corruption injected inside a CTE overlay and shown to
-- return false. Passes clean (integrity_passed = true) on live production today.
--

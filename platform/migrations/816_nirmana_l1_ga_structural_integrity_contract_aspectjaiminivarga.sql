-- 816_nirmana_l1_ga_structural_integrity_contract_aspectjaiminivarga.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-second follow-up F-A14 widening pass for
-- ga_structural, seventeenth in the 800-819 range (adjudication #2057, continuation 4).
-- Migration 815 added karaka_bhava_concordance (56/64 by the corrected running tally -- see
-- D-L1-106 for the 54-vs-55 off-by-one correction, and D-L1-105 for the underlying 64-not-57
-- denominator finding) and discovered F-A24, a genuinely-red Aries-lagna-hardcode writer bug.
-- This adds aspect_jaimini_per_varga.
--
-- aspect_jaimini_per_varga is the per-varga sibling of migration 803's aspect_jaimini -- the
-- SAME pure 12-sign Jaimini Rasi drishti combinatorial rule (no dependency on birth data,
-- longitude, ayanamsha_id, or even lagna -- all three SIGN_TYPES branches reduce to the same
-- `offset not in [1, 11]` exclusion), just emitted identically for EVERY one of the 29 vargas
-- rather than only D1. Because the rule is birth-data-independent, this category is IMMUNE to
-- the F-A24 class of bug (no Lagna dependency at all) -- confirmed, not assumed: this migration's
-- own conjuncts re-derive the full classical zodiacal-order rule per varga and find 0 violations
-- across all 29 vargas, unlike karaka_bhava_concordance's bhava-position dependency on the actual
-- D1 lagna.
--
...
--         SIGN_TYPES[source_sign] table (movable/fixed/common) -- a field migration 803's D1-only
--         version does not carry, new to this per-varga emission.
...
--         re-derived per-varga.
--   (e23) completeness: for every (chart, ayanamsha, build, varga) combination and every valid
--         sign pair, a row MUST exist -- migration 803's (dd4) re-derived per-varga.
--   (f23) exact count invariant: each (chart, ayanamsha, build, varga) combination must hold
--         exactly 108 rows -- migration 803's (ee4) re-derived per-varga. 0/435 combinations
--         violate (29 vargas x 15 chart/ayanamsha/build); 46980 total rows confirmed.
--   (g23) symmetric mutual invariant: if (varga, s1, on_s2) is stored, (varga, s2, on_s1) must
--         also be stored -- migration 803's (ff4) re-derived per-varga.
--   (h23) fact_subject format: must equal "{varga}_{source_sign}" -- re-derived from the row's
--         own stored value_jsonb.varga/source_sign fields.
--   (i23) fact_key format: must equal "on_{target_sign}" -- the same self-consistency check for
--         the target side.
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's/810's/811's/812's/813's/814's/815's
-- two-hundred-and-five original conjuncts (a) through (j22) forward VERBATIM, including
-- conjuncts (b), (e), (f), (e7), and (j22), which remain GENUINELY RED TODAY exactly as migration
-- 815 already documented (F-A15/F-A17/F-157/F-A18/F-A24). Because these five are all red, the
-- combined 215-conjunct SELECT evaluates to false on live production today; the new conjuncts
...
-- 57 of 64 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these fifty-seven categories' natural key needs re-asserting

-- 819_nirmana_l1_ga_structural_integrity_contract_sambandhagrade.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-fifth follow-up F-A14 widening pass for
-- ga_structural, twentieth in the 800-819 range (adjudication #2057, continuation 4) -- the
-- LAST migration this range can hold; 819 is the final free number (a fresh range grant will be
-- needed before F-A14 can widen further). Migration 818 added bhava_significance_link (59/64 by
-- the corrected running tally -- see D-L1-106 for the 54-vs-55 off-by-one correction, and
-- D-L1-105 for the underlying 64-not-57 denominator finding), discovering F-A25. This adds
-- sambandha_grade.
--
-- sambandha_grade (`_build_sambandha_per_varga_rows`) computes a 4-fold planet-pair relationship
-- grade per varga: a conjunction score (REAL degree-based orb, not sign/house alone), a mutual
-- Parashari aspect score, a parivartana (sign-exchange) score, and a mutual-reception score,
-- averaged to one grade. Unlike every other category widened so far, the conjunction component
-- genuinely needs each graha's actual ecliptic degree -- data NO already-shipped sibling category
-- stores for non-D1 vargas (graha_dignity_per_varga carries only sign/house). Rather than
-- fabricate a degree-based re-derivation this migration cannot honestly perform for 28 of 29
-- vargas, this migration ships strong domain/format/self-consistency conjuncts for all rows, plus
-- a genuine partial cross-reference for the one varga (D1) where real degree-based ground truth
-- already exists: migration 783's own `conjunction_within_orb` category, which stores the actual
-- orb_deg for every D1 graha pair. This is the same "don't always need to re-derive the full
-- source algorithm" precedent migration 800 established and migration 813 reused.
--
...
--         round((conjunction_score + mutual_aspect_score + exchange_score + reception_score) /
--         4.0, 4) -- the writer's own averaging formula, re-derived from the row's own four
--         stored component scores (not a fresh geometric derivation of those components).
--   (d26) conjunction_score domain: must be one of the writer's own four orb-threshold constants
--         (0.0, 0.25, 0.75, 1.0) -- no fifth value is ever legitimate.
--   (e26) mutual_aspect_score domain: must be one of the writer's own two constants (0.0, 1.0).
--   (f26) exchange_score domain: must be one of the writer's own two constants (0.0, 1.0).
--   (g26) reception_score domain: must be one of the writer's own two constants (0.0, 0.5).
--   (h26) fact_subject varga-prefix consistency: fact_subject must begin with
--         "{varga}_" -- re-derived from the row's own stored value_jsonb.varga field.
--   (i26) D1-only conjunction_score cross-reference: for D1 rows whose graha pair (either
--         ordering) has a sibling row in migration 783's `conjunction_within_orb` category, the
--         stored conjunction_score must match the writer's own orb-threshold classification
--         (<=5 deg -> 1.0, <=10 -> 0.75, <=20 -> 0.25, else 0.0) applied to that sibling's real
--         orb_deg -- a genuine partial ground-truth check (30/30 D1 pairs matched, all clean),
--         not a coincidental agreement.
--
...
-- 817's/818's two-hundred-and-thirty-two original conjuncts (a) through (i25) forward VERBATIM,
-- including conjuncts (b), (e), (f), (e7), (j22), and (i25), which remain GENUINELY RED TODAY
-- exactly as migration 818 already documented (F-A15/F-A17/F-157/F-A18/F-A24/F-A25). Because
-- these six are all red, the combined 241-conjunct SELECT evaluates to false on live production
-- today; the new conjuncts were verified INDIVIDUALLY (their own subquery in isolation) rather
-- than via the full combined SELECT.
--
...
-- 60 of 64 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these sixty categories' natural key needs re-asserting

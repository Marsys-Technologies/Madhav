-- 782_nirmana_l1_ga_structural_integrity_contract_tarabala.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Eighth follow-up F-A14 widening pass for ga_structural,
-- third in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- nway_config_per_varga; migration 781 added kala_sarpa_per_varga (9/57). This adds
-- tara_bala_natal_baseline, taking coverage to 10/57.
--
-- Cross-writer note: like bhadra_flag/panchaka_flag (migration 755), tara_bala_natal_baseline is
-- physically EMITTED by ga_panchanga_writer.py (`_emit_tara_bala_baseline`), not
-- ga_structural_writer.py -- but it is OWNED by ga_structural per fact_category_ownership, so
-- ga_structural's integrity_check_sql is the correct place to verify it (D-CND-03 scopes to the
-- owning asset's registry row, not the emitting writer file).
--
-- tara_bala_natal_baseline stores 27 rows per (chart, ayanamsha) -- one per transit nakshatra,
-- fact_subject = "TRANSIT_NAK_{3-letter code}" (the writer's own NAKSHATRA_SHORT table) -- each
-- classified into one of the 9 classical Tara qualities via:
--   tara_pos = ((transit_nak_id - birth_nak_id) % 27) + 1
--   tara_pos_in_cycle = (tara_pos - 1) % 9 + 1
-- mapped through the writer's own _TARA_QUALITY dict, with birth_nak_id sourced from ga_panchanga's
-- own panchanga_nakshatra_moon category (the same authoritative birth-nakshatra reference already
-- used by migration 755's conjunct (d) for panchaka_flag).
--
--   (u) fact_value_text domain: must be one of the 9 classical Tara quality names (Janma/Sampat/
--       Vipat/Kshema/Pratyak/Sadhaka/Vadha/Mitra/Atimitra) -- the writer's own _TARA_QUALITY dict,
--       no tenth value ever legitimate. 0/405 violations live.
--   (v) fact_value_text must equal the full re-derivation of the writer's own two-step modulo
--       formula from fact_subject's embedded transit-nakshatra code and
--       panchanga_nakshatra_moon.number (birth_nak_id) -- re-derived directly rather than restated.
--       Per D-L1-55 (Postgres's `%` returns a same-sign-as-dividend remainder, so a negative
--       dividend can produce a negative remainder that never satisfies downstream logic), a +270
--       (10*27) safety margin is added before the first modulo and a +90 (10*9) margin before the
--       second, guaranteeing a positive dividend in both cases without changing the result mod 27
--       / mod 9. 0/405 violations live.
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries migrations 745's/755's/756's/757's/758's/759's/780's/781's twenty original conjuncts (a)
-- through (t) forward VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED
-- TODAY exactly as migration 781 already documented (F-A15/F-A17/F-157 fixes landed, tracked
-- charts not yet rebuilt). Because (b), (e), (f) are all red, the combined 22-conjunct SELECT
-- evaluates to false on live production today; conjuncts (u)/(v) were verified INDIVIDUALLY (their
-- own NOT EXISTS subquery in isolation) rather than via the full combined SELECT.
--
...
-- kala_sarpa_per_varga / tara_bala_natal_baseline, 10 of 57 owned categories). D-CND-03:
-- chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No distinctness
-- conjunct -- none of these ten categories' natural key needs re-asserting

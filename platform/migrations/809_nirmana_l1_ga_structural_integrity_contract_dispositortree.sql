-- 809_nirmana_l1_ga_structural_integrity_contract_dispositortree.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-fifth follow-up F-A14 widening pass for
-- ga_structural, tenth in the 800-819 range (adjudication #2057, continuation 4). Migration 745
-- covered graha_vargottama_amplification_factor; migration 755 added
...
--
-- dispositor_tree (`_build_dispositor_tree_per_varga_rows`) is the richest category widened so
-- far: one row per graha (fact_key 'tree_position') storing {varga, parent, children, depth,
-- is_root, sign} PLUS one CHART summary row per varga (fact_key 'summary') storing {varga,
-- roots, root_count}. Unlike migration 806's dispositor_chain_per_varga (which needed a sibling
-- category cross-reference for signs), dispositor_tree stores its OWN sign directly, making the
-- classical parent-derivation conjunct FULLY SELF-CONTAINED -- the SAME SIGN_LORDS table already
-- used by migrations 793/799/806/etc. re-derives `parent` purely from the row's own sign field.
-- The remaining conjuncts are genuine cross-ROW checks WITHIN this single category: the mutual
-- parent-child invariant (a non-root graha's parent must list it back as a child) and the
-- CHART-summary-vs-per-graha-rows round trip (every declared root is genuinely is_root, and
-- every is_root graha is genuinely declared).
--
--   (a16) fact_value_text (the parent subject) must equal the classical SIGN_LORDS lookup of
--         value_jsonb.sign, re-derived purely from the row's own stored field. 0/... per-graha
--         rows checked, 0 violations live.
...
--   (d16) value_num domain: must be NULL if and only if value_jsonb.depth = -1 (a cycle member
--         with no reachable own-sign root) -- the writer's own `float(depth) if depth >= 0 else
--         None` branch. When not NULL, value_num must equal value_jsonb.depth exactly.
...
--         children array must list it back -- the writer's own single `children_map` built in
--         the same pass, re-derived as a cross-row check within dispositor_tree itself.
...
--         per-graha row, for the SAME (chart, ayanamsha, build, varga) -- round-trip
--         completeness in one direction.
--   (i16) every per-graha row with is_root=true must be named in the CHART summary's "roots"
--         array, for the SAME (chart, ayanamsha, build, varga) -- round-trip completeness in the
--         other direction.
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's one-hundred-and-fifty-two original conjuncts
-- (a) through (f15) forward VERBATIM, including conjuncts (b), (e), (f), and (e7)/F-A18, which
-- remain GENUINELY RED TODAY exactly as migration 808 already documented. Because (b), (e), (f),
-- and (e7) are all red, the combined 161-conjunct SELECT evaluates to false on live production

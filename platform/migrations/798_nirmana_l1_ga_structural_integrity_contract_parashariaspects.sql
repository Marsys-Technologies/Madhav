-- 798_nirmana_l1_ga_structural_integrity_contract_parashariaspects.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-fourth follow-up F-A14 widening pass for
-- ga_structural, nineteenth in the 780-799 range (adjudication #2012) -- the SECOND-TO-LAST
-- number in this range (only 799 remains free; adjudication #2057 filed this cycle requesting
-- the continuation). Migration 745 covered graha_vargottama_amplification_factor; migration 755
-- added bhadra_flag/panchaka_flag; migration 756 added vargottama_per_varga; migration 757
-- added parivartana_per_varga; migration 758 added combustion_per_varga; migration 759 added
...
-- migration 796 added the Group C Bhava Bala extended bundle (8 categories); migration 797
-- added aspect_matrix_summary (37/57). This adds BOTH aspect_parashari_given and
-- aspect_parashari_received in one migration -- a genuine two-category bundle (mirror-image
-- given/received views of the exact same underlying Parashari aspect data, emitted in lockstep
-- by the SAME per-graha, per-offset loop in `_build_aspect_rows`) -- taking coverage to 39/57.
--
-- Both categories store the classical Parashari graha-dṛṣṭi (whole-sign aspect) table:
-- `brahmagyan/aspects.py`'s `get_graha_aspects` (a dedicated, dependency-free canonical
-- authority, the same status as SIGN_LORDS) returns a FIXED, ALL-1.0-STRENGTH offset set per
-- graha -- Sun/Moon/Mercury/Venus get only the universal 7th aspect; Mars adds 4th/8th;
-- Jupiter/Rahu/Ketu add 5th/9th; Saturn adds 3rd/10th. `aspect_parashari_given` stores this
-- keyed by the CASTING graha (fact_subject=graha, fact_key=house_{target}); `aspect_parashari_
-- received` stores the inverse view keyed by the RECEIVING house (fact_subject=HOUSE_{target},
-- fact_key=from_{graha}) -- already the sibling category migration 797's aspect_matrix_summary
-- conjunct (l) reads from.
--
-- Fully re-derived from first principles: each graha's own house is read from `ga_positions`'
-- own graha_position.house_d1 (T0 layer-root authority, §N.5), the classical offset table is
-- hardcoded directly (it IS the authority, per the same doctrine as SIGN_LORDS), and
-- target_house = ((house - 1 + offset - 1) % 12) + 1 mirrors the writer's own formula exactly.
-- Hand-verified for three grahas before designing conjuncts (JUP house 9 -> targets {1,3,5};
-- MAR house 7 -> targets {10,1,2}; SAT house 7 -> targets {9,1,4}; KET_MEAN house 8 -> targets
-- {12,2,4}), all matching live data exactly.
--
--   (a6) aspect_parashari_given.fact_value_num domain: must equal 1.0 -- the classical scheme has
--       no fractional aspects (every offset in `_GRAHA_PARASHARI_ASPECTS` carries strength 1.0).
...
--       graha_position.house_d1) under its offset table. 0/285 violations live.
...
--       "from_"||given.subject, same value) for the SAME (chart, ayanamsha, build) -- the
--       writer's own same-loop dual-emission, re-derived as a genuine cross-row check rather
--       than a bare restatement. 0/285 violations live.
...
-- 786's/787's/788's/789's/790's/791's/792's/793's/794's/795's/796's/797's eighty-two original
-- conjuncts (a) through (l) forward VERBATIM, including conjuncts (b), (e), and (f), which
-- remain GENUINELY RED TODAY exactly as migration 797 already documented (F-A15/F-A17/F-157
-- fixes landed, tracked charts not yet rebuilt). Because (b), (e), (f) are all red, the combined
-- 90-conjunct SELECT evaluates to false on live production today; the new conjuncts were
...
-- aspect_parashari_given / aspect_parashari_received, 39 of 57 owned categories). D-CND-03:
-- chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No distinctness
-- conjunct -- none of these thirty-nine categories' natural key needs re-asserting

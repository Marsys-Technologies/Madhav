-- 807_nirmana_l1_ga_structural_integrity_contract_centrality.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-third follow-up F-A14 widening pass for
-- ga_structural, eighth in the 800-819 range (adjudication #2057, continuation 4). Migration
...
-- 48/57.
--
-- graha_centrality (`_build_graph_centrality_per_varga_rows`) computes, per varga, an undirected
-- Parashari aspect-graph degree centrality per graha: Edge(g1,g2) exists if g1 aspects g2's
-- house OR g2 aspects g1's house (via `_graha_aspects_house`, the SAME canonical
-- `get_graha_aspects` offset authority already reused by migrations 798/802/805) OR they share a
-- sign. The row stores {varga, degree_centrality, connected_to} but NOT the graha's own
-- house/sign, so the full classical edge re-derivation cross-references the sibling
-- `graha_dignity_per_varga` category's own house/sign fields for BOTH grahas in every stored
-- edge -- the same sibling-category reference pattern migration 806 established. A9 CAUGHT
-- DURING AUTHORING: the naive SQL offset formula `((h2-h1) % 12) + 1` silently breaks for
-- negative differences (SQL's `%` follows the dividend's sign, unlike Python's floor-mod) --
-- verified by first testing it against all 11500 live edges and finding 2338 false violations
-- purely from sign-mismatched modulo arithmetic; fixed to `MOD(h2-h1+120, 12) + 1` (a safe
-- wraparound offset), re-verified at 0/11500 violations, mutation-confirmed to still catch a
-- real illegitimate edge.
--
--   (a14) fact_value_num must equal value_jsonb.degree_centrality -- the same count stored
...
--         adjacency construction (`adjacency[g1].add(g2); adjacency[g2].add(g1)`), re-derived as
--         a genuine cross-row check. 0/11500 edge-endpoints checked, 0 violations live.
...
--         be justified by g1 aspects g2's house, OR g2 aspects g1's house (both via the
--         classical Parashari offset table, safe-wraparound arithmetic), OR they share the same
--         sign -- houses/signs sourced from the sibling graha_dignity_per_varga category for
--         BOTH grahas. 0/11500 edges checked, 0 violations live, all 11500 matched.
--
...
-- 801's/802's/803's/804's/805's/806's one-hundred-and-forty original conjuncts (a) through (g13)
-- forward VERBATIM, including conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY
-- RED TODAY exactly as migration 806 already documented. Because (b), (e), (f), and (e7) are all
-- red, the combined 146-conjunct SELECT evaluates to false on live production today; the new

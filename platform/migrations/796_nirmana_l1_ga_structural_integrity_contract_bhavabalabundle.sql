-- 796_nirmana_l1_ga_structural_integrity_contract_bhavabalabundle.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-second follow-up F-A14 widening pass for
-- ga_structural, seventeenth in the 780-799 range (adjudication #2012). Migration 745 covered
...
-- (28/57). This adds the Group C "Bhava Bala extended" bundle -- EIGHT categories in one
-- migration, the arc's largest bundle jump yet (after migration 788's 4-category Group H bundle
-- and migration 791's 3-category Group O bundle): bhava_bala_positional, bhava_bala_directional,
-- bhava_bala_temporal, bhava_bala_aspectual, bhava_bala_occupant, bhava_bala_lord,
-- bhava_bala_total_extended, and house_strength_classification_rollup -- taking coverage to
-- 36/57.
--
-- All eight categories are emitted by the SAME per-house loop in
-- `_build_bhava_bala_extended_rows` (Group C: Bhava Bala extended), with a genuine cross-field
-- dependency chain justifying the bundle per the established multi-category bundling
-- discipline (never mere adjacency): bhava_bala_total_extended.total is the arithmetic mean of
-- the other six sub-score categories, and house_strength_classification_rollup.classification
-- is a threshold function of bhava_bala_total_extended.total. Three of the six sub-scores
-- (positional, directional, temporal) are PURE FUNCTIONS OF HOUSE NUMBER ALONE -- no chart data
-- needed at all, fully classical/static -- so their conjuncts are complete re-derivations with
-- zero cross-reference risk. The remaining three (aspectual, occupant, lord) depend on the
-- chart's actual planetary positions and are covered by domain bounds derived directly from the
-- writer's own formula structure (aspectual: 0.5 + 0.125*benefics_aspecting for 0-3 benefics;
-- occupant: 0.5 + 0.1*occupant_count; lord: the writer's own 4-value dignity-strength dict).
--
-- Verified against ALL 180 live rows per category (not a sample) before landing. Confirmed
-- non-vacuous: the classification's three branches all fire live (strong=15, normal=111,
-- weak=54), not a degenerate single-value column.
--
--   (c) bhava_bala_positional full re-derivation: 1.0 for angular houses (1,4,7,10), 0.875 for
--       trikona (5,9 -- house 1 already claimed by angular, checked first per the writer's own
--       branch order), 0.375 for dusthana (6,8,12), 0.5 otherwise -- a pure function of house
--       number, no chart data needed. 0/180 violations live.
...
--   (e) bhava_bala_temporal full re-derivation: 0.75 for angular houses, 0.5 otherwise -- a pure
--       function of house number. 0/180 violations live.
--   (f) bhava_bala_aspectual domain: must be one of {0.5, 0.625, 0.75, 0.875} -- the writer's own
--       0.5 + 0.125*n formula for n benefics (Jupiter/Venus/Mercury) aspecting, n in [0,3].
--       0/180 violations live.
--   (g) bhava_bala_occupant domain: must be 0.5 + 0.1*n for integer n in [0,9] (max grahas that
...
--   (h) bhava_bala_lord domain: must be one of the writer's own four dignity-strength values
--       {1.0, 0.875, 0.5, 0.25}. 0/180 violations live.
--   (i) bhava_bala_total_extended self-consistency: total must equal the arithmetic mean of the
--       SAME house's six sibling sub-score categories (positional/directional/temporal/
--       aspectual/occupant/lord) -- a genuine cross-category re-derivation within the same
--       bundle, not a bare restatement. 0/180 violations live.
--   (j) house_strength_classification_rollup full re-derivation: classification must equal the
--       writer's own threshold function of the SAME house's bhava_bala_total_extended.total
--       (>=0.75 strong, >=0.5 normal, else weak) -- re-derived by reading the sibling category
--       rather than restated. 0/180 violations live, confirmed non-vacuous across all three
--       branches.
--
...
-- 786's/787's/788's/789's/790's/791's/792's/793's/794's/795's seventy-two original conjuncts (a)
-- through (b5) forward VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY
-- RED TODAY exactly as migration 795 already documented (F-A15/F-A17/F-157 fixes landed,
-- tracked charts not yet rebuilt). Because (b), (e), (f) are all red, the combined 80-conjunct
-- SELECT evaluates to false on live production today; the new conjuncts were verified
-- INDIVIDUALLY (their own subquery in isolation) rather than via the full combined SELECT.
--
...
-- bhava_bala_temporal / bhava_bala_aspectual / bhava_bala_occupant / bhava_bala_lord /
-- bhava_bala_total_extended / house_strength_classification_rollup, 36 of 57 owned categories).
-- D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No
-- distinctness conjunct -- none of these thirty-six categories' natural key needs

-- 799_nirmana_l1_ga_structural_integrity_contract_specialstaterollup.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-fifth follow-up F-A14 widening pass for
-- ga_structural, twentieth and LAST in the 780-799 range (adjudication #2012 -- the
-- continuation adjudication #2057 is open requesting the next block). Migration 745 covered
-- graha_vargottama_amplification_factor; migration 755 added bhadra_flag/panchaka_flag;
-- migration 756 added vargottama_per_varga; migration 757 added parivartana_per_varga;
-- migration 758 added combustion_per_varga; migration 759 added graha_yuddha_per_varga;
-- migration 780 added nway_config_per_varga; migration 781 added kala_sarpa_per_varga;
-- migration 782 added tara_bala_natal_baseline; migration 783 added conjunction_within_orb;
-- migration 784 added aspect_tajik; migration 785 added graha_yoga_karaka_flag; migration 786
-- added graha_dispositor_chain; migration 787 added composite_dispositor_strength; migration
-- 788 added the Group H avastha bundle; migration 789 added nakshatra_dispositor_chain;
-- migration 790 added chandra_bala_natal_baseline; migration 791 added the Group O tri-deva
-- bundle; migration 792 added graha_functional_class_per_ascendant; migration 793 added
...
--
-- graha_special_state_rollup (Group M/T, `_build_special_state_rows`, the same function that
-- emits graha_effective_dignity_modified_by_aspects covered by migration 793) stores FIVE
-- independent boolean flags per graha: is_combust, is_retrograde, is_vargottama, is_debilitated,
-- is_exalted -- already flagged as "already investigated, not F-A14'd yet" in migration 793's
-- header comment.
--
-- Four of the five flags are fully re-derivable against an existing L1 authority, all verified
-- clean and non-vacuous:
--   - is_combust re-derives from ga_positions' own graha_position.combustion_state ('combust').
--   - is_retrograde re-derives from ga_positions' own graha_position.retrograde_flag ('retrograde').
--   - is_debilitated / is_exalted re-derive from the SAME classical exaltation/debilitation
--     tables (from graha_position.sign) already embedded in SQL since migration 794's conjunct
--     (b4) -- confirmed all 270 rows (135 x 2 keys) join-matched, 0 disagreements.
--
-- ****F-A18 (NEW FINDING, GENUINELY RED, NEVER SUPPRESSED)****: the fifth flag, is_vargottama,
-- uses the SAME buggy inline navamsha-degree formula (`nav_starts` sign-cycling table + float
-- division) that F-A15 already found and fixed for `graha_vargottama_amplification_factor` (a
-- DIFFERENT category, emitted by the DIFFERENT function `_build_shadbala_extension_rows`) --
-- but this SECOND occurrence, in `_build_special_state_rows`, was NEVER updated to read
-- ga_vargas' authoritative D9 `chart_divisionals.varga_vargottama_flag` the way the first one
-- now does (confirmed live: `_build_special_state_rows`'s `is_vargottama` still computes
-- `nav_para`/`nav_starts`/`nav_sign` inline, unchanged). This is a live, ongoing §N.5 violation
-- that will keep producing wrong values on EVERY future build until the writer itself is fixed
-- -- unlike F-A15/F-A17/F-157, which are stale pre-fix DATA waiting on a chart rebuild, this is
-- a genuinely unfixed WRITER bug, discovered here for the first time. Verified: disagrees with
-- ga_vargas' D9 authority on the EXACT SAME 4/105 rows as F-A15's own tracked residual
-- (1c826d5a/surya_siddhanta_classical/Venus; cb73cd3d/raman/Saturn; cb73cd3d/
-- surya_siddhanta_classical/Jupiter; cb73cd3d/surya_siddhanta_classical/Saturn) -- the SAME
-- underlying pre-fix computation, now confirmed duplicated into a second, still-unfixed call
-- site. Per "never weaken a gate, ship the real red conjunct" (the F-A15/F-A17/F-157
-- precedent), this conjunct is landed here honestly RED, not suppressed, narrowed, or deferred.
-- A follow-up writer fix (mirroring F-A15's own fix: replace the inline formula with a call to
-- the ALREADY-EXISTING `_get_varga_vargottama_flag(conn, chart_id, ayanamsha_id, "D9", g_name)`
-- helper) is a separate, future bounded unit of work -- not attempted in this same cycle, per
-- this campaign's own established discover-then-fix-later cadence (F-A15/F-A16/F-A17 were all
-- discovered during F-A14 migration authoring and fixed in later, separate cycles).
--
--   (a7) graha_special_state_rollup fact_value_text domain: must be 'true' or 'false' across
--        ALL five fact_keys -- no third value is ever legitimate for any of these boolean
--        flags. 0/675 violations live.
--   (b7) is_combust must equal ga_positions' own graha_position.combustion_state = 'combust' for
--        the same (chart, ayanamsha, subject). 0/135 violations live.
...
--        re-derivation from graha_position.sign (the same table already embedded since
--        migration 794's conjunct (b4)). 0/270 violations live (135 rows x 2 keys).
--   (e7) ****F-A18, GENUINELY RED TODAY****: is_vargottama must equal ga_vargas' own
--        authoritative D9 chart_divisionals.varga_vargottama_flag for the same (chart,
--        ayanamsha, graha) -- re-derived here directly rather than restated, mirroring
--        migration 792's conjunct (b) exactly. GENUINELY RED on 4/105 rows -- the writer itself
--        has not yet been fixed for this second call site (unlike F-A15's own now-fixed
--        original occurrence). Will clear only once the writer is fixed AND the 2 affected
--        charts next rebuild -- tracked here as a real, honest defect, not suppressed.
--
...
-- 786's/787's/788's/789's/790's/791's/792's/793's/794's/795's/796's/797's/798's ninety original
-- conjuncts (a) through (h6) forward VERBATIM, including conjuncts (b), (e), and (f), which
-- remain GENUINELY RED TODAY exactly as migration 798 already documented (F-A15/F-A17/F-157
-- fixes landed, tracked charts not yet rebuilt). Because (b), (e), (f), and now (e7)/F-A18 are
-- all red, the combined 96-conjunct SELECT evaluates to false on live production today; the new
-- conjuncts were verified INDIVIDUALLY (their own subquery in isolation) rather than via the
...
-- aspect_parashari_given / aspect_parashari_received / graha_special_state_rollup, 40 of 57
-- owned categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare
-- count pin (C12). No distinctness conjunct -- none of these forty categories' natural key

-- 780_nirmana_l1_ga_structural_integrity_contract_nway.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Sixth follow-up F-A14 widening pass for ga_structural, first
-- in the new 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- combustion_per_varga; migration 759 added graha_yuddha_per_varga (7/57). This adds
-- nway_config_per_varga, taking coverage to 8/57.
--
-- nway_config_per_varga (_build_nway_per_varga_rows) detects stellium clusters: 3+ classical
-- grahas sharing the same sign within a varga.
--
-- Three conjuncts:
--   (m) fact_value_num (the graha count) must be >= 3 -- re-derives the writer's own
--       `if len(grahas) >= 3` filter as a domain check. 0/223 violations live.
--   (n) fact_value_num must equal the length of the value_jsonb.grahas array -- internal
--       self-consistency between the two places this same count is stored. 0/223 violations
--       live.
--   (o) every graha named in value_jsonb.grahas (varga != 'D1' -- see migration 758's header note
--       on the D1 dual-source exclusion, which applies identically here) must actually sit in
--       the stellium's own claimed sign, per ga_vargas' own chart_divisionals varga_position
--       data for the SAME varga -- re-derived directly rather than restated. Checked unscoped
--       first: 1/764 violations, on D1 (the fourth _per_varga category to hit this exact shape,
--       after vargottama_per_varga/F-A17, combustion_per_varga, and graha_yuddha_per_varga) --
--       scoped out on that established basis rather than re-investigating. 0/746 violations live
--       once D1 is excluded (D2 onward).
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries migrations 745's/755's/756's/757's/758's/759's twelve original conjuncts (a) through
-- (l) forward VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED TODAY
-- exactly as migration 759 already documented (F-A15/F-A17/F-157 fixes landed, tracked charts not
-- yet rebuilt). Because (b), (e), (f) are all red, the combined 15-conjunct SELECT evaluates to
-- false on live production today; conjuncts (m)/(n)/(o) were verified INDIVIDUALLY (their own NOT
...
-- 8 of 57 owned categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No
-- bare count pin (C12). No distinctness conjunct -- none of these eight categories' natural key

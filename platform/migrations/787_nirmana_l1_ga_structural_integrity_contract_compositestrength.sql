-- 787_nirmana_l1_ga_structural_integrity_contract_compositestrength.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirteenth follow-up F-A14 widening pass for ga_structural,
-- eighth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- graha_dispositor_chain (14/57). This adds composite_dispositor_strength, taking coverage to
-- 15/57.
--
-- composite_dispositor_strength stores one numeric row per graha (fact_key = 'terminal_strength'
-- -- a naming artifact from the pre-EL-40 formula the writer's own comment discloses; the value is
-- actually the MEAN of dignity-strength over every chain member, not the terminal graha's
-- strength alone): composite_strength = mean(dignity_strength(member) for member in the SAME
-- graha's graha_dispositor_chain.chain array), where dignity_strength maps chart_output's own
-- dignity_status field through the writer's fixed {"exalted":1.0,"own_sign":0.875,"neutral":0.5,
-- "debilitated":0.25} table.
--
-- dignity_status itself is never independently persisted as its own fact_category (unlike
-- graha_dignity_per_varga, which uses a DIFFERENT, independently-computed 5-way
-- classify_dignity() scheme -- already investigated and correctly ruled a genuine vocabulary
-- mismatch, not cross-referenceable, per cycle 49's finding on graha_dignity_per_varga vs
-- ga_vargas' varga_dignity), so a full per-member re-derivation is not achievable without
-- reimplementing PyJHora's own exaltation/debilitation tables from scratch -- disproportionate
-- scope for one bounded conjunct pass, the same judgment already applied to kala_sarpa_per_varga's
-- arc-membership walk (migration 781). Instead this migration ships three self-consistency/
-- domain/cross-category conjuncts against the row's own value plus its already-verified
-- graha_dispositor_chain sibling (migration 786):
--
--   (nn) fact_value_num domain: must be within [0.25, 1.0] -- the achievable range of a mean over
--        one or more values drawn from the writer's fixed four-value dignity-strength set (no
--        chain is ever empty; the writer's own `chain: list[str] = [g_name]` guarantees at least
--        one member). 0/135 violations live.
...
--        composite_dispositor_strength row must have a matching graha_dispositor_chain row for
--        the same (chart, ayanamsha, build, subject), and vice versa. 0/135 violations live in
--        either direction.
--   (pp) cross-category re-derivation: composite_strength * chain_length (from the sibling
--        graha_dispositor_chain row's own `length` field) must be within a length-scaled
--        tolerance of an exact multiple of 0.125 -- every one of the writer's four
--        dignity-strength values (0.25/0.5/0.875/1.0) is itself a multiple of 0.125, so a genuine
--        mean over `length` such values, once multiplied back by `length`, must reconstruct to a
--        multiple of 0.125. The tolerance (0.0001 * length) is sized to the writer's own
--        round(mean, 4) storage precision (composite_strength itself carries up to ±0.00005
--        rounding error, which the length multiplication propagates linearly) -- confirmed
--        against live data: several rows (e.g. Sun/raman, composite 0.7188, length 4) land at
--        0.0016 in naive ratio-space purely from this expected rounding, motivating the
--        length-scaled tolerance rather than a fixed one. 0/135 violations live.
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries migrations 745's/755's/756's/757's/758's/759's/780's/781's/782's/783's/784's/785's/786's
-- thirty-nine original conjuncts (a) through (mm) forward VERBATIM, including conjuncts (b), (e),
-- and (f), which remain GENUINELY RED TODAY exactly as migration 786 already documented
-- (F-A15/F-A17/F-157 fixes landed, tracked charts not yet rebuilt). Because (b), (e), (f) are all
-- red, the combined 42-conjunct SELECT evaluates to false on live production today; conjuncts
-- (nn) through (pp) were verified INDIVIDUALLY (their own subquery in isolation) rather than via
-- the full combined SELECT.
--
...
-- graha_yoga_karaka_flag / graha_dispositor_chain / composite_dispositor_strength, 15 of 57 owned
-- categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count pin
-- (C12). No distinctness conjunct -- none of these fifteen categories' natural key needs

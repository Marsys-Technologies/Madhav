-- 808_nirmana_l1_ga_structural_integrity_contract_chartcluster.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-fourth follow-up F-A14 widening pass for
-- ga_structural, ninth in the 800-819 range (adjudication #2057, continuation 4). Migration 745
...
--
-- chart_cluster (`_build_chart_cluster_per_varga_rows`) computes Parashari aspect-graph
-- connected components (union-find) per varga, using the EXACT SAME edge rule as migration 807's
-- graha_centrality (same `_graha_aspects_house` calls over the same adjacency). The row stores
-- {varga, cluster_id, total_clusters} but no per-graha edge list, so its conjuncts lean on two
-- kinds of check: internal self-consistency/completeness within chart_cluster's own rows, and a
-- genuine cross-CATEGORY re-derivation against the sibling graha_centrality category's own
-- `connected_to`/`degree_centrality` fields (built from the identical graph) rather than
-- rebuilding the union-find from scratch in SQL.
--
--   (a15) fact_value_num must equal value_jsonb.cluster_id -- the same id stored twice. 0/3915
--         violations live.
...
--         group must report the identical total_clusters value -- it is a chart-varga-wide
--         constant computed once by the writer, not a per-graha value. 0/... groups violate.
...
--         confirms no gap or phantom count between the writer's counter and what actually got
--         stored. 0/... groups violate.
--   (e15) direct-edge-implies-same-cluster: for every direct edge in the sibling
--         graha_centrality category's own connected_to array, both endpoint grahas must share
--         the same cluster_id in chart_cluster for the SAME (chart, ayanamsha, build, varga) --
--         a genuine cross-category invariant from the two categories sharing one union-find
--         input graph. 0/11500 edge-endpoints checked, 0 violations live.
...
--         (chart, ayanamsha, build, varga) group may share its cluster_id -- a zero-degree node
--         can only ever form its own singleton connected component. 0/164 isolated grahas
--         violate.
--
...
-- 801's/802's/803's/804's/805's/806's/807's one-hundred-and-forty-six original conjuncts (a)
-- through (f14) forward VERBATIM, including conjuncts (b), (e), (f), and (e7)/F-A18, which
-- remain GENUINELY RED TODAY exactly as migration 807 already documented. Because (b), (e), (f),
-- and (e7) are all red, the combined 152-conjunct SELECT evaluates to false on live production

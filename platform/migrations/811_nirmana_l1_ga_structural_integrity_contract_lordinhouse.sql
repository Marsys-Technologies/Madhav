-- 811_nirmana_l1_ga_structural_integrity_contract_lordinhouse.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-seventh follow-up F-A14 widening pass for
-- ga_structural, twelfth in the 800-819 range (adjudication #2057, continuation 4). Migration
...
-- to 52/57.
--
-- lord_in_house_per_varga (`_build_lord_relationship_rows`'s "lord_in_house_per_varga — 12
-- rows" block, the sibling category to migration 805's lord_aspects_lord_per_varga from the SAME
-- function) is FULLY SELF-CONTAINED: value_jsonb stores {varga, house, sign, lord,
-- lord_house_in_varga}, everything needed to re-verify the row's own classical house-lord
-- assignment and its parent function's own f-string constructions. Because `house_sign(h)` uses
-- the D1 lagna ALONE (`SIGN_NAMES[(lagna_sign_num - 1 + h - 1) % 12]`) regardless of which varga
-- is being processed, the sign at a given house number is a genuine VARGA-INDEPENDENT constant
-- for the whole chart -- a powerful cross-row invariant unique to this category, re-derived here
-- as conjunct (e18) rather than the more common cross-category or cross-asset reference pattern.
--
--   (a18) value_jsonb.lord must equal the classical SIGN_LORDS lookup of value_jsonb.sign --
--         the same 12-sign table already used throughout the arc, re-derived purely from the
--         row's own stored sign field.
...
--         value_jsonb.sign must be IDENTICAL across every varga -- house_sign(h) is computed
--         from the D1 lagna alone and never varies by varga, a genuine structural certainty of
--         the writer's own single house_sign() closure shared by every varga call.
...
--         (Sun/Moon/Mars/Mercury/Jupiter/Venus/Saturn) -- SIGN_LORDS never maps to a node, the
--         same domain already established for migration 805's lord_a/lord_b.
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's/810's one-hundred-and-sixty-seven
-- original conjuncts (a) through (f17) forward VERBATIM, including conjuncts (b), (e), (f), and
-- (e7)/F-A18, which remain GENUINELY RED TODAY exactly as migration 810 already documented.
-- Because (b), (e), (f), and (e7) are all red, the combined 174-conjunct SELECT evaluates to

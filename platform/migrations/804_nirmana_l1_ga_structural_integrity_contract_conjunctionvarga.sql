-- 804_nirmana_l1_ga_structural_integrity_contract_conjunctionvarga.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirtieth follow-up F-A14 widening pass for ga_structural,
-- fifth in the 800-819 range (adjudication #2057, continuation 4). Migration 745 covered
...
--
-- conjunction_per_varga (`_build_varga_relationship_rows`'s "Conjunctions per varga" block,
-- inside the R1 multi-varga enumeration) stores, for every pair of grahas sharing a sign in a
-- given varga (D1 through D2700, one call per varga), a row keyed "{VARGA}_{S1}_{S2}"
-- (fact_key "orb_deg"). D1 is a SPECIAL CASE (same "D1 dual-independent-PyJHora-source caveat"
-- already established by migrations 758/759/783/793/800): the writer computes a real
-- degree-based orb from raw longitudes and emits ANY pair within 10°, REGARDLESS of whether the
-- two grahas actually share a sign (same_sign is stored but not gating for D1) -- confirmed live:
-- 5/30 D1 rows genuinely have same_sign=false. For every OTHER varga, the writer's own `if sign1
-- != sign2: continue` guard means only genuine same-sign pairs are ever emitted, with orb_deg
-- hardcoded to 0.0 and same_sign always true -- confirmed live: 1689/1689 non-D1 rows have
-- same_sign=true, value_num=0.0, exactly as the writer's own branch guarantees.
--
--   (a11) unit/varga correlation: unit must equal 'deg' iff varga='D1', 'same_sign' iff
--         varga<>'D1' -- the writer's own `unit="deg" if varga == "D1" else "same_sign"`
--         ternary, re-derived as a data check on what actually got stored. 0/1719 violations
--         live.
--   (b11) value_num domain: for varga='D1', must be within [0, 10.0] (the writer's own `if orb >
--         10.0: continue` filter); for varga<>'D1', must equal exactly 0.0 (the writer's own
--         hardcoded `orb_val = 0.0` for the same-sign branch). 0/1719 violations live.
--   (c11) same_sign domain for non-D1: must be true for every varga<>'D1' row -- the writer's
--         own `if sign1 != sign2: continue` guard, which by construction means every row that
--         survives to be appended for a non-D1 varga is same_sign=true. Deliberately scoped away
--         from D1, where same_sign is NOT gating (5/30 D1 rows are genuinely same_sign=false --
--         a real classical possibility, not a defect). 0/1689 violations live.
...
--         RAH_MEAN/KET_MEAN-aware split already established by migration 783's conjuncts
--         (x)/(y)) must never be equal -- the writer's own `for g2 in ALL_GRAHAS[i+1:]` loop
--         structure guarantees this for every future build. 0/1719 violations live.
--   (e11) no reversed-duplicate pair: for a stored "{VARGA}_{S1}_{S2}" row, "{VARGA}_{S2}_{S1}"
--         must never also be stored for the same (chart, ayanamsha, build) -- re-derives the
--         writer's own unordered-pair-emitted-once loop guarantee, the same invariant migration
--         783's conjunct (x) already established for conjunction_within_orb, now re-verified for
--         the per-varga category. 0/1719 violations live.
--   (f11) pair ordering invariant: s1's index in the writer's own ALL_GRAHAS ordering
--         (Sun/Moon/Mars/Mercury/Jupiter/Venus/Saturn/Rahu/Ketu) must strictly precede s2's
--         index -- the same invariant as migration 783's conjunct (y), re-verified for the
--         per-varga category. 0/1719 violations live.
...
--         re-derived directly from the classical source table (the same varga_position
--         authority already used by migration 793's D1-Sun/graha degree join), catching any
--         drift between the writer's own in-memory varga_state and the canonical chart_divisionals
--         record. Excludes D1 per the established dual-source caveat. 0/1689 violations live
--         (all 1689 non-D1 rows matched).
--   (h11) house cross-reference, non-D1: value_jsonb.house must equal ga_vargas' own
--         authoritative chart_divisionals varga_position.house_from_varga_lagna for the SAME
--         (chart, ayanamsha, varga, graha) -- re-derived directly rather than restated. Excludes
--         D1 per the same dual-source caveat. 0/1689 violations live (all 1689 non-D1 rows
--         matched).
--
...
-- 801's/802's/803's one-hundred-and-seventeen original conjuncts (a) through (ff4) forward
-- VERBATIM, including conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY
-- exactly as migration 803 already documented. Because (b), (e), (f), and (e7) are all red, the
-- combined 125-conjunct SELECT evaluates to false on live production today; the new conjuncts

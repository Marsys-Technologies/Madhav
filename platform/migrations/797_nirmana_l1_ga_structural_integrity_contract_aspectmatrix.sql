-- 797_nirmana_l1_ga_structural_integrity_contract_aspectmatrix.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-third follow-up F-A14 widening pass for
-- ga_structural, eighteenth in the 780-799 range (adjudication #2012). Migration 745 covered
...
-- migration 796 added the Group C Bhava Bala extended bundle (8 categories, 36/57). This adds
-- aspect_matrix_summary, taking coverage to 37/57.
--
-- aspect_matrix_summary.aspects_received_count (Group A: Aspects, near the end of
-- `_build_aspect_rows`) is a per-house tally: for each house 1-12, the writer counts how many
-- IN-MEMORY `aspect_parashari_received` rows (already built earlier in the SAME function call)
-- have `fact_subject == house_key`. Since `aspect_parashari_received` is itself a real, already-
-- stored chart_facts category (285 live rows), this count is fully re-derivable by counting the
-- actual STORED sibling rows for the same (chart, ayanamsha, build, house) rather than trusting
-- the writer's own in-memory tally -- a genuine same-asset cross-category re-derivation (the
-- exact shape already established for e.g. migration 787's composite_dispositor_strength /
-- graha_dispositor_chain pairing). Confirmed the join is non-vacuous before landing: 150/180
-- live rows have a nonzero received count (only 30 houses genuinely receive zero aspects), and
-- the value distribution (0-5) is non-degenerate.
--
--   (k) fact_value_num domain: must be a non-negative integer <= 9 -- the theoretical ceiling
--       given ALL_GRAHAS has 9 members (7 classical + Rahu/Ketu), each contributing at most one
--       aspect per house per the writer's own per-graha aspect-offset loop. 0/180 violations
--       live.
...
--       re-derived directly from the sibling category rather than restated. 0/180 violations
--       live, confirmed non-vacuous (150/180 nonzero matches).
--
...
-- 786's/787's/788's/789's/790's/791's/792's/793's/794's/795's/796's eighty original conjuncts
-- (a) through (j2) forward VERBATIM, including conjuncts (b), (e), and (f), which remain
-- GENUINELY RED TODAY exactly as migration 796 already documented (F-A15/F-A17/F-157 fixes
-- landed, tracked charts not yet rebuilt). Because (b), (e), (f) are all red, the combined
-- 82-conjunct SELECT evaluates to false on live production today; conjuncts (k)/(l) were
...
-- bhava_bala_total_extended / house_strength_classification_rollup / aspect_matrix_summary, 37
-- of 57 owned categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No
-- bare count pin (C12). No distinctness conjunct -- none of these thirty-seven categories'
-- natural key needs re-asserting (chart_facts_unique_null_formula already covers it).
SELECT

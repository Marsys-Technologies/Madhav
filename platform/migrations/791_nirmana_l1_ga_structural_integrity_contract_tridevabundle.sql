-- 791_nirmana_l1_ga_structural_integrity_contract_tridevabundle.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Seventeenth follow-up F-A14 widening pass for ga_structural,
-- twelfth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- 790 added chandra_bala_natal_baseline (21/57). This adds THREE categories at once --
-- pranic_strength_per_graha, jaimini_tri_deva_role_per_graha, graha_tri_deva_role_strength --
-- taking coverage to 24/57.
--
-- All three are emitted together by the SAME per-graha loop in `_build_esoteric_rows` (Group O:
-- Esoteric / Jaimini), each D1-sourced from chart_output's own already-loaded dignity_status/
-- house fields plus the writer's own fixed PRANIC_BASE_SCORES / TRI_DEVA_ROLES tables -- not two
-- independent PyJHora position computations. Bundled into one migration because
-- graha_tri_deva_role_strength has a genuine, direct cross-field dependency on BOTH sibling
-- categories (role_strength = prana_score * role_multiplier(tri_deva_role)), making the three a
-- cohesive unit rather than disconnected passes -- the same bundling discipline established by
-- migration 788's Group H avastha bundle.
--
-- Notable classical-table quirk confirmed before designing the re-derivation conjunct: Jupiter
-- appears in BOTH TRI_DEVA_ROLES["brahma"] and TRI_DEVA_ROLES["vishnu"], but the writer's own
-- `for role, planets in TRI_DEVA_ROLES.items(): ... break` loop always resolves it to "brahma"
-- (Python dict iteration is insertion-ordered, and "brahma" is defined first) -- confirmed
-- against all 135 live rows that Jupiter is stored as "brahma" 100% of the time, never "vishnu".
-- The re-derivation conjunct below matches this exact, deterministic tie-break rather than
-- either ignoring the ambiguity or treating Jupiter's membership in "vishnu" as equally valid.
--
--   (vv2) pranic_strength_per_graha.prana_score domain: must be within [0.3375, 1.2] -- the
--         achievable range given the writer's own PRANIC_BASE_SCORES (0.50-0.80), dignity
--         modifiers (0.75-1.25), and house modifiers (0.9-1.2), all multiplied together.
--         0/135 violations live.
--   (ww2) jaimini_tri_deva_role_per_graha.tri_deva_role domain: must be one of the writer's own
--         four values ('brahma', 'vishnu', 'shiva', 'neutral') -- no fifth value is ever
...
--         brahma-checked-first tie-break for Jupiter's dual membership, re-derived directly from
--         fact_subject rather than restated. 0/135 violations live.
--   (yy2) graha_tri_deva_role_strength.role_strength cross-field re-derivation: must equal the
--         SAME graha's own pranic_strength_per_graha.prana_score multiplied by the writer's own
--         fixed per-role multiplier (brahma=1.1, vishnu=1.2, shiva=0.9, neutral=1.0) read from
--         the SAME graha's own jaimini_tri_deva_role_per_graha.tri_deva_role -- a genuine
--         two-category cross-reference, re-derived directly rather than restated. 0/135
--         violations live.
--
...
-- 786's/787's/788's/789's/790's fifty-seven original conjuncts (a) through (uu2) forward
-- VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED TODAY exactly as
-- migration 790 already documented (F-A15/F-A17/F-157 fixes landed, tracked charts not yet
-- rebuilt). Because (b), (e), (f) are all red, the combined 61-conjunct SELECT evaluates to
-- false on live production today; conjuncts (vv2) through (yy2) were verified INDIVIDUALLY
...
-- graha_tri_deva_role_strength, 24 of 57 owned categories). D-CND-03: chart-partitioned /
-- row-wise, attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these twenty-four categories' natural key needs re-asserting (chart_facts_unique_null_formula
-- already covers it).
SELECT

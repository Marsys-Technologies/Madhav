-- 806_nirmana_l1_ga_structural_integrity_contract_dispositorchainvarga.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-second follow-up F-A14 widening pass for
-- ga_structural, seventh in the 800-819 range (adjudication #2057, continuation 4). Migration
...
-- added conjunction_per_varga; migration 805 added lord_aspects_lord_per_varga (46/57). This adds
-- dispositor_chain_per_varga, taking coverage to 47/57.
--
-- dispositor_chain_per_varga (the "Dispositor chains per varga" block in
-- `_build_varga_relationship_rows`) is the per-varga sibling of migration 786's
-- graha_dispositor_chain: for each graha, walks its classical SIGN_LORDS dispositor chain up to
-- 8 hops, stopping on cycle detection. Unlike migration 786's version, this per-varga row does
-- NOT store a parallel "signs" array alongside "chain" -- only {varga, chain, chain_length,
-- start_sign}. The classical chain-step re-derivation therefore cross-references EACH chain
-- member's sign-in-this-varga via the SAME asset's own `graha_dignity_per_varga` category
-- (sibling emission, same varga, same build) rather than a self-contained parallel array --
-- confirmed live: all 3915 rows' chain[0] resolve to a matching graha_dignity_per_varga row
-- (0 unmatched), and all 8179 chain-step transitions (sum of chain_length-1 across all rows)
-- resolve and agree with the classical SIGN_LORDS table (0 violations).
--
--   (a13) fact_value_text must equal the chain array joined with '→' -- the same chain stored
--         twice (human-readable string + structured array). 0/3915 violations live.
...
--         initialization), re-derived from fact_subject rather than restated. 0/3915 violations
--         live.
--   (d13) no duplicate elements in the chain array -- the writer's own `if lord in visited:
--         break` guard (checked BEFORE appending) means a graha can never appear twice in a
--         stored chain. 0/3915 violations live.
...
--         the sibling `graha_dignity_per_varga` category's own stored `sign` field for the SAME
--         (chart, ayanamsha, build, varga, graha) -- re-derived from a genuine sibling-category
--         reference rather than restated. 0/3915 violations live (all 3915 rows matched).
...
--         (SIGN_LORDS -- the same 12-sign classical table already used by migrations 793's
--         conjunct (g)/786's conjunct (ll)), where the previous graha's sign is sourced from the
--         sibling `graha_dignity_per_varga` category rather than a parallel array -- walked
--         step-by-step via generate_series over the JSON array. 0/8179 pairs checked (sum of
--         chain_length-1 across all 3915 rows), 0 violations live, all 8179 steps matched.
--
...
-- 801's/802's/803's/804's/805's one-hundred-and-thirty-three original conjuncts (a) through
-- (h12) forward VERBATIM, including conjuncts (b), (e), (f), and (e7)/F-A18, which remain
-- GENUINELY RED TODAY exactly as migration 805 already documented. Because (b), (e), (f), and
-- (e7) are all red, the combined 140-conjunct SELECT evaluates to false on live production

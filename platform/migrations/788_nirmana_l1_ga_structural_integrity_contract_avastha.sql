-- 788_nirmana_l1_ga_structural_integrity_contract_avastha.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Fourteenth follow-up F-A14 widening pass for ga_structural,
-- ninth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- graha_dispositor_chain; migration 787 added composite_dispositor_strength (15/57). This adds
-- FOUR categories at once -- graha_avastha_baladi, graha_avastha_jagrad, graha_avastha_deepta,
-- graha_avastha_lifetime_exposure_summary -- taking coverage to 19/57.
--
-- All four are emitted together by the SAME `_build_avastha_rows` loop over grahas_data (Group H:
-- Avasthas), each a D1-sourced classification derived from chart_output's own already-loaded
-- dignity_status/house/retrograde fields -- not two independent PyJHora position computations, so
-- none of the four carries the D1 dual-source disclaimer. Bundled into one migration because the
-- categories are tightly coupled (graha_avastha_lifetime_exposure_summary's own value_jsonb
-- literally re-quotes the other three's current-cycle values as a same-loop-iteration copy) and
-- widening them together is a genuinely bounded, cohesive unit of work rather than four
-- disconnected passes.
--
--   (qq) graha_avastha_baladi.baladi_state domain: must be one of the writer's own BALADI_STATES
--        five values ('bal','kumar','yuva','vriddha','mrit') -- no sixth value is ever
...
--        branch outcomes ('deepta','svastha','mudita','shanta','vikala','kopa','dina') -- despite
--        the writer's own comment describing "9 states", only these seven are ever reachable from
--        the actual if/elif chain; confirmed against all 135 live rows before fixing the domain
--        at seven rather than trusting the comment's stale count. 0/135 violations live.
--   (tt)/(uu)/(vv) graha_avastha_lifetime_exposure_summary.value_jsonb.current_baladi /
--        current_jagrad / current_deepta must each equal the SAME graha's own
--        graha_avastha_baladi / graha_avastha_jagrad / graha_avastha_deepta fact_value_text for
--        the same (chart, ayanamsha, build) -- the writer's own same-loop-iteration copy,
--        re-derived as a genuine cross-category consistency check rather than restated. 0/135
--        violations live for each of the three.
...
--        (dignity IN ('exalted','own_sign')) as their very first branch, so this is a genuine
--        two-way implication derivable purely from the writer's own branch ORDERING (the deepta
--        function's house/retro/debilitated branches are only ever reached when dignity is
--        neither exalted nor own_sign, i.e. exactly when jagrad_state is NOT 'jagrad') --
--        verified by hand-tracing both branch chains, not assumed. Note this is deliberately an
--        iff only for the 'jagrad' case; the 'sushupta'/'swapna' split does not map to a single
--        deepta value each (a debilitated graha can still hit an earlier house/retro branch in
--        the deepta chain), so no analogous full iff was attempted for those two -- shipping the
--        one relationship that is genuinely and provably an iff rather than forcing a broader,
--        false one. 0/135 violations live.
--
...
-- 786's/787's forty-two original conjuncts (a) through (pp) forward VERBATIM, including
-- conjuncts (b), (e), and (f), which remain GENUINELY RED TODAY exactly as migration 787 already
-- documented (F-A15/F-A17/F-157 fixes landed, tracked charts not yet rebuilt). Because (b), (e),
-- (f) are all red, the combined 49-conjunct SELECT evaluates to false on live production today;
-- conjuncts (qq) through (ww) were verified INDIVIDUALLY (their own subquery in isolation) rather
...
-- graha_avastha_lifetime_exposure_summary, 19 of 57 owned categories). D-CND-03: chart-partitioned
-- / row-wise, attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none
-- of these nineteen categories' natural key needs re-asserting (chart_facts_unique_null_formula
-- already covers it).
SELECT

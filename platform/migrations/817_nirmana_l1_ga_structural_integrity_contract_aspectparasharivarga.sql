-- 817_nirmana_l1_ga_structural_integrity_contract_aspectparasharivarga.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-third follow-up F-A14 widening pass for
-- ga_structural, eighteenth in the 800-819 range (adjudication #2057, continuation 4). Migration
-- 816 added aspect_jaimini_per_varga (57/64 by the corrected running tally -- see D-L1-106 for
-- the 54-vs-55 off-by-one correction, and D-L1-105 for the underlying 64-not-57 denominator
-- finding), confirmed immune to F-A24. This adds aspect_parashari_per_varga.
--
-- aspect_parashari_per_varga (the "Parashari aspects per varga" block inside
-- `_build_varga_relationship_rows`) emits, for every graha present in a varga, one row per
-- classical Parashari aspect it casts (the universal 7th for Sun/Moon/Mercury/Venus; the special
-- 4th/8th for Mars; 5th/9th for Jupiter/Rahu/Ketu; 3rd/10th for Saturn -- the SAME
-- `get_graha_aspects` canonical authority already reused since migration 807). Unlike
-- karaka_bhava_concordance (migration 815, F-A24), this category has NO Lagna dependency at all
-- -- house/sign come from the graha's own varga position, not from any bhava-number-to-sign
-- mapping -- so it is immune to that bug class, confirmed by this migration's own conjuncts (see
-- (f24) below, cross-referencing the sibling `graha_dignity_per_varga` category).
--
--   (a24) fact_value_num domain: must equal 1.0 -- every offset in the writer's own
--         `get_graha_aspects` table (`brahmagyan/aspects.py`) carries strength 1.0; no lesser
--         strength is ever emitted.
...
--         aspect-casting arithmetic, re-derived directly from the row's own stored source_house
--         and offset fields.
--   (d24) offset domain per graha: value_jsonb.offset must be one of the writer's own
--         `get_graha_aspects` offsets for that graha's token (hardcoded here as the authority,
--         the same table already reused since migration 807).
--   (e24) graha-token domain: the token after "{varga}_" must be one of the nine known
--         PLANET_TO_SUBJECT tokens -- a genuine domain check (an earlier draft reconstructed
--         fact_subject from its own suffix, caught as tautological during mutation-testing and
--         fixed before landing, the same defect class migration 814's original (e21) hit).
--   (f24) source_house/source_sign cross-reference: must match the SAME (chart, ayanamsha,
--         build, varga, graha)'s sibling `graha_dignity_per_varga` category exactly -- confirming
--         this category has no independent (and therefore no Lagna-dependent) position source.
...
--         must equal the writer's own classical offset-count for that graha (3 for
--         Mars/Jupiter/Saturn/Rahu/Ketu, 1 for Sun/Moon/Mercury/Venus).
--
...
-- two-hundred-and-fifteen original conjuncts (a) through (j23) forward VERBATIM, including
-- conjuncts (b), (e), (f), (e7), and (j22), which remain GENUINELY RED TODAY exactly as migration
-- 816 already documented (F-A15/F-A17/F-157/F-A18/F-A24). Because these five are all red, the
-- combined 223-conjunct SELECT evaluates to false on live production today; the new conjuncts
...
-- 58 of 64 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these fifty-eight categories' natural key needs re-asserting

-- 810_nirmana_l1_ga_structural_integrity_contract_houcompstrength.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-sixth follow-up F-A14 widening pass for
-- ga_structural, eleventh in the 800-819 range (adjudication #2057, continuation 4). Migration
...
--
-- graha_in_house_composite_strength (`_build_composite_strength_rows`, per the writer's own
-- extensive M-14 commentary) stores, for every (graha, house) pair, THREE sibling rows sharing
-- one fact_subject "{GRAHA}_IN_HOUSE_{h}": bphs_weighted, simple_multiplication, and
-- cross_formula_divergence -- OR, when the real GA3 shadbala/bhava_bala facts are missing for
-- that graha/house, exactly ONE floored bphs_weighted row (value_num NULL, canonical-or-floor
-- per the writer's own documented policy; 0/1620 comp_subjects floored on the canonical charts
-- today). The writer's own comments establish the exact algebraic relationship between the two
-- formulas: bphs_score = simple_score x shadbala_ratio x aspect_modifier, where shadbala_ratio
-- in [0,1] and aspect_modifier in {0.75, 1.0} -- meaning bphs_score can NEVER exceed
-- simple_score (both factors are <= 1), a genuine non-trivial invariant re-derived here as
-- conjunct (b17) rather than restated. cross_formula_divergence is by the writer's own
-- definition `abs(bphs_score - simple_score)`, fully re-derivable from the two sibling rows
-- without needing the underlying shadbala_ratio/bhava_ratio at all (conjunct a17).
--
--   (a17) cross_formula_divergence must equal abs(bphs_weighted - simple_multiplication) for the
--         SAME comp_subject -- the writer's own `divergence = abs(bphs_score - simple_score)`
--         definition, re-derived directly from the two sibling rows rather than restated.
--   (b17) bphs_weighted must never exceed simple_multiplication -- a genuine algebraic certainty
--         from the writer's own formula relationship (bphs = simple x shadbala_ratio x
...
--         must contain EXACTLY 3 rows with no floored member, OR exactly 1 floored row -- no
--         partial state is ever legitimate, re-deriving the writer's own
--         all-three-or-floored-alone emission structure.
--   (e17) fact_subject format: must match "{GRAHA_TOKEN}_IN_HOUSE_{1-12}" -- the writer's own
--         `f"{subject}_IN_{house_key}"` construction, re-derived as a domain check on what
--         actually got stored.
--   (f17) floored-row invariant: value_num must be NULL if and only if
--         verification_pass_status='floored' -- the writer's own canonical-or-floor branch,
--         re-derived as a data check on what actually got stored (0 floored rows exist on the
--         canonical charts today, but the invariant holds vacuously and is mutation-provable
--         regardless).
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's one-hundred-and-sixty-one original
-- conjuncts (a) through (i16) forward VERBATIM, including conjuncts (b), (e), (f), and
-- (e7)/F-A18, which remain GENUINELY RED TODAY exactly as migration 809 already documented.
-- Because (b), (e), (f), and (e7) are all red, the combined 167-conjunct SELECT evaluates to
-- false on live production today; the new conjuncts were verified INDIVIDUALLY (their own

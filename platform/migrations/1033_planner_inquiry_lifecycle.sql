...
BEGIN
  ...
  IF NOT has_column_privilege('purna_inquiry_owner', 'public.profiles', 'id', 'REFERENCES')
    OR NOT has_column_privilege('purna_inquiry_owner', 'public.charts', 'id', 'REFERENCES') THEN
    RAISE EXCEPTION 'Pūrṇa lifecycle migration requires exact referenced-table privileges';
  END IF;
END $$;

-- 815_nirmana_l1_ga_structural_integrity_contract_karakabhavaconcordance.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Forty-first follow-up F-A14 widening pass for
-- ga_structural, sixteenth in the 800-819 range (adjudication #2057, continuation 4). Migration
-- 814 added convergence_count (55/64 by the corrected running tally -- see D-L1-106 for the
-- 54-vs-55 off-by-one correction, and D-L1-105 for the underlying 64-not-57 denominator
-- finding). This adds karaka_bhava_concordance.
--
-- karaka_bhava_concordance (`_build_karaka_bhava_concordance_per_varga_rows`) compares each
-- significance's natural karaka against its classical bhava's sign lord, via the natural
-- Parashari friendship table (BPHS Ch.3, `NATURAL_PLANET_RELATIONS`). All of bhava_num,
-- natural_karaka, bhava_lord, and the full concordance decision tree are fully re-derivable in
-- SQL from the writer's own classical dicts (SIGNIFICANCE_TO_BHAVA, NATURAL_KARAKAS, SIGN_LORDS,
-- NATURAL_PLANET_RELATIONS) -- no cross-category join needed for the writer's OWN arithmetic to
-- be checked, matching the "fully self-contained category" pattern of migrations 793/802/805.
--
-- **F-A24 (GENUINELY RED, a real writer bug, confirmed and profiled before shipping)**: unlike
-- migration 811's `_build_lord_relationship_rows` (which correctly reads D1's actual ascendant
-- via `chart_output.get("ascendant", {}).get("sign_id", ...)`, valid for every varga per its own
-- documented §N.4 "structural approximation" design), `_build_karaka_bhava_concordance_per_varga_rows`
-- has NO access to `chart_output` at all -- its signature only receives `varga_state`. It tries
-- `varga_state.get("Lagna") or varga_state.get("LAGNA")`, which only resolves for D1 (where
-- `_extract_chart_state` sets a LAGNA pseudo-entry); for every OTHER varga, `_load_varga_positions`
-- is graha-only (D-L1-104's own finding) and carries no Lagna key at all, so `lagna_data` is
-- `None` and the code silently falls back to `lagna_sign_num = 1` (Aries) for EVERY non-D1 varga,
-- regardless of the chart's actual D1 lagna. This is invisible on two of the three test charts
-- (`482012f1`/`1c826d5a`) purely because their own D1 lagna genuinely IS Aries -- but chart
-- `cb73cd3d`'s D1 lagna is Cancer, and its 28 non-D1 vargas' `bhava_sign`/`bhava_lord` values are
-- confirmed WRONG (still Aries-derived) on exactly 4200 rows (28 vargas x 30 significances x 5
-- ayanamshas), profiled and root-caused BEFORE writing any conjunct, the same discipline D-L1-104
-- established: 0 violations on D1 and on the two Aries-lagna charts, 4200/4200 on cb73cd3d's
-- non-D1 rows, confirmed via a ground-truth cross-reference to `lord_in_house_per_varga`'s own
-- (already-correct) D1 H1 sign field rather than assumed. Shipped honestly RED per the
-- never-weaken-a-gate doctrine, matching the F-A15/F-A17/F-157/F-A18 precedent -- the writer fix
-- (threading the real D1 ascendant into this function, mirroring migration 811's own correct
-- pattern) is left as a follow-up, not attempted in this migration-authoring cycle.
--
...
--   (e22) bhava_lord cross-check: value_jsonb.bhava_lord must equal SIGN_LORDS[bhava_sign] --
--         the same classical sign-lord table reused since migration 792.
...
--         `_get_planet_concordance` decision tree (BPHS Ch.3 natural friendship table, hardcoded
--         here as a 54-row directed relation VALUES list) must reproduce the stored concordance.
--   (i22) fact_subject format: must equal "{varga}_{significance}".
--   (j22) **F-A24, GENUINELY RED TODAY** -- bhava_sign must equal the D1-lagna-based
--         house_sign(bhava_num) invariant (varga-independent, per migration 811), cross-
--         referenced against `lord_in_house_per_varga`'s own D1 H1 sign as ground truth. 0/12,150
--         non-cb73cd3d-non-D1 rows violate; 4200/4200 of cb73cd3d's non-D1 rows violate (28
--         vargas x 30 significances x 5 ayanamshas) -- confirmed the Aries-lagna-hardcode bug
--         above, will clear once cb73cd3d's chart is rebuilt with the writer fix.
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's/810's/811's/812's/813's/814's one-
-- hundred-and-ninety-five original conjuncts (a) through (i21) forward VERBATIM, including
-- conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY exactly as migration
-- 814 already documented. This migration's own (j22)/F-A24 is a FIFTH genuinely-red conjunct,
-- newly discovered here, not carried forward. Because (b), (e), (f), (e7), and now (j22) are all
-- red, the combined 205-conjunct SELECT evaluates to false on live production today; the new
-- conjuncts were verified INDIVIDUALLY (their own subquery in isolation) rather than via the full
-- combined SELECT.
--
...
-- convergence_count / karaka_bhava_concordance,
-- 56 of 64 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of
-- these fifty-six categories' natural key needs re-asserting

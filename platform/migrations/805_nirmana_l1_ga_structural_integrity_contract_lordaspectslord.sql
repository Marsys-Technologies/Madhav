-- 805_nirmana_l1_ga_structural_integrity_contract_lordaspectslord.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-first follow-up F-A14 widening pass for
-- ga_structural, sixth in the 800-819 range (adjudication #2057, continuation 4). Migration 745
...
-- 46/57.
--
-- lord_aspects_lord_per_varga (`_build_lord_relationship_rows`'s "lord_aspects_lord_per_varga"
-- block) stores, for every ordered pair of house-lords (lord_A, lord_B) where lord_A's classical
-- Parashari aspect (the SAME `get_graha_aspects` canonical authority already used by migrations
-- 798/802) lands on lord_B's own house-in-varga, one row keyed "{VARGA}_{LORD_A}" /
-- "aspects_{LORD_B}". Like migrations 793/802, this category is FULLY SELF-CONTAINED:
-- value_jsonb already stores {varga, lord_a, lord_a_house, lord_b, lord_b_house, aspect_offset,
-- strength}, everything needed to re-verify the row's own geometry and classical aspect
-- membership without any cross-category or cross-asset join. Since `SIGN_LORDS` maps only to
-- the seven classical grahas, lord_a/lord_b are confirmed live to never include Rahu/Ketu (a
-- narrower domain than migration 802's aspecting_graha, which does include the nodes).
--
--   (a12) fact_value_num domain: must equal 1.0 -- the classical Parashari scheme has no
--         fractional aspects (same domain as migrations 798/802). 0/2748 violations live.
...
--         lord_a: continue` guard, re-derived as a data check on what actually got stored.
--         0/2748 violations live.
--   (d12) (lord_a, aspect_offset) classical validity: must be a legitimate Parashari aspect pair
--         per the hardcoded classical offset table (Mars 4/7/8; Jupiter 5/7/9; Saturn 3/7/10;
--         all others only the universal 7th) -- the same classical-validity re-derivation as
--         migration 802's conjunct (d10), narrowed to the 7 classical grahas that can ever be a
--         SIGN_LORDS value (no Rahu/Ketu branch is ever reachable here). 0/2748 violations live.
--   (e12) lord_a/lord_b domain: both must be one of the seven classical grahas
--         (Sun/Moon/Mars/Mercury/Jupiter/Venus/Saturn) -- SIGN_LORDS never maps to a node, so
--         neither lord_a nor lord_b can ever legitimately be Rahu or Ketu. 0/2748 violations
--         live.
...
--         lord_a_house/aspect_offset fields -- the same geometry as migration 802's conjunct
--         (c10). 0/2748 violations live.
--   (g12) fact_key format self-consistency: must equal 'aspects_' || the PLANET_TO_SUBJECT
--         mapping of value_jsonb.lord_b -- the writer's own f-string key construction, re-derived
--         from the row's own stored lord_b rather than restated. 0/2748 violations live.
...
-- 801's/802's/803's/804's one-hundred-and-twenty-five original conjuncts (a) through (h11)
-- forward VERBATIM, including conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY
-- RED TODAY exactly as migration 804 already documented. Because (b), (e), (f), and (e7) are all
-- red, the combined 133-conjunct SELECT evaluates to false on live production today; the new

-- 803_nirmana_l1_ga_structural_integrity_contract_jaimini.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-ninth follow-up F-A14 widening pass for
-- ga_structural, fourth in the 800-819 range (adjudication #2057, continuation 4). Migration 745
-- covered graha_vargottama_amplification_factor; migration 755 added bhadra_flag/panchaka_flag;
...
-- to 44/57.
--
-- aspect_jaimini (`_build_aspect_rows`, the "Jaimini Rasi drishti (12×12 matrix)" block) is the
-- SIMPLEST category widened so far: it is not just self-contained per row (like migrations 793
-- and 802) but entirely CHART-INDEPENDENT -- a pure 12-sign combinatorial rule with no
-- dependency on birth data, longitude, or even ayanamsha_id at all (the writer's own
-- `s1_type`/`SIGN_TYPES` branch computes IDENTICAL exclusion logic in all three cases --
-- movable/fixed/common all reduce to `offset not in [1, 11]` per the writer's own comments), so
-- every ayanamsha's copy is byte-identical and the whole rule is fully re-derivable in SQL from
-- nothing but the 12 sign names' classical zodiacal order. Confirmed against live production: 15
-- (chart, ayanamsha, build) combinations, each holding exactly 108 rows (12 signs x 9 valid
-- targets after excluding self and the two immediately-adjacent signs), value_num always 1.0.
-- Because `offset(s2,s1) = 12 - offset(s1,s2)` and {1,11} is closed under that map, the relation
-- is also provably SYMMETRIC (s1 aspects s2 iff s2 aspects s1) -- a genuine cross-row invariant
-- distinct from the full re-derivation, verified as conjunct (ff4) below.
--
--   (aa4) fact_value_num domain: must equal 1.0 -- only true aspects are ever appended by the
--         writer's `if has_aspect:` guard; no row is ever emitted with a lesser strength.
--         0/1620 violations live (global, all charts/ayanamshas/builds).
...
--         cyclic zodiacal distance between s1 and s2 (re-derived purely from the 12 canonical
--         sign names' fixed order, not restated) must be >= 2 -- re-derives the writer's own
--         `offset not in [1, 11]` exclusion directly. 0/1620 violations live.
--   (dd4) completeness: for every (chart, ayanamsha, build) combination and every (s1, s2) pair
--         whose cyclic zodiacal distance is >= 2, a row MUST exist -- the converse of (cc4),
--         confirming the writer never silently drops a legitimate pair. 0 missing pairs across
--         all 15 combinations x 108 expected pairs = 1620 checked.
--   (ee4) exact count invariant: each (chart, ayanamsha, build) combination must hold exactly
--         108 rows (12 signs x 9 valid targets) -- a coarser but independent cross-check of
--         (dd4)'s per-pair completeness. 0/15 combinations violate; 1620 total rows confirmed.
--   (ff4) symmetric mutual invariant: if (s1, on_s2) is stored, (s2, on_s1) must also be stored
--         for the SAME (chart, ayanamsha, build) -- re-derives the provable symmetry of the
--         underlying rule (see header note) as a genuine row-to-row cross-check, independent of
--         the classical zodiacal-order re-derivation (cc4)/(dd4) already establish. 0/1620
--         violations live.
--
...
-- 801's/802's one-hundred-and-eleven original conjuncts (a) through (f10) forward VERBATIM,
-- including conjuncts (b), (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY exactly as
-- migration 802 already documented. Because (b), (e), (f), and (e7) are all red, the combined
-- 117-conjunct SELECT evaluates to false on live production today; the new conjuncts were
-- verified INDIVIDUALLY (their own subquery in isolation) rather than via the full combined
-- SELECT.
--
...
-- aspect_received_by_special_point / aspect_jaimini, 44 of 57 owned categories). D-CND-03:
-- chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No
-- distinctness conjunct -- none of these forty-four categories' natural key needs re-asserting

-- 793_nirmana_l1_ga_structural_integrity_contract_effdignity.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Nineteenth follow-up F-A14 widening pass for ga_structural,
-- fourteenth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- migration 792 added graha_functional_class_per_ascendant (25/57). This adds
-- graha_effective_dignity_modified_by_aspects, taking coverage to 26/57.
--
-- graha_effective_dignity_modified_by_aspects (Group M "Special states",
-- `_build_special_state_rows`) is FULLY SELF-CONTAINED: unlike most recently-widened categories,
-- no cross-category join or external authority reference is needed at all. The row's own
-- value_jsonb stores `{formula, base_dignity, contributions: [{graha, aspect_strength,
-- functional_class, delta}, ...]}`, and fact_value_num (effective_dignity_score) is a
-- deterministic function purely of these already-stored fields:
-- effective_dignity_score = round(clamp(base_dignity + sum(contributions[].delta) * 0.1, 0, 1), 4).
-- Manually hand-verified against two live rows before designing the conjuncts (SUN: base=0.5,
...
--
--   (cc3) fact_value_num domain: within [0.0, 1.0] -- the writer's own clamp bounds
--         (`min(max(...), 0.0), 1.0)`). 0/135 violations live.
--   (dd3) value_jsonb.base_dignity domain: must be one of the writer's own dignity_scores dict
--         values (exalted=1.0, own_sign=0.75, neutral=0.5, debilitated=0.25) -- no fifth value is
--         ever legitimate. 0/135 violations live.
--   (ee3) full cross-field re-derivation: fact_value_num must equal
--         round(clamp(base_dignity + sum(contributions[].delta) * 0.1, 0.0, 1.0), 4), re-derived
--         directly from the row's own SELF-CONTAINED value_jsonb. 0/135 violations live.
--   (ff3) per-contribution delta re-derivation: each contributions[] entry's delta must equal
--         round(0.25 * aspect_strength, 4) when functional_class is benefic
--         (functional_benefic/yogakaraka/temporal_benefic), round(-0.25 * aspect_strength, 4) when
--         malefic (temporal_malefic/functional_malefic), else 0.0 -- the writer's own
--         _BENEFIC_FUNCTIONAL_CLASSES / _MALEFIC_FUNCTIONAL_CLASSES bucket membership. 0/223
--         contribution entries checked (sum across all 135 rows), 0 violations live.
--
...
-- 786's/787's/788's/789's/790's/791's/792's sixty-four original conjuncts (a) through (bb3)
-- forward VERBATIM, including conjuncts (b), (e), and (f), which remain GENUINELY RED TODAY
-- exactly as migration 792 already documented (F-A15/F-A17/F-157 fixes landed, tracked charts not
-- yet rebuilt). Because (b), (e), (f) are all red, the combined 68-conjunct SELECT evaluates to
-- false on live production today; conjuncts (cc3)/(dd3)/(ee3)/(ff3) were verified INDIVIDUALLY
...
-- graha_effective_dignity_modified_by_aspects, 26 of 57 owned categories). D-CND-03:
-- chart-partitioned / row-wise, attribution-preserving. No bare count pin (C12). No distinctness
-- conjunct -- none of these twenty-six categories' natural key needs re-asserting

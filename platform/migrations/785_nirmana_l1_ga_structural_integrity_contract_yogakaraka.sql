-- 785_nirmana_l1_ga_structural_integrity_contract_yogakaraka.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Eleventh follow-up F-A14 widening pass for ga_structural,
-- sixth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- aspect_tajik (12/57). This adds graha_yoga_karaka_flag, taking coverage to 13/57.
--
-- graha_yoga_karaka_flag stores one boolean row per classical graha (fact_subject = graha,
-- fact_key = 'is_yoga_karaka', fact_value_text = 'true'/'false'), emitted by
-- _build_functional_class_rows alongside graha_functional_class_per_ascendant. The writer's own
-- formula is explicit and narrow: `is_yoga_karaka = (g_name == lord_9 == lord_10)` -- a graha must
-- be lord of BOTH the 9th and 10th house simultaneously. Both lord_9 and lord_10 come from
-- _get_house_lord, which resolves purely from the chart's ascendant sign (a single value, not two
-- independent PyJHora computations) via the classical SIGN_LORDS table -- this is NOT the D1
-- dual-independent-PyJHora-source shape (that caveat is specifically about per-graha longitude/
-- position data computed twice by separate invocations; house-lordship here is a single
-- deterministic lookup from one already-resolved ascendant sign), so no D1-avoidance disclaimer is
-- needed for this category.
--
-- Investigated the classical structure of the writer's own exact formula before designing
-- conjuncts (not to fix it -- routing a possible writer-scope finding is out of bounds for this
-- bounded F-A14 authoring pass -- but to confirm what invariant the stored data actually obeys):
-- under the classical SIGN_LORDS table, adjacent signs never share a lord EXCEPT the Capricorn/
-- Aquarius pair (both Saturn, pre-outer-planet rulership) -- so `lord_9 == lord_10` is true for
-- exactly one lagna (Taurus, where 9H=Capricorn and 10H=Aquarius) and false for the other eleven.
-- None of the three charts currently in production has a Taurus lagna, so today's 105 live rows
-- are honestly all 'false' -- not a stale/empty-detector artifact (a real 'true' row DOES exist as
-- a reachable value under the writer's own formula, it is simply not exercised by the charts on
-- file today); the conjuncts below are written to hold regardless of which lagna a future chart
-- has, not tuned to the current all-false data.
--
--   (ff) fact_value_text domain: must be 'true' or 'false' -- no third value is ever legitimate
--        for a boolean flag. 0/105 violations live.
--   (gg) at most one graha per (chart, ayanamsha, build) may have is_yoga_karaka='true' -- a
--        genuine cross-row invariant the writer's own equality-chain formula guarantees (only the
--        one graha literally equal to both lord_9 and lord_10, if such a graha exists at all, can
--        ever satisfy the condition -- structurally impossible for two different grahas to both
--        equal the same lord_9/lord_10 pair). 0/105 violations live (vacuously satisfied by
--        today's all-false data, but a real, mutation-provable detector: flipping any second row
--        to 'true' within the same chart/ayanamsha/build is caught).
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries migrations 745's/755's/756's/757's/758's/759's/780's/781's/782's/783's/784's thirty-one
-- original conjuncts (a) through (ee) forward VERBATIM, including conjuncts (b), (e), and (f),
-- which remain GENUINELY RED TODAY exactly as migration 784 already documented (F-A15/F-A17/F-157
-- fixes landed, tracked charts not yet rebuilt). Because (b), (e), (f) are all red, the combined
-- 33-conjunct SELECT evaluates to false on live production today; conjuncts (ff)/(gg) were
...
-- graha_yoga_karaka_flag, 13 of 57 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of these
-- thirteen categories' natural key needs re-asserting (chart_facts_unique_null_formula already
-- covers it).
SELECT

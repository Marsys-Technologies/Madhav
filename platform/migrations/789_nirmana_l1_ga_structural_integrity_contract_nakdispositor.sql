-- 789_nirmana_l1_ga_structural_integrity_contract_nakdispositor.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Fifteenth follow-up F-A14 widening pass for ga_structural,
-- tenth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- added the Group H avastha bundle (19/57). This adds nakshatra_dispositor_chain, taking
-- coverage to 20/57.
--
-- nakshatra_dispositor_chain (GAP-4) walks the nakshatra-lord chain (graha/Lagna →
-- nakshatra_lord(its own nakshatra) → nakshatra_lord(that lord's nakshatra) → ... until a
-- repeat), storing {chain, length, nakshatras, cycle_at_step, constituent_fact_ids} in
-- value_jsonb. Unlike graha_dispositor_chain (migration 786), this walk does NOT recompute
-- anything from a hardcoded classical table -- the writer's own docstring is explicit that it
-- "References ga_nakshatra fact_ids" and reads each step's nakshatra_lord DIRECTLY from
-- graha_nakshatra_join (an L1-authority reference per §N.5). This makes the strongest possible
-- conjunct available: re-deriving the chain-walk by reading the SAME source-of-truth table the
-- writer itself consults, not an independently-embedded classical table -- and confirming the
-- constituent_fact_ids reference genuinely resolves to that source row (the exact §N.5 concern
-- this campaign has tracked since the MSR drift handoff: "constituent_facts_array... MUST
-- resolve").
--
-- Subjects include Lagna (fact_subject LAGNA) alongside the 9 classical grahas -- confirmed
-- graha_nakshatra_join carries a real nakshatra_lord entry for LAGNA too, so it participates in
-- the chain-walk re-derivation on equal footing, not excluded as a special case.
--
--   (nn2) chain[0] domain: must equal the row's own graha/Lagna name (the writer's own
--         `chain: list[str] = [planet_name]` initialization) -- re-derived from fact_subject
--         rather than restated. 0/150 violations live.
...
--         whether the loop found a genuine repeat (cycle_at_step = len(chain)-1 at the exact
--         point of the repeat's append) or the `effective_cycle` fallback fired
--         (chain_length - 1), the two formulas coincide exactly, so this equality holds
--         unconditionally rather than only in the "genuine cycle" branch. 0/150 violations
--         live.
--   (qq2) the nakshatras array must have length-1 entries for graha subjects, but length-2 for
--         Lagna specifically -- Lagna has no graha_position.nakshatra entry (confirmed: 0 rows
--         live), so the writer's own `if nak:` guard silently skips appending Lagna's own
--         nakshatra (but not any later grahas' nakshatras in the same chain) -- an honest,
--         explained gap in the writer's own data availability, not a defect this migration
--         needs to fix; the conjunct is scoped to match this exactly-reproducible pattern
--         rather than either ignoring it or treating it as a violation. 0/150 violations live
--         (confirmed the naive length-1-for-everyone version produces exactly 15/150 false
--         violations, all on fact_subject='LAGNA', before landing the corrected version).
...
--         graha_nakshatra_join.nakshatra_lord value (capitalized per the writer's own
--         Rahu/Ketu special-casing) -- walked step-by-step via generate_series, re-reading the
--         SAME L1-authority table the writer itself consults rather than an independently
--         re-derived classical rule. 0/693 pairs checked (sum of length-1 across all 150 rows),
--         0 violations live.
--   (ss2) constituent_fact_ids[0], when present, must resolve to an ACTUAL
--         graha_nakshatra_join.nakshatra_lord row for the SAME (chart, ayanamsha, subject) --
--         the §N.5 constituent-facts-must-resolve concern, verified directly rather than
--         assumed. 0/150 violations live.
--
...
-- 786's/787's/788's forty-nine original conjuncts (a) through (ww) forward VERBATIM, including
-- conjuncts (b), (e), and (f), which remain GENUINELY RED TODAY exactly as migration 788 already
-- documented (F-A15/F-A17/F-157 fixes landed, tracked charts not yet rebuilt). Because (b), (e),
-- (f) are all red, the combined 55-conjunct SELECT evaluates to false on live production today;
-- conjuncts (nn2) through (ss2) were verified INDIVIDUALLY (their own subquery in isolation)
-- rather than via the full combined SELECT.
--
...
-- graha_avastha_lifetime_exposure_summary / nakshatra_dispositor_chain, 20 of 57 owned
-- categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count
-- pin (C12). No distinctness conjunct -- none of these twenty categories' natural key needs

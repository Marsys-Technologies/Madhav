-- 800_nirmana_l1_ga_structural_integrity_contract_chartcog.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Twenty-sixth follow-up F-A14 widening pass for
-- ga_structural, FIRST in the new 800-819 range (adjudication #2057, continuation 4, granted
-- after the 780-799 range was fully exhausted by migration 799). Migration 745 covered
...
--
-- chart_center_of_gravity (near graha_dispositor_chain/composite_dispositor_strength) is a
-- PER-VARGA CHART-LEVEL ROLLUP: for each of 29 vargas, `_build_chart_cog_per_varga_rows` walks
-- every one of the 9 ALL_GRAHAS members' own sign-lord dispositor chain (via the classical
-- SIGN_LORDS table, already embedded in SQL since migration 757's conjunct (g)) to a terminus
-- (self-disposing planet or a detected cycle, capped at 13 hops), tallies how many chains
-- terminate at each planet, and stores the plurality terminus (`final_dispositor`) plus the
-- count of distinct termini (`cluster_count`). Two fact_keys per (chart, ayanamsha, varga):
-- final_dispositor (fact_value_text=winning graha, fact_value_num=its chain count,
-- value_jsonb.tally=the full per-graha terminus tally) and cluster_count (fact_value_num=count
-- of distinct termini).
--
-- A full re-derivation would require walking all 9 grahas' dispositor chains across all 29
-- vargas in SQL (a 13-hop recursive walk per graha per varga, reading either graha_position (D1)
-- or chart_divisionals (D2+) for each hop's sign) -- a correct but substantially heavier
-- structure than the campaign's other conjuncts. Per the established "don't always need to
-- re-derive the full source algorithm" precedent (kala_sarpa_per_varga migration 781,
-- composite_dispositor_strength migration 787), this migration instead ships STRONG INTERNAL
-- CROSS-FIELD CONSISTENCY conjuncts that would catch essentially any corruption of the writer's
-- own tally/final_dispositor/cluster_count relationship, verified against ALL 435 live rows
-- (chart, ayanamsha, varga combinations) -- not a sample.
--
--   (a8) final_dispositor fact_value_text domain: must be one of the seven classical grahas
--        that can ever be a SIGN_LORDS value (Sun/Moon/Mars/Mercury/Jupiter/Venus/Saturn) --
--        Rahu/Ketu can never terminate a chain since they are never a sign lord. 0/435
--        violations live.
--   (b8) cluster_count / final_dispositor's own fact_value_num (chains_terminating_here) domain:
--        both must be integers in [1, 9] -- the achievable range given exactly 9 walking grahas
--        per varga. 0/435 + 0/435 violations live.
--   (c8) cluster_count self-consistency: must equal the count of distinct keys in the SAME
--        row's own sibling final_dispositor row's value_jsonb.tally -- the writer's own
--        `len(tally)` computation, re-derived by counting the stored JSON object's own keys
--        rather than restated. 0/435 violations live.
--   (d8) final_dispositor cross-field consistency: fact_value_num (chains_terminating_here)
--        must equal value_jsonb.tally[fact_value_text] -- the writer's own
--        `tally[final_disp]` lookup, re-derived directly from the row's own stored tally rather
--        than restated. 0/435 violations live.
--   (e8) final_dispositor genuine-argmax invariant: no OTHER entry in value_jsonb.tally may
--        exceed fact_value_num -- re-derives the writer's own `max(tally, key=lambda x:
--        tally[x])` selection as a real cross-entry comparison (ties are legitimate and not
--        asserted away, only a strict violation of the max property is caught). 0/435
--        violations live.
--   (f8) tally-sum invariant: the sum of ALL value_jsonb.tally entries must equal exactly 9 --
--        every one of the 9 ALL_GRAHAS members walks to some terminus every time (the writer's
--        own `for g in graha_sign:` loop, and graha_sign always has all 9 entries given
--        ALL_GRAHAS' own position data is always available). 0/435 violations live.
--
...
-- 786's/787's/788's/789's/790's/791's/792's/793's/794's/795's/796's/797's/798's/799's
-- ninety-five original conjuncts (a) through (e7) forward VERBATIM, including conjuncts (b),
-- (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY exactly as migration 799 already
-- documented. Because (b), (e), (f), and (e7) are all red, the combined 101-conjunct SELECT
...
-- chart_center_of_gravity, 41 of 57 owned categories). D-CND-03: chart-partitioned / row-wise,
-- attribution-preserving. No bare count pin (C12). No distinctness conjunct -- none of these
-- forty-one categories' natural key needs re-asserting (chart_facts_unique_null_formula already
-- covers it).
SELECT

-- 812_nirmana_l1_ga_structural_integrity_contract_netargala.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Thirty-eighth follow-up F-A14 widening pass for
-- ga_structural, thirteenth in the 800-819 range (adjudication #2057, continuation 4). Migration
...
--
-- net_argala_per_varga (`_build_net_argala_per_varga_rows`) computes, per varga, a net argala
-- score per house: the count of grahas at each of 4 classical argala-offset houses (2nd, 4th,
-- 5th, 11th from the target) minus the count at each of 4 virodha-offset houses (3rd, 10th,
-- 9th, 12th). Re-deriving this via the sibling graha_dignity_per_varga category's own house
-- counts FIRST produced 40/5220 apparent violations, ALL confined to varga='D1' -- investigated
-- to a precise, verified root cause rather than assumed a defect: `_build_net_argala_per_varga_rows`
-- iterates `varga_state.items()` directly, and for D1 that dict is built by
-- `_extract_chart_state`, which ALWAYS inserts a `"LAGNA"` pseudo-entry at house=1 (line ~890).
-- The sibling graha_dignity_per_varga category, by contrast, iterates the classical grahas only
-- (never Lagna), so it under-counts house 1's occupancy by exactly 1 for D1. For every OTHER
-- varga, `varga_state` is built by `_load_varga_positions`, which is documented as
-- graha-only (no Lagna key) -- confirmed live: 0/5040 non-D1 rows disagree without any
-- adjustment at all. The re-derivation therefore adds Lagna's own house-1 presence back in, but
-- ONLY for varga='D1' -- re-verified at 0/5220 violations globally after the fix.
--
--   (a19) fact_value_num must equal value_jsonb.net_argala -- the same count stored twice.
--   (b19) fact_subject format: must equal "{varga}_HOUSE_{house}" -- the writer's own
--         `f"{varga}_HOUSE_{tgt_h}"` construction, re-derived from the row's own stored
--         varga/house fields.
...
--         the 4 classical argala-offset houses minus the sum at the 4 virodha-offset houses,
--         sourced from the sibling graha_dignity_per_varga category's own house field for the
--         SAME (chart, ayanamsha, build, varga) -- PLUS Lagna's own house-1 occupancy, but ONLY
--         for varga='D1' (see header note; SIGN_LORDS/ALL_GRAHAS classical offset arithmetic, the
--         same safe non-negative modulo range already used throughout the arc). 0/5220
--         violations live (5040 non-D1 rows need no Lagna adjustment; 180 D1 rows do).
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's/810's/811's one-hundred-and-seventy-four
-- original conjuncts (a) through (g18) forward VERBATIM, including conjuncts (b), (e), (f), and
-- (e7)/F-A18, which remain GENUINELY RED TODAY exactly as migration 811 already documented.
-- Because (b), (e), (f), and (e7) are all red, the combined 179-conjunct SELECT evaluates to

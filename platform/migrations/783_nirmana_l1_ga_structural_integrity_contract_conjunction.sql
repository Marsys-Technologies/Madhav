-- 783_nirmana_l1_ga_structural_integrity_contract_conjunction.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Ninth follow-up F-A14 widening pass for ga_structural,
-- fourth in the 780-799 range (adjudication #2012). Migration 745 covered graha_vargottama_
...
-- tara_bala_natal_baseline (10/57). This adds conjunction_within_orb, taking coverage to 11/57.
--
-- conjunction_within_orb stores one row per graha pair (from ALL_GRAHAS, i < j) whose D1
-- longitudes fall within a fixed 10-degree orb, fact_subject = "{s1}_{s2}" (both tokens drawn
-- from PLANET_TO_SUBJECT -- note RAH_MEAN/KET_MEAN themselves contain an underscore, so a naive
-- split_part(x,'_',2) parse is WRONG for any pair involving Rahu/Ketu; this migration's conjuncts
-- parse s1 by checking the RAH_MEAN_/KET_MEAN_ prefix first, falling back to split_part only
-- otherwise, then take s2 as the remainder -- confirmed against all 30 live rows, which include a
-- genuine SAT_KET_MEAN subject that a naive split would have mis-parsed as SAT/KET rather than
-- SAT/KET_MEAN).
--
-- This is a pure-D1 category (no varga dimension) computed from ga_structural's own in-memory
-- chart_output -- the SAME D1 source already flagged by the D1 dual-independent-PyJHora-source
-- caveat (root-caused for F-A17). A full re-derivation against ga_vargas' own chart_divisionals D1
-- data would risk re-surfacing that already-tracked disagreement as a false "new defect" (the same
-- dead-end this arc has now recognized four times for _per_varga categories); this pass instead
-- ships two self-consistency/domain conjuncts against the row's own already-stored fields, the
-- same discipline established for kala_sarpa_per_varga (migration 781) and used again here for a
-- pure-D1 category rather than a _per_varga one:
--
--   (w) fact_value_num (orb_deg) domain: must be within [0, 10.0] -- re-derives the writer's own
--       `if orb <= 10.0` emission filter (and the physical non-negativity of an orb) as a domain
--       check. 0/30 violations live.
--   (x) no reversed-duplicate pair: for a stored "{s1}_{s2}" row, "{s2}_{s1}" must never also be
--       stored for the same (chart, ayanamsha, build) -- the writer's own `for g2 in
--       ALL_GRAHAS[i+1:]` loop guarantees each unordered pair is emitted exactly once; re-derived
--       here as a genuine cross-row consistency check rather than restated. 0/30 violations live.
--   (y) pair ordering invariant: s1's index in the writer's own ALL_GRAHAS ordering must precede
--       s2's index -- re-derives the writer's own `i, g1` / `ALL_GRAHAS[i+1:]` loop structure as a
--       domain check on what actually got stored (a genuine structural invariant the loop
--       guarantees, not a bare restatement). 0/30 violations live.
--
-- integrity_check_sql is a single UPDATE ... SET column, not additive SQL -- this migration
-- carries migrations 745's/755's/756's/757's/758's/759's/780's/781's/782's twenty-two original
-- conjuncts (a) through (v) forward VERBATIM, including conjuncts (b), (e), and (f), which remain
-- GENUINELY RED TODAY exactly as migration 782 already documented (F-A15/F-A17/F-157 fixes landed,
-- tracked charts not yet rebuilt). Because (b), (e), (f) are all red, the combined 25-conjunct
-- SELECT evaluates to false on live production today; conjuncts (w)/(x)/(y) were verified
...
-- kala_sarpa_per_varga / tara_bala_natal_baseline / conjunction_within_orb, 11 of 57 owned
-- categories). D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count pin
-- (C12). No distinctness conjunct -- none of these eleven categories' natural key needs

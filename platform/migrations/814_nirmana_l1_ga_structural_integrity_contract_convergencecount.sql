-- 814_nirmana_l1_ga_structural_integrity_contract_convergencecount.sql
--
-- NIRMĀṆA L1 Gaṇita — W3 IMPLEMENT. Fortieth follow-up F-A14 widening pass for
-- ga_structural, fifteenth in the 800-819 range (adjudication #2057, continuation 4). Migration
-- 813 added contradiction_pair (54/64 by the corrected running tally -- see D-L1-105 for the
-- governance-table registry-gap finding that established the true denominator is 64, not 57).
-- This adds convergence_count.
--
-- convergence_count (`_build_convergence_count_per_varga_rows`) stores TWO row shapes per varga:
-- one row per graha (entity='graha', total_edges = count of undirected Parashari-aspect-or-
-- conjunction edges touching that graha) and one row per house 1-12 (entity='house', total_edges
-- = count of the SAME edges with at least one endpoint occupying that house, an edge with BOTH
-- endpoints in the same house counted once, not twice -- the writer's own `if h2 != h1` guard).
-- The graha-entity adjacency test is BYTE-IDENTICAL to migration 807's `graha_centrality`
-- adjacency test (same `_graha_aspects_house` calls, same same-sign fallback) -- so rather than
-- re-deriving the aspect graph a second time, this migration cross-references the ALREADY-
-- VERIFIED graha_centrality category directly: a graha's total_edges here must equal its
-- degree_centrality there. For the house-entity rows, this migration reconstructs the full edge
-- set from graha_centrality's own `connected_to` arrays (deduplicated per unordered pair) joined
-- to each endpoint's house via the sibling `graha_dignity_per_varga` category, then aggregates by
-- house exactly as the writer does -- a genuine full re-derivation of the house-level rollup that
-- still avoids re-deriving the aspect/conjunction logic itself, since that logic's OUTPUT
-- (the edge list) is already a proven, shipped category.
--
...
--   (b21) entity domain: value_jsonb.entity must be one of the writer's own two shapes
--         ("graha", "house") -- no third shape is ever legitimate.
--   (c21) value_num/value_jsonb consistency: fact_value_num must equal
--         value_jsonb.total_edges -- the same count stored twice.
...
--         known PLANET_TO_SUBJECT tokens -- a genuine domain check, not a self-derived
--         tautology (see the conjunct's own comment for the mutation-testing catch).
...
--         varga, graha)'s graha_centrality.degree_centrality -- the two categories' adjacency
--         tests are identical in the writer, so this is a genuine, provable equality, not a
--         coincidental agreement.
--   (i21) house-row full re-derivation: total_edges must equal the aggregated edge count
--         reconstructed from graha_centrality's own connected_to arrays (deduplicated per
--         unordered pair) joined to each endpoint's house via graha_dignity_per_varga, summed
--         per house exactly as the writer's own `house_edges[h1] += 1` / `house_edges[h2] += 1
--         if h2 != h1` construction does. 0/5220 house rows violate; the full 3915-row graha
--         cross-reference (h21) also verified 0 violations.
--
...
-- 801's/802's/803's/804's/805's/806's/807's/808's/809's/810's/811's/812's/813's one-hundred-
-- and-eighty-six original conjuncts (a) through (g20) forward VERBATIM, including conjuncts (b),
-- (e), (f), and (e7)/F-A18, which remain GENUINELY RED TODAY exactly as migration 813 already
-- documented. Because (b), (e), (f), and (e7) are all red, the combined 195-conjunct SELECT
-- evaluates to false on live production today; the new conjuncts were verified INDIVIDUALLY
-- (their own subquery in isolation) rather than via the full combined SELECT. This migration also
-- corrects the "target: chart_facts, scoped to" comment block below, which had been copy-pasted
-- unchanged since at least migration 810 and had drifted stale (still listing only 45 of the by-
-- then 53+ categories actually covered) -- a real, low-severity documentation-accuracy defect in
-- a comment that ships live inside the asset_registry row, now brought current to all 55.
--
...
-- convergence_count,
-- 55 of 64 owned categories -- corrected here to actually list every category the
-- combined SELECT covers; this comment had been copy-pasted unchanged since at least
-- migration 810 and had drifted stale at 45, several migrations behind the real coverage).
-- D-CND-03: chart-partitioned / row-wise, attribution-preserving. No bare count
-- pin (C12). No distinctness conjunct -- none of these fifty-five categories' natural key
-- needs re-asserting

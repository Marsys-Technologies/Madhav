#!/usr/bin/env python3
"""
Standalone runner for bo_anveshana against prod DB (Cloud SQL Auth Proxy at 127.0.0.1:5433).
Bypasses discover_all()/_auto_discover() which hard-fails on ga_nakshatra (jhora not installed).

Usage:
    python run_bo_anveshana_prod.py [--dry-run]
"""
import os, sys, uuid, logging, argparse
logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s %(message)s")
logger = logging.getLogger("bo_anveshana_runner")
...
import pipeline.orchestrator.writers.bo_anveshana  # noqa: F401 — registers via @register
...
def main():
    ...
    writer_cls = _REGISTRY.get("bo_anveshana")
    if writer_cls is None:
        print("ERROR: bo_anveshana not in registry", file=sys.stderr)
        sys.exit(1)
    ...
    conn.commit()
    conn.close()
    logger.info("bo_anveshana COMPLETE: rows_inserted=%d notes=%s", result.rows_inserted, result.notes)

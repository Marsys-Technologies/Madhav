...
entry points that drive real chart builds). It carries the MR-39 remediation
— a SESSION-scoped `SET idle_in_transaction_session_timeout = 1800000`
(30 minutes) at connection setup (never `ALTER DATABASE`/`ALTER ROLE`) —
bounding the server-side idle-in-txn killer without disabling it entirely
(S7459 fix: the prior `= 0` turned genuine connection hangs into permanent
stalls requiring manual pg_terminate_backend intervention). MR-39 closed the
remaining gap: several standalone build-runner scripts at the sidecar root
(`run_heavy_writer_standalone.py`, `run_elev_beta_d_rebuild.py`,
`run_elev_beta_integration_rebuild.py`) drove writer substeps via a bare
`psycopg.connect()` that bypassed this factory entirely and therefore lacked
the same protection; those now route through `connect()` below.
...
def connect() -> psycopg.Connection:
    ...
    # idle_in_transaction_session_timeout=1800000 (30 min) bounds the server-side
    # idle-in-txn killer — high enough that legitimately slow ayanamsha substeps
    # (up to ~20 min of pure CPU with no DB traffic) survive, but finite so a
    # genuinely hung/deadlocked connection auto-recovers rather than blocking
    # indefinitely (S7459 finding: the prior =0 turned connection hangs into
    # permanent stalls requiring manual pg_terminate_backend intervention).
    ...
    # idle-in-transaction for >10 minutes gets killed prematurely. An explicit SET,
    # issued as a real statement, cannot be stripped by a proxy — it is
    # indistinguishable from any other query on the wire.

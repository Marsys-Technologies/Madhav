"""
bo_vargottama_dhana — Vargottama amplification + Dhana-axis MSR signal layer (L2 Bodha)
============================================================================================
D-2 Lane V-5 (CR-36). PURE L2 DERIVATION over existing L1 `vargottama_per_varga`
and `graha_position` facts — no new astronomical compute. Emits TWO
signal_type_classes: `vargottama_amplification` (cross-frame D1/D9
confirmation, fires only when true) and `dhana_axis` (complete 2nd/11th
house tenancy + classical lord placement).

Salience: vargottama_amplification class_prior=1.15, dhana_axis
class_prior=1.05 — ratified DIS.019/DR-6.

Owned signal_type_classes: this writer owns EXACTLY `vargottama_amplification`
and `dhana_axis` (D-1.5b `owned_signal_type_classes` allowlist discipline).
...
from bodha_writers.vargottama_dhana_emitter import (
    ...
    _fetch_vargottama_facts,
    _fetch_graha_positions,
)
...
@l2_producer("bo_vargottama_dhana")
class BoVargottamaDhanaWriter(WriterBase):
    """bo_vargottama_dhana: vargottama_amplification + dhana_axis MSR signal layer."""
    ...
    def run(self, ctx: ContextSpec) -> WriterResult:
        ...
        for aya in CANONICAL_AYANAMSHAS:
            ...
            if ctx.dry_run:
                logger.info(
                    "[bo_vargottama_dhana dry_run] %s — %d vargottama facts, %d position buckets found",
                    aya, len(vargottama_facts), len(positions))
                continue

            if not positions:
                logger.warning("[bo_vargottama_dhana] %s — no graha_position facts; "
                                "ga_positions must have failed for this ayanamsha", aya)
                continue

            rows = build_vargottama_rows(
                chart_id=chart_id, ayanamsha_id=aya, build_id=build_id,
                vargottama_facts=vargottama_facts, positions=positions, now=now,
            ) + build_dhana_axis_rows(
                chart_id=chart_id, ayanamsha_id=aya, build_id=build_id,
            ...
            logger.info("[bo_vargottama_dhana] %s — deleted %d prior, inserting %d signals",

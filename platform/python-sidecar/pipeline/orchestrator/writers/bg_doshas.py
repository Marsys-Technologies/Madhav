...
class DoshasWriter(WriterBase):
    ...
    def run(self, ctx: ContextSpec) -> WriterResult:
        ...
        ontology_ins = counts.get('ontology_inserted', 0)
        ref_ins = counts.get('ref_inserted', 0)
        return WriterResult(
            asset_id=self.asset_id,
            rows_inserted=catalog_ins + ontology_ins + ref_ins,
            rows_skipped=counts.get('catalog_skipped', 0),
            duration_seconds=time.time() - t0,
            notes=(
                f"brahma_dosha_catalog: +{catalog_ins}; "
                f"brahma_ontology(dosha): +{ontology_ins}; "
                f"reference_doshas: +{ref_ins}; "
                f"total_owned={catalog_ins + ontology_ins + ref_ins}"
            ),
        )

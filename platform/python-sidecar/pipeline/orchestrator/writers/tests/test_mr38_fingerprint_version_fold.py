"""MR-38 gate: fold the writer's row-shape/output-contract into the delta fingerprint.
...
FIX
---
`ROW_SCHEMA_COLUMNS` is derived directly from `INSERT_PROD_SQL` (the actual
SQL template executed, not a hand-maintained duplicate list that could itself
drift) and folded into `compute_substep_fingerprint`'s hashed payload
alongside `engine_version`. A future MR that adds/removes/renames a column in
`INSERT_PROD_SQL` therefore automatically changes every substep's fingerprint
— no human has to remember to bump `ENGINE_VERSION` for a row-shape change
ever again. This composes cleanly with any future `ENGINE_VERSION` bump
(a distinct, orthogonal input to the same hashed payload).

"""bo_sudarshana must never write a random signal_id.

Nirmana #1770/#1804 (CONDUCTOR ruling on #2524/#2535, 2026-09-09): `signal_id`
was `str(uuid.uuid4())` at the emit site (bodha_writers.sudarshana_emitter.
build_signal_row), so every rebuild minted fresh identities for the same
signals -- the exact CASCADE/orphan hazard #1770 already ruled on for any
...
test_bo_laksana_signal_identity's cursor-based one) -- bo_sudarshana.py
already calls conn.execute() directly elsewhere (_fetch_sign_facts).
"""
...
SOURCE = Path(__file__).resolve().parents[1] / "bo_sudarshana.py"
EMITTER_SOURCE = Path(__file__).resolve().parents[4] / "bodha_writers" / "sudarshana_emitter.py"
...
def _row(sid, aya="lahiri", stype="sudarshana_agreement:SUN", cfg=None, varga="D1") -> dict:
    return {
        "signal_id": sid,
        ...
        "varga_id": varga,
        "configuration_jsonb": cfg if cfg is not None else json.dumps({"agreement": "confirmed_3frame"}),
    }


def test_assigns_the_database_derived_identity_onto_the_row() -> None:
    from pipeline.orchestrator.writers.bo_sudarshana import assign_deterministic_signal_ids

    conn = _FakeConn()
    rows = [_row(None), _row(None, stype="sudarshana_agreement:MOON")]
...
def test_reports_collapse_honestly_rather_than_assuming_none() -> None:
    ...
    assert assign_deterministic_signal_ids(
        _FakeConn(), [_row(None), _row(None, stype="sudarshana_agreement:MOON")]
    ) == 0
...
def test_preserialised_config_derives_the_same_id_as_the_object() -> None:
    ...
    build_signal_row() stores configuration_jsonb pre-serialised (json.dumps) for
    the INSERT's ::jsonb cast. Passing that string into the identity payload
...
def test_no_emit_site_can_construct_a_random_signal_id() -> None:
    ...
    module it calls into -- the emit site actually lives in sudarshana_emitter.py,
    not bo_sudarshana.py)."""

"""
bo_special_lagna — Special-lagna domain-salience MSR signal layer (L2 Bodha)
================================================================================
D-2 Lane V-5 (CR-76). PURE L2 DERIVATION over existing L1 `special_lagna`
facts — no new astronomical compute. Emits one `special_lagna` MSR signal
per (Indu/Sree/Ghati/Hora lagna x ayanamsha), each carrying `domain_salience`
so it ranks correctly WITHIN its classically-assigned domain despite the
chart-wide 0.90 class-prior discount (DR-6).

Salience: class_prior=0.90, subsystem='special_lagna' — ratified DIS.019/DR-6.

Owned signal_type_classes: this writer owns EXACTLY `special_lagna`.
...
BO_SPECIAL_LAGNA_OWNED_SIGNAL_TYPE_CLASSES: list[str] = ["special_lagna"]
...
def assign_deterministic_signal_ids(conn: Any, rows: list[dict]) -> int:
    ...
    if collapsed:
        logger.warning(
            "bo_special_lagna: %d of %d rows collapsed onto deterministic identities",
            collapsed,
            len(rows),
        )
    return collapsed
...
class BoSpecialLagnaWriter(WriterBase):
    ...
    def run(self, ctx: ContextSpec) -> WriterResult:
        ...
        for aya in CANONICAL_AYANAMSHAS:
            facts_by_lagna = _fetch_special_lagna_facts(conn, chart_id, aya)
            if ctx.dry_run:
                logger.info("[bo_special_lagna dry_run] %s — %d lagna fact buckets found",
                            aya, len(facts_by_lagna))
                continue

            rows: list[dict] = []
            for lagna_key in _TARGET_LAGNAS:
                facts = facts_by_lagna.get(lagna_key)
                if not facts:
                    logger.warning("[bo_special_lagna] %s — no special_lagna facts for %s; skipping",
                                   aya, lagna_key)
                    continue
                row = build_signal_row(
                    chart_id=chart_id, ayanamsha_id=aya, build_id=build_id,
                    lagna_key=lagna_key, facts=facts, now=now,
                )
                if row is None:
                    continue
            ...
            deleted = replace_prior_msr_for_chart(
                conn, chart_id, aya, BO_SPECIAL_LAGNA_OWNED_SIGNAL_TYPE_CLASSES,
            )
            logger.info("[bo_special_lagna] %s — deleted %d prior, inserting %d signals",

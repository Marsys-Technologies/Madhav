...
PRIMITIVE_ROWS: list[tuple] = [
    ...
    # primitives, transcribed verbatim from platform/src/lib/vidhi/registry_data.ts by the
    # vidhi-registry parity gate's own dump (scripts/census/dump_vidhi_registry.ts) — never
    # hand-retyped independently of the TS source.
    ('ahead_read', 1, 'VIEW 2 — AHEAD ("what is coming?"): dated, shaped forward-looking temporal windows + probabilistic projections, composed into an argument-shaped reading (thesis → evidence → verdict → falsifier). Machine-band default: compiled into every domain deepdive alongside now_read + priority_read.', 'temporal', 'kala_ahead_get', {'chart_id': '{chart_id}', 'domain': '{domain}', 'question_frame': '{question_frame}'}, 'kala_bundle_get', None, [], []),
    ('elect_read', 1, 'VIEW 3 — ELECT ("when should I…?"): Mode-3 (ACTIVITY ELECTION) act-time slate for a named undertaking, ranked and graded, paired with its preparatory rite and the rite\'s own best time served as ONE answer (the Mode-3 routing rule, Elevation §8 — ELECT is the sole server of Mode 3). Routing default: an undertaking-shaped question compiles the `undertaking_election` floor headed by this primitive.', 'temporal', 'kala_elect_get', {'chart_id': '{chart_id}', 'undertaking': '{undertaking}', 'question_frame': '{question_frame}'}, 'kala_muhurta_get', None, [], []),
    ('explain_read', 1, 'VIEW 6 — EXPLAIN ("why do you say that?"): the PACT-protocol causal chain (promise in the rāśi → confirmation in the varga → activation in the daśā → trigger in the transit) naming its weakest link, composed into an argument-shaped reading. Universally pre-authorized: every compiled floor item, in every floor, carries one EXPLAIN hop (the `explain_on_request` Anusaraṇa rule — Six Views Design §8 "every served row id pre-authorizes one EXPLAIN hop").', 'temporal', 'kala_explain_get', {'chart_id': '{chart_id}', 'domain': '{domain}', 'question_frame': '{question_frame}'}, 'pact_query', None, [], []),
    ('now_read', 1, 'VIEW 1 — NOW ("what is my temporal state right now?"): active temporal-activation windows + the current Kāla-Darshana confluence, composed into an argument-shaped reading on the elevated kala_* envelope. Machine-band default: compiled into every domain deepdive alongside ahead_read + priority_read (Six Views Design §8 — "timing is never optional in a deepdive").', 'temporal', 'kala_now_get', {'chart_id': '{chart_id}', 'question_frame': '{question_frame}'}, 'kala_windows_get', None, [], []),
    ('priority_read', 1, 'VIEW 5 — PRIORITIZE ("what matters most right now?"): the salience-ranked signal ledger (computed_salience, domains_affected, trigger_type) — the attention-budget surface. Machine-band default: compiled into every domain deepdive alongside now_read + ahead_read.', 'temporal', 'kala_priority_get', {'chart_id': '{chart_id}', 'question_frame': '{question_frame}'}, 'kala_priority_ranking_get', None, [], []),
    ('ritual_read', 1, 'CAPABILITY — YAJÑA-SETU (Modes 1–2 ONLY; Mode 3 lives in elect_read — the Mode-3 routing rule, Elevation §8): Mode 1 (OPPORTUNITY SCAN — ranked (window, rite) pairs over a horizon) and Mode 2 (PATTERN SEARCH — a declared sky-pattern spec graded against the lattice). A Mode-3-shaped call (naming an `undertaking`) is honestly redirected (`wrong_view` + the elect_read pointer), never passed through. Routing default: a ritual/yajña/vrata-shaped question compiles the `ritual_yajna` floor headed by this primitive.', 'remedy', 'kala_ritual_get', {'chart_id': '{chart_id}', 'horizon': '{horizon}', 'question_frame': '{question_frame}'}, 'kala_muhurta_get', None, [], []),
    ('story_read', 1, 'VIEW 4 — STORY ("what has my life been?"): the daśā-chaptered life-arc (parva quality, theme keywords, high-convergence chapters) deduped by span+level, composed into a developmental narrative. Routing default: a biography/life-narrative-shaped question compiles the `biography_narrative` floor headed by this primitive.', 'temporal', 'kala_story_get', {'chart_id': '{chart_id}', 'question_frame': '{question_frame}'}, 'kala_life_arc_get', None, [], []),
    ('upaya_read', 1, 'CAPABILITY — UPĀYA-SETU: PACT-link intervention diagnosis (weak-promise/weak-confirmation/weak-activation) + alternate-routing search + efficacy-tiered remedy candidates, composed into an argument-shaped reading. `category: remedy` — stripped when `intervention: false` (a remedy read is never useful without an intervention ask, matching every other remedy-category primitive).', 'remedy', 'kala_upaya_get', {'chart_id': '{chart_id}', 'domain': '{domain}', 'question_frame': '{question_frame}'}, 'bodha_remedies_get', None, [], []),
]

"""ka_gochara writer — W2G's per-chart materialization (GOCHARA-2.0 / GOCHARA-UTKARSA W6.4).

ṢAḌ-DARŚANA wave W2G (item 19), lane G REWORK. Originally named
`ka_gochara_v2_materialize`; renamed to `ka_gochara` at W6.4 cutover (UTK-R2)
per the native ruling ("retire ka_gochara self-test, rename v2_materialize →
ka_gochara"). The former zero-row self-test `ka_gochara` global-scope asset has
been deleted from `asset_registry` (migration 563).
  ...
  2. W2G writes exclusively to its own surface (own asset_id
     `ka_gochara` + own table `kala_gochara_windows_v2`, per
...
FROZEN ORCHESTRATOR CONTRACT (§N.2), conformed to, not extended:
  * `@register('ka_gochara')` on a `WriterBase` subclass;
...
class KaGocharaWriter(WriterBase):
    ...
    def plan_substeps(self, ctx: ContextSpec) -> list[SubStep]:
        ...
        try:
            ...
            logger.warning("[ka_gochara] could not discover event_classes for "
    ...
    def run_substep(self, ctx: ContextSpec, step: SubStep) -> WriterResult:
        ...
        for row in result.rows:
            ...
            try:
                ...
                collisions += 1
                logger.warning(
                    "[ka_gochara] row insert collision for chart=%s event_class=%s "
...
__all__ = ["BODIES", "TABLE", "KaGocharaWriter"]

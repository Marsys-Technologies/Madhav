#!/usr/bin/env python3
"""
Direct runner for ka_yojaka against prod DB (Cloud SQL Auth Proxy at 127.0.0.1:5433).
Bypasses the full orchestrator UI to run the writer standalone.

Usage:
    python run_ka_yojaka_prod.py [--dry-run]
"""
import os, sys, uuid, logging, argparse
logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s %(message)s")
logger = logging.getLogger("ka_yojaka_runner")
sys.path.insert(0, os.path.dirname(__file__))

import psycopg2
from pipeline.orchestrator.writers import ContextSpec, _REGISTRY
import pipeline.orchestrator.writers.ka_yojaka  # noqa: F401 — registers via @register
...
def main():
    ...
    writer_cls = _REGISTRY.get("ka_yojaka")
    if writer_cls is None:
        print("ERROR: ka_yojaka not in registry", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"ka_yojaka: {result.rows_inserted} rows written for chart {CHART_ID}")

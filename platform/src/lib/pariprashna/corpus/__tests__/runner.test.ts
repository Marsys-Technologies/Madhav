...
describe('runCorpus', () => {
  it('skips the not-runnable fixtures (door_parity, all 5) and runs the rest, producing a schema-valid report', async () => {
    const runTurn = async (fixture: CorpusFixture): Promise<TurnObservation> => baseObservation({ fixture })
    const notRunnableCount = CORPUS_FIXTURES.filter((f) => !f.expected.runnable).length
    const runnableCount = CORPUS_FIXTURES.length - notRunnableCount
    ...
    expect(report.summary.fixtures_total).toBe(CORPUS_FIXTURES.length)
    expect(report.summary.fixtures_run).toBe(runnableCount)
    expect(report.summary.fixtures_skipped).toBe(notRunnableCount)

    const doorParityResults = report.fixture_results.filter((f) => f.query_class === 'door_parity')
    expect(doorParityResults.length).toBeGreaterThan(0)
    for (const r of doorParityResults) {
      expect(r.ran).toBe(false)
      expect(r.skipped_reason).toContain('G4-B')
      expect(r.dimension_results).toEqual([])
    }
  })
  ...
  it('aggregates mean_score_by_dimension correctly across fixtures that were actually run', async () => {
    ...
    const runTurn = async (fixture: CorpusFixture): Promise<TurnObservation> => baseObservation({ fixture })
    const runnableFixtures = CORPUS_FIXTURES.filter((f) => f.expected.runnable)

    const report = await runCorpus({
      fixtures: runnableFixtures,
      runTurn,
      target: { kind: 'mocked_pipeline', description: 'in-memory fixture mock' },
    })

    expect(report.summary.fixtures_run).toBe(runnableFixtures.length)
  ...
  it('respects an injected fixtures subset', async () => {
    ...
    expect(report.summary.fixtures_total).toBe(onlyFactual.length)
    expect(report.fixture_results).toHaveLength(onlyFactual.length)
  })
})

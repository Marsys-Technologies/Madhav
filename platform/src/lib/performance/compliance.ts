 ...
 * Mirrors `inferLayer()` in `platform/src/app/api/chat/consume/route.ts`.
 */
...
/**
 * Parse citations out of synthesis text. Returns one CitationLite per
 * `(→ ...)` marker found. The layer is heuristically inferred from the
 * citation body using prefix conventions (`FORENSIC`, `MSR`, etc.).
 * Used by `writeConsumePerformanceRow` to feed B.10.
 */
export function parseCitations(synthesisText: string): CitationLite[] {
  const text = synthesisText ?? ''
  const markers = text.match(/\(→[^()]+\)/g) ?? []
  return markers.map((raw, idx) => {
    ...
    return { source_layer: layer, citation_id: `cite_${idx}_${body.slice(0, 40)}` }
  })
}

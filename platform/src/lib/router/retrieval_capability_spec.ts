...
export const RETRIEVAL_CAPABILITY_SPEC: readonly RetrievalCapabilityEntry[] = [
  ...
  // ── GISMCP-R2 + UDA additions (added to spec at CI-AUDIT-2026-05-30) ──────
  { tool_name: 'query_transits_over_natal', description: 'Active transits of all 9 grahas over the native natal chart positions. Returns transit planet, natal planet, aspect type, orb, and whether the transit is separating or applying.', data_surface: 'L1 — sidecar /transits_over_natal endpoint + ephemeris.', supported_params: '{ chart_id?: string; date?: string (ISO); orb_deg?: number }', optimal_patterns: ['Current transits: {}', 'Specific date: {date:"2026-06-01"}'], cost_tier: 'medium', requires_temporal: true },
  { tool_name: 'query_yogas_active_now', description: 'Yogas currently active for the native based on running dasha and transit. Returns yoga name, type, strength, and dasha trigger.', data_surface: 'L1 — chart_facts yoga rows + dasha current period.', supported_params: '{ chart_id?: string; date?: string; ayanamsha_id?: string }', optimal_patterns: ['Active yogas now: {}', 'For specific date: {date:"2026-06-01"}'], cost_tier: 'medium', requires_temporal: true },
  { tool_name: 'get_planet_avastha', description: 'Planetary avastha (state/condition) for a specific planet — baladi (infant to dead), jagradi (waking/dreaming/sleeping), lajjitadi, and deeptadi avasthas.', data_surface: 'L1 — chart_facts avastha category rows.', supported_params: '{ chart_id?: string; planet: string; ayanamsha_id?: string }', optimal_patterns: ['Sun avastha: {planet:"Sun"}', 'Moon avastha: {planet:"Moon"}'], cost_tier: 'low', requires_temporal: false },
  { tool_name: 'get_shadbala_full', description: 'Full shadbala (six-fold planetary strength) breakdown for all 9 grahas. Returns sthana, dig, kala, cheshta, naisargika, and drig balas plus the total rupa and ishta/kashta.', data_surface: 'L1 — chart_facts shadbala category rows (migration 072+).', supported_params: '{ chart_id?: string; ayanamsha_id?: string }', optimal_patterns: ['Full shadbala: {}', 'Compare planetary strengths: {}'], cost_tier: 'medium', requires_temporal: false },
  { tool_name: 'query_jaimini_chara_dasha', description: 'Jaimini Chara Dasha active period for the native — current mahadasha rashi, start/end dates, and the running antardasha. Foundational Jaimini timing tool.', data_surface: 'L1 — sidecar /jaimini_chara_dasha endpoint.', supported_params: '{ chart_id?: string; date?: string; ayanamsha_id?: string }', optimal_patterns: ['Current Jaimini dasha: {}', 'For date: {date:"2026-06-01"}'], cost_tier: 'medium', requires_temporal: true },
  { tool_name: 'query_planetary_period_predictions', description: 'Classical shastra predictions for the current MD/AD planetary period from BPHS, Phaladeepika, and Brihat Jataka. Returns prediction text, source text, and confidence level.', data_surface: 'L1 — rag_chunks filtered by current dasha lords.', supported_params: '{ chart_id?: string; mahadasha?: string; antardasha?: string; limit?: number }', optimal_patterns: ['Current period predictions: {}', 'Saturn-Rahu period: {mahadasha:"Saturn",antardasha:"Rahu"}'], cost_tier: 'high', requires_temporal: false },
  { tool_name: 'query_dasamsha_career', description: 'D10 (Dasamsha) career indicators — ascendant, planet placements with dignity, career_indicators array, and 10th lord analysis.', data_surface: 'L1 — chart_facts D10 category rows.', supported_params: '{ chart_id?: string; ayanamsha_id?: string }', optimal_patterns: ['Career analysis: {}', 'D10 overview: {}'], cost_tier: 'medium', requires_temporal: false },
  { tool_name: 'query_shashtiamsha', description: 'D60 (Shashtiamsha) — past karma divisional chart. Returns D60 ascendant, planet signs, and shashtiamsha names (e.g. Ghora, Amrita) for each graha.', data_surface: 'L1 — chart_facts D60 category rows.', supported_params: '{ chart_id?: string; ayanamsha_id?: string }', optimal_patterns: ['D60 chart: {}', 'Past karma indicators: {}'], cost_tier: 'medium', requires_temporal: false },
  { tool_name: 'query_eclipse_transits', description: 'Solar and lunar eclipse proximity for a date range. Returns eclipse date, type (solar/lunar), totality, and whether the native\'s natal positions are within orb.', data_surface: 'L1 — sidecar /eclipse_transits endpoint + ephemeris.', supported_params: '{ chart_id?: string; start_date?: string; end_date?: string; orb_deg?: number }', optimal_patterns: ['Upcoming eclipses: {}', 'Eclipse window: {start_date:"2026-01-01",end_date:"2027-01-01"}'], cost_tier: 'medium', requires_temporal: true },
  { tool_name: 'query_planet_war', description: 'Graha yuddha (planetary war) detection — which planets are currently within 1° of each other, who wins the war, and the astrological implications.', data_surface: 'L1 — chart_facts planet_war category rows + ephemeris.', supported_params: '{ chart_id?: string; date?: string }', optimal_patterns: ['Current planetary wars: {}', 'For date: {date:"2026-06-01"}'], cost_tier: 'low', requires_temporal: true },
  { tool_name: 'query_drekkana_drishti', description: 'D3 (Drekkana) divisional chart with drishti (aspect) analysis — per-planet drekkana sign, sign type (fiery/earthy/airy/watery), drishti targets, and mutual aspects.', data_surface: 'L1 — chart_facts D3 category rows.', supported_params: '{ chart_id?: string; ayanamsha_id?: string }', optimal_patterns: ['Drekkana aspects: {}', 'D3 chart with aspects: {}'], cost_tier: 'medium', requires_temporal: false },
  { tool_name: 'query_remedies_prescribed', description: 'Classical remedies prescribed for the native based on afflicted planets and houses — mantras, gemstones, fasting, charity, and yantra recommendations from authoritative texts.', data_surface: 'L1 — chart_facts remedies category rows.', supported_params: '{ chart_id?: string; planet?: string; house?: number; category?: string }', optimal_patterns: ['All remedies: {}', 'Saturn remedies: {planet:"Saturn"}'], cost_tier: 'low', requires_temporal: false },
  { tool_name: 'tara_balam_for_native', description: 'Tara Bala (Star Strength) for the native — the relative strength of the Moon nakshatra to each of the 27 nakshatras, used for muhurta and transit selection.', data_surface: 'L1 — sidecar /tara_balam endpoint.', supported_params: '{ chart_id?: string; date?: string }', optimal_patterns: ['Tara bala now: {}', 'For date: {date:"2026-06-01"}'], cost_tier: 'low', requires_temporal: true },
  { tool_name: 'chandra_balam_for_native', description: 'Chandra Bala (Moon Strength) score (1–12) indicating the relative strength of the current Moon sign to the native\'s natal Moon sign.', data_surface: 'L1 — sidecar /chandra_balam endpoint.', supported_params: '{ chart_id?: string; date?: string }', optimal_patterns: ['Chandra bala now: {}', 'For date: {date:"2026-06-01"}'], cost_tier: 'low', requires_temporal: true },
  { tool_name: 'muhurta_finder', description: 'Auspicious time-window finder — scores time slots by panchanga factors (tithi, vara, nakshatra, yoga, karana) plus tara bala and chandra bala. Returns ranked windows with factor breakdown.', data_surface: 'L1 — sidecar /muhurta_finder endpoint.', supported_params: '{ chart_id?: string; start_date: string; end_date: string; activity?: string; min_score?: number }', optimal_patterns: ['Best windows next week: {start_date:"2026-06-01",end_date:"2026-06-07"}', 'Marriage muhurta: {activity:"marriage",start_date:"...",end_date:"..."}'], cost_tier: 'high', requires_temporal: true },
  { tool_name: 'query_tara_balam', description: 'Canonical-name alias for tara_balam_for_native. Tara Bala (Star Strength) for the native.', data_surface: 'L1 — sidecar /tara_balam endpoint.', supported_params: '{ chart_id?: string; date?: string }', optimal_patterns: ['Tara bala now: {}', 'For specific date: {date:"2026-06-01"}'], cost_tier: 'low', requires_temporal: true },
  { tool_name: 'query_chandra_balam', description: 'Canonical-name alias for chandra_balam_for_native. Chandra Bala (Moon Strength) score.', data_surface: 'L1 — sidecar /chandra_balam endpoint.', supported_params: '{ chart_id?: string; date?: string }', optimal_patterns: ['Chandra bala now: {}', 'For specific date: {date:"2026-06-01"}'], cost_tier: 'low', requires_temporal: true },
  { tool_name: 'jaimini_chara_dasha', description: 'Canonical-name alias for query_jaimini_chara_dasha. Active Jaimini Chara Dasha period for the native.', data_surface: 'L1 — sidecar /jaimini_chara_dasha endpoint.', supported_params: '{ chart_id?: string; date?: string }', optimal_patterns: ['Current Jaimini dasha: {}', 'For specific date: {date:"2026-06-01"}'], cost_tier: 'medium', requires_temporal: true },
  { tool_name: 'jaimini_chara_dasha_full', description: 'Full Jaimini Chara Dasha timeline from birth to age 120 — all mahadasha rashis with start/end dates and nested antardashas.', data_surface: 'L1 — sidecar /jaimini_chara_dasha_full endpoint.', supported_params: '{ chart_id?: string; ayanamsha_id?: string }', optimal_patterns: ['Full dasha timeline: {}', 'Complete Jaimini schedule: {}'], cost_tier: 'high', requires_temporal: false },
  { tool_name: 'query_chart_facts', description: 'Surgical chart_facts lookup by category. Returns structured L1 facts (planet positions, house cusps, dignities, divisional charts, yogas, shadbala, ashtakavarga, etc.) for the requested category.', data_surface: 'L1 — table chart_facts (147 categories, migration 134+).', supported_params: '{ chart_id?: string; category: string; ayanamsha_id?: string; limit?: number }', optimal_patterns: ['Planetary positions: {category:"planets"}', 'House cusps: {category:"house_cusps"}', 'Yogas: {category:"yogas"}'], cost_tier: 'low', requires_temporal: false },
  { tool_name: 'query_varshphal', description: 'Canonical MCP name alias for query_varshaphala. Varshaphala (Tajika) annual Solar Return chart for a specific year or the active year window.', data_surface: 'L1 — table varshaphala (migration 025).', supported_params: '{ chart_id?: string; year?: number; year_start?: number; year_end?: number; ayanamsha?: string }', optimal_patterns: ['Current year Varshaphala: {}', 'Specific year: {year:2026}'], cost_tier: 'low', requires_temporal: true },
] as const

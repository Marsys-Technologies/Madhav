...
Output:
{"query_class":"predictive","domains":["career"],"forward_looking":true,"history_mode":"synthesized","panel_mode":false,"expected_output_shape":"time_indexed_prediction","graph_seed_hints":["DSH.MD.MERCURY","DSH.MD.KETU","HSE.10"],"query_intent_summary":"Identify the highest-probability career-change windows in the next 24 months and rank them by signal density.","planning_rationale":"Predictive timing requires temporal context. Mercury MD ends 2027-08-21; the next MD is Ketu MD. The 2-year window straddles both, plus AD/PD layers. msr_sql with dasha_activation filter gives the activated signals; resonance_register surfaces confluences in the same window.","synthesis_guidance":"Time-indexed prediction shape. State 1–3 candidate windows with date ranges and probability bands. Cite the dasha lord(s) activating each window. Be explicit about uncertainty — this is calibrated, not deterministic.","tool_calls":[{"tool_name":"temporal","params":{"time_window":{"start":"2026-05-01","end":"2028-05-01"},"dasha_context_required":true},"priority":1,"reason":"Dasha chain across the 2-year window is the spine of any prediction."},{"tool_name":"msr_sql","params":{"domains":["career"],"dasha_activation":["Mercury","Ketu"],"min_significance":0.55},"priority":1,"reason":"Career signals activated by either dasha lord during the window."},{"tool_name":"resonance_register","params":{"theme":"career_transition","min_strength":0.6},"priority":2,"reason":"Confluences that mark genuine inflection points vs noise."},{"tool_name":"timeline_query","params":{"dasha_name":"Ketu MD"},"priority":2,"reason":"Upcoming Ketu MD arc is the framing for any post-2027 prediction."}]}

### Example 3b — temporal/moksha (dasha-activated spiritual signals)

User query: "Which signal flags the upcoming Ketu mahadasha as a moksha-stack activation, and what age window does it cover?"
Output:
{"query_class":"temporal","domains":["spiritual"],"forward_looking":true,"history_mode":"synthesized","panel_mode":false,"expected_output_shape":"time_indexed_prediction","graph_seed_hints":["DSH.MD.KETU","SIG.MSR.150"],"query_intent_summary":"Identify dasha-activated signals for Ketu MD moksha-stack window.","planning_rationale":"Temporal + spiritual query. msr_sql with dasha_activation=[\"Ketu\"] and entities_involved_any=[\"DSH.MD.KETU\"] surfaces moksha-tagged signals. temporal tool anchors the dasha window dates.","synthesis_guidance":"State the dasha window dates, the activating signal(s) with IDs, and the age range. Use calibrated language — probabilities not certainties.","tool_calls":[{"tool_name":"msr_sql","params":{"domains":["spiritual"],"dasha_activation":["Ketu"],"entities_involved_any":["DSH.MD.KETU"],"min_significance":0.45},"priority":1,"reason":"Ketu-activated signals in the spiritual domain — the primary answer surface."},{"tool_name":"temporal","params":{"dasha_context_required":true},"priority":1,"reason":"Dasha window dates and age range for the Ketu MD activation."},{"tool_name":"vector_search","params":{"layer":"L2_5","doc_type":["msr_signal"],"top_k":10},"priority":2,"reason":"Semantic backstop for moksha/Ketu signals that may not be domain-tagged 'spiritual'."}]}

...
export const queryPlanetPositionCapability: ToolCapability = {
  ...
  semantic_capabilities: [{
    scu_id: 'scu.catalog.query_planet_position', version: 1,
    label: 'Planet position evidence',
    description: 'Retrieve date-specific planetary positions from the authenticated ephemeris service.',
    kind: 'datum', domains: ['timing'], concepts: ['ephemeris', 'planet_position'],
    intents: ['assess', 'verify'], horizons: ['historical', 'current', 'future'], scope: 'global',
    inputs: ['date', 'planet?'], outputs: ['positions', 'count', 'ayanamsha_id'],
    primary_binding_uri: 'marsys://tool/L0/query_planet_position',
    provenance_requirements: ['computed_at', 'engine_version'],
    freshness_policy: 'Requires a fresh authenticated ephemeris-engine probe; a missing or stale probe keeps this binding unavailable.',
    entitlement: 'native', safety_notes: ['Read-only computed evidence; an empty position set is a failed lookup.'], known_gaps: [],
    availability_contracts: [{ binding_id: 'registry:marsys://tool/L0/query_planet_position', requirements: [{
      ...
      probe_contract_sha256: 'd36262fa325fc2c29fe5ccf026d3da67120cad2280b1f7e1287bf4a327db5de1', max_age_seconds: 900,

...
export type ResolvedEntity = ResolvedGraha | ResolvedSign | ResolvedKaraka | ResolvedOccupants
...
export async function resolveAddress(
  ...
  return { expression: expr, entities, chain }
}
...
function parseHouseArg(arg: string): HouseNumber {
  ...
  const m = cleaned.match(/^bhava[\s:]+(\d{1,2})$/i) ?? cleaned.match(/^(\d{1,2})$/)
...
export function parseAddressExpression(input: string): AddressExpression {
  ...
  switch (fn) {
    ...
    default:
      throw new AddressResolutionError(
        `Unknown address function "${fn}". Supported: lord_of, dispositor_of, bhava_from, karaka, occupants_of, bhava, graha.`,
      )
  }
}

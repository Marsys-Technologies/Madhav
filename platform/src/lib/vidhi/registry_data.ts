 ...
 *
 * 52 versioned primitives (design §3 targets "~30"; expanded to guarantee full §B0.4
 * mandatory-surface coverage; the 51st–52nd — `argala_read` / `dispositor_closure_read` — added by
 * PARIŚODHANA B2 (Ω8 floor-wiring), which also widened `special_lagna_read` (full lagna+saham+Upapada)
 * and `ashtakavarga_scan` (full per-varga AV family) and spliced the Ω8 reachability band into every
 * `*_deepdive` floor so the served floor matches REGENERATED_FLOORS_v1_0.json. Earlier expansion:
...
export const VIDHI_PRIMITIVES: readonly VidhiPrimitive[] = [
  ...
  },
  {
    ...
    category: 'structural',
    live_tool: 'ganita_transit_anchors_get',
  ...
  },
  {
    ...
    version: 1,
    // PARIŚODHANA B2 (Ω8 floor-wiring): widened from a 2-lagna filter (Indu/Sree) to the FULL
    // special-lagna + saham + Upapada set — 7 special lagnas (Indu/Sree/Ghati/Hora/Bhava/
    // Varnada/Vighati) + 70 sahams + Upapada = 78 concepts (REGENERATED_FLOORS_v1_0.json). The
    // `lagnas` filter is dropped from the default args so the primitive serves the whole stored
    // special-lagna fact set (ganita_special_lagnas_get with just chart_id serves all stored
    // special-lagna facts — CR-16 note below); a floor may still narrow via args_override.
    definition: 'Reads the FULL special-lagna + saham set — 7 special lagnas (Indu, Sree, Ghati, Hora, Bhava, Varnada, Vighati) + 70 sahams + Upapada — with domain salience.',
    category: 'structural',
  ...
  },
  {
    ...
    version: 1,
    // PARIŚODHANA B2 (Ω8 floor-wiring): widened from D1-only bindu-sign to the FULL per-varga
    // Ashtakavarga family (ashtakavarga_bindu / _sign / _per_varga / pinda_* / *_shodhana /
    // kakshya — 504 concepts/domain per REGENERATED_FLOORS_v1_0.json). The `category: 'ashtakavarga'`
    // selector is the family prefix ganita_chart_facts_get matches against, so it already routes the
    // whole AV family; this widening is the definition catching up to what the floor slice accounts.
    definition: 'Full Ashtakavarga family scan (per-varga bindu / bindu-sign / pinda / shodhana / kakshya) per graha/house — not D1-only.',
    category: 'structural',
    live_tool: 'ganita_chart_facts_get',
  ...
  // ── PARIŚODHANA B2 (Ω8 floor-wiring) — two new structural-sweep primitives ──
  // Both re-probed against the live serving surface: argala_natal_matrix / virodha_argala_natal_matrix
  // / net_argala_per_varga are served by ganita_chart_facts_get (get_argala face; coverage_matrix.ts)
  // and composite_dispositor_strength by ganita_chart_facts_get (get_dispositors face) with the
  // CGM chain classes via bodha_mechanisms_get — no new computation (B.10). These were REGISTERED in
  // REGENERATED_FLOORS_v1_0.json as domain-relevant floor items but had no primitive to wire them.
  {
    // Argala (intervention) + virodha-argala structure on the bhavas — 1044 concepts/domain.
    primitive_id: 'argala_read',
    version: 1,
    definition:
      'Argala (intervention) + virodha-argala matrix per varga for a house/point — the natal ' +
      'argala_natal_matrix, virodha_argala_natal_matrix, and net_argala_per_varga (intervention ' +
      'structure on the bhāvas). A structural sweep, not a single-fact read.',
    category: 'structural',
    live_tool: 'ganita_chart_facts_get',
    tool_args: { chart_id: '{chart_id}', category: 'net_argala_per_varga' },
    fallback_face: 'ganita_structural_get',
    known_gap: null,
    mandatory_tags: [],
    cr27_prevents: [],
  ...
  {
    // Full dispositor-chain closure — 607 concepts/domain — the composite dispositor strengths +
    // the convergent-dispositor-chain / dispositor-cycle CGM mechanism classes.
    primitive_id: 'dispositor_closure_read',
    version: 1,
    definition:
      'Full dispositor-chain closure: composite_dispositor_strength facts + the convergent- ' +
      'dispositor-chain / dispositor-cycle mechanism classes (the who-ultimately-controls-whom ' +
      'lordship closure across the chart).',
    category: 'structural',
    live_tool: 'ganita_chart_facts_get',
    tool_args: { chart_id: '{chart_id}', category: 'composite_dispositor_strength' },
    fallback_face: 'bodha_mechanisms_get',
    known_gap: null,
    mandatory_tags: [],
    cr27_prevents: [],
  },
...
/**
 * Ω8 reachability band (PARIŚODHANA B2). The acharya-band items each `*_deepdive` floor gains so
 * its SERVED floor matches the TCI-regenerated floor in `REGENERATED_FLOORS_v1_0.json` (the floor
 * CONSUMER the Ω8 lane parked as "blocked-on-α"): the domain-relevant divisional vargas, the
 * widened Ashtakavarga + special-lagna/saham family scans, the argala + dispositor-closure
 * structural sweeps, and the cross-ayanamsha agreement axis. Every item resolves to a live tool
 * (B.10) — no new computation; the underlying categories were confirmed served (get_argala /
 * get_dispositors / ashtakavarga family via ganita_chart_facts_get). `from` is a high starting
 * order kept UNIQUE within each floor (registry_completeness asserts order uniqueness, not
 * contiguity; band partitioning in the compiler places these at the end of the acharya band). Each
 * floor passes ONLY the items it does not already carry, so no (primitive, args) read is
 * duplicated — the parity gate checks PRESENCE, satisfied by the union of pre-existing + Ω8 items.
 */
function omega8Band(opts: {
  from: number;
  divisionalVargas?: readonly string[];
  ashtakavarga?: boolean;
  specialLagna?: boolean;
  argala?: boolean;
  dispositorClosure?: boolean;
  crossAyanamshaPoint?: string;
}): FloorItem[] {
  const items: FloorItem[] = [];
  let order = opts.from;
  for (const varga of opts.divisionalVargas ?? []) {
    items.push({ primitive_id: 'divisional_facts', order: order++, band: 'acharya_floor', args_override: { varga } });
  }
  if (opts.ashtakavarga) items.push({ primitive_id: 'ashtakavarga_scan', order: order++, band: 'acharya_floor' });
  if (opts.specialLagna) items.push({ primitive_id: 'special_lagna_read', order: order++, band: 'acharya_floor' });
  if (opts.argala) items.push({ primitive_id: 'argala_read', order: order++, band: 'acharya_floor' });
  if (opts.dispositorClosure) items.push({ primitive_id: 'dispositor_closure_read', order: order++, band: 'acharya_floor' });
  if (opts.crossAyanamshaPoint) {
    items.push({ primitive_id: 'cross_ayanamsha_variation', order: order++, band: 'acharya_floor', args_override: { point: opts.crossAyanamshaPoint } });
  }
  return items;
}
...
const WEALTH_DEEPDIVE_ITEMS: readonly FloorItem[] = [
  ...
  // PARIŚODHANA B2 (Ω8): the lagnas narrowing ['indu','sree'] is dropped so the flagship wealth
  // floor serves the FULL special-lagna + saham + Upapada set (78 concepts), matching the regen floor.
  { primitive_id: 'special_lagna_read', order: 14, band: 'acharya_floor' },
  ...
  // PARIŚODHANA B2 (Ω8 reachability band; orders 40+). wealth floor_vargas {D1,D2,D9,D11} — D2
  // already present (order 10); adds D1/D9/D11 + the argala + dispositor-closure sweeps. Ashtakavarga,
  // special-lagna, and cross-ayanamsha (Jupiter) are already carried above.
  ...omega8Band({ from: 40, divisionalVargas: ['D1', 'D9', 'D11'], argala: true, dispositorClosure: true }),
];
...
const CAREER_DEEPDIVE_ITEMS: readonly FloorItem[] = [
  ...
  // PARIŚODHANA B2 (Ω8 reachability band; orders 40+). career floor_vargas {D1,D9,D10} — D9/D10
  // already present (orders 5/6); adds D1 + the special-lagna/saham, argala, dispositor-closure and
  // cross-ayanamsha (Sun, the karyesha kāraka) sweeps. Ashtakavarga already carried above.
  ...omega8Band({ from: 40, divisionalVargas: ['D1'], specialLagna: true, argala: true, dispositorClosure: true, crossAyanamshaPoint: 'SUN' }),
];

...
describe('POST /api/build/start — input validation', () => {
  ...
  it('returns 400 when ayanamsha_role is invalid', async () => {
    const res = await POST(makeReq({ chart_id: 'c1', ayanamsha_role: 'nope' }))
    expect(res.status).toBe(400)
  })
})
...
describe('POST /api/build/start — happy path', () => {
  it('owner enqueues a build and gets { build_id, task_name }', async () => {
    ...
    const res = await POST(makeReq({ chart_id: 'c1', ayanamsha_role: 'jh_true_chitra' }))
    expect(res.status).toBe(200)
    const body = (await res.json()) as { build_id: string; task_name: string }
    expect(body.build_id).toBe('build-7')
    expect(body.task_name).toMatch(/^projects\//)
    ...
    expect(arg.ayanamshaRole).toBe('jh_true_chitra')

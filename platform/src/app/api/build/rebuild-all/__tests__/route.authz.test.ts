 ...
 * could enqueue a full-chart rebuild event against ANY chart_id, regardless of
 * ownership. Gate level is 'write' (permission === 'all'), matching the sibling
 * B-008 fixes (POST /api/cockpit/runs, /api/cockpit/refresh): a `chart_grants`
 * 'view' grantee may read a chart, not trigger a rebuild against it.
 */
...
function makeReq(body: object): NextRequest {
  return new NextRequest('http://localhost/api/build/rebuild-all', {
    method: 'POST',
...
function setupMocks(opts: {
  ...
  mockQuery.mockImplementation((sql: string) => {
    ...
    if (/INSERT INTO build_events/.test(sql)) {
      return Promise.resolve({ rows: [{ build_id: 'generated-build-id' }], rowCount: 1 })
    }
...
describe('POST /api/build/rebuild-all — V3-E-010 cross-tenant build_events write', () => {
  it('DENIES a non-owner guest — and writes NO build_events row', async () => {
    ...
    const res = await POST(makeReq({ chart_id: VICTIM_CHART }))
  ...
  it('ALLOWS the owner — the legitimate rebuild-all path must keep working', async () => {

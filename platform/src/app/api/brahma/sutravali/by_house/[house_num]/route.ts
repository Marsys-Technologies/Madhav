/**
 * GET /api/brahma/sutravali/by_house/[house_num]
 *
 * Proxy to Python sidecar: /api/brahma/sutravali/by_house/{house_num}
 * Returns all sutravali rules for the given house number (1-12).
 * Internal endpoint used by MCP resource marsys://resource/sutravali/all-by-house/{n}
 *
...
export async function GET(
  ...
  const { house_num } = await params
  const houseN = parseInt(house_num, 10)
  if (isNaN(houseN) || houseN < 1 || houseN > 12) {
    return NextResponse.json({ error: 'house_num must be 1-12' }, { status: 400 })
  }
  const limit = req.nextUrl.searchParams.get('limit') ?? '500'

  try {
    const sidecarResp = await fetch(
      `${SIDECAR_URL}/api/brahma/sutravali/by_house/${houseN}?limit=${limit}`,
      {
    ...
    console.error('[sutravali/by_house] sidecar fetch error:', err)

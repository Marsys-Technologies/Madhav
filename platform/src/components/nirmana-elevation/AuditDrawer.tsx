...
export function AuditDrawer({ snapshot, assetId, open, onOpenChange }: {
  ...
  if (!open) {
    return <div className="flex justify-end">
      <button type="button" aria-haspopup="dialog" onClick={() => onOpenChange(true)} className="inline-flex items-center gap-2 rounded-lg border border-brand-border bg-brand-surface px-3 py-2 text-xs font-medium text-brand-text-2 transition-colors hover:border-brand-gold-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold-1">
  ...
  return <>
    <div className="fixed inset-0 z-40 bg-black/45" aria-hidden="true" onClick={() => onOpenChange(false)} />
    <aside ref={drawerRef} role="dialog" aria-modal="true" aria-label="Campaign evidence audit" tabIndex={-1} className="fixed inset-y-0 right-0 z-50 flex w-full max-w-lg flex-col overflow-y-auto border-l border-brand-border bg-brand-bg shadow-2xl focus:outline-none">
      <header className="sticky top-0 flex items-start justify-between gap-3 border-b border-brand-border bg-brand-bg px-4 py-3">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold text-brand-text-1">Campaign evidence audit</h2>
          <p className="mt-1 text-xs text-brand-text-3">{assetId ? <>Filtered to <span className="font-mono">{assetId}</span></> : 'All campaign assets'} · Data quality: <span className="capitalize">{snapshot.data_quality.verdict}</span></p>
        </div>
        <button type="button" aria-label="Close audit drawer" onClick={() => onOpenChange(false)} className="rounded p-1.5 text-brand-text-3 transition-colors hover:bg-brand-surface hover:text-brand-text-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-gold-1"><X aria-hidden="true" className="size-4" /></button>
      </header>

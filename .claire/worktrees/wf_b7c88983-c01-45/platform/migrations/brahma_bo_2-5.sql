...
--   DROP FUNCTION IF EXISTS bodha_query_signals_with_lens(TEXT, TEXT, TEXT, NUMERIC, NUMERIC, TEXT, INT, INT);
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $$
    SELECT
        ...
        -- agreement_score = avg(confidence × salience) — higher when signals agree strongly
...
'BO-2-5 concordance lens: groups bodha_signals by domain+valence, ranks by agreement_score = avg(confidence×salience). High score = school consensus; low score = weak or split signals. Native chart (Abhisek Mohanty 1984-02-05): expect Saturn-career signals to cluster near top.';
...
RETURNS TABLE (
    ...
    graph_edge_id   TEXT,   -- NULL if no CONTRADICTS edge in bodha_graph
...
AS $$
    SELECT
        ...
        -- tension_score: high when both sides are high-confidence contradictions
    ...
    JOIN public.bodha_signals b
        ...
        AND a.signal_id < b.signal_id   -- avoid symmetric duplicates
        AND (
              -- benefic vs malefic is the primary contradiction
              (a.valence = 'benefic'  AND b.valence = 'malefic')
           OR (a.valence = 'malefic' AND b.valence = 'benefic')
              -- context-dependent vs. malefic is also tension-surfacing
...
'BO-2-5 contradiction lens: surfaces same-domain signal pairs with opposing valence (benefic vs malefic). tension_score = product of confidences × saliencies. Links to bodha_graph CONTRADICTS edges when they exist. For native chart: expect ≥1 contradiction-hub (conflicting signals on same domain).';


-- ── §3 — Negative Space Lens ──────────────────────────────────────────────────
--
-- Computes what is NOT signalled: domains and grahas where the chart has no
-- signals at all, or only low-confidence signals (< 0.4). The absence of signals
-- in a domain is itself astrologically informative.

CREATE OR REPLACE FUNCTION bodha_negative_space(
    p_chart_id  TEXT
)
RETURNS TABLE (
    check_type          TEXT,   -- 'domain_absent' | 'graha_absent' | 'domain_weak'
    label               TEXT,   -- the domain or graha name
    signal_count        BIGINT,
...
AS $$
    ...
    -- 1. Domains with no signals at all
    SELECT
        'domain_absent'::TEXT                  AS check_type,
        ed.domain                              AS label,
        0::BIGINT                              AS signal_count,
        0::NUMERIC                             AS max_confidence,
        'Domain '|| ed.domain ||' has NO signals — potential blank in the reading' AS interpretation_hint
    ...
    -- 2. Domains where all signals are weak (max confidence < 0.4)
    SELECT
        'domain_weak'::TEXT                    AS check_type,
        ad.domain                              AS label,
        ad.signal_count,
        ROUND(ad.max_confidence::NUMERIC, 4)  AS max_confidence,
        'Domain '|| ad.domain ||' exists ('||ad.signal_count||' signals) but max confidence='||
            ROUND(ad.max_confidence::NUMERIC,2)||' — weak coverage, consider deepening' AS interpretation_hint
    FROM actual_domains ad
    WHERE ad.max_confidence < 0.4

    UNION ALL

    -- 3. Grahas with no signals
    SELECT
        'graha_absent'::TEXT                   AS check_type,
        eg.graha                               AS label,
        0::BIGINT                              AS signal_count,
        0::NUMERIC                             AS max_confidence,
        'Graha '|| eg.graha ||' has NO signals — potential blind spot or very quiet planet' AS interpretation_hint
...
-- ── §4 — Salience Lens ────────────────────────────────────────────────────────
--
-- Ranks signals by a composite salience score:
--   composite_salience = confidence × salience × reinforcement_weight × domain_weight
--
-- reinforcement_weight: 1 + (count of REINFORCES edges in bodha_graph for this signal / 10)
--   — signals that are reinforced by more graph edges rank higher
--
-- domain_weight: configurable per call; defaults are:
--   career=1.0, wealth=0.9, relationships=0.85, health=0.8, spirit=0.75, others=0.7
...
'BO-2-5 salience lens: ranks signals by composite_salience = confidence × base_salience × (1+reinforcement_count/10) × domain_weight. Domain weights: career=1.0, wealth=0.9, ..., house_domain=0.65. Reinforced signals (REINFORCES edges in bodha_graph) rank higher.';


-- ── §5 — Unified query_signals with lens parameter ────────────────────────────
--
-- Extends the base query_signals pattern (from BO-2-1) with a lens= parameter
-- that activates one of the four lens computations. When lens is NULL, falls
-- back to the plain signal list ordered by salience DESC, confidence DESC.

CREATE OR REPLACE FUNCTION bodha_query_signals_with_lens(
    p_chart_id      TEXT,
    p_lens          TEXT    DEFAULT NULL,   -- concordance|contradiction|salience|negative_space|NULL
...
BEGIN
    CASE LOWER(COALESCE(p_lens, 'default'))
        ...
        ELSE
            -- Default: plain signal list (BO-2-1 behaviour)
...
'BO-2-5 unified entry point: bodha_query_signals_with_lens(chart_id, lens, ...). lens=concordance|contradiction|salience|negative_space|NULL. NULL falls back to plain BO-2-1 signal list. All lens functions query bodha_signals + bodha_graph in-place; no new tables.';
...
--   bodha_graph (BO-2-2) must be seeded for concordance/contradiction/salience lenses
--   that use REINFORCES/CONTRADICTS edges.
--
-- MCP tool: platform-mcp/src/tools/bo_2-5.ts → query_signals (lens param added)
--
-- Python engine: platform/python-sidecar/brahmagyan/bodha/bo_2-5.py
--
-- Acceptance gate (BO-2-5):
--   Native chart test: SELECT * FROM bodha_contradiction_lens('<native_chart_id>', 0.3)
--     → must return ≥1 row (at least one contradiction-hub)
--   SELECT * FROM bodha_negative_space('<native_chart_id>')
--     → may return 0 rows if all domains/grahas are covered (healthy chart coverage)
--   SELECT * FROM bodha_salience_lens('<native_chart_id>', 0, 0, 5) LIMIT 5

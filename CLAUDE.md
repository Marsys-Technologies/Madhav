...
- **Phase 4C — Panchang Module (concurrent workstream)** — canonical_id `PHASE_4C_PANCHANG_BRIEF_v1_0`, path `00_ARCHITECTURE/BRIEFS/PHASE_4C_PANCHANG_BRIEF_v1_0.md`. **STATUS: WAVE_1_COMPLETE_PENDING_PR (4C-9 closed 2026-05-20; PSHIP integration in progress).** Workstream declared 2026-05-19. Branch `feature/panchang-ship` (cut from main, PSHIP series). Scope: `query_panchanga` RetrievalTool (SQL-backed, extended with JSONB columns via S3H) + `PANCHANG_DAILY` L1.5 asset + `/panchang` UI page with Muhurat Finder + iCal export + Ask-Madhav prompt deep links. Architecture: Option H hybrid (SQL precompute for planner tool; live sidecar engine for /panchang UI + Muhurat Finder). Integration sessions: PSHIP-S2H (shared-file merge, this session), PSHIP-S3H (query-tool reconciliation + schema extension), PSHIP-S4H (planner prompt, human gate), PSHIP-S5H (verification), PSHIP-S6H (PR+deploy, human gate). Engine tests: 230/230 passing. Wave 1 source on `feature/phase-4c-panchang`.
- **Conductor — Autonomous Session Orchestrator** — canonical_id `CONDUCTOR_PROMPT_v1_0`, path `00_ARCHITECTURE/CONDUCTOR/CONDUCTOR_PROMPT_v1_0.md`. **STATUS: ACTIVE — orchestrating PSHIP integration sessions.** The Conductor is project-spanning orchestration infrastructure. It walks `session_queue.yaml`, spawns sub-agents per brief, gates each session via shell tests, and halts for human approval at required checkpoints. Wave 1 scope: Phase 4C only (PSHIP-S2H through PSHIP-S6H). Context budget: 20 sub-agents per orchestrator chat; user re-kicks in a fresh chat for the next batch. Smoke test SMOKE-S0 PASSED on 2026-05-19. **Key files:** `00_ARCHITECTURE/CONDUCTOR/README.md` (operator docs), `session_queue.yaml` (live queue state), `CONDUCTOR_LOG.md` (run history), `CONDUCTOR_HALT_LOG.md` (open halts). **Architectural rule:** Conductor commits must touch ONLY `00_ARCHITECTURE/CONDUCTOR/` paths.

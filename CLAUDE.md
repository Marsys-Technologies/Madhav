...
- **Phase 4C — Panchang Module (concurrent workstream)** — canonical_id `PHASE_4C_PANCHANG_BRIEF_v1_0`, path `00_ARCHITECTURE/BRIEFS/PHASE_4C_PANCHANG_BRIEF_v1_0.md`. **STATUS: WAVE_1_COMPLETE — PR #105 merged to main (merge SHA `3b3405c`, 2026-05-20).** Workstream declared 2026-05-19 under the Phase O / Chat V2 concurrent-workstream precedent. Worktree `/Users/Dev/Vibe-Coding/Apps/Panchang/` on branch `feature/phase-4c-panchang` (base: main). Scope: `query_panchanga` RetrievalTool + `PANCHANG_DAILY` L1.5 asset + `/panchang` UI surface with Muhurat Finder + iCal export + Ask-Madhav prompt deep links. Wave 1 (4C-0 through 4C-9) complete. Phase 4B (sunrise derivation + MEAN_NODE rebuild) is a prerequisite for 4C.2 but does NOT block Wave 1 close. Next: split-PR strategy per HANDOFF_WAVE_1.md (cherry-pick Conductor → main as PR 1; Phase 4C close as PR 2). **Chat-side enrichment shipped via PR #110** (merge SHA `9bdcac24`, 2026-05-20): migration 069 (5 JSONB columns + GIN indexes on `panchanga_daily` + `panchanga_daily_staging`), `query_panchanga` 5 enrichment field groups (`special_yogas`, `choghadiya`, `hora`, `inauspicious`, `auspicious`), `PLANNER_PROMPT_v2_0.md` v2.0.7 R-PA subclauses (f)+(g) + R-PCI suppression rule + bootstrap writer. Branch `feature/panchang-enrichment` merged and deleted. Post-merge operator steps **PENDING** (human, do not automate): (1) `psql $DATABASE_URL -f platform/migrations/069_extend_panchanga_daily.sql`; (2) `bootstrap_panchanga.py --dry-run` validate; (3) `bootstrap_panchanga.py` full rebuild (~60 min, ~73K rows); (4) staging swap per `RUNBOOK_EPHEMERIS_REBUILD_v1_0.md §4`.

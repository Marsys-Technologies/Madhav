...
- **Chat V2 R7 — Polish Round** — canonical_id `CHAT_V2_R7_MASTER_PLAN`, brief path `00_ARCHITECTURE/chat_v2_briefs/round7/R7_MASTER_PLAN_v1_0.md`, queue `round7/session_queue.yaml`. **STATUS: ACTIVE (kick-off 2026-05-20).** Branch `chat-v2/round7-polish`, worktree `/Users/Dev/Vibe-Coding/Apps/MadhavR7`. 7 sessions (R7-S1 through R7-S7): citation double-wrap fix → GFM footnote citations (R6.2) → enrich citations (R6.3) → citation panel auto-open → truncation Continue button → composer draft persistence → a11y polish. Flag namespace `MARSYS_FLAG_R7_*`. No DB migrations. Merge train position 1 (smallest blast radius). **Isolation rule:** R7 may not touch R8/R9 file domains or any governance/L1/L2.5 layer.
- **Chat V2 R8 — Capabilities Round** — canonical_id `CHAT_V2_R8_MASTER_PLAN`, brief path `00_ARCHITECTURE/chat_v2_briefs/round8/R8_MASTER_PLAN_v1_0.md`, queue `round8/session_queue.yaml`. **STATUS: ACTIVE (kick-off 2026-05-20).** Branch `chat-v2/round8-capabilities`, worktree `/Users/Dev/Vibe-Coding/Apps/MadhavR8`. 8 sessions (R8-S1 through R8-S8): branches persistence → branch picker UI → sidebar FTS search → pin/archive/folders → token estimate → slash command menu → vision pipeline → conversation export. Flag namespace `MARSYS_FLAG_R8_*`. DB migrations: `conversation_branches`, `conversation_folder*` tables, `pg_trgm` index on message body. Merge train position 2 (after R7). **Isolation rule:** R8 may not touch R7 files post-merge or any R9 file domains; `search/route.ts` is shared with R9-S2 (coordinate on rebase).
- **Chat V2 R9 — Elevation Round** — canonical_id `CHAT_V2_R9_MASTER_PLAN`, brief path `00_ARCHITECTURE/chat_v2_briefs/round9/R9_MASTER_PLAN_v1_0.md`, queue `round9/session_queue.yaml`. **STATUS: ACTIVE (kick-off 2026-05-20).** Branch `chat-v2/round9-elevation`, worktree `/Users/Dev/Vibe-Coding/Apps/MadhavR9`. 4 sessions (R9-S1 through R9-S4): Projects abstraction (new tables + API + synthesis prompt integration) → semantic conversation search (vector embeddings + hybrid trgm+cosine) → persona library (ModelStylePicker + settings page) → inline tool flow timeline (admin-only disclosure in AssistantMessage). Flag namespace `MARSYS_FLAG_R9_*`. DB migrations: `projects`, `project_files`, `project_conversations`, `conversation_message_embeddings`, `personas` tables. **Operator prerequisite for R9-S2**: verify `pgvector` extension in production Postgres. Merge train position 3 (heaviest; merges last). See `MERGE_TRAIN_ORDER_v1_0.md` for full rebase sequence and conflict-resolution guidance (`search/route.ts` is the expected conflict point).
...
*End of CLAUDE.md v2.6 (amended 2026-05-20 — R7/R8/R9 elevation workstreams declared ACTIVE in §E; "Four workstreams" → "Seven workstreams"; governance branch `chat-v2/governance-r7-r9-setup` authored 19 sub-briefs + 3 master plans + 3 queues + MERGE_TRAIN_ORDER). Prior: v2.5 (2026-05-18 §M.17 — Chat V2 Big Bang COMPLETE; sealing-merge `6c431f9` PR #82); v2.4 (2026-05-13 M5 opened); v2.3 (2026-05-13 Gate II.5 close); v2.2 (2026-05-11 Pipeline-Transform-S1 close); v2.1 (2026-05-11 Phase 11B). Governance rebuild closed; M4 CLOSED 2026-05-02; M5 ACTIVE — M5-A is the active sub-phase.*

...
# Idempotent apply for the edge LB + CDN + Cloud Armor IaC.
# DO NOT run from a feature/Conductor worktree.
...
case "$CMD" in
  ...
  apply)
    ...
    echo
    echo "LB_IP=$(terraform output -raw lb_ip)"
    echo "Point DNS A-record for the configured domain here."
    ;;
  destroy)

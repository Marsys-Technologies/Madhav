#!/usr/bin/env python3
"""
ingest_bhrigu_nandi_nadi.py — Ingest Bhrigu Nandi Nadi (R.G. Rao translation).
Nadi/BNN tier. Source: archive.org.
Target: ≥200 chunks, ≥90% embedded (lenient threshold — Nadi texts are shorter).
"""
...
logger = logging.getLogger("ingest_bhrigu_nandi_nadi")

TEXT_KEY = "bhrigu_nandi_nadi"
TEXT_METADATA = {
    "text_key": TEXT_KEY, "title": "Bhrigu Nandi Nadi", "author": "Bhrigu Nandi",
    "tradition": "nadi", "school": "bnn", "tier": 4,
    "language_original": "tamil", "translation_author": "R.G. Rao",
    "source_url": "https://archive.org/search?query=bhrigu+nandi+nadi",
    "procurement_date": str(date.today()),
}
GCS_PATH = "L8/classical_texts/nadi_bnn/bhrigu_nandi_nadi_chunks.jsonl"
CHUNK_THRESHOLD = 200
...
KNOWN_IDENTIFIERS = [
    "BhriguNandiNadi",
    "bhrigu-nandi-nadi",
    "BhriguNandiNadiRGRao",
    "bhrigu_nandi_nadi",
    "NandiBhrigu",
    "BhriguNadiRaoTranslation",
    "bhrigunandi",
]
...
def run(dry_run: bool = False) -> int:
    logger.info("=== Bhrigu Nandi Nadi Ingestion START (dry_run=%s) ===", dry_run)

    text_files = find_text_files()
    if not text_files:
        logger.warning("Bhrigu Nandi Nadi not found on archive.org — PROCUREMENT_GAP")
        print(json.dumps({"status": "PROCUREMENT_GAP", "text_key": TEXT_KEY,
                          "note": "Text not found at known archive.org identifiers. "
                                  "Manual procurement may be required: search archive.org for "
                                  "'bhrigu nandi nadi' or 'R.G. Rao nadi'"}))
        return 1
    ...
    logger.info("=== Bhrigu Nandi Nadi Ingestion COMPLETE ===")

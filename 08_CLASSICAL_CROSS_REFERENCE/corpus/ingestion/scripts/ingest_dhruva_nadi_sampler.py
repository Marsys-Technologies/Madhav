#!/usr/bin/env python3
"""
ingest_dhruva_nadi_sampler.py — Ingest Dhruva Nadi (sampler / partial corpus).
Nadi/BNN tier. Source: archive.org (partial expected).
Target: ≥100 chunks (lenient — sampler), ≥85% embedded.
"""
...
logger = logging.getLogger("ingest_dhruva_nadi_sampler")

TEXT_KEY = "dhruva_nadi_sampler"
TEXT_METADATA = {
    "text_key": TEXT_KEY, "title": "Dhruva Nadi (Sampler)", "author": "Dhruva Nadi",
    "tradition": "nadi", "school": "nadi", "tier": 4,
    "language_original": "kannada", "translation_author": "Various",
    "source_url": "https://archive.org/search?query=dhruva+nadi",
    "procurement_date": str(date.today()),
    "attribution_baseline_confidence": 0.55,  # lower baseline — sampler corpus
}
GCS_PATH = "L8/classical_texts/nadi_bnn/dhruva_nadi_sampler_chunks.jsonl"
CHUNK_THRESHOLD = 100  # lenient — partial corpus
EMBED_THRESHOLD = 85.0
...
KNOWN_IDENTIFIERS = [
    "satyajatakamdhruvanadiramans.k.astrology_202003_357_T",  # verified: 455KB djvu.txt (Satyajatakam Dhruva Nadi)
    "dhruva-nadi",                                            # verified: Dhruva Nadi djvu.txt (small)
    "DhruvaNadi",
    "dhruva_nadi",
    "DhruvaNadiKannada",
]
...
def find_text_files() -> list[tuple[str, str]]:
    for ident in KNOWN_IDENTIFIERS:
        ...
        if chosen:
            results = []
            for f in chosen[:3]:  # limit to 3 for sampler
...
def run(dry_run: bool = False) -> int:
    logger.info("=== Dhruva Nadi Sampler Ingestion START (dry_run=%s) ===", dry_run)

    text_files = find_text_files()
    if not text_files:
        logger.warning("Dhruva Nadi not found on archive.org — PROCUREMENT_GAP")
        print(json.dumps({"status": "PROCUREMENT_GAP", "text_key": TEXT_KEY,
                          "note": "Text not found at known archive.org identifiers. "
                                  "Dhruva Nadi is a Kannada-language Nadi text with limited "
                                  "digital availability. Search archive.org for 'dhruva nadi' "
                                  "or try Scribd / JyotishDigital."}))
        return 1
    ...
    summary = {
        ...
        "note": "Partial sampler corpus — chunk count may be below threshold for full texts"
    }
    print(json.dumps(summary))
    logger.info("=== Dhruva Nadi Sampler Ingestion COMPLETE ===")
    if chunk_count < CHUNK_THRESHOLD:
        logger.warning("chunk_count %d < threshold %d (sampler corpus — check procurement)", chunk_count, CHUNK_THRESHOLD)
        return 1

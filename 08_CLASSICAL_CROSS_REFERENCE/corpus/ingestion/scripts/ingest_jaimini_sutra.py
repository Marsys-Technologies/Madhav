#!/usr/bin/env python3
"""
ingest_jaimini_sutra.py — Ingest Jaimini Sutra (Jaimini / B. Suryanarain Rao trans.)
into the M8 classical corpus. Tier 2 text.
Target: ≥150 chunks, ≥95% embedded.
...
logger = logging.getLogger("ingest_jaimini_sutra")

TEXT_KEY = "jaimini_sutra"
TEXT_METADATA = {
    "text_key": TEXT_KEY,
    ...
    "translation_author": "B. Suryanarain Rao",
    "source_url": "https://archive.org/search?query=jaimini+sutras+jyotish",
    "procurement_date": str(date.today()),
}
GCS_PATH = "L8/classical_texts/tier2/jaimini_sutra_chunks.jsonl"
CHUNK_THRESHOLD = 150
...
KNOWN_IDENTIFIERS = [
    "Jaiminisutras1955EditionByBSRao",  # 277KB B.S. Rao 1955 ed (verified 2026-05-14)
    "1_20230902_20230902_1035",          # 921KB (verified 2026-05-14)
    "jaimini-kva-a-4-size",              # KV Abhyankar commentary
    "JaiminiSutras",
    "jaimini-sutras",
    "JaiminiSutraRao",
    "in.ernet.dli.2015.404779",
]
...
def find_best_identifier() -> tuple[str, list[str]] | None:
    ...
    # Search fallback
    for query in ["jaimini sutras jyotish", "jaimini sutras astrology suryanarain"]:
        search = fetch_url(f"https://archive.org/advancedsearch.php?q={query.replace(' ', '+')}&output=json&rows=5")
        if not search:
            continue
        try:
...
def run(dry_run: bool = False) -> int:
    logger.info("=== Jaimini Sutra Ingestion START (dry_run=%s) ===", dry_run)

    result = find_best_identifier()
    if not result:
        logger.warning("Jaimini Sutra not found — PROCUREMENT_GAP")
    ...
    logger.info("=== Jaimini Sutra Ingestion COMPLETE ===")

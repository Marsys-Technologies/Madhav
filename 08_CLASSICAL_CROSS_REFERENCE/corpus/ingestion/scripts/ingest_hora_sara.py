#!/usr/bin/env python3
"""
ingest_hora_sara.py — Ingest Hora Sara (Prithuyasas, R. Santhanam trans.).
Tier 3. Source: archive.org HoraSaraRSanthanamEng (~493KB djvu.txt).
...
logger = logging.getLogger("ingest_hora_sara")

TEXT_KEY = "hora_sara"
TEXT_METADATA = {
    "text_key": TEXT_KEY, "title": "Hora Sara", "author": "Prithuyasas",
    "tradition": "parashari", "school": "parashari", "tier": 3,
    "language_original": "sanskrit", "translation_author": "R. Santhanam",
    "source_url": "https://archive.org/details/HoraSaraRSanthanamEng",
    "procurement_date": str(date.today()),
}
GCS_PATH = "L8/classical_texts/tier3/hora_sara_chunks.jsonl"
CHUNK_THRESHOLD = 200
...
KNOWN_IDENTIFIERS = [
    "HoraSaraRSanthanamEng",   # verified 2026-05-14: 493KB djvu.txt
    "HoraSara",
    "hora-sara",
    "hora_sara_santhanam",
]
...
def run(dry_run: bool = False) -> int:
    logger.info("=== Hora Sara Ingestion START (dry_run=%s) ===", dry_run)

    text_files = find_text_files()
    if not text_files:
        logger.warning("Hora Sara not found — PROCUREMENT_GAP")
    ...
    logger.info("=== Hora Sara Ingestion COMPLETE ===")

#!/usr/bin/env python3
"""
ingest_brihat_samhita.py — Ingest Brihat Samhita (Varahamihira, M.R. Bhat / V. Subrahmanya Sastri trans.).
Tier 3. Primary: VarahamihirasBrihatSamhitaByVSubrahmanyaSastri (1.36MB djvu.txt).
...
logger = logging.getLogger("ingest_brihat_samhita")

TEXT_KEY = "brihat_samhita"
TEXT_METADATA = {
    "text_key": TEXT_KEY, "title": "Brihat Samhita", "author": "Varahamihira",
    "tradition": "parashari", "school": "parashari", "tier": 3,
    "language_original": "sanskrit", "translation_author": "V. Subrahmanya Sastri",
    "source_url": "https://archive.org/details/VarahamihirasBrihatSamhitaByVSubrahmanyaSastri",
    "procurement_date": str(date.today()),
}
GCS_PATH = "L8/classical_texts/tier3/brihat_samhita_chunks.jsonl"
CHUNK_THRESHOLD = 300
...
KNOWN_IDENTIFIERS = [
    "VarahamihirasBrihatSamhitaByVSubrahmanyaSastri",           # 1.36MB verified 2026-05-14
    "aqbw_brihat-samhita-part-1-by-varahamihira-translated-by-m-ramakrishna-bhat-sans",  # 1.8MB
    "BrihatSamhita",
    "brihat-samhita",
    "in.ernet.dli.2015.284175",
]
    ...
    for ident in KNOWN_IDENTIFIERS:
        ...
        if chosen:
            ...
            for f in chosen[:3]:
                ...
                logger.info("Fetching %s (~%s bytes)", f['name'][:50], f.get('size', '?'))
                text = fetch_url(url)
...
def run(dry_run: bool = False) -> int:
    logger.info("=== Brihat Samhita Ingestion START (dry_run=%s) ===", dry_run)

    text_files = find_text_files()
    if not text_files:
        logger.warning("Brihat Samhita not found — PROCUREMENT_GAP")
    ...
    logger.info("=== Brihat Samhita Ingestion COMPLETE ===")

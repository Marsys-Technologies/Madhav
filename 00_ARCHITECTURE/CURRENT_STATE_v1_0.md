...
> ⚠️ **AUTHORITATIVE STATE (2026-07-15): Doctrine-Waves autonomous campaign ACTIVE — current_wave = D-1.5a.** L0–L5 build arc is CLOSED/SEALED (see §E of CLAUDE.md); the active work is the D-1 → D-4 doctrine-remediation wave sequence governed by `00_ARCHITECTURE/DOCTRINE_CAMPAIGN_EXECUTION_PLAN_v1_0.md` (v1.1 FINAL) and run per `00_ARCHITECTURE/llm_consumption_audit/briefs/doctrine_waves/CONDUCTOR_PROTOCOL.md`. D-1 (judgment substrate + CR-87 hotfix) merged 2026-07-14 (`4bebb622`, PR #561); post-deploy re-verification (register `POST_REMEDIATION_CONSUMPTION_REGISTER_v1_0.md §J/§K`) found the valence engine inverted (CR-90) and the detector lane unreachable from the deployed MCP (CR-92) — D-1.5a exists to close that gap before D-2 (Vidhi Engine) is authorized. Live wave state ledger: `00_ARCHITECTURE/llm_consumption_audit/briefs/doctrine_waves/STATE_D-1.5a.md`. Gate: register §K.2 (12 assertions) + A5/A7, executable via `platform/scripts/audit/doctrine_harness/run.ts` (Lane A-0, D-1.5a) — the harness's assertion definitions are the canonical copy of the gate per protocol §8.8(v). The banners this replaces (BRAHMA re-architecture SEALED 2026-06-02; "M6 INCOMING" / `OPERATOR_ACTIONS_PENDING.md` gate, 2026-05-31) are superseded — the BRAHMA rebuild and the M-series macro-phase banners are historical; the project's active-work framing since 2026-07-13 is the doctrine-waves campaign, not the M-series macro-phase arc. See SESSION_LOG.md Night-1 (2026-07-14) and doctrine-waves campaign brief pack (SESSION_LOG 2026-07-15) entries for full provenance.

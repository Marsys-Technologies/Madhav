---
artifact: MADHAV_PRODUCT_DEFINITION_v3_1.md
...
changelog:
  - "3.1 (2026-09-24, native-directed cleanse): §1 rewritten so the defining promise leads with the actual differentiator — data engineering computing at a scale and depth no single mind can hold, and AI deriving insight from it. §2 primary audience corrected to serious layperson AND advanced Jyotish acharya, two presentation modes over one depth. Lifespan interpretation restored. Life Event Log placed behind a single switch. REMOVED as out of scope for this product at this time: cross-subject hypothesis testing and the contrastive research observatory; lost-knowledge recovery and labelled reconstruction; expert-panel assessment protocol; the four-pathway life-event compliance table; voluntary-practice/coercion/monetized-fear clauses; third-party, minors and consent-enforcement clauses; the death, illness and fertility exclusions; the scholarly-laboratory and research-workbench horizons. RETAINED deliberately: the prohibition on invented computation, source, detector, confidence or score, which is engineering doctrine rather than compliance, and ablation testing."
---
...
ambition is extension of reach, not a claim that any individual reading is superior.
...
scholarship, exact computation and explanation are valuable in their own right.
...
concept coverage and predictive usefulness. There is no mandatory pass count and no instruction to
think indefinitely — effort follows the question and the emerging evidence.
...
The following is a target portfolio, not a current availability list.
...
| P09 | "Does this yoga form, and when might it manifest?" | Prerequisites, formation, exceptions, bhaṅga, qualified expression, multiple activation routes, nearest versus strongest window. |
...
question is an elevation requirement when a missing capability is the reason.

## 3. The Jyotish substance: what depth actually requires

Coverage obligations, not a compulsory sequence. A method must have qualified sources, conventions,
prerequisites and exceptions before it is applied.
...
and when. Daśā systems and nested periods, gochara, transit conditions, annual and return methods.
...
condition, recurrence and inferred manifestation. Compare nearest and stronger later windows under a
...
**Out of scope for this product.** Recovery of lost Jyotish knowledge, reconstruction of missing
sources, and the proposing and testing of new astrological hypotheses across populations of subjects
are not features of this product. Madhav interprets and predicts charts using qualified existing
knowledge. Discovery here means discovering what is true **within a chart** that was not previously
visible — not discovering new astrology.
...
Do not fabricate alternatives to satisfy a count. Keep deterministic fact, structural prior, classical
...
qualified manifestation alternatives → permitted external-outcome forecast.**
...
Compare closest eligible triggers and better-supported later windows under explicit criteria.
...
timing, assumptions, information cutoff, evidence, method versions and original wording. Computed
...
These are semantic responsibilities, not permission to add tables, alter assets or switch on dormant
writers.
...
## 10. Every surface carries the same product promise
...
substantive findings, conjoint interpretation, qualification and receipt semantics. Raw MCP supplies
...
|---|---|---|
| L0 — Brahmagyan | Qualified vocabulary, sources, rules, constants, reference systems, ephemeris and calendar foundations | Canonical identity, source fidelity, qualified semantics, method boundaries. |
| L1 — Gaṇita | Reproducible subject calculations under declared inputs and conventions | Correct fact identity, values, units, precision and provenance. Downstream consumers refer to L1 facts; they do not recompute them. |
| L2 — Bodha | Whole-chart structural understanding, configurations, relationships, mechanisms, tensions and alternatives | Qualified contributions beyond isolated placements; complete prerequisites and exceptions. |
| L3 — Kāla | Which structures are engaged by which clocks, when, under what conditions and with which alternatives | Structure–time intersections, temporal hierarchy, inhibiting and enabling factors, recurrence. |
| L4 — Phala | Qualified manifestation and permitted outcome interpretation | An explicit bridge from configuration and timing to named life distinctions; no unqualified composite score. |
| L5 — Mīmāṃsā | Challenge, adjudication, scope of validity and qualified learning | Preserved failures, independent evidence, correct denominators, leakage-free evaluation. |
...
- No automatic rectification, no outcome-driven personal tuning, and no autonomous decisions taken on
  the person's behalf.
- Rectification is an explicit, labelled hypothesis process, never a silent correction.
...
| Temporal integrity | Correct boundaries, zones, hierarchy, horizons, nearest versus strongest separation, and absence-versus-unavailable semantics. |
...
| Delivery fidelity | Complete findings survive stream, save, resume, replay and export; no wrong-chart carryover. |
...
Valuable directions, not currently activated and not a reason to delay core elevation.
...
- Every layer and asset brief states: its consumer contribution, the relevant Jyotish concepts, the
  preserved kernel, the exact delta, its authoritative dependencies, and its manifestation or temporal
  role.

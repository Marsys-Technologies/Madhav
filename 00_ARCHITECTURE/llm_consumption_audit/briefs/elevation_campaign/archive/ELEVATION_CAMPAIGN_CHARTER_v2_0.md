...
(c) were NOT served in the last N answers. **This is the direct measurement of the native's ~20%
unknown-unknown, and its target is zero.** Runs in the standing battery, ships in the morning
report, and any non-zero entry becomes an automatic register item for the next campaign.
...
### Ω — Verification (Verifier-owned, the campaign's flagship acceptance)

1. A **naive** "how is my wealth?" — no expert prompting — routes to `depth: deepdive` and returns,
...
4. **Chart-scoped rebuild** — delete-then-insert per §N.3, both canonical charts, via the FROZEN
   orchestrator (new/fixed writers YES; contract changes NO). Then **downstream refresh**: any L2+
   signal citing corrected facts (`constituent_facts_array` per §N.5) rebuilt or flagged; run the
   MSR drift check from `MSR_COMPUTED_VALUE_DRIFT_HANDOFF_v1_0.md`.
5. **EL-38 data leg** — adjudicate the all-zero argala matrix; fix the writer and rebuild if wrong.
...
| **C7 Ω3 accounting assertion** | γ | α (K1 receipt gate) | The 100%-accounting invariant α's CI asserts. |
...
2. **G0 reproduce-or-reclassify.** A lane's first act is re-running its recipes against prod. Symptom
   absent → `NOT-REPRODUCED` + evidence, item closes with **no code churn**. This is a legitimate,
   evidence-backed close (some register symptoms may be stale — R5.1-era fixes).
...
4. **Evidence block per EL**, appended to `ledgers/ELEVATION_V2_RUN_LEDGER.md`:
   `{el_id, status, before_payload_ref, after_payload_ref, probes_run, charts, verifier_notes}`.
   **The §15 matrix is updated ONLY by a Verifier.**

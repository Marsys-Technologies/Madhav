...
gate: {run: true, green: [1,5,6a,9,10,12,A5], red: [2,3,4,6b,7,8,11,A7], note: "first live gate run post-rebuild found #7 (final proof) red despite composite score correctly moving live (verified via direct MCP call: 1.15->2.78, convergent_moderate->convergent_strong). Root cause: shared response_budget.ts PASS-2 hard-cap trimmer zeroing bearing_yogas because it's now the biggest trimmable section (was reliably empty pre-fix). Hotfix in flight: wave/D-1.5a/hotfix-bearing-yogas-trim (commit 08664345), hardFloor opt-in field, verifier af8e55650478692fc running. Re-run gate after this merges+deploys."}

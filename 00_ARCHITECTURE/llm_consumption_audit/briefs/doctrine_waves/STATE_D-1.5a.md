...
gate: {run: true, green: [1,4,5,6a,7,8,9,10,12,A5], red: [2,3,6b,11,A7], note: "Native confirmed: keep going. Investigated #6b/#11/A7 -- all 3 were HARNESS bugs (double-nested content.content.rows parsing predating A-beta's route double-wrap fix; naive stale-description substring false-positiving on a correction notice quoting the old text to retract it; #6b requiring a literal fact_category the brief's own A3 text explicitly allows substituting with a firings_pointer, which is live/correct). Hotfix PR #566 (harness fix, commit 534bfc0b) RECEIPTED ACCEPT with live-reproduction verification. Live-verified post-fix: #6b + #11 now green. A7 stays honestly red -- genuine separate serving-layer gap (facet=aspects not surfacing parashari categories), writer-level fix independently verified correct by A-gamma's tests, diagnosis documented for native triage as a distinct follow-up. Remaining red after this merges: #2/#3 (live Mars/Jupiter aspect valence specimens not found -- investigating next), A7 (documented separate gap, not blocking wave close per its own must_not_touch scope)."}

...
gate: {run: true, green: [1,4,5,6a,7,8,9,10,12,A5], red: [2,3,6b,11,A7], note: "#6b/#11 confirmed harness bugs, fixed+merged (PR #566). Investigated #3 deeply: root cause is NOT ayanamsha contamination (1st fix attempt correctly REJECTED by verifier -- all 5 ayanamshas store the SAME correct value) but a genuinely missing asset_registry.depends_on edge: bo_laksana never declared ga_vichara as a dependency (deferred since migration 367, never landed), so bo_laksana built at 22:05 UTC using STALE ga_vichara data from before its 23:10 UTC fix. Migration 437 adds the edge (migration-guard APPROVED). Also found+fixed a 2nd, PROVEN-live bug in a sibling function (_load_varga_ratification: JUP/wealth ratification_factor 1 vs 1.4 across ayanamshas). PR #567 RECEIPTED ACCEPT (2nd verification round after 1st was rejected), CI running. CRITICAL sequencing per verifier: merge+deploy alone changes nothing -- migration 437 must be APPLIED and bo_laksana REBUILT before re-running gate. #2 (Jupiter specimen not found at all) still needs investigation after this lands. A7 remains documented-red (separate serving-layer gap, out of scope)."}

...
/**
 * b1_batch2_driver.ts — D-4b B-1 chunked re-run, BATCH 2/3 (key="batch2").
 *
 * Scores ONLY the 5 contenders assigned to batch2 by
 * B1_RUN_MANIFEST_v1_0.json's `batching.batches[1]`: naisargika, mudda,
 * kalachakra, narayana, sade_sati. Against the manifest's cited
 * D4B_PREREGISTRATION_PACKET_v1_0.md v1.2 §1 event set (56 events; 54
 * scored, per the same §0 recommendation batch1 already applied — excluding
 * the birth structural-anchor and the congenital-onset stammering entry).
 *
 * The 54-event ROWS list below is copied VERBATIM from
 * b1_batch1_driver_v1_0.ts (same packet, same git blob sha
 * 9b6713db8c2551a937ff2070e498da1f12526966, same J1-J7 judgment calls) — not
 * re-derived from a fresh read of the packet — because BRIEF_D4B §1's
 * "identical everything" rule requires every batch to score the SAME event
 * set identically; re-deriving independently per batch would risk silent
 * drift between batches' event lists (a different judgment call on an
 * ambiguous packet row) that would corrupt the eventual assembly. batch1's
 * own header documents that this list was read verbatim from the cited blob
 * this run (2026-07-22); this driver inherits that same read, not a second
 * one.
 *
 * Imports the REAL, already-merged harness modules (permission_model.ts,
 * harness.ts, dr17_grading.ts, curve_controls.ts) — no reimplementation of
 * any scoring math.
 *
 * Substrate: all 5 batch2 contenders are PERMISSION_SYSTEM_IDS
 * (permission_model.ts) — each resolves its curve via ONE live HTTP call
 * per (chart, eventClass, range) to the already-deployed Python sidecar's
 * `/api/compute/permission_curve` route (confirmed reachable + returning
 * real per-system intensity points this session: naisargika/marriage-window
 * probe returned points_computed=31, target_count=23, live weight/intensity
 * values, HTTP 200). No local dasha-period substrate is needed (unlike
 * batch1's pratyantar_lord) — `chart.substrate` is the empty bag
 * `permissionSystemModel.bind()` never reads it.
 *
 * Event-class resolution: SAME 3-of-54 resolved events as batch1
 * (EVT.2010.XX.XX.01 / EVT.2013.12.11.01 / EVT.2025.07.XX.01, per
 * gochara_resonance_map's 3-class live-populated scope for this chart --
 * career_advancement=22/major_gain=35/marriage=23 -- already independently
 * live-verified in the batch1 pass this same session-day; not re-queried a
 * second time here since batch1's own artifact already records the
 * event_date<2020-01-01-scoped live verification and this driver reuses
 * that same resolved ROWS list, not a fresh query). All 5 batch2 contenders
 * are honestly SKIPPED (not fallback-scored) for the other 51 unresolved
 * events, matching F-1's established design and batch1's own precedent.
 */
...
import type { CurvePoint } from '../../../../../platform/scripts/audit/t0_retrodiction/lib/curve'
...
const RUN_REF_DATE = '2026-07-22' // this session's own reference "now" (J3), same as batch1
...
// VERBATIM COPY of b1_batch1_driver_v1_0.ts's ROWS (54 of 56 rows; birth EVT.1984.02.05.01 and
// congenital-onset EVT.1995.XX.XX.02 excluded per §0's own recommendation) -- cross-batch
// event-set identity, see module header.
const ROWS: Row[] = [
...
async function main() {
  ...
  const BATCH2_PERMISSION_IDS = ['naisargika', 'mudda', 'kalachakra', 'narayana', 'sade_sati']
  const permModels = allPerm.filter((m) => BATCH2_PERMISSION_IDS.includes(m.modelId as string))
  if (permModels.length !== 5) throw new Error(`expected 5 permission models, got ${permModels.length}`)
  ...
  for (const row of ROWS) {
    ...
    console.error(`[batch2] event ${idx}/${ROWS.length} ${row.eventId} (category=${row.category}, resolved=${row.resolvedEventClass ?? 'UNRESOLVED'}) range=${range[0].toISOString().slice(0,10)}..${range[1].toISOString().slice(0,10)}`)
    ...
    writeFileSync(`${SCRATCH}/b1_batch2_results_raw.json`, JSON.stringify(results, null, 2))
    writeFileSync(`${SCRATCH}/b1_batch2_progress.json`, JSON.stringify({ eventsDone: idx, eventsTotal: ROWS.length, negativeCrpsFoundSoFar: negativeCrpsFound.length }, null, 2))
  }

  console.error(`[batch2] DONE. negativeCrpsFound=${negativeCrpsFound.length}`)
  writeFileSync(`${SCRATCH}/b1_batch2_negative_crps_check.json`, JSON.stringify(negativeCrpsFound, null, 2))
  console.error('[batch2] wrote', `${SCRATCH}/b1_batch2_results_raw.json`)
}

main().catch((err) => {
  console.error('[batch2] FATAL:', err)
  process.exit(1)
})

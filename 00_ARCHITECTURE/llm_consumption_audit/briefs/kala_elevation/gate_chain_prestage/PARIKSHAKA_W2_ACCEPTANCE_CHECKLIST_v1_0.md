...
| # | Check | Evidence | Disposition |
|---|---|---|---|
| W2.8.a | The stage-6.5 insight-synthesis rows (the 8-type catalog, `kala_insights` table) appear FIRST in at least one composed reading (the "reading-leads-with-insight" rule enforced in `argument_composer.ts`) — checked by inspecting a real composed response's ordering, not by trusting the composer's own claim. | `SELECT count(*) FROM kala_insights` (both chart_ids) → **0 rows, both charts**, re-verified live AFTER this session's fresh rebuild (so this is not stale — the field was genuinely just recomputed and still produced zero insights). Root-caused by reading `writer.py::_run_stage65` + `stage65_insights.py` directly (not guessed): every one of the 7 non-LEL detectors legitimately fails to fire given the real data density — `detect_concurrence` needs ≥3 distinct competence-class systems agreeing per window (v0_classical baseline-only windows don't have this); `detect_rarity_firing` needs a non-NULL cohort match (NULL per W2.5 above); `detect_absence_of_expected` is correctly restricted to non-skipped classes with zero windows (482012f1: all 7 classes skipped, so 0 candidates by design; 1c826d5a: its 2 attempted classes both HAVE windows, so absence correctly does not fire for them); `detect_scarcity`/`detect_reversal` need ≥2 windows per class (both charts have ≤1 per class); `detect_contrast` needs a baseline snapshot (explicitly `None` on every rebuild per §N.3 delete-then-insert — writer's own comment confirms this is by design). The writer's own `WriterResult.notes` field for a zero-insight stage65 run reads `'honest_empty:no_insight_predicate_fired'` — an honest, named empty, not a silent gap. | **PARKED-HONEST — genuinely unable to demonstrate on either canonical chart today, not a code defect.** Traced every one of the 7 detectors' non-firing to real, explicable data sparsity (confirmed by reading the detector logic, not asserted). The Gate W2 criterion ("insight rows lead readings... checked by inspecting a real composed response's ordering") cannot currently be exercised live on EITHER canonical chart because there is no insight row to inspect the ordering of. This is a corpus-density problem (same root as W2.3/W2.5's N_e-overlap gap), not a bug — but it is a real, currently-unsatisfied Gate W2 item. |
...
gap found), **W2.8/insight rows** (zero rows on both charts, traced to real data sparsity,
not a bug), and **W2.10/specificity gate** (real HARD-mode enforcement, but scoped to built

...
| 33 | Absence-of-expected detector | W3 | BUILT — PARĪKṢAKA ACCEPT-WITH-DEBT | PR #1085 merged 18:23:34Z | `detect_absence_from_pratijna()` in `stage65_insights.py`; fires when max_grade >= 0.60 AND zero field windows; B.3 provenance via fact_ids union; 70 Python tests; threshold-boundary mutation KILLED; DEBT: monotonicity test uses `<=` (weak) — constant carrier_salience mutation survives; `<` needed for distinct above-threshold grades |
| 34 | Contrastive EXPLAIN | W3 | BUILT — PARĪKṢAKA ACCEPT-WITH-DEBT | PR #1085 merged 18:23:34Z | `compute_field_diff()` Python + `computeFieldDiff()` TypeScript; anti-symmetry property KILLED in both languages; 8 Python + 11 TypeScript tests; `computeFieldDiff` exported but NOT yet wired into handler (scope-correct for Item 34); DEBT: Python/TS diverge on zero-lambda current (Python: -inf delta, TS: skip — informational); dead `__post_init__` on `FieldSnapshot` (cosmetic) |
| 35 | Planner wiring verified LIVE (hard gate) | W5 | NOT-STARTED | — | — |
| 36 | Contender lattice + adjudication engine | W3 | **SUBSTRATE VERIFIED-FIXED** + **ENGINE BUILT — PARĪKṢAKA ACCEPT-WITH-DEBT** | PR #930 (substrate); PR #1083 merged 18:23:02Z — `kala_lattice_query.ts` Pareto-dominance adjudication engine; 27 TypeScript tests; 3 mutations KILLED (dominance bypass, Convention B arithmetic, census citation); DEBT: cosmetic row count in PR desc/migration comment off by 1 (51 rows/9 families actual, not 45/7); target_floor 439 should be 440 |
| 37 | Ritual-resonance + paddhati profile | W3/W4 | BUILT — PARĪKṢAKA ACCEPT-WITH-DEBT | PR #1084 merged 18:23:22Z | `query_kala_paddhati_profile` capability registered (was missing — `fetchPaddhatiProfile` in `kala_sky_pattern.ts` degraded to `honest_empty`); code correct (no `convention_status` filter in SQL); 15 tests; storage layer (migrations 533/534/537) pre-existing. DEBT (PARĪKṢAKA): Rail-2 test checks description string only — mutation adding `convention_status` WHERE-filter survives with all tests green; integration test (needs live DB + declared_not_computed seed) is the correct fix; limitation honestly documented in test block comment |
...
| 41 | Muhūrta Factor Census + corpus extraction | W3 | **VERIFIED-FIXED** (PR #930) + **W3-ENG UPDATED — PARĪKṢAKA ACCEPT-WITH-DEBT** | PR #930 (original census); PR #1083 merged 18:23:02Z — updated census via `bg_parihara_rules.py`; PARĪKṢAKA independent count: 51 rows / 9 families (PR desc claims 45/7, migration comment claims 50 — both cosmetically wrong; mutation on null citation_or_gap_note KILLED); DEBT: cosmetic count discrepancy; pre-existing jhora env failure in test_writer_registered (not caused by PR) |

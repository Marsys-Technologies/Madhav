# Shard 9b — esoteric_point_shiva

**shard_id:** `9b-esoteric_point_shiva`
**charts:** 482012f1-… (Abhisek) · 1c826d5a-… (Abhinandan)

## Exact SQL run (proven 9b 5-cell recipe, `<CAT>`=esoteric_point_shiva)
```sql
WITH sig AS (SELECT DISTINCT ms.signal_id, ms.chart_id, ms.signature_tier, ms.signal_type_class,
  ...
  WHERE f.fact_category = 'esoteric_point_shiva'
...
## 5-cell verdicts
1. **Consumed?** YES (35 signals each chart; 70 facts → 35, ~50%).
...
## Findings
- **F1 (class 2 WRONG; domain mis-mapping; MED):** Shiva sphuta (spiritual/moksha deity point) maps exclusively to default `character|relationship`; no spirituality/dharma/moksha routing. Part of shard-wide default-collapse. Suspected layer: L-writer (bo_laksana domain-mapping). Call = SQL above, cell4.

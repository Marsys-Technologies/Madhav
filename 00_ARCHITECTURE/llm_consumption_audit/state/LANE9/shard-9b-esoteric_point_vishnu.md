# Shard 9b — esoteric_point_vishnu

**shard_id:** `9b-esoteric_point_vishnu`
**charts:** 482012f1-… (Abhisek) · 1c826d5a-… (Abhinandan)

## Exact SQL run (proven 9b 5-cell recipe, `<CAT>`=esoteric_point_vishnu)
```sql
WITH sig AS (SELECT DISTINCT ms.signal_id, ms.chart_id, ms.signature_tier, ms.signal_type_class,
  ...
  WHERE f.fact_category = 'esoteric_point_vishnu'
...
## Findings
- **F1 (class 2 WRONG; domain mis-mapping; MED):** Vishnu sphuta (dharma/sustenance deity point) maps exclusively to default `character|relationship`; no dharma/spirituality routing. Part of shard-wide default-collapse. Suspected layer: L-writer (bo_laksana domain-mapping). Call = SQL above, cell4.

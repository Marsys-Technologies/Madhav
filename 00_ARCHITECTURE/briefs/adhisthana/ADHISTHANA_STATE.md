...
| A5 | THE FACT IDENTITY INDEX (`chart_fact_identity` + deterministic parser) | PR #1105 OPEN (not yet merged) — **remediation in progress.** PARĪKṢAKA independently re-derived the whole claim at FULL SCALE against live data (not sampled): all 125,592 rows of chart `482012f1` checked via independent SQL invariants (zero mismatches), 100% coverage number reproduced via an independently-written driver script, idempotency re-proven with a second live run + MD5 content checksum, R19/migration confirmed clean by direct inspection. **Verdict YELLOW — 3 real, narrow issues found, fixes dispatched back to the builder, not yet returned:** (1) a genuine 12-row gap (`fact_key` shape `co_<GRAHA>_<GRAHA>`, e.g. `co_MAR_VEN` — nakshatra co-tenancy pairs) was silently swept into an identity-free bucket by classifying on `fact_subject` instead of the `fact_key` that actually carries the identity — squarely within the parser's own chartered scope (4 sibling pair-conventions already handled); (2) the headline "≈31%-of-rows sign/rashi dimension" discovery is a reporting-arithmetic error — independently recomputed 3 ways, true figure is **≈44%** (underlying `sign_num` data itself confirmed correct; this is a narration bug, not a functional one); (3) the coverage report claims per-shape counts are reproducible via a committed reconnaissance script that was never actually committed (only 3 commits exist on the branch). None of these threaten the parser's structural soundness or the >99% target. |

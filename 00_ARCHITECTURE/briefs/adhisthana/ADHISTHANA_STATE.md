...
| A8 | Checkpoint artifacts (Factor→Fact Coverage Matrix + V4 Rubric Spec draft) | DISPATCHED — two parallel research/writing agents (document-only, no engine code): Part 1 (`A8_FACTOR_FACT_COVERAGE_MATRIX_v1_0.md`, live-verified against `bo_pratijna_karyatva.py`'s existing 27-class KaryatvaMap registry + `chart_fact_identity`/`chart_facts`) and Part 2 (`V4_RUBRIC_SPEC_v0_9.md`, per-factor bands from `reference_planets` dignity data, per-class weights summing to 1, denial configs, R12/R18 formulas, a priori thresholds). Not yet returned. |
| — | **Rung P3** (= A8's hand-worked artifact) | NOT RUN — blocked on A8 |

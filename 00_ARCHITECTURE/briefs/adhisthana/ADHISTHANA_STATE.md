...
| A5 | THE FACT IDENTITY INDEX (`chart_fact_identity` + deterministic parser) | PR #1105 OPEN (not yet merged). Migration 552, `fact_identity_parser.py` (28 named rules, per-row provenance), 106/106 TDD tests, idempotent population script, `A5_COVERAGE_REPORT_v1_0.md`. **Claimed: 100.0000% coverage, zero gaps, all 3 charts (417,268 total rows), 14 individually-dispositioned identity-free classes.** Reconnaissance found the dot-separated `D9.MAR` compound has zero live rows (contradicts original grounding notes) and an undocumented ≈31%-of-rows rashi/sign dimension. **Master plan §3 A5 explicitly mandates PARĪKṢAKA verification against the LIVE key inventory (not fixtures) before acceptance — dispatched, not yet returned.** A 100%-coverage claim on the keystone table is exactly the kind of claim that needs adversarial re-derivation, not a self-report read. |

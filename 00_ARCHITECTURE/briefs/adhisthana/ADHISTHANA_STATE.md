...
| A6 | Gates (registry-parity script, subject-wellformedness lint, graha/varga census in CI) | **PR OPEN** (this branch, `adhisthana/lane-a6-gates` → `adhisthana/integration`). (i) `platform/scripts/governance/registry_parity_gate.py` — `--self-test`/`--live` split per the `msr_referential_integrity.py` pattern; checks graha/varga/house/event_class across Python SSoT ↔ TS SSoT ↔ live `brahma_ontology`/`brahma_event_ontology` ↔ producer-emitted DISTINCT values. Live-run against `482012f1`/`1c826d5a`/`cb73cd3d` 2026-08-08 found a REAL pre-existing gap (see AB5 below) and, after excluding two confirmed non-identity `chart_divisionals` sentinel conventions (documented in the script itself), zero violations on varga/house/event_class. (ii) `platform/scripts/governance/check_fact_subject_wellformedness.py` — regex scanner over `ga_writers/`, `bodha_writers/`, `pipeline/orchestrator/writers/`; self-test fixtures include a literal reproduction of both real Lane A1 defect sites; live repo scan confirmed 0 violations (Lane A1's fix is the entire baseline); mutation-proven against the REAL (fixed) `ga_condition_writer.py` — silent on the fixed file, catches it the instant one site is reverted to the pre-fix shape. (iii) confirmed graha/varga census tests (`test_graha_vocabulary_census.py`, `graha_vocabulary_census.test.ts`, `test_l0_ontology.py`) already ride the general CI test run (`pytest tests/` in the Governance Gates job; `npm test` in the Unit Tests job) — no dedicated "domain census" CI job exists for `test_domain_vocabulary_census.py` either, so no new wiring was needed, only confirmed. Both new scripts wired into a new `registry-parity-gate` CI job (`.github/workflows/ci.yml`), self-test only (no DB/npx in CI, same as `msr_referential_integrity.py`). |
...
| AB5 | `brahmagyan/graha_vocabulary._GRAHA_ALIASES` recognizes 14 alias forms as valid graha INPUT (`SU`/`MO`/`MA`/`ME`/`JU`/`VE`/`SA`/`RA`/`KE` 2-letter shorthand + `RAH`/`RAH_TRUE`/`KET`/`KET_TRUE`) that are NOT registered as `brahma_ontology` synonyms for `entity_class='planet'` — confirmed live against `482012f1`/`1c826d5a`/`cb73cd3d` via `registry_parity_gate.py --live --domain graha` (2026-08-08). Not a producer-emission bug (no producer actually emits any of the 14 into `chart_facts` — separately confirmed 0 `unresolved_producer_value` hits for graha once two `chart_divisionals` sentinel conventions were excluded, see the script's own comments), purely a registry-completeness gap on the alias-acceptance side: something could `norm_graha('RAH')` successfully in Python today with no matching `ref_entity_resolve('RAH')` in the registry. | Lane A6(i) `registry_parity_gate.py --live` | OPEN — out of Lane A6's scope (A6 is the GATE, not the registry-content fix; the gate's job is exactly to surface findings like this one for a future lane to disposition, per Lane A2/A3's original `entity_class='varga'`/storage-code-synonym pattern) |
...
- **2026-08-08 (Lane A6 session, Sonnet 5, isolated worktree, branch
  `adhisthana/lane-a6-gates` from `origin/adhisthana/integration` @ `23a770cbb`):** Built
  all three A6 deliverables (see the A6 row above for the full account). Both new
  governance scripts self-test DB-free and hermetic (CI hard gate); `registry_parity_gate.py
  --live` was run for real against the live DB (read-only, credential never printed/committed)
  and found one real finding (AB5, above) plus two chart_divisionals sentinel conventions
  now documented and excluded in the script. `check_fact_subject_wellformedness.py`'s
  mutation proof ran against the REAL `ga_condition_writer.py` (not just synthetic
  fixtures): silent on the current fixed file, catches the violation the instant a real
  site is reverted to the pre-fix `graha.upper()` shape. PR opened against
  `adhisthana/integration`, not merged by this session per instructions.

...
changelog:
  - "1.0 update 2026-09-20: Records DP-SD-021, the native-authorized execution platform split — L3 Kāla moves to Claude Code (integration branch codex/madhav-l3-claude-code) while Pūrṇa Anveṣaṇa remains in Codex, superseding only the L3 execution brief's 'sole execution destination' clause and dead c9bd worktree pin. Every other DP-SD-017/018/019/020 clause is retained. See §13."
...
## 13. Execution platform split — DP-SD-021

| Control | Binding |
|---|---|
| Native decision | Run Pūrṇa Anveṣaṇa and the L3 Kāla data-plane campaign in parallel on separate platforms rather than timesharing one Codex fleet. Recorded native agreement, 2026-09-20 (autonomous overnight execution conversation): plan adopted; autonomous overnight execution authorized; commit/merge/push/deploy authorized; budgets 2x the execution package's estimate. |
| Supersedes (only) | `MADHAV_DATA_PLANE_L3_KALA_EXECUTION_BRIEF_v1_0.md` §2/§8 wording naming the Codex task "Execution — Data Plane" (`01a0998a-8240-7631-97ce-36c6d4734fde`, branch `codex/madhav-data-plane-execution`, worktree `/Users/Dev/.codex/worktrees/c9bd/Madhav`, now gone) as "the sole execution destination. No replacement task is created." Every other clause of DP-SD-017/018/019/020 — `may_touch`/`must_not_touch`, activation prohibitions, delivery target, L4/L5 hold — is retained unchanged. |
| Decision | (1) L3 Kāla execution destination = Claude Code sessions conducted from the integration branch `codex/madhav-l3-claude-code` (worktree `/Users/Dev/madhav-l3/integration`), created from verified protected `main` ≥ `32e5a9f0e` (subsequently rebased forward as main advanced; base at DP-SD-021 authoring time is `cdc701afa`). (2) Pūrṇa Anveṣaṇa execution destination = Codex, unchanged. (3) The Codex task "Execution — Data Plane" and its continuation automation (`l3-data-plane-overnight-continuation`) remain PAUSED; they are not a second conductor for L3. (4) Shared-surface partition rules per `MADHAV_DUAL_CAMPAIGN_EXECUTION_PLAN_v1_0.md` §3.3 (production-mutation lease, migration-number partition Pūrṇa 1042–1069 / L3 1070–1119 / shared ≥1120, generated-artifact regenerate-never-hand-edit, `registry/layers/L3_kala/**` owned by L3, disjoint asset/table territory); interlocks per §3.4 (`L3-REQ-nn` / `PA-REQ-nn` in `CAMPAIGN_COORDINATION.md`). |
| Rationale | Platform-level isolation of quota, task/goal state and automation heartbeats — the actual source of Codex-fleet contention observed 2026-09-16 through 09-19; the git-level surfaces (branches, migrations, generated artifacts, one handler directory) are cheaply partitionable and this repo has done exactly this before (SAMPŪRTI ∥ GOCHARA-UTKARṢA, 2026-08-10). The recorded `Accepted 0/22` headline was structurally forced by the shut delivery gate and empty upstream L1/L2 generation heads (both campaigns' `candidate_verified`/`live_verified` were `NOT_RUN` for the identical plumbing reason), not by execution quality on either side. |
| Evidence | `MADHAV_DUAL_CAMPAIGN_EXECUTION_PLAN_v1_0.md` (this document's sibling, Appendix A/B/C) and `MADHAV_L3_AUTONOMOUS_EXECUTION_PACKAGE_v1_0.md`, both in this directory. |
| Non-claim | This decision does not itself close any L3 asset, alter the L3 acceptance bar, grant new credentials/IAM, or authorize touching `.github/workflows/deploy.yml`, the data-plane/Pūrṇa ownership scripts, or migrations 1033–1041 from the L3 side. |

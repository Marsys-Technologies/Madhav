...
- `2026-09-06T~09:3xZ — L3-W3 — TWENTIETH DIRTY-PR fix this run: #1943 (F-PARVA-1,
  ...
  same shape as #1903/#1913/#1940: L3_STATE.md + the layer-pins file conflicted,
  `ka_jivana_parva.py` itself and the writer-digests file both auto-merged cleanly
  (confirmed correct by regenerating anyway, zero diff). Resolved via the established
  script (verified 3 total mentions of "F-PARVA-1" are genuinely 1 heading + 2 distinct,
  non-duplicated body references — one this PR's own text, one a much older unrelated
  entry). Verified `test_ka_jivana_parva_a8.py` + `test_migration_679_parva_level_
  column.py` (28/28, incl. live-DB integration tests), full `tests/l3/` (1424 passed, 0
  failures), both `--check` invocations exit 0, migration 679 confirmed still not
  deployed (expected), `tsc --noEmit` clean. Force-pushed, re-armed auto-merge, confirmed
  `MERGEABLE`.
  Also hit the SAME stash-reapply-orphaning issue a third time (on this cycle's own state
  branch) — caught it again via `git diff` before committing, discarded, wrote this entry
  fresh. Establishing going forward: do not trust a stash of `L3_STATE.md` across a
  cycle that also rebases a different branch's own copy of the file — write fresh instead
  of reapplying, every time, rather than re-litigating the same surprise each cycle.
  Twenty DIRTY-PR fixes total this run. #1903 (F-CONC-6 prerequisite) still not merged.

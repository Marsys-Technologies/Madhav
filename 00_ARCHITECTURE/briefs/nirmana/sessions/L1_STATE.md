...
last_updated: 2026-09-07 — C8 v2.3 cycle 197; **#2276 RULED (Option 1, generation-scoped redispatch) and implemented same-cycle.** `create_campaign_run`'s `accepted_rebuild_observed` guard in `dispatch_nirmana_campaign_wave.py` no longer refuses a new run forever for any asset that ever reached acceptance under any past generation — it now scopes the refusal to rows still bound to the asset's CURRENT live-registry generation (read directly from the already-validated, immutable `registry_fingerprint_sha256` field on the event's own payload, a disclosed simpler deviation from the ruling's literal `decision_digest`-chain path, flagged to the Conductor on #2276). Verified live before opening the PR: `ga_dashas`' bound fingerprint (`8523e32a...`) genuinely no longer matches its current live fingerprint (`b85a6d11...`, post-migration-882) — confirms the fix actually unblocks it. 4 new tests (stale-generation allowed / current-generation still refused / mixed wave / fail-closed malformed payload), 45/45 pass. PR #2295 opened, auto-merge armed. `ga_transit_anchors`'s separate #2224 orphaned-generation trap is untouched by this ruling — remains its own open item. `capsule_audit.sql` §1/§2 clean throughout.
---

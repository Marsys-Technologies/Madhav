...
last_updated: 2026-09-06 — C8 v2.3 cycle 114; fixed a genuine RED on #2136 (line-drift in a pin allowlist entry caused by the F-C21 insertion) then closed F-D5 (PR #2140) — get_yoga_firings.ts's ORDER BY was a non-total order, missing ayanamsha_id/id tiebreak; had to reconcile a real merge conflict against a concurrent PR that added a catalog JOIN + real OFFSET pagination to the same query
---

...
last_updated: 2026-09-07 — C8 v2.3 cycle 181; **F-B32 slice 8: new `get_structural_signals.ts` tool closes a 15-category gap for `ga_structural` (PR #2244), the first genuinely new-endpoint W3 work this campaign's F-B32 backlog explicitly flagged as out of scope for a list-edit slice.** `ganita_structural_get` was a registered canonical-face name with zero backing capability. All 15 categories confirmed single-writer-owned by `ga_structural_writer.py` alone (no competing L0/L1/L3 writer) with live rows 1-5,220; 4 more (`bhava_significance_link`/`net_argala_per_varga`/`panchadha_maitri`/`sandhi_flag`) deliberately left open — multi-writer-owned, needs a dedicated disambiguation pass. Side-fix: corrected coverage_matrix.ts's own stale claim that `karaka_web_per_varga` was uncovered — it was already served via `get_karakas.ts`, a pre-existing entry the cycle-156 sweep missed. New integration test caught and correctly handled two real page-cap edge cases (relational/graph domains each exceed the 2000-row limit) rather than silently passing on a lucky page. PR #2244 open, autoMergeRequest armed, mid-CI. #2113/#2180/#2224 checked -- no new Conductor reply. #2242 (cycle 180's ga_nakshatra fix) and #2228 (prior state PR) both confirmed MERGED to main during this cycle's close -- state branch rotated cycle6->cycle7 accordingly
---

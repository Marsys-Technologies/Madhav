...
last_updated: 2026-09-07 — C8 v2.3 cycle 182; **F-B32: `get_sensitive_points.ts` closes 2 more mischaracterized categories (PR #2247), continuing the disambiguation cycle 181's own comment started.** `esoteric_point_sphuta_fertility`/`esoteric_point_yogi_system` (70/25 live rows) were lumped into cycle-156's "needs new-endpoint work" bucket alongside genuinely unreachable categories — re-checked, both are cleanly single-writer-owned by `ga_sensitive_writer.py` alone and simply missing from `get_sensitive_points.ts`'s existing `esoteric_point_*` family (an oversight, not ambiguity). Added directly; picked up automatically by the existing `tradition:'esoteric'` filter. Wrote the coverage_matrix.ts comment update to stay self-consistent regardless of PR merge order with #2244 (still pending, not assumed merged). **PR hygiene caught a genuine RED gate on the state PR itself (#2246)**: "Unit Tests" failed with `Test timed out in 5000ms` in `domain_vocabulary_census.test.ts` — unrelated to this state-only PR's markdown changes (zero application code touched); re-triggered via `gh run rerun --failed`, confirmed CI-load timing flake not a real regression before treating it as resolved. #2113/#2180/#2224 checked -- no new Conductor reply
---

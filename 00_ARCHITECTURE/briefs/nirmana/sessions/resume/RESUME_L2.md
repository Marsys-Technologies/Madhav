# SESSION PROMPT — L2 (Bodha) — RESUME (v2.2, 2026-09-05)

You are the **L2** session of the NIRMĀṆA v2.1 parallel campaign: 22 `bo_*` assets, all six
waves W1→W6, migrations 660–669, branches `codex/nirmana-l2-*`, PR prefix `L2:`, worktree
`~/nirmana-s/l2` (recreate with
`git -C /Users/Dev/Vibe-Coding/Apps/Madhav worktree add ~/nirmana-s/l2 origin/main --detach` if gone).
...
while [ ! -f NIRMANA_HOLD ]; do
  claude --dangerously-skip-permissions \
    "$(cat 00_ARCHITECTURE/briefs/nirmana/sessions/resume/RESUME_L2.md)"
...
# §L2 — YOUR LANE SPECIFICALLY

**You have been down the longest (since 00:14Z) and you are the critical path.** Three things
landed while you were gone that are yours.

**Read #1770 first, top to bottom.** The native ruled directly (D-NATIVE-05). **You were right on
every count** — right to hold, right to verify #1748 rather than accept it, right to call it
charter-level. Your correction of a *favourable* conclusion about your own table is on the record as
the campaign's best process result.

**Your assignments, in order:**
1. **`_idempotency.py` docstrings, lines 55 and 110 — `NO ACTION` → `CASCADE`, with the child list.**
   Smallest, highest-value change in the campaign: that false comment is how #1748 reached the wrong
   conclusion and how the Conductor ratified it. It is now doctrine **D-CND-16**.
2. **Deterministic `signal_id` — a MUST** (D-NATIVE-05 action 8). Same pattern as L4's #1754, and
   **no bigserials in the key**. **D-CND-11 binds**: the key excludes every graded, calibrated or
   recomputed quantity. **Coordinate with L5** — their `mimamsa_attribution` / `mimamsa_load_bearing`
   are `text` while yours is `uuid`, so they cannot carry a real FK to you without a type change; a
   deterministic `uuid` on your side plus a real FK on theirs makes the orphan class *structurally
   impossible*. One decision, two layers.
3. **`bodha_triangulation.signal_ids[]` (405 rows)** — disposition owed: a real FK with an intended
   delete rule, **or** documented orphan-tolerance **with a detector**. The array column makes a
   conventional FK awkward — say so explicitly if that drives the choice.
4. **#1767 (L1's #1750 handoff), #1755, #1760, #1775** — rebase, re-arm, land.
5. **#1729** — the `VERIFICATION_RESCALE` fix is yours to implement (`bodha_writers/formulas.py` is
   your package); L1 supplies the weights. Aliases resolve *through* `verification_vocab`, never
   restated; all 13 members explicit; the silent 0.60 fallback becomes a loud failure.
6. **#1720** — `system_convergence_count`: declare your degeneration threshold **before** measuring,
   publish the distribution, and take the honest NULL (Option 2) without re-filing if it degenerates.
7. **C13** — blast-radius statements on all 22 routes. **`bo_laksana` is your own worked example**:
   you routed it `changed` assuming an in-layer rebuild, and it cascades across three layers.

**Your `bo_laksana` dispatch is what the cascade ordering is built around**, and it is held until
WP-6 (#1781) is live. When released: claim a slot, **`weight: monster`, runs SOLO**, and L3 must
first confirm on #1770 that its five `kala_*` tables are re-runnable.

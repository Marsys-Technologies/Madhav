# SESSION PROMPT — L3 (Kāla) — RESUME (v2.2, 2026-09-05)

You are the **L3** session of the NIRMĀṆA v2.1 parallel campaign: 23 `ka_*` assets, all six
waves W1→W6, migrations 670–679, branches `codex/nirmana-l3-*`, PR prefix `L3:`, worktree
`~/nirmana-s/l3` (recreate with
`git -C /Users/Dev/Vibe-Coding/Apps/Madhav worktree add ~/nirmana-s/l3 origin/main --detach` if gone).
...
while [ ! -f NIRMANA_HOLD ]; do
  claude --dangerously-skip-permissions \
    "$(cat 00_ARCHITECTURE/briefs/nirmana/sessions/resume/RESUME_L3.md)"
...
# §L3 — YOUR LANE SPECIFICALLY

**Two of your filings shaped the whole campaign.** #1734 (the E-gate is only as sound as
`depends_on`) and #1794 (the CI blocker) — the second of which **you filed at 01:27 and died before
the Conductor ruled it at 04:05**. It is fixed: **PR #1795**, a rule correction not an allowlist
entry, because `DISTINCT ON (fact_subject, fact_key)` **is** a `fact_key` pin. Your both-guard-
versions attribution method is now the campaign's standard for CI regressions.

**Your migration 670 (#1792) merged** — 19 chart-partitioned integrity contracts.

**In order:**
1. **#1770 needs your answer, and L2's dispatch is waiting on it:** confirm your five `kala_*`
   tables (`kala_activation` 672,551 · `kala_convergence` 35,365 · `kala_darshana` 1,500 ·
   `kala_obstruction` 1,283 · `kala_bhavishya` 200) **are re-runnable from a rebuilt MSR base.**
   **If any carries state that cannot be regenerated, say so — the ruling changes.** This is the one
   fact neither L2 nor the Conductor can check from outside your writers.
2. **Sequence your MSR-dependent assets after `bo_laksana` freezes** and **price the re-run into your
   cost ledger** — it is planned regeneration from a corrected base, not damage.
3. **`kala_activation_predicates` (150,150 rows)** — disposition owed. No FK, so it **orphans rather
   than cascades**. Real FK with an intended delete rule, or documented orphan-tolerance **with a
   detector**.
4. **Your five red contracts SHIP RED.** Four localise to chart `cb73cd3d`, now formally **DAMAGED**
   (D-CND-17). *Scoping a detector to the charts that pass is the definition of weakening it* — that
   is your line and it is now precedent. Those four assets are HELD from `integrity_verified`, not
   failed.
5. **#1734's `depends_on` audit** — declared-vs-actual, both directions, for all 23. Your method is
   the model the other four layers were told to copy. `ka_gochara_resonance` and `ka_graha_sancara`
   stay held until it is done; `ka_graha_sancara` also has two real defects to fix (`row[0]` against
   a `dict_row`; a self-test asserting natal Moon = Aquarius against 12:00-UT `ephemeris_daily`).
6. **C13** — blast-radius statements on all 23 routes.

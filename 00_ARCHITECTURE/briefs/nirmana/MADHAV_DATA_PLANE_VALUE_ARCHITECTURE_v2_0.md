...
parent_definition: ../../MADHAV_PRODUCT_DEFINITION_v3_0.md
source_revision: 45120d72dd4e968234d5e77cc1d665e74bc595d6
application_source_base: 731e311f0b8f5f84db2f152b93951e1d3d50d89a
...
authority: "Native authorized this documentation-only planning session and extended its actual managed-profile exception. Authorization to prepare this proposal is not adoption of its recommendations or implementation authority."
...
The ambition is **beyond-Acharya depth through qualified connectedness**: more relevant concepts and chains examined correctly, more meaningful alternatives distinguished, more precise temporal discrimination where earned, and fewer important omissions. This is a design ambition, not a demonstrated claim of astrological or predictive superiority. Engineering verification, interpretive value and empirical outcome performance remain different proofs.
...
- What is supported as interpretation, what can responsibly become a specific permitted forecast, and which bridge is still missing.
...
The P identifiers below refer to the product master. The examples are requirements, not personal readings. Each obligation has a named owning contribution; not every query needs every asset.
...
| V11 Relational/entity/collective research | P14; master §15 | Reuse qualified subject/reference foundations; add distinct relationship/entity/method contracts only after separate authority. No invented entity birth or automatic cohort admission. |
| V12 Excluded outputs | P24; master §13 | Enforce exclusions at selection, computation purpose, retrieval and delivery. Medical/āyurdaya corpus presence does not enable individual illness, mortality or medical-timing claims. |
...
| L4 — Phala | What could that structure-in-time mean in the person's stated life domain? | Qualified manifestation alternatives, permitted outcome propositions, falsifiers and action constraints. | Automatic certainty or calibration; a generic domain score as a specific outcome. |
...
`bg_ontology` and its source definitions are candidate existing identity authorities; `bg_reference`, `bg_formula_constants`, specialized references and vocabularies supply distinct semantics. The L0 plan must decide the exact authority boundary from producer/consumer evidence. Do not indiscriminately merge these assets or replace every local constant with a runtime database request.

Generated, versioned Python/TypeScript snapshots are legitimate. They must identify their source release and pass parity tests. Unknown and ambiguous aliases stay explicit. Canonicalizing a node label must not erase true/mean-node conventions; canonicalizing a rule name must not erase school variants or disputed readings.
...
A source may support formation but not the proposed timing operator or manifestation claim. Those are separate qualification gaps. Ten derivative quotations of one source are not ten independent authorities. An allegedly lost doctrine needs identified witnesses and explicit reconstruction status, never an AI-invented verse.
...
The six-view design is not a target constraint. Preserve useful views as projections where they help consumers; do not maintain rival scoring authorities simply to preserve screen boundaries.
...
Electional/remedial/rectification assets keep their separate method and authority gates. L4 does not calibrate itself. If an affirmative, specific permitted forecast is earned, emit it with the exact outcome and temporal scope. If not, retain the useful interpretation/activation and explain the absent bridge rather than making every question permanently unanswerable or inventing a claim.

### 6.6 Mīmāṃsā: challenge without contaminating the answer
...
| DP08 Temporal mechanism | L2+qualified clocks → L3 consumers | Same mechanism/configuration ID, engaged participants, qualified necessary/optional conditions, enablement/inhibition, alternate routes and horizon. No topology-only gating. |
...
| DP18 Future qualification | Approved L5/research artifact → named later consumers | Immutable admission/version/feature lineage, permitted population and cutoff, independent evaluation; no automatic personal tuning. |
...
### 9.2 Four purpose paths

| Path | Use across the plane | Boundary |
|---|---|---|
| Event-free prospective interpretation | Admitted reference/chart/method evidence with permissible question specification. | PPR-31 bars life events/outcomes, including indirect features, personal selectors, summaries and rectification derived from them. |
| Permitted historical inquiry | L0 vocabulary; L1 separate event-time context; L2 structural alternatives; L3 temporal alignment/unmatched windows; L4 manifestation distinctions. | Hindsight and exposure explicit; keep event-conditioned overlays separate from event-free structural/temporal products. C1 permission does not admit C3 data to synthesis. |
| Protected evaluation | Frozen claims and admissible observations under L5; capture, confirmation, observation, eligibility and score denominators distinct. | C3 stays out of synthesis prompts/providers. Original wording, outcome and probability are not rewritten to fit. |
| Future context-conditioned prediction | Potentially valuable narrowly admitted pre-cutoff practical context. | Proposal/amendment only; requires explicit PPR-31/data-purpose decisions, feature lineage and separate evaluation before implementation. Not part of the initial elevation execution. |

“Across all layers” means every justified consumer receives the minimum complete purpose-qualified projection. It does not mean every table gets the raw diary. Derived values retain the source restrictions. Existing `SUBJECT_CONSENT_ENFORCEMENT: OFF_BY_DESIGN` posture and separate authorization/privacy/research gates remain unchanged.
...
### 9.4 Voluntary observation briefs

Where useful and voluntarily chosen, preserve rival propositions, the observation that would support or contradict each, relevant period and precision, ambiguity/insufficient-observation conditions, when criteria were established and when the person saw the interpretation. Preserve ordinary periods and genuinely observed non-events separately from silence; do not prescribe constant self-monitoring.

Classify the brief as forecast, retrospective hypothesis or another interpretive inquiry before routing its observations. Criteria established after an outcome are retrospective. The brief is not research enrollment, a clinical intervention or permission to feed observations into prospective generation. Existing purpose and C3/PPR-31 boundaries remain unchanged. These are added fields/views over existing inquiry, claim and observation owners, not a new asset by default.
...
Prefer these missing *capabilities* within existing owners: qualified rule/applicability dossiers; exact entity/convention adapters; full structural/temporal mechanism views; scope-aware capability metadata; evidence-dependence and omitted-concept checks; precise outcome/manifestation contracts; purpose-qualified observations; correction and coherent-generation propagation; complete reading delivery; and controlled comparison operators.
...
| Delivery sentinel | A decisive low-ranked/non-default field reaches synthesis and appears with its conjoint interpretation in all completed managed parts, replay and permitted export. |
...
1. Consumer value and distinctive layer responsibility, inherited P/V/DP obligations and exclusions.
...
6. Upstream demand and downstream offers with field/grain/context/lineage contracts, named owners and tests.
...
| Preserve-first upstream work | Extend from L3 demands to independent L0–L5 consumer responsibilities, scholarship, exact lookup, continuity, controlled comparison and permitted non-natal horizons. |
| Historical accepted assets | Preserve receipts; review changed/unproven semantic/serving seams without relabeling historical closure as absent. |

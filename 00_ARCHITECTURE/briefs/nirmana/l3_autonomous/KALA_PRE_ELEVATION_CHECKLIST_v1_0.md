...
| C3 | **Vedha: CLOSED code-side, pending merge of PR #2731.** `house_vedha` rows now carry a digest of the exact reference rows consumed; a detector reads *stale* for a changed rule or for rows with no fingerprint (the 132-row case); the `'4.0'` candidate build refuses on stale (`bb7644ee4`, three mutations each caught). The 132 production rows stay stale until a rebuild — correct, and now detectable. **Mūrti: STILL OPEN** — `bb7644ee4` touches `ka_vedha_gochara` only; `kala_moorti_nirnaya` (143 rows) has no fingerprint and no detector. **New finding of the §N.7 class** (audit 1.9 §7): the vedha writer had stamped every row `verse_cited` — including the six `UNSOURCED` node rows — and substituted a citation where none existed; fixed at `4d8f83050` | [V] |

...
date: 2026-09-24
responds_to: "KALA_SYNERGY_AUDIT_v1_0.md / KALA_SYNERGY_BINDING_v1_0.md / KALA_SYNERGY_AMENDMENTS_v1_0.md §Gochara (strategic session madhav-fc, commit 63b5fb429 on l3/kala-elevation-readiness)"
...
| "`claim_grain` … prose only, column unchecked" | worse: no production code path sets `claim_grain` or `time_basis` at all (0 literals under `services/gochara_*`); the only writer is `tests/l3/gochara/test_wp4_decomposed.py:612`, which writes `"exact_instant"` — a value in neither the prose (`date_grain`/`instant_grain`) nor the binding | the column is unpinned in code as well as in DDL |
...
| 4 | `completeness_state` / `claim_grain` unconstrained; F06 six named nowhere | **real gap**, deeper than stated (no code sets `claim_grain`; live `qualified` is not an F06 name) | D-S2: adopt the six F06 names on `completeness_state` with `qualified → applied`, CHECK both columns, pin `claim_grain ∈ {instant_grain, date_grain}` and `time_basis` in `WP1_CONTRACTS.md` §3.1 and in the kernel dataclasses. `inapplicable` stays out of `target_resolution_state` (ruled) |

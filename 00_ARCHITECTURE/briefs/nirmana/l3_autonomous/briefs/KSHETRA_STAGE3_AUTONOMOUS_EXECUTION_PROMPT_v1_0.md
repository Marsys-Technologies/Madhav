...
changelog:
  - "1.8 (2026-09-24) — the six open native questions RULED under delegation (ruling sheet 1.13): B8-6 evaluation-only knot source with detectors; PG349/PG353 NEITHER (covariate unqualified, not seated); B8-4 shape ratified with group_id content-addressed; B8-7 declared alias map with valence NULL + L2 DEMAND. B-4 (shared grid kept, P1 per-class not funded, refinement dormant-not-disabled) and B-2 (terminal assets only) bind outside this prompt and are recorded in Phase 5. Executor not started."
  - "1.7 (2026-09-24) — Phase 2 item (b′): the DAG edge guard verified statically — writer.py:2344's read of kala_gochara_windows is a latent HARD violation on main (sole active owner ka_gochara not in the closure; CI skips the live test without DATABASE_URL); item (b)'s \"evaluation edge\" is not a state the guard can hold. Three options stated with their trade; executor decides or escalates. Gochara held the ka_gochara edge out of 1084 at Kshetra's request (64bdc10da)."
...
**Phase 2 — The seams, by DAG stage (plan §4 stage 3 order).**
  ...
  **Synergy-binding items (strategic session's `KALA_SYNERGY_AUDIT_v1_0.md` @ 63b5fb429; the author spot-verified
  ...
  kernel's `contact_id` — this is **RULED 2026-09-24 under native delegation** (ruling sheet 1.13 §NATIVE-DELEGATED RULINGS, B8-6): the episodes are an internal **knot source** for a continuous-time integral, not a contact verdict, so they stay `evaluation`-only with `comparable_with = different_convention` and you do **not** consume `kala_gochara_contacts` (which does not exist in production). No episode may be served as a transit claim — add the detector that fails if one reaches a served surface. Where Kṣetra cites a transit *as evidence for a window* rather than as a knot, it consumes Gochara by `window_ref`, and B6 binds there; (v) [verified: 0 hits] no coverage object on any result and no `independence_group` inherited from
  ...
  **(b′) The DAG edge guard — verified statically 2026-09-24, and it changes (b).** `pipeline/orchestrator/dag_edge_guard.py` loads only `is_active = true` assets (`:96-103`), so the RETIRED sweep drops out and `ka_gochara` is the sole registered per-chart owner of `kala_gochara_windows` (seed `target_table` + `count_sql`) — the table is hard-checkable. The guard attributes a file to an asset only by an `@register(...)` marker in that file (`_asset_for_file`, `:158-160`): `stage4_field.py` carries none and is invisible to it, but `writer.py` carries `@register('ka_kshetra')` and reads `FROM kala_gochara_windows` in a live SQL string at `writer.py:2344`; the transitive closure of Kshetra's `depends_on` (`ka_dasha_kala`, `ka_gochara_resonance → bg_transit_rules`, `ga_panchanga`, `bo_*`, `bg_*`) does not reach `ka_gochara`. So the live guard (`tests/test_dag_edge_guard.py::test_live_registry_has_no_hard_violations`, run only when `DATABASE_URL` is set — which is why CI has not caught it) would report a HARD violation on today's `main`: *ka_kshetra reads kala_gochara_windows (produced by ka_gochara) but ka_gochara is NOT in depends_on closure*. The guard has no role concept; "evaluation edge, not a build gate" is not a state it can express. **Three honest options — the executor states which, with the trade, and puts it to the native if it cannot decide:** (i) declare `ka_gochara` in `depends_on` (K-1's fix; Gochara held that line out of migration 1084 at Kshetra's request, 64bdc10da): a hard per-chart build gate — `asset_runner` requires the dependency `lit`/`service_ok` — on an asset whose registry row is itself wrong today (`target_table` says `kala_gochara_windows`, its writer writes `kala_gochara_windows_v2`; Gochara's WP10 step 5 re-pins it, not done); (ii) remove the legacy cross-check read at `writer.py:2330-2347` and its `legacy_sweep_xref` provenance edges (`:948-953`, `:1059-1064`) — an evaluation-only read of a table the W6 cutover rewrites, whose authority the `'v1'` fall-through already misstates — and record the removal as a deliberate loss of the legacy agreement signal; (iii) run the live guard first (`DATABASE_URL` against the registry, read-only) to confirm the finding on the executor's own environment, then choose. Do not do what (b) above implied — declare nothing and rely on a role the register cannot hold.
...
**Phase 4 — S1 ingestion packet (gated on Phase 3 PASS) and the interfaces.**
  **S1:** consume `ka_vedha_gochara` (all `vedha_kind`s, admitted under the uniform rule — today
  ...
  (E7b needs `ka_tithi_pravesha`'s own source qualification first — C8). **S5:** denominator prose; the null's non-comparability flag and `1/R` resolution survive every projection. **Binding B2/B4/B5 columns (KALA_SYNERGY_BINDING v2.2 — carried in spirit, now named):** emit `epistemic_class` (F04), `completeness_state` with exactly the six values `{applied, inapplicable, unavailable, unqualified, contradictory_unresolved, unexplored}`, `operator_role` (F12), `tier_basis ∈ {relative_uncalibrated, calibrated:<gate_id>}`; inherit `independence_group` as the **union** of every witness's groups and carry it with `comparable_with` on any scored projection (jsonb shape **RULED 2026-09-24**: `[{group_id, family, roots[], members[], basis}]`, `basis` always `declared_lineage`, and **`group_id` content-addressed as sha256 over `(family, sorted(roots))` — never a surrogate**, so the union is well-defined across producers; `members[]` is per-asset and concatenated in the union, never deduped away); emit `coverage` on every result in the seven-field shape `{requested_horizon, completed_horizon, resolution, partitions_searched[], exclusions[], unsearched_regions[], completion_detector}`, and an empty result is **a row and a coverage object, never "no row"**. **R-6 four-way separation (B8-7) — RULED 2026-09-24:** emit the R-6 axes as a **declared alias map**, not as a re-decomposition of λ: `activity` ← the row's computed intensity (`kala_field`'s `(alpha, gamma)` / `lambda_start`), `applicability` ← the F06 `completeness_state` above, `availability` ← the `coverage` object above — three aliases, no new columns — plus one genuinely new emission, `valence`, which is **NULL with reason `class_polarity_not_declared_upstream`**: `bodha_pratijna` carries no polarity column and no event-class catalogue exists, so deriving a sign from the 27 class names would re-derive what L2 owns (§N.5) and invent a default (§N.7 item 6). Raise the L2 DEMAND packet for `event_class.valence`; do not compute it here. The stage-1 symbol `polarity` stays an input grain and is **not** promoted to a window-level valence. Declare the map in brief §1 and prove each alias in §6 with a detector that fails when it is absent or mis-valued.
...
**Phase 5 — P6 → P2 → P1, each against its equivalence contract (brief §4.6).**
  ...
  **P1 scope, RULED 2026-09-24 (B-4, ruling sheet 1.13): per-class knot grids are NOT funded — keep the shared chart-level grid.** Measured on the canonical chart: `kala_field` = 8,570,075 rows = 25 classes × exactly 342,803 segments, uniform, and `refinement_depth = 0` / `refinement_exhausted = false` on every row. `breakpoints()` (`stage4_field.py:880-896`) is class-independent by construction and the per-class operation (SM-R-7 Option B filtering) only *removes* knots, so a per-class knot set is necessarily a **subset** of the chart-level union; and `integrator._refine`'s τ = 0.02 detector reports nothing needs refining at a ≈2.6 h mean segment. This closes the standing `refinement_depth` **dormant-vs-disabled** question as *dormant, with a live detector* — do not re-open it without falsifying the invariant. **Add that invariant to F-KSHETRA-11's family:** for any class, the class knot set ⊆ the chart-level breakpoint union, failing if a class ever introduces a knot outside it. A shared x-axis is **not** comparability — B2's `comparability_class` must still be declared (or a single class declared) before any ranking over Kṣetra windows. **B-2, same ruling:** Kṣetra counts **0** toward the campaign's "Accepted N/22" until its own terminal gate (`t3 FROZEN`, plan stage 6) — `PRODUCER_READY` at this Phase 5 exit does **not** increment N; report stage-family progress as a separate `stages_accepted: k/9` figure, never summed into N.

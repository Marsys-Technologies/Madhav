...
house/saṅkrānti (26.9-11, 26.29), decanate phase (26.25). The muhūrta tārā-bala form stays [U]. **Condition mirrored from the Kshetra session (2026-09-23), adopted:** reuse of the served vedha producer is **per `vedha_kind`, not per table** — `house_vedha` rows enter as F06 `applied` (admitted L0 chain `bg_phaladeepika_vedha` — Phaladīpikā PG353, ADJUDICATION-11; W0-accepted; CR-102 frame — **not** "BPHS ch.29", which is Bhāva Padas in the corpus (vol1:23168) and was a mis-citation inherited from FOUNDATION_SAFETY §5:207, corrected 2026-09-23 on the Kshetra session's finding, author-verified); laṭṭā **exists at L0** too (`bg_phaladeepika_latta`, migration 528, PG338-339 ślokas 42-44) and still enters `unqualified`, not via corpus extraction; **`sarvatobhadra` and `latta` enter as `unqualified`** until source-qualified (FOUNDATION_SAFETY §5: sarvatobhadra "remains unqualified"; laṭṭā separately scoped) — never `applied` by virtue of sharing the table. And `kala_vedha_gochara` windows are **DATE-grain**: any child sub-peak that cites them stamps `precision_regime` accordingly. This also binds C11 today.

...
### D-1 — M-6 minimum n: **20 per stratum (crit ≥8, α=0.0321, power 0.868 at 2.5×) · 50 instrument-level (crit ≥16, α=0.0308, power 0.904 at 2×) · `method_version` never pooled**

**[RE-SET 2026-09-24 on the native's written instruction, verbatim:** *"The numbers that you've suggested are very high. Can you considerably And reasonably. bring it down? And please close it."* **— the author's decision under that instruction, kept apart from the native's words.** The false-pass rate is unchanged (α ≤ 0.05 at both gates); what moved is the smallest lift the gate can see at ~80% power. Exact binomial, null 0.20:
...
**Why these and not lower.** Below n≈15 a stratum gate only sees a tripling and single flukes dominate the tail; 20/50 is the largest cut that keeps the per-stratum gate able to see a 2.5× lift and the instrument gate a doubling, at ~87–90% power, with α still under 5%. **Honest consequence, carried into every E6 report:** a genuine doubling per stratum is now seen only 58% of the time, and a 1.6× lift instrument-wide 55% of the time — so "no signal" means *not large enough to see at this n*, never *nothing there*. The gate opens sooner; what it can certify is a large effect. Code, tests and evidence (`exposure.py`, `test_ka_sangam_e6_exposure.py`, `S20`) moved with the numbers in the same commit; `KALA_DELEGATED_DECISIONS` D-F (35/100) is superseded by this instruction.**]**

~~(heading as it stood before this instruction: 35 per stratum (crit ≥12, α=0.0344, power 0.805) · 100 instrument-level (crit ≥28, α=0.0342, power 0.833))~~

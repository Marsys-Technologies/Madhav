...
Śrīpati (Kimi) / whole-sign only?** (d) **Node convention: mean (L1, Kimi, me) / true (scanner)?** — **Re-corrected 2026-09-23 (Gochara session's finding, author-verified at source): the split is four-way, and `ephemeris_daily` is TRUE despite its contract.**
  ...
  **Reading-visible consequence — stated in class, not as a specific pāda (withdrawn to class 2026-09-23 on the strategic session's reconciliation, which the author accepts):** a mean-vs-true Rāhu shift of ~1° against 3°20′ pādas **can move a natal pāda**, so this ruling is visible in a reading, not only in code. For your own chart the two sides are of unequal standing:
  - **Mean (authoritative, cited per §N.5, not recomputed):** L1 `chart_facts` `c520713087b97470`, subject **`RAH_MEAN`**, `longitude_sidereal = 49.0330441°` (`lahiri_chitrapaksha`, pyjhora/1.0.0) → Rohiṇī **pāda 3** (fact `060bb63b81a073bb`). This sits 0.97° from the 50°00′ boundary — far outside any backend error — so **pāda 3 under the mean convention stands**. L1 names its convention in the subject key; the `ephemeris_daily` store does not.
  - **True / consumed side: undetermined on this host.** Birth-instant true-node recomputations here (author 50.054°, strategic session 50.050°) are **Moshier** (no `.se1` on any worktree) and sit only **0.05°** from the boundary; the value Kāla actually **consumes** — the stored `ephemeris_daily` knot at 00:00 UT, 49.9941° converted — sits **0.0059°** from it. Moshier's error (up to ~0.1°) exceeds both margins, so **no pāda assignment for the true/consumed value can go to you as a measured fact** until produced against the production ephemeris (`/app/ephe` with `sepl_18/semo_18/seas_18.se1` and checksums recorded). The three circulating numbers were three quantities (two birth-instant recomputations, one stored day-knot), not a disagreement.

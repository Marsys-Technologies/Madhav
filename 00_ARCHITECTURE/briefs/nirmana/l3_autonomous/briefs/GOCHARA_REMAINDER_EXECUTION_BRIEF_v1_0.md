...
| **the merge of PR #2731** | **REAL, NOT OURS, and being fixed — expect two red pins tests until PR #2733 merges; do NOT touch the pins** | **Cause (from the L0 session, who reproduced it: baseline `b6690928f` gives 2 failed / 60 passed):** two tests in `test_nirmana_analysis_layer_pins.py` check a deliberately rewound pre-repair fixture against a baseline that now carries the repair successors, so they fail for every PR based on or merged onto main at or after the L0 merge (`b6690928f`, 2026-09-24). The tool's own `--check` stays green. **Nothing needs re-sealing or re-admitting on our side.** The fix is L0's PR #2733. **History of this row, kept because it went wrong twice:** it first said the merge WILL fail because main's pins name a commit that is not an ancestor of main. That commit fact is true (verified, 12 references) but is NOT the cause. It was then retracted on the strength of `governance-gates` passing on #2724, #2725, #2726 and #2727 — but those all merged on 2026-09-23 or before the L0 merge, so they could not test the post-merge state, and the retraction was itself wrong. The Kṣetra stream's two queue ejections are plausibly this same cause. |

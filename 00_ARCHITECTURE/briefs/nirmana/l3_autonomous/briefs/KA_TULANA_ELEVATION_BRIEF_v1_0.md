...
7. **`WindowInput` adapter by `window_ref` (B3)** from `kala_convergence`: `t_start/t_end/t_peak`
   ...
   the partition inputs, `generation`, `mode`. **Mapping declared — and it is NOT a two-way rename.**
   `independence_groups` (Saṅgam brief) → `independence_group` (binding) is a spelling rename.
   `comparability_class` is **not** a spelling of `comparable_with`: binding v2.3 §B5 corrects the
   v2.0–v2.2 instruction that ordered that rename (Saṅgam raised it). The two coexist and mean
   different things — `comparable_with` is a **relation** between this row and a reference row
   (`{self, same_convention_same_inputs, same_convention_newer_inputs, different_convention}`, four
   values, D-S6), while `comparability_class` (stage3 `exposure.py:65-82`, `ka_sangam/<signature_
   class>`) is a **partition key** saying what a projection may rank *within*. This kernel consumes
   both: `comparability_class` selects the partition, `comparable_with` qualifies the pair inside it,
   and cross-class pairs are excluded as `incomparable` (§9) rather than ranked. **None of these
   columns is persisted on any branch today.**

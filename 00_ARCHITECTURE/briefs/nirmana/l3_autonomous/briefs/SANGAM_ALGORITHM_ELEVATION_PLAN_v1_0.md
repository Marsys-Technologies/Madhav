...
evidence: evidence_sangam/ (v2 harness) — 13 assertion-based falsifiers, each with a negative control that must fail; manifest-checked runner; OUTPUT_2026-09-23T012041.txt (S8 on SWIEPH with .se1 checksums; re-pointed 2026-09-23 per K2-05 — the previously cited 2026-09-22T2342 run had S8 passing as an oracle on Moshier, which the on-disk S8 v2.1 now refuses via NOT_RUN(3)) = SUITE-PASS (13/13 positive, 13/13 negative controls fail); OUTPUT_…T234203.txt retained = the run that caught two of the author's own script errors
...
|---|---|
| M-1 | *(rows mix the native's ruling with the sheet's author layer; per-element markers added 2026-09-23 per K2-08 — `[N]` native, `[A]` author/sheet layer)* **[N]** R-1/R-3/E1 as written, with **mean node** **[A]** (disposition (b): true L0 knots retained, mean derived at read in the L0 service, **the 1.016204° / 3658.3″ mean↔true gap** declared per row — K2-07), **Placidus-as-stored cusps** (frame on every row; Śrīpati only as a ratified variant), **legacy scan retired after one generation**, **no node dṛṣṭi** (nodes as gochara agents and targets only). **[A]** R-3's convention vector: `ephemeris_backend`, `epoch_convention`, `ayanamsa_application` (= `apparent_flg_sidereal`). **[S3-E 2026-09-23, RRV-16]** M-1's mean-node convention is NOT executable inside Saṅgam today: the scanner is hardcoded TRUE_NODE (`transit_search.py:10,64`, frozen, Gochara-owned). Until the N-7-gated producer amendment lands, Saṅgam's own scan remains true-node and every new row must carry the convention vector with that declared; the ruling binds the contract, not the frozen scanner. |
| M-2 | **[N]** E2 as written; **4 = indeterminate-leaning-adverse** (Phaladīpikā 23.11 = `phaladeepika:PG299:C1`, verified at the table 2026-09-23; note 3 and 4 both read "fear" there — the 4/≤3 boundary is BPHS's); **6/8/12 inversion does not ship**; BPHS bands primary. **[A]** producer completeness receipt now; kakṣyā deferred; Kshetra S1 shares the source. |
| M-3 | **§4.5 governs.** R-1's directed events are produced **upstream by the Gochara stream** — Path B (kernel, N-5/N-7) if the native approves N-7 on the Gochara brief, else Path A (bounded `transit_search` amendment). ~~**E1 and E3 stay gated until N-7 is ruled**~~ **[RELEASED 2026-09-23: N-7 RULED by the native on `GOCHARA_RULING_SHEET_v1_0.md` (`50b5e1822`) — kernel path, `kala_gochara_contacts`; its consequence line releases Saṅgam E1/E3 from the gate. They build when S-2 (the kernel's contact events / `find_episodes`) exists — unbuilt for lack of a producer, not of authority.]** E3's bounded tier is granted in principle. `_resolve_transit_planet` is replaced by event consumption at that point, not extended. TRANSIT_REDESIGN prompt → SUPERSEDED-and-reissued. **[S3-E 2026-09-23, RRV-02]** The same gate covers R-3(b)/R-4's scanner-signature-dependent detector SPECs: until N-7 (or a named Path-A owner) lands, step-3 detector work runs only its scanner-independent scope (testimony identity, fail-loud context reads, R-6 kernel); the frame/convention vector is authored into the event-consumption interface so it activates with the producer. N-7 status at 2026-09-23: ~~**UNRULED** (verified by full-text search of the campaign tree)~~ **[CORRECTED 2026-09-23: RULED at 03:55 IST on `l3/gochara-elevation` — the search covered the wrong branch.]** |
| M-7 | R-6 as written. |
| M-4 | E4 as written; **[N]** **D30-for-DOSHA HELD** — **[A]** domain varga primary, D30 computed and carried as labelled `secondary_dosha`, excluded from score paths **and served surfaces and consumer SQL**; the falsifier must **exist in the MANIFEST and pass before the first D30 row ships** — until then the exclusion is an intention, not a verified property (D-4, amended per K2-04); cancellation detector precondition binds. |
| M-5 | E5 as written. |
| M-6 | E6 as written; L5 consumption opens on the two preconditions. **`EMPIRICALLY_EVALUATED` gate (D-1, AMENDED per K2-01/02/10/12): per-stratum n=20, critical ≥8, α=0.0321, power 0.868 at 0.20→0.50 (0.584 at a doubling); instrument-level n=50, critical ≥16, α=0.0308, power 0.904 at 0.20→0.40 (0.553 at 1.6×) — **re-set 2026-09-24 on the native's written instruction from 35/100; α unchanged, detectable lift raised, code/tests/evidence moved in the same commit**, pooled within one frozen `method_version` via a stratified CMH-type estimand with no significant opposite-sign stratum; never pooled across versions; figures are first-order approximations — two-chart clustering requires chart-level concordance plus a published overdispersion check; the 0.20 null comes from the exposure model a priori; below either gate → `PROVISIONAL_INSUFFICIENT_N` with actual n; the exposure manifest publishes the measured window-issuance rate.** `ambiguous` → native-adjudicated; **excluded from n but retained in the reported rate's adverse end** — every stratum reports `[hits/all , (hits+ambiguous)/all]` and the claim is stated against the lower bound (which is Kimi's M-6 condition 4 verbatim, so nothing native-adjacent is superseded); censoring rate published with a **10% `censoring_elevated` label / 20% block gradient** (D-2, amended 2026-09-23 per K2-03/K2-11). Evaluation population: consenting charts with real outcomes only; synthetic `evaluation_eligible = false` (D-3). |
...
| **R-6 Score-kernel separation** *(new, RR-05)* | Replace the single I-16 output with separate fields: `activity` (geometric/clock intensity, ≥0), `valence` (signed, from dignity/nature/AV/route), `applicability` (F06 per method), `availability` (inputs present), and — only where separately qualified — a calibrated probability. **Dignity leaves the necessary product**; it becomes valence. I-16's multiplicative-veto form survives only for genuine necessity (orb presence, route satisfied). Legacy rows carry `kernel_version = legacy_i16` and are **never pooled or ranked** with new rows. Consumer projections choose fields explicitly. **[S3-E 2026-09-23 (RRV-03)]** Within-class ordering contract (binding design pin — every consumer cut is an ORDER BY, and R-6 removes the old rankable scalar): each `comparability_class` declares one ordering key — `activity DESC, contact instant ASC` (deterministic tiebreak) — with `valence`/`applicability`/`availability` carried as data, never pooled into the key. Enforcement is SQL-level partitioning (`comparability_class`/`kernel_version` in every consumer query), not policy. **[S3-E 2026-09-23 (RRV-08)]** The static daśā prior (`engine.py:1108` default 0.5, `:1142` fallback return) is an availability state, not support: eligibility service failure → zero manufactured supporting contribution, `availability.dasha = unavailable`, the daśā current recorded as absent — never as neutral support. | S11 (today) · SPEC R-6 · **M-7** |
...
change. `ka_taranga` aggregation unit — **[S3-E 2026-09-23: RULING APPLIED, discrepancy recorded (RRV-05)]** the sheet's M-5 resolution row rules the unit: **occupied-day union per month per (method-contract × valence-sign)** — additive, duplicate-invariant, respecting partial months. The earlier "occupied duration / episode / route" open-choice text in this paragraph is superseded by the ruling (the ruling sheet is the binding document; the discrepancy is recorded here, not smoothed). **[S3-E 2026-09-23 (RRV-05)]** month boundary timezone pinned: the chart's birth timezone (from `birth_params`), not UTC and not run-time local — an occupied-day union in IST vs UTC splits different months. **[S3-E 2026-09-23: RULING APPLIED (RRV-06)]** Aborted approaches (station before exactness) are kept as labelled children — `approached, never perfected`; sign-level applicability survives (classically "no itthaśāla", per the sheet's M-5 record). Whether aborted approaches count in E6's episode numerator (`perfected: false`, Kimi leans yes) is **not ruled** — recorded open, to be settled with the native before E6's exposure manifest defines its numerator; until then they are carried as children but excluded from any frequency numerator, and E6's output must state which convention it used.
...
behaviour*. Current: `OUTPUT_2026-09-23T012041.txt (S8 on SWIEPH with .se1 checksums; re-pointed 2026-09-23 per K2-05 — the previously cited 2026-09-22T2342 run had S8 passing as an oracle on Moshier, which the on-disk S8 v2.1 now refuses via NOT_RUN(3))` **SUITE-PASS 13/13**; the retained
...
5. **Ephemeris:** S8 ran, on **Moshier** (no `.se1` files; requested SWIEPH, returned MOSEPH). v0.3's
   "no ephemeris test" was wrong; corrected. **[S3-E 2026-09-23, RRV-10]** Environment changed under this text: the `.se1` files landed 2026-09-22 17:20 (`/tmp/se1`) and the currently cited runs (`OUTPUT_2026-09-23T012041.txt` and later, incl. the reviewer's `OUTPUT_2026-09-23T051310.txt`) are **SWIEPH with sha256 checksums**. The fixture policy is re-anchored to the backend detector, not host state: every geometric fixture states engine, flags, ayanāṃśa, node convention and tolerance; the authoritative backend fact is the returned `retflag` per call. **[S3-E 2026-09-23, RRV-12 — policy line, previously unwritten]** Assertions of **logic** (counts, grouping, interval membership, topology) may run on Moshier with the backend recorded; assertions of **accuracy or tolerance against Swiss** are `NOT_RUN` without `.se1` present (Moshier-vs-Moshier cannot fail, §N.8). S8 v2.1 applies the stricter all-or-nothing rule to itself (exits `NOT_RUN(3)` on any fallback); both rules are now written, S8's stricter self-rule standing as the per-script ceiling.
...
## §10 — Stage-3 entry-gate dispositions (D-8 / Astra third review v1.0)

**Gate.** D-8 (ruling sheet §CLOSE) made the third independent review of this v1.0 a stage-3 entry
gate. It ran 2026-09-23: `ASTRA_REVIEW_SANGAM_ALGO_PLAN_v1_0.md`, verdict
**PROCEED_WITH_AMENDMENTS**, findings RRV-01…RRV-16 (4 HIGH, 5 MODERATE, 5 MINOR, 2 NOTE), plan
SHA-256 `8f97963fee71a2c74d877cc29f8f378f06e3048a42f3e34c112cb83e10f51119`. The reviewer's gate
condition — RRV-01…RRV-09 dispositioned in plan/brief text before code — is discharged by this
section and the companion amendments. All dispositions below are **[S3-E]** stage-3 executor
dispositions of author-layer review findings: none re-opens a native ruling, none is doctrine.
Where a disposition exceeds implementation detail (RRV-05/06 numerator question), the residue is
recorded open and routed, not decided.

| # | Severity | Disposition (binding on stage 3 unless marked carried) |
|---|---|---|
| RRV-01 (HIGH) | E2 producer-receipt ownership | **Dispositioned without a may_touch change.** The completeness/validity receipt is computed stage-3-side: the Saṅgam writer joins the AV sign-fact rows it consumes against L1 `chart_facts` graha_position facts (same chart, same ayanāṃśa). A planet with AV rows but no L1 position fact → those rows are `provenance: fabricated_zero`; a planet with an L1 fact and all-zero AV → `measured_zero`. This recovers exactly the distinction RR-02 proved unrecoverable at reader level, from data L1 already serves (§N.5 — referenced, never restated), with **no edit to `ga_strength_writer.py`** (outside `may_touch`, frozen L1 writer surface). The strictly-better producer-column version is routed as an L1-owner bounded packet (receipt columns only, no numeric change) — recorded as a follow-up in durable state, not a stage-3 blocker. |
| RRV-02 (HIGH) | Step 3 silently gated on N-7 | **Adopted inline (§0R M-3 [S3-E] note).** R-3(b)/R-4 detector SPECs that need the scanner signature amendment gate on the same N-7/Path-A-owner decision as E1/E3. Stage 3 proceeds now with the scanner-independent scope: R-3(a) geometry-cache key, R-3(b) testimony-identity fields, fail-loud lagna/Moon (S12), R-1 provenance binding, R-2 intersection oracle, R-6 kernel. The frame/convention vector is authored into the event-consumption interface (§4.5 mechanism) so it activates with the producer. N-7 status at 2026-09-23: **UNRULED** (verified by tree-wide search). The Path-A owner question remains the native's (K3 Q9#1; nobody has named one) — recorded open. |
| RRV-03 (HIGH) | No within-class ordering contract | **Adopted inline at R-6 (binding design pin):** per-`comparability_class` ordering key `activity DESC, contact instant ASC`; valence/applicability/availability carried as data, never pooled; enforcement as SQL-level `comparability_class`/`kernel_version` partitioning in every consumer query. SPEC R-6 gains the assertion: two equal-activity opposite-valence rows both survive to a served page. |
| RRV-04 (HIGH) | Two consumers unnamed | **Adopted.** `mi_adhilepa` (L5 calibration overlays binding the volatile `convergence_id`, unordered LIMIT 500) and `ph_nimitta` (L4 per-domain ROW_NUMBER + nearest-peak LIMIT 1 attach) join the consumer obligation set; `mimamsa_convergence_adjustment` and `mi_adhilepa` join the §6.3 dependent map. `mi_adhilepa`'s surrogate-id binding is recorded as an **R-5 / L3-U07 dependency** (L5 is sealed — packet and test only, never a stage-3 edit). The `UNRESOLVED_USE` list (`mi_kula`, `ph_muhurta`, `ph_pratikara`, `ph_sodhana`, `taranga_kernel`, `ka_temporal/date_resolver`, `kala_temporal.ts`) gets a per-item "disclose or bind" disposition at the phase whose output it touches — carried as a stage-3 checklist item, first pass at Phase 2 exit. |
| RRV-05 (MOD) | Ruled taranga unit missing from plan E5 | **Ruling applied + discrepancy recorded inline (§3 E5).** The sheet's M-5 resolution row (binding) rules the unit; plan E5's open-choice text is superseded. Month-boundary tz pinned: the chart's birth timezone from `birth_params`. |
| RRV-06 (MOD) | Aborted approaches missing from plan E5 | **Ruling applied inline (§3 E5).** Aborted approaches kept as labelled children (`approached, never perfected`; sign-level applicability survives). E6-numerator inclusion (`perfected: false`) ~~is **not ruled**~~ **[RULED 2026-09-23 — author under the native's second delegation, sheet §Residual B-3: INCLUDED, `perfected` a mandatory reported covariate]**; the remainder of this row was the interim and — recorded open, routed to the native before E6's numerator is defined; until ruled, excluded from frequency numerators with the convention stated on output. |
| RRV-07 (MOD) | Stale gate numbers in companion changelog | **Adopted.** Companion changelog corrected in place with a dated correction line (35/crit≥12/α=0.0344/power 0.805; 100/crit≥28/α=0.0342/power 0.833). |
| RRV-08 (MOD) | Static daśā prior bypasses availability | **Adopted inline at R-6.** Eligibility failure → `availability.dasha = unavailable`, zero manufactured support. SPEC R-6 fixture added. |
| RRV-09 (MOD) | §6.3 canonical framing unpinned | **Adopted.** §6.3 comparison uses a canonical serialization with an explicit field-exclusion list (surrogate row id, `computed_at`, generation-head pointers) and canonical JSONB framing (sorted keys), compared per stable id; the harness runs on a schema-faithful disposable DB; dependent map extended per RRV-04. Without the exclusion list the comparison either fails benign rebuilds or loosens until a swap passes — the unframed-hash attack one level up. |
| RRV-10 (MINOR) | Stale ephemeris text | **Adopted inline (§9.5).** Fixture policy re-anchored to the backend detector; SWIEPH run cited. |
| RRV-11 (MINOR) | K2-08 markers partial | **Adopted inline (§0R M-2 row).** |
| RRV-12 (MINOR) | Moshier-policy line unwritten | **Adopted inline (§9.5).** Logic-on-Moshier-with-backend-recorded vs accuracy-is-NOT_RUN; S8's stricter self-rule as per-script ceiling. |
| RRV-13 (MINOR) | D-2 interval denominator unpinned | **Carried to Phase 5 (E6).** "all" in `[hits/all, (hits+ambiguous)/all]` = all issued windows in the stratum including censored and unobserved; the exposure manifest defines "fully observed" against outcome-collection lag. Pinned there or E6 does not land. |
| RRV-14 (MINOR) | E1 angle-family assertion missing | **Carried to the E1/E3 phase** (gated on N-7; the SPEC is authored with the event-consumption interface so it is runnable the day the producer lands). The emitted angle set per contract is asserted: Parāśari = 7th-all + 3/10 Saturn, 5/9 Jupiter, 4/8 Mars with fractional slabs; 60°/90° only under a Tājika label with Tājika strengths/orbs; a contract's label may not carry another family's angles or strengths. |
| RRV-15 (NOTE) | NEG sincerity escape | **Permanent discipline, recorded.** The runner checks structure, not that a NEG path is evidence-bearing (the reviewer demonstrated a no-op script that passes both ways). Standing stage-3 rule: every new evidence script's NEG path is read line-by-line in review; a NEG path that does not mutate evidence-bearing state is rejected. |
| RRV-16 (NOTE) | Mean-node ruling not executable in Saṅgam today | **Adopted inline (§0R M-1).** Scanner hardcoded TRUE_NODE (frozen); ruling binds the contract, not the scanner; new rows declare the vector honestly until the producer amendment lands. |

**K2 verification designation discharged:** the Astra v1.0 review verified K2-01…K2-12 landed
(10 fully, K2-05/K2-08 partial — both closed by RRV-11/RRV-12 inline) and independently confirmed
the DIS.031 predicate-level recheck (100 chunks, no nodal aspect grant; PG133 "Rahu similar to
Saturn" is a results-analogy, not a dṛṣṭi rule).

*Changelog — **1.0-stage3** (2026-09-23): §10 added (D-8 dispositions, RRV-01…16); inline [S3-E]
amendments at §0R M-1/M-2/M-3, §3 E5, §3 R-6, §9.5. Executor amendment layer, marked; native
rulings untouched.*

...
- **§7.B (tranche 1, steps 0–5): RUN 2026-09-24 under native authorization (E-015 resolved-with-path); HALTED at step 5 — tranche NOT green.** The frontmatter flags were read fresh from disk and are TRUE (both tranches authorized by `GOCHARA_NATIVE_RULINGS_2026-09-24_v1_0.md` §3). Run executed per brief §12.15 order — **step 3 first** — with the Cloud SQL proxy on 127.0.0.1:5433. E-015 privilege resolution: no Secret Manager/local credential existed for `data_plane_schema_owner`/`data_plane_migrator`/`postgres` and the `postgres` superuser is itself locked out of schema public; path used — rotated `data_plane_migrator` (GitHub env secret `DATA_PLANE_MIGRATOR_DATABASE_URL` updated to the pinned proxy format), `SET ROLE data_plane_schema_owner` → **temporary** `GRANT CREATE ON SCHEMA public TO amjis_app`; also rotated `postgres` and set the previously-unset `DATA_PLANE_OWNERSHIP_ADMIN_DATABASE_URL`. Per-step verdicts:
  ...
  - **Step 4 — GREEN with scope note** (`4670bebab`; applied **only 1080/1081** through the script's own machinery with APPLY_SET/EXPECTED_COLUMNS scoped, per the native's tranche-1 reading of A-2; production post-check v1=38287, 3.0=1830, `gochara_resonance_map` 1595 rows, century `is_active=false`). **1082/1083/1084/1087 NOT applied — E-016, native ruling needed** (follow-up tranche vs post-merge deploy pipeline; 1087's columns are what §12.3's step-6 writer populates, so tranche 2 may block on this too). Evidence: `evidence/step04_evidence.md`.
  - **Step 5 — FAILED, tranche HALT (E-017).** `platform/migrations/1091_wp10_ka_gochara_registry_repin.sql` (numbered per E-009 re-scan — max 1090 across all origin/* heads on origin/sangam/stage3; MIG-1 guard PASS) aborted on `malformed array literal: "[kala_gochara_windows, ...]"` — production `asset_registry.clear_tables` is `text[]`, the script wrote `'[...]'`, and the rehearsal harness declared the column TEXT so rehearsal never caught it (**rehearsal-fidelity gap recorded**). Single-transaction abort; production verified unchanged (count_sql still '2.0', no snapshot table, generations v1=38287/3.0=1830). **Corrected 1091 prepared (`'{...}'` ×2 + header note) but deliberately NOT re-run** — the E-015 authorization covered the privilege path, not script surgery; fail-closed doctrine halts the tranche for native ruling. The temporary CREATE grant was **revoked and verified** (`has_schema_privilege=f`) at halt; re-grant is one command on resume. Evidence: `evidence/step05_evidence.md` (includes the §12.14 digest-pin sweep: seed carries old count_sql/depends_on but owns only `target_table` on conflict so no seed update blocks step 5; **conjunct-(j)/1072 interplay flagged** — unapplied 1072 would set `target_table='kala_gochara_windows_v2'` and break conjunct (j) if it lands after a successful step 5).
...
- **§7.C (tranche 2, steps 6–10): BLOCKED — NOT STARTED.** Precondition assessment from disk (2026-09-24):

  | Precondition (brief frontmatter + §7.C) | State |
  |---|---|
  | `PRODUCTION_TRANCHE_2_AUTHORIZED` flag | TRUE (ruling §3) |
  | Tranche 1 evidence green | **UNMET** — step 5 FAILED (E-017) |
  | P-1 merged to `main` | **UNMET** — PR #2731 OPEN (`mergedAt: null`); `register_gochara_windows.ts` packet changes (+401/−63) are branch-only |
  | WP8 M-1 ratified in the sheet OR fallback | MET — ratified with the value split (`GOCHARA_NATIVE_RULINGS_2026-09-24_v1_0.md` §2) |
  | WP9 §5.2 (M-8) merged | **UNMET** — `services/ka_vedha_gochara/` changes (+553/−19) are branch-only on PR #2731 |
  | N-19 artifacts on `main` | **UNMET** — `KALA_COST_PROFILE_v1_0.md` / `KALA_BASELINE_v1_0.md` exist on this branch, not on `origin/main` |

  Tranche-1-incomplete alone blocks 7.C; steps 6–10 were not started on this run's own judgment. Per E-012, tranche 2 would anyway stop at step 7's `windows_present` gate (the `'4.0'` windows projection has no writer; the gates are deliberately not weakened). The §12.9 staleness-gate test (12.9c) was not re-run — its disposable DB (`gochara-wp6-disposable`, 55433) was torn down at the earlier closure; recreate per §12.5 docs if the native wants a fresh fingerprint reading.
...
| §7.B tranche 1 | **HALTED at step 5 — steps 0/1/3/4 GREEN, 2 NOT_RUN, 5 FAILED (E-017); E-015 resolved-with-path; E-016 open (1082/1083/1084/1087 unapplied); native ruling needed** | see §7 above |
...
- **WP10 tranche 1:** RUN and HALTED at step 5 (E-017 — script defect, `text[]` literal; steps 0/1/3/4 GREEN, step 2 NOT_RUN). E-015's privilege precondition was resolved with the native's authorized path (temporary CREATE grant, since revoked and verified). Tranche 1 is **not green**.
- **WP10 tranche 2:** not started (four of six §7.C preconditions UNMET — see §7 table; would also stop at step 7 per E-012).
...
- **E-016:** step 4 applied only 1080/1081; **1082/1083/1084/1087 NOT applied to production** — native ruling needed (follow-up tranche vs deploy pipeline; tranche 2's step-6 writer may depend on 1087's columns).
- **E-017:** step 5 FAILED on a `clear_tables` `text[]` literal defect the rehearsal harness could not see (harness declared the column TEXT — rehearsal-fidelity gap). Corrected 1091 prepared, NOT re-run; tranche halted for native ruling; conjunct-(j)/1072 interplay flagged for merge time.

## 4. Migrations landed

1080/1081 (renumbered resonance target state + ledger coverage publication), 1082 (WP9 overlay stamps + `upstream_fingerprint` on mūrti, §12.9/§12.12), 1083 (L5 `contact_id`), 1084 (K-1/V-1 registry edges — the `ka_kshetra → ka_gochara` edge HELD OUT per `64bdc10da`), 1087 (§12.3: inclusivity, six-state F06 completeness CHECK, time_basis CHECK, tier_basis). **1085 was RETIRED and its files removed (E-010); 1086 was renamed to the L1 layer and is not this tranche's.** **1080/1081 are APPLIED to production** (step 4, 2026-09-24, `4670bebab`); 1082/1083/1084/1087 remain unapplied anywhere shared — E-016 awaits the native's ruling. 1091 (step-5 registry re-pin) exists corrected in `platform/migrations/` but is **NOT applied** — step 5 failed and halted (E-017).

## 5. Protected data / §3 hard list

No OCR/source-data files were modified (read-only consultation of already-cited passages only); no applied migration was renumbered; WriterBase conformance preserved (orchestrator conformance tests green). **Production writes in this run were exactly and only the authorized tranche-1 ones:** step 3's guard function + two row/truncate triggers and three `build_protected_assets` rows (step03); step 1's SELECT grants to `data_plane_builder`; step 4's migrations 1080/1081; the temporary CREATE grant to `amjis_app` (granted for the tranche, **revoked and verified at halt**); and the two password rotations + GitHub env-secret updates under E-015's resolution. No data rows in `kala_gochara_windows` were modified (generations v1=38287 / 3.0=1830 verified unchanged at every step boundary); century stays `is_active=false`; step 5 aborted in-transaction and wrote nothing. Everything else remains disposable-DB-only.

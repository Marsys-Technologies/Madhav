...
- **§7.B (tranche 1, steps 0–5): GREEN — completed 2026-09-24 across two sessions.** First session: run under E-015's authorized path, halted at step 5 on the 1091 `text[]` literal defect (E-017). Second session: the native ruled (verbatim, 2026-09-24T11:39:01Z: **"Approved on point number two. Go ahead to everything."**), step 5 resumed and passed. The frontmatter flags were read fresh from disk and are TRUE (both tranches authorized by `GOCHARA_NATIVE_RULINGS_2026-09-24_v1_0.md` §3). Run executed per brief §12.15 order — **step 3 first** — with the Cloud SQL proxy on 127.0.0.1:5433. E-015 privilege resolution: no Secret Manager/local credential existed for `data_plane_schema_owner`/`data_plane_migrator`/`postgres` and the `postgres` superuser is itself locked out of schema public; path used — rotated `data_plane_migrator` (GitHub env secret `DATA_PLANE_MIGRATOR_DATABASE_URL` updated to the pinned proxy format), `SET ROLE data_plane_schema_owner` → **temporary** `GRANT CREATE ON SCHEMA public TO amjis_app`; also rotated `postgres` and set the previously-unset `DATA_PLANE_OWNERSHIP_ADMIN_DATABASE_URL`. Per-step verdicts:
  - **Step 0 — GREEN** (`cacc72440`, cherry-pick `-x` of `eb00da67d`; migrations 1071/1072, no collision — origin/main head is 1070; route.protected-assets.test 2/2; `npm run guard:migration-numbers` PASS). Evidence: `evidence/step00_evidence.md`.
  - **Step 1 — GREEN** (`e8cbce231`; SELECT grants to `data_plane_builder` on `kala_gochara_windows` + `_archive_20260805`, verified as grantee: 40117 and 35620 rows). Evidence: `evidence/step01_evidence.md`.
  - **Step 2 — NOT_RUN** (`9a786de9e`; no 2026-08-23 dump available locally — recorded, not waived).
  - **Step 3 — GREEN** (`8a7f5e692`; guard function + both triggers live, build_protected_assets seeded ×3, century `is_active=false`, generations v1=38287/3.0=1830 unchanged; all four gate probes pass — DELETE/TRUNCATE/re-label refused loudly, synthetic '4.0' INSERT passes in a rolled-back tx). Evidence: `evidence/step03_evidence.md` (SUCCESS section appended after the earlier STOPPED record).
  - **Step 4 — GREEN** (`4670bebab` scoped 1080/1081; then, under the 2026-09-24T11:39Z native ruling resolving E-016, the full committed APPLY_SET ran green: **1082/1083/1084/1087 applied to production** — 1082 stamp columns ×5, 1083 L5 `contact_id` ×2, 1084 edges AS COMMITTED with the `ka_kshetra → ka_gochara` edge verified HELD OUT, 1087 §12.3 columns (41 on contacts); information_schema diff empty; post-checks v1=38287, 3.0=1830, `gochara_resonance_map` 1595 rows, century `is_active=false`; temporary grant re-used and re-revoked, verified `=f`). Evidence: `evidence/step04_evidence.md` (both sections). **E-016 RESOLVED.**
  - **Step 5 — FAILED then RESUMED GREEN (E-017 resolved).** First attempt: `platform/migrations/1091_wp10_ka_gochara_registry_repin.sql` (numbered per E-009 re-scan; MIG-1 guard PASS) aborted on `malformed array literal: "[kala_gochara_windows, ...]"` — production `asset_registry.clear_tables` is `text[]`, the script wrote `'[...]'`, and the rehearsal harness declared the column TEXT so rehearsal never caught it (**rehearsal-fidelity gap recorded**; harness fix remains an open reviewer item). Single-transaction abort; production verified unchanged. Under the native's 2026-09-24T11:39Z ruling the corrected 1091 (`'{...}'` ×2 + header note) was applied as `amjis_app` (`3fcf6a586`): clean apply (snapshot ×2 rows, 3 UPDATEs, DO gate probe, COMMIT); gates all green — count_sql relation = target_table; new count executes → 0 for both authority charts (cockpit-0-between-5-and-6 EXPECTED); Clear-proof rehearsal 16/16 on a fresh disposable DB; production `integrity_check_sql` executes → `integrity_passed = t`; conjunct-(j) state recorded (1072 NOT applied; target_table stays `kala_gochara_windows`; the merge-time interplay note stands). Temporary CREATE grant revoked at gate-green, verified `=f`. Evidence: `evidence/step05_evidence.md` (FAILED and RESUMED-GREEN sections).
  - **Incident disclosure (honesty):** early in the E-015 resolution an malformed `export PW_<name-with-dashes>=...` line caused bash to echo four secret values (`nirmana-campaign-control-db-password`, `nirmana-evidence-ingress-db-password`, `retrieval-census-ro-db-password`, `amjis-inquiry-db-password`) to stderr in the session transcript. No values were written to any committed file; consider rotating those four secrets.
- **§7.C (tranche 2, steps 6–10): ENTERED under the native's 2026-09-24T11:39Z ruling; HALTED at step 6 (E-018) — steps 6–10 NOT RUN.** The native ruled the merge-state preconditions satisfied by authorization (P-1 / WP9 §5.2 / N-19 live on this branch, PR #2731); tranche-1-green was achieved first (step 5 resumed). Final precondition table at entry:

  | Precondition (brief frontmatter + §7.C) | State at entry |
  |---|---|
  | `PRODUCTION_TRANCHE_2_AUTHORIZED` flag | TRUE (ruling §3) |
  | Tranche 1 evidence green | MET — step 5 resumed GREEN (`3fcf6a586`) |
  | P-1 merged to `main` | RULED SATISFIED by the native (branch, PR #2731) |
  | WP8 M-1 ratified OR fallback | MET — ratified with the value split (ruling §2); candidate-1 parameters `linear_no_box × 5.0°` |
  | WP9 §5.2 (M-8) merged | RULED SATISFIED by the native (branch) |
  | N-19 artifacts on `main` | RULED SATISFIED by the native (branch) |

  **Halt at step 6, two independent causes (E-018; evidence `evidence/step06_evidence.md`):**
  1. **§12.9 staleness gate RED on production** — gate verified to exist and its tests green (40/40 incl. the 13 previously-skipped, on fresh disposable DBs 55433/55434, torn down after); but every production overlay row on both authority charts predates 1082 and carries NULL `upstream_fingerprint` (135/135 + 72/72 on `1c826d5a…`; 132/132 + 71/71 on `482012f1…`; 0 mismatched). Prescribed remediation (overlay rebuild with this branch's writers) NOT performed inside a halted tranche — it rewrites live-served rows and belongs as its own reviewed change (E-018 item ii).
  2. **The step-6 episode enumeration driver does not exist** — step06 consumes `--episodes-json`/`--coverage-json` from "the kernel pipeline"; nothing on the branch enumerates a chart's full episode set (only tests and step06 call `ledger.write_contacts`; `find_episodes` serves from the ledger). Writing it is the '4.0' writer's core — new engineering, barred as improvisation, and §10 bars live-chart writes before the proof-matrix gates run for it.

  **E-012 remains in force:** the '4.0' windows projection writer does not exist, so step 7's `windows_present` gate is RED by design and step 8 refuses to flip — the authority flip was physically unreachable this run even with a green step 6. Steps 7, 8, 9, 10 NOT RUN. Production untouched by tranche 2: no '4.0' rows anywhere; generations v1=38287 / 3.0=1830; authorities both `'3.0'`.
- R-1..R-6 take effect only at a resonance rebuild sequenced at WP10 step 6 — which has not occurred; R-1..R-6 remain inert.
...
| §7.B tranche 1 | **GREEN** — steps 0/1/3/4/5 GREEN, 2 NOT_RUN (recorded, not waived); E-015/E-016/E-017 all resolved under the 2026-09-24T11:39Z native ruling | see §7 above |
...
- **WP10 tranche 1:** **GREEN** (steps 0/1/3/4/5; step 2 NOT_RUN — no dump locally). E-015/E-016/E-017 resolved under the native's 2026-09-24T11:39Z ruling ("Approved on point number two. Go ahead to everything."); temporary CREATE grants used and revoked with verification at each boundary.
- **WP10 tranche 2:** ENTERED under the ruling; **HALTED at step 6 (E-018)** — §12.9 gate RED on production (unfingerprinted overlay rows) and no episode-enumeration driver exists; E-012's missing windows projection writer independently bars the flip. Steps 6–10 NOT RUN.
...
- **E-015:** tranche 1 initially halted at step 3 — `amjis_app` lacked CREATE on schema `public`. **RESOLVED-WITH-PATH (2026-09-24)** under the native's "go ahead": rotated `data_plane_migrator`, updated env secret `DATA_PLANE_MIGRATOR_DATABASE_URL`, `SET ROLE data_plane_schema_owner` → temporary GRANT CREATE to `amjis_app`; `postgres` rotated and `DATA_PLANE_OWNERSHIP_ADMIN_DATABASE_URL` set. Grant revoked at the step-5 halt (verified `=f`); residual question whether it should be standing (default per Sept cutover design: no).
- **E-016:** **RESOLVED** (native ruling 2026-09-24T11:39Z) — 1082/1083/1084/1087 applied to production via the step04 machinery, full committed APPLY_SET; 1084's `ka_kshetra → ka_gochara` edge verified HELD OUT.
- **E-017:** **RESOLVED** (same ruling) — corrected 1091 applied as `amjis_app`, all step-5 gates green, grant revoked/verified. Open reviewer residue: the rehearsal harness's `clear_tables TEXT` declaration should become `text[]`; the conjunct-(j)/1072 merge-time note stands (1072 still unapplied).
- **E-018:** tranche 2 HALTED at step 6 — §12.9 gate RED on production (overlay rows unfingerprinted; rebuild prescribed but deferred as its own reviewed change); step-6 episode enumeration driver missing; E-012 windows writer missing. Native decisions requested: owners for the two writers; whether the overlay fingerprint rebuild runs now; confirmation of the M-1 candidate-1 parameters.

## 4. Migrations landed

1080/1081 (renumbered resonance target state + ledger coverage publication), 1082 (WP9 overlay stamps + `upstream_fingerprint` on mūrti, §12.9/§12.12), 1083 (L5 `contact_id`), 1084 (K-1/V-1 registry edges — the `ka_kshetra → ka_gochara` edge HELD OUT per `64bdc10da`), 1087 (§12.3: inclusivity, six-state F06 completeness CHECK, time_basis CHECK, tier_basis). **1085 was RETIRED and its files removed (E-010); 1086 was renamed to the L1 layer and is not this tranche's.** **1080/1081/1082/1083/1084/1087 are all APPLIED to production** (1080/1081 in tranche-1 step 4 `4670bebab`; the rest under the E-016 ruling, `5ee6280bd`); **1091 (step-5 registry re-pin) APPLIED** under the E-017 ruling (`3fcf6a586`).

## 5. Protected data / §3 hard list

No OCR/source-data files were modified (read-only consultation of already-cited passages only); no applied migration was renumbered; WriterBase conformance preserved (orchestrator conformance tests green). **Production writes in this campaign were exactly and only the authorized tranche ones:** step 3's guard function + two row/truncate triggers and three `build_protected_assets` rows; step 1's SELECT grants to `data_plane_builder`; step 4's migrations 1080/1081 and, under the E-016 ruling, 1082/1083/1084/1087; step 5's migration 1091 (registry re-pin, applied under the E-017 ruling after the corrected literals); the temporary CREATE grants to `amjis_app` (granted per step need, **revoked and verified `=f` at each boundary**); and the two password rotations + GitHub env-secret updates under E-015's resolution. No data rows in `kala_gochara_windows` were modified (generations v1=38287 / 3.0=1830 verified unchanged at every step boundary); century stays `is_active=false`; no `'4.0'` rows exist anywhere; tranche 2 wrote nothing. Everything else remains disposable-DB-only (both disposable containers torn down at close; `docker ps -a` confirms none remain).

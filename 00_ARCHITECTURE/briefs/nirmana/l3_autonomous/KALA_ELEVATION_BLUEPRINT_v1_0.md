...
read live**. That leaves **22 ± 1**. The L3 seal counted 12; ten were added after it. Any headline
printed over this denominator must say which of the 22 it counted.
...
- **Release:** DAG-safe micro-batches through the merge queue; a queued PR locks its branch (push
  elsewhere, don't force); migrations at **1075+** (1070–1074 are claimed by four unmerged branches), both directories checked, never edited once applied.
...
**Corrected at v4.0 — that baseline does not exist.** Earlier versions cited `KALA_BASELINE_v1_0.md`
as if frozen. No such file exists anywhere in the repository. The only baseline artifact is
`MADHAV_DATA_PLANE_L3_W0_BENCHMARK_BASELINE_v1_0.md`, which is a **performance** baseline (wall and
CPU timings) carrying zero references to L3-Q01–Q13, no proving cases and no ordinary period. So the
instrument this section's proof depends on has never been built, and the layer today can show it
was *changed*, not that it was *elevated*. Building it is the single highest-leverage action in
`KALA_BLUEPRINT_REVIEW_v1_0.md`.

**The empirical tier now has a ruled gate.** The E6 evaluation gate is set by the native directly at
...
which. Anything short reports `PROVISIONAL_INSUFFICIENT_N` with its actual n.

---

## §9 — Decisions, holds, risks

Rebuilt at v4.0 from the rulings on the three sheets and the delegated-decisions record. The v2.0
queue that stood here is preserved in the chronicle.

### 9.1 Ruled — no decision outstanding

| was | ruling | where |
|---|---|---|
| **D6** node frame + paired disposition + sibling conventions | **Mean** (M-1); disposition **(b)** — true knots retained, mean derived at read, disagreement declared per row (Kṣetra 7); epoch and ayanāṃśa-application declared on the row → **L0 items 6–7**. **No graha-dṛṣṭi from the nodes** across the instrument (M-1 · Saṅgam D-6 · Gochara **N-14**), a same-generation λ regeneration whose cost is accepted knowingly | three sheets; `KALA_DELEGATED_DECISIONS` D-E |
| **D7** M-3 producer shape and owner | **Gochara stream owns**; kernel path (N-5/N-7); E1/E3 gated on that landing | Saṅgam M-3; Gochara N-5/N-7 |
| **D8** corpus admission | **Moot — the premise was false.** Phaladīpikā was always in the corpus (564 chunks, the richest vedha source). The rules are *mis-cited*, not unsourced: re-citation at page grain → **L0 item 1**. The uniform `corpus_verifiable` stamp stands with its outcome corrected per `vedha_kind` | chronicle §11.21–§11.25; Kṣetra ruling 8 (rewritten twice) |
| century writer disposition | `is_active=false` at runbook step 3, reversible at WP10, nothing deleted; production write declared at step 5 | **N-6a**, F-30 |
| E6 gate numbers | **n = 20 / 50**, set by the native directly, superseding the delegated 35/100 | Saṅgam D-1 (superseded), `9f29d61ea` |
| reviewer for the three conflicted rulings | fresh-context agent + isolated second tool; both produced corrections re-verified at source | Kṣetra close, PR #2724 |
| the residual open items on both sheets | delegated in writing; author decisions kept typographically separate, each reversible in one line | Saṅgam §CLOSE D-1…D-8; Kṣetra rulings 1–10 |
| the four consolidated items put to the native | delegated to the strategic session; decided as D-A…D-F, one withdrawn (D-C contradicted N-6a), one superseded (D-F by the native) | `KALA_DELEGATED_DECISIONS_v1_0.md` v1.4 |
| the L0 repair | **AUTHORIZED 2026-09-24** — eight items, own worktree, reviewer from outside the three streams | `L0_REPAIR_EXECUTION_PROMPT_v1_0.md` |

### 9.2 Still the native's — two items, both one word
...
| **E2** | ~~Authorize Kṣetra stage 3~~ **RULED 2026-09-24 — AUTHORIZED.** Native, verbatim: *"I want to authorize Kshetra … I have just authorized stage three now."* Stage 3 (source elevation) only; the native explicitly does **not** open stage 4: *"I don't want to start the stage four."* | Kṣetra's stage-3 execution may begin; stage 4 stays closed |
...
### 9.4 Holds (clear on evidence, not on a ruling)

W1 on the production cutover (G2) · Kshetra populated replacement until W7 · Bhavishya until its
W6 packet · RI-01 on production-owner authority · **both DB endpoints refusing** (checklist B4) —
two staged SQL actions and every live measurement wait on it.
...
- An L2 rebuild during the campaign empties five tables (build-order rule + `cascade_check.sql`
  until W1) · Pūrṇa/L3 collision on `kala_views` (interface packets only) · every cost figure
  unmeasured (promise no schedule) · migration-number collisions 1070–1074.
...
retraction and correction of the 23–24 September campaign, in order — now live verbatim in
`KALA_ELEVATION_CHRONICLE_v1_0.md`. They were the truth while the body above went stale; at v4.0 the
body was rewritten from them and they became history. Cite the chronicle for **how** something was
established; cite this document for **what** is currently true. The failure-mode ledger is chronicle
§11.23; the corpus re-audit task is §11.26; the Sarvatobhadra recovery is §11.24–§11.25.

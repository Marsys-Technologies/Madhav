...
| **L1 Gaṇita** | `L1_DEPENDS_ON_AUDIT_v1_0.md` | 🟡 IN PROGRESS | 19/19 assets have at least one audit pass; 16/19 now have their declared edges fully re-verified in both directions (rounds 3-5, cycles 141-143 — `ga_yoga`, `ga_dashas`, `ga_sensitive_degree`, `ga_nakshatra`, `ga_condition`, `ga_structural`, plus `ga_vargas`/`ga_sensitive` needing no further check), 3 remain (`ga_sade_sati`, `ga_medical`, `ga_tajaka`); 16 confirmed findings (11 self-reported on #1744 + 5 new post-W2: `ga_yoga → ga_positions` hidden [#2180], `ga_panchanga`'s 2 false edges, `ga_yoga → ga_dashas` false + `ga_yoga → ga_vargas` hidden via a reused cross-writer helper, `ga_structural → ga_panchanga` false) · 8 assets confirmed CLEAN overall · `ga_dashas → ga_vargas` hidden edge has a MEASURED live correctness consequence (build `6479bb56`, concurrent start, MVCC stale read) · own §3 names what a fuller L3-grade sweep would still add |

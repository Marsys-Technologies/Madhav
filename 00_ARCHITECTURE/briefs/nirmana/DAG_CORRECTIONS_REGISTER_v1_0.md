...
| **L1 Gaṇita** | `L1_DEPENDS_ON_AUDIT_v1_0.md` | 🟡 IN PROGRESS | 19/19 assets now have at least one audit pass (round 2 grep-swept the 8 not covered by round 1; round 3 fully re-verified `ga_yoga`'s own declared edges in both directions, finding 2 more); 15 confirmed findings (11 self-reported on #1744 + 4 new post-W2: `ga_yoga → ga_positions` hidden [#2180], `ga_panchanga`'s 2 false edges, `ga_yoga → ga_dashas` false + `ga_yoga → ga_vargas` hidden via a reused cross-writer helper) · 8 assets confirmed CLEAN · `ga_dashas → ga_vargas` hidden edge has a MEASURED live correctness consequence (build `6479bb56`, concurrent start, MVCC stale read) · own §3 names what a fuller L3-grade sweep would still add |

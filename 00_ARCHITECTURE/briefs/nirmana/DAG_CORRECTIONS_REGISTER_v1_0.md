...
| **L1 Gaṇita** | `L1_DEPENDS_ON_AUDIT_v1_0.md` | 🟡 IN PROGRESS | 19/19 assets now have at least one audit pass (round 2 grep-swept the 8 not covered by round 1); 13 confirmed findings (11 self-reported on #1744 + `ga_yoga → ga_positions` hidden edge, Conductor-confirmed on #2180 + `ga_panchanga`'s 2 false edges, new cycle 140) · 8 assets confirmed CLEAN · `ga_dashas → ga_vargas` hidden edge has a MEASURED live correctness consequence (build `6479bb56`, concurrent start, MVCC stale read) · own §3 names what a fuller L3-grade sweep would still add |

#!/usr/bin/env bash
# nikasha-test Phase 1 extension — add tables referenced by count_sql/integrity_check_sql
# that no registry target_table names (the inspector reads them; fidelity requires them).
set -uo pipefail
...
spsql -d nikasha_sandbox -t -A -c "SELECT relname FROM pg_class JOIN pg_namespace n ON n.oid=relnamespace WHERE n.nspname='public' AND relkind IN ('r','v')" | sort -u > "$SB/_have.txt"
comm -23 "$SB/_refs.txt" "$SB/_have.txt" > "$SB/_missing.txt"
say "missing: $(wc -l < "$SB/_missing.txt" | tr -d ' ') tables"

TARGS=(); while IFS= read -r t; do TARGS+=(-t "$t"); done < "$SB/_missing.txt"
[ ${#TARGS[@]} -eq 0 ] && { say "nothing to add"; exit 0; }
$PGBIN/pg_dump -s --no-owner --no-privileges "${TARGS[@]}" > "$SB/schema_ext.sql" 2>>"$SB/build.log" || { say "pg_dump FAILED"; exit 1; }
spsql -d nikasha_sandbox -v ON_ERROR_STOP=0 -f "$SB/schema_ext.sql" > "$SB/schema_ext_load.log" 2>&1
say "ext schema errors: $(grep -c ERROR "$SB/schema_ext_load.log")"

: > "$SB/copy_report_ext.tsv"
while IFS= read -r t; do
  ...
  if [ "$k" = "v" ]; then
    ...
    spsql -d nikasha_sandbox -c "CREATE OR REPLACE VIEW \"$t\" AS $def" 2>/dev/null \
      && echo -e "$t\tview\tview\tview" >> "$SB/copy_report_ext.tsv" \
      || { say "VIEW FAIL $t"; echo -e "$t\tview\tview\tCOPY_FAIL" >> "$SB/copy_report_ext.tsv"; }
    continue
  fi
  ...
  psql -t -A -c "$q" > "$SB/_b.copy" 2>"$SB/_b.err" || { say "COPY-OUT FAIL $t: $(head -1 "$SB/_b.err")"; echo -e "$t\t$rule\t$pc\tCOPY_FAIL" >> "$SB/copy_report_ext.tsv"; continue; }
  spsql -d nikasha_sandbox -c "COPY \"$t\" ($cols) FROM STDIN" < "$SB/_b.copy" 2>"$SB/_b.err" || { say "COPY-IN FAIL $t: $(head -1 "$SB/_b.err")"; echo -e "$t\t$rule\t$pc\tCOPY_FAIL" >> "$SB/copy_report_ext.tsv"; continue; }
  sc=$(spsql -d nikasha_sandbox -t -A -c "SELECT count(*) FROM \"$t\"")
  echo -e "$t\t$rule\t$pc\t$sc" >> "$SB/copy_report_ext.tsv"
done < "$SB/_missing.txt"
rm -f "$SB/_b.copy" "$SB/_b.err" "$SB/_sql.txt"
say "extension done: $(awk -F'\t' '$4!="COPY_FAIL"' "$SB/copy_report_ext.tsv" | wc -l | tr -d ' ') ok, $(grep -c COPY_FAIL "$SB/copy_report_ext.tsv" || true) failed"

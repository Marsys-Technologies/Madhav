...
ROOT = Path(subprocess.run(["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True)
            .stdout.strip() or ".")
# T3 sandbox variant: ledger directory is overridable so --emit-gaps never touches production ledgers.
...
def idem_scan(asset_id: str, files: list[str], convention: str) -> tuple[str, list[str]]:
    """§N.3, over real SQL strings (docstrings excluded). T3 change: when the writer's own SQL
    shows no pattern, follow the writer's `from <sidecar package> import ...` delegation one hop
    and scan the seeder module — the property the gate certifies (a rebuild replaces its own
    rows) lives where the INSERTs live. Convention match is reported, not silently equated."""
    if not files:
        return NA, ["no writer file"]
    sqls = []
    for name in files:
    ...
    if convention == "upsert" and upsert:
        return PASS, ["ON CONFLICT present in the writer"]
    if convention != "upsert" and delete:
        return PASS, ["DELETE FROM present (delete-then-insert)"]
    if not (upsert or delete):
        seeder_hits = []
        for name in files:
            src = (WRITERS / name).read_text(encoding="utf-8", errors="replace")
            for mod in re.findall(r"from\s+((?:brahmagyan|pipeline)(?:\.[\w]+)+)\s+import\s+", src):
                p = SIDECAR / (mod.replace(".", "/") + ".py")
                if not p.exists():
                    continue
                sblob = "\n".join(_code_strings(_parse(p)))
                sup = bool(re.search(r"ON CONFLICT", sblob, re.I))
                sdel = bool(re.search(r"DELETE\s+FROM", sblob, re.I))
                if sup or sdel:
                    mech = "ON CONFLICT (upsert)" if sup else "DELETE-then-INSERT"
                    match = (convention == "upsert") == sup
                    seeder_hits.append(
                        f"idempotent in delegated seeder {p.name}: {mech}"
                        + (" (matches layer convention)" if match
                           else f" (layer convention is {convention} — mechanism differs, property holds)"))
        if seeder_hits:
            return PASS, seeder_hits
...
def measure(layer_key: str) -> dict:
    ...
    for aid, r in reg.items():
        ...
        # T3: a per-asset carriage detector registered at <CTRL>/detectors/<asset>_D<1|2|3>.py
...
def emit_gaps(census: dict) -> tuple[int, int, int, int]:
    """T3 closing variant. Append-only ledger with deterministic ids (`<asset>-<criterion>`).

    - check FAIL/PARTIAL/NO_DETECTOR, no row with this gap_id yet        -> append OPEN
    - check FAIL/PARTIAL/NO_DETECTOR, latest row OPEN                    -> skip (already open)
    - check FAIL/PARTIAL/NO_DETECTOR, latest row CLOSED                  -> append OPEN (RE-OPENED;
      the defect regressed — a loop that only counts up is not a loop)
    - check PASS/N/A now, latest row OPEN                                -> append CLOSED
      (closure BY MEASUREMENT: the same detector now passes; the row quotes the new measured value)
    - check PASS/N/A, latest row CLOSED or no row                        -> nothing to do
    Rows whose gap_id is not a deterministic `<asset>-<criterion>` id (hand-written rows) are
    never touched: they carry no detector binding, so no measurement can close them.
    """
    ...
    with path.open("a", encoding="utf-8") as f:
        for a in census["assets"]:
            for crit, res in a["measurements"].items():
                ...
                failing = res["v"] in (FAIL, PARTIAL, NO_DET)
                ...
                if failing:
                    ...
                    if prior is not None and prior.get("state", "OPEN").upper() == "OPEN":
                        f.write(json.dumps(dict(
                            ...
                            change="", detector=f"asset_census_closing.py --layer {census['layer']} ({crit})",
                            owner="asset_census", gate="this asset's certification",
                            state="CLOSED", ts=ts), ensure_ascii=False) + "\n")
                        closed += 1

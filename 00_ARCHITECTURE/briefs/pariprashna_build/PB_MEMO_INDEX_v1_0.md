...
own opening note for the full account (also relevant to register item `INF-3`, since **CLOSED as
EXPLAINED by DVA Ruling 56** — which also ruled that the rename itself is *not* standing practice:
a genuine governance deliverable is handed back to the Conductor to write, not renamed around the
filter).
...
| 3 | `PARK_PB-3_L-5_MIMAMSA_CALIBRATION_WRITE` | `00_ARCHITECTURE/briefs/pariprashna_build/PARK_PB-3_L-5_MIMAMSA_CALIBRATION_WRITE.md` | **PARKED-HONEST — design questions now RULED, implementation not yet built** (corrected 2026-07-31: this row previously read "OPEN — parked, awaiting a Pratinidhi MEMO"; DVA Ruling 55 decided the memo's §5 item 1 and Ruling 79 the remainder — see `PB_CAMPAIGN_CLOSE_v1_0.md` §4) | Lane L-5's park-with-costed-spec: persisting a conversational calibration row into `mimamsa_calibration` needs a schema change (Option A: extend the shared `mi_pramana` table with a discriminator column; Option B, recommended: a new `mimamsa_conversational_calibration` table) that L-5 cannot self-authorize. The Brier computation and the ledger-side outcome write were built and round-trip for real; only the final persistence sink is parked. |
...
| 6 | `BRIEF_PB-3` | `00_ARCHITECTURE/briefs/pariprashna_build/BRIEF_PB-3.md` | **RECOVERED AND ON `origin/main`** — corrected 2026-07-31 (DVA Ruling 81; this row previously read "CONFIRMED ABSENT — never committed to git anywhere," which was false). `git log --all --diff-filter=A -- '*BRIEF_PB-3.md'` shows the file added by lane `A1-PRESERVE` at commit `b74d953b` (2026-07-30 02:15 IST) and merged to `origin/main` via PR #896 (`0d919718`) — a real 260-line wave brief (`artifact: BRIEF_PB-3`, `status: FROZEN — opens when PB-2 closes green (or ship-degraded per Pratinidhi MEMO)`, `blocks: PB-4`). It is a distinct file from the separately-recovered partial draft at `ledgers/RECOVERED_BRIEF_PB-3_PRIOR_DRAFT_fuse_hidden0000000500000001.md`. Register item WT-2 and `REPORT_PB-3.md` §G item 1 / its "Corrections" section still describe the gap as open — both are now stale on this point and should be updated by whichever session next owns them (out of this lane's declared scope). | The wave brief PB-3 executed against — recovered, not lost. |
...
- `CAMPAIGN_PB_MASTER_BRIEF_v1_0.md` (`00_ARCHITECTURE/briefs/pariprashna_build/CAMPAIGN_PB_MASTER_BRIEF_v1_0.md`,
  recovered by `A1-PRESERVE` at `b74d953b` and **now on `origin/main`** via merged PR #896 —
  corrected 2026-07-31; this line previously read "not yet on `origin/main` as of this writing")
  — the multi-wave campaign brief that establishes the Pratinidhi delegation `MEMO_PB-3_0`
  and this run's own PB-4 gate both operate under.
- `BRIEF_PB-4.md` (`00_ARCHITECTURE/briefs/pariprashna_build/BRIEF_PB-4.md`, same recovery, **also
  now on `origin/main`** via merged PR #896) — PB-4 PŪRṆATĀ's wave brief, `status: FROZEN — opens
  when PB-3 closes green (or ship-degraded per Pratinidhi MEMO)`. PB-3 closed SHIP-DEGRADED (item 5
  above), satisfying the brief's own opening condition; PB-4's execution status is tracked in
  `PB_CAMPAIGN_CLOSE_v1_0.md` §5, not here.

...
status: CLOSED
closed: >
  Campaign arc closed by its own conductor at R47; the file's absolute final line is
  the terminal marker `RUN-TERMINAL: ARC-COMPLETE`. This frontmatter field was added
  when the sealed body was landed on main (the seal had previously existed only on
  branch sampurti/integration, while main's copy stayed frozen at `status: LIVE`
  five days stale). Frontmatter now agrees with the body; no new claim is asserted here.

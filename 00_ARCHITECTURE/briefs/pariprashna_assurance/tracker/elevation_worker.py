...
def builtin_probes(
    source_repo: Path,
    accepted_identity_url: str,
    *,
    run_command: Callable[[list[str]], str] = _run_command,
    fetch_json: Callable[[str], dict[str, Any]] = _fetch_loopback_json,
    tests_dir: Path | None = None,
    edir_path: Path | None = None,
) -> dict[str, Callable[[str | None], dict[str, Any]]]:
    """Concrete read-only probes for all six governed adapters.

    ``codex_tasks``, ``tests_evidence`` and ``edir`` are local, deterministic reads
    (process scan / test run / register file) -- never a fabricated placeholder.
    """
    repo = str(source_repo)
    tests_path = tests_dir if tests_dir is not None else source_repo / "tests" / "pariprashna_assurance_tracker"
    register_path = edir_path if edir_path is not None else source_repo / "00_ARCHITECTURE/briefs/pariprashna_assurance/EDIR_V3_REGISTER_v1_0.md"
    ...
    def codex_tasks(_cursor: str | None) -> dict[str, Any]:
        scan = subprocess.run(["pgrep", "-fl", "codex"], check=False, capture_output=True, text=True, timeout=10)
        if scan.returncode not in (0, 1):
            raise ValueError(scan.stderr.strip() or f"process scan exited {scan.returncode}")
        processes = [line for line in scan.stdout.splitlines() if line.strip()]
        payload = {"provenance": "local process scan (pgrep -fl codex)", "state": "ACTIVE" if processes else "IDLE", "processes": processes}
        return {"cursor": _digest(payload), "payload": payload}
    ...
    def edir(_cursor: str | None) -> dict[str, Any]:
        if register_path.is_file():
            digest_value = hashlib.sha256(register_path.read_bytes()).hexdigest()
            payload = {"provenance": "EDIR_V3_REGISTER file probe", "state": "OPEN", "sha256": digest_value, "size_bytes": register_path.stat().st_size}
        else:
            payload = {"provenance": "EDIR_V3_REGISTER file probe", "state": "NOT_YET_OPENED"}
        return {"cursor": _digest(payload), "payload": payload}

...
def main() -> None:
    p = argparse.ArgumentParser(); p.add_argument("--runtime", required=True); p.add_argument("--p0b-only", action="store_true"); p.add_argument("--p1-enabled", action="store_true"); p.add_argument("--p2-enabled", action="store_true")
    sub = p.add_subparsers(dest="command", required=True)
    sub.add_parser("provision-credentials")
    sub.add_parser("provision-p1-credentials")
    sub.add_parser("provision-p2-credentials")
    event = sub.add_parser("emit"); event.add_argument("--actor", required=True); event.add_argument("--token", required=True); event.add_argument("--key", required=True); event.add_argument("--type", required=True); event.add_argument("--payload", required=True); event.add_argument("--evidence", default="[]"); event.add_argument("--stream"); event.add_argument("--expected-seq", type=int); event.add_argument("--writer-instance-id", help="required for --type scenario_executed; a stable id for this writer process, distinct from any other concurrently-running session")
    ...
    args = p.parse_args(); store = EventStore(args.runtime, p0b_only=args.p0b_only, p1_enabled=args.p1_enabled, p2_enabled=args.p2_enabled)
    try:
        if args.command == "provision-credentials": result = store.provision_local_credentials()
        ...
        elif args.command == "provision-p2-credentials": result = store.provision_p2_credentials()
            ...
            request = {"actor_id": args.actor, "idempotency_key": args.key, "event_type": args.type, "payload": json.loads(args.payload), "evidence": json.loads(args.evidence), "stream_id": args.stream, "expected_stream_seq": args.expected_seq}
            if args.writer_instance_id: request["writer_instance_id"] = args.writer_instance_id

...
| 902 | NIRMANA | 902_bg_sarvatobhadra_grid_freshness_sentinel.sql (asset_freshness sentinel row for bg_sarvatobhadra_grid — empty-by-ruling ADJUDICATION-11 — completes migration 553's throughput sentinel symmetrically) | CLAIMED — Conductor overnight ruling on #2393, unblocks `ka_vedha_gochara` DEP-ASSERT; allocated via `reserve_migration_number.py` (disk ceiling 901 across both migration dirs, re-verified live pre-claim) on branch `codex/nirmana-conductor-2393-vedha-freshness-sentinel` |
| 903+ | — | next free; claim here before use — **the "next free" number drifts fast under concurrent campaigns; re-verify live via `git ls-tree -r --name-only origin/main -- platform/supabase/migrations platform/migrations` before trusting this table** | — |

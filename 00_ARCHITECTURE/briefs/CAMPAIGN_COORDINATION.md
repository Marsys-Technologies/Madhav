...
|---|---|---|---|---|---|
| MADHAV-JATAKA-INFORMATION-NAV-DEPLOY-20260927 | JĀTAKA PORTAL UI / Codex | **Native-authorized release of the already-tested Jātaka Information navigation change at local commit `220835239`, bound to protected `main` `ee9b6c7e8a4333cc7ac08470e223ed2f38de0817` at lease-open.** Owns only: push `codex/jataka-information-nav`; open and review the focused PR; satisfy protected CI/merge-queue controls; merge through the normal protected path; observe the exact-main automatic web deployment; verify the deployed revision and the authenticated Jātaka Information → Atlas flow; and release with evidence. The diff is UI/routing/tests only: no migration file, database/data mutation, rebuild, feature-flag, credential, secret, IAM, infrastructure, MCP/sidecar/pipeline change, or foreign-campaign/worktree mutation. The standard deploy workflow may perform its routine migration replay, but this release introduces no migration. Roll back by reverting the focused PR and redeploying if required checks fail after merge, the web revision is unhealthy, authorization weakens, or the live Information/Atlas flow regresses. | 2026-09-27 01:03 IST | 2026-09-27 03:33 IST | **ACTIVE — PROTECTED PR/MERGE, WEB DEPLOY, LIVE VERIFICATION** |
| MADHAV-JATAKA-INFORMATION-NAV-20260927 | JĀTAKA PORTAL UI / Codex | **Code-only coordination record for the native-requested Jātaka navigation adjustment.** Owns one isolated worktree and only the minimum portal source/tests needed to add a bottom Information entry to the Jātaka left rail, expose Atlas as the first item beneath it, and remove Atlas from the Cockpit top navigation. No deployment, production build/rebuild, migration, database/data mutation, credential/infrastructure change, merge, or foreign-campaign/worktree mutation. This record satisfies the state-changing session handshake without claiming or exercising production authority. | 2026-09-27 00:48 IST | 2026-09-27 04:48 IST | **RELEASED — LOCAL SOURCE COMPLETE** (2026-09-27 01:00 IST; branch `codex/jataka-information-nav`, commit `220835239`; Information is bottom-aligned above the user menu on desktop/mobile, Atlas is its first admin-only item at `/information/atlas`, `/cockpit/atlas` redirects, and the Cockpit header no longer lists Atlas. ESLint 0 errors/590 inherited warnings; TypeScript PASS; full unit 1,142 files/12,423 tests PASS; no deploy, production build/rebuild, merge, migration, database/data, credential or infrastructure action.) |
| MADHAV-L3-KALA-W1-DISPATCH-20260920 | L3 KĀLA / Claude Code | **Narrow W1 L1-generation dispatch lease only, bound to protected `main` `cfbab01e6d92bed7ff45c83895b6c9f49e2208b8` (no L3 file changes on that tip; PR #2695 carrying the digest-path + ayanamsha-default fixes is separate and unmerged at lease-open).** Re-checked the Pūrṇa lease table immediately before claiming: the topmost row (`MADHAV-PURNA-DELIVERY-DATA-PLANE-ALLOWLIST-SUCCESSOR-20260920`) is RELEASED at 01:50 IST, ~2h25m before this claim, and no newer Pūrṇa row exists above it. This lease owns only: one production DB backup (operation-specific, taken before any write); dispatch of `platform/scripts/dispatch_frozen_rebuild.py` (fixed local copy, digest verified against current-main `nirmana-writer-digests.json`) for exactly two assets — `ga_positions` then `ga_dashas` — against canonical chart `482012f1-710e-4a25-994a-93821f5871aa`, using the `data-plane-builder-db-url` secret (read-only-verified `data_plane_builder` session_user, INSERT confirmed on `build_runs`/`build_run_assets`); observation of `build_runs`/`build_run_assets`/`l1_data_plane_generation_heads` state to terminal; and release with evidence. No merge of any Pūrṇa PR, no migration application, no credential/infrastructure change, no L2+/foreign-campaign mutation, no rollback-function invocation (documented non-viable for a first generation). Stop on any dispatch failure after one retry (two identical failures halt the sequence per the governing packet). | 2026-09-20 04:16 IST | 2026-09-20 05:16 IST | **RELEASED — dispatch attempted for `ga_positions`, dry run + digest match confirmed, real dispatch failed structurally: the job's own bound identity (`data_plane_builder`) has zero role memberships and no grant on `asset_registry`, so `runner.py`'s `_verify_registry_still_matches_manifest` raises `psycopg.errors.InsufficientPrivilege` before any write. Confirmed by direct privilege introspection (`has_table_privilege`, `pg_auth_members`), not inferred. `build_runs` row self-terminalized via an orphan watchdog (`state='failed'`) — no hand-edit needed. `l1_data_plane_generation_heads` remains empty; no partial write occurred (0 rows, atomicity intact). Did not attempt `ga_dashas` or a second `ga_positions` try — the root cause is deterministic and structural, not transient, so a retry would not add information. See §6 LOG for full detail and the required grant.** |
| MADHAV-PURNA-DELIVERY-DATA-PLANE-ALLOWLIST-SUCCESSOR-20260920 | PŪRṆA ANVEṢAṆA / Codex | **Final data-plane allowlist and service-delivery successor lease bound only to protected `main` `66b962f2994f0a7500c285025740cd5861534d8c` (PR #2690).** The predecessor exact-main automatic run `35461533371` failed closed before migration or service mutation because protected `main` `449dde8305db6d532ddfa939748641ad61e9a7a2` still rejected the required `purna_inquiry_owner` public-schema USAGE grant as global ACL drift. PR #2690 conditionally grants that owner USAGE during future data-plane preflight when the role exists and conditionally requires exactly that USAGE in the canonical ACL allowlist, while preserving no CREATE and clean installations where the role does not yet exist; focused tests passed 67/67, the full local platform suite passed 12,383/12,383, and all PR plus merge-group checks passed. The live database is already owner/serving USAGE-only with no CREATE; no further credential or privilege mutation is authorized unless a mandatory exact-SHA attestation proves the expected posture absent. This lease owns only: exact-main automatic-deploy observation; mandatory data-plane/Nirmāṇa/Pūrṇa/strict-isolation and bootstrap-absence re-attestation through the protected workflow; one exact force-all protected dispatch if needed; routine migration replay; web/MCP/sidecar/pipeline candidate build, canary and promotion; actual ready-revision/traffic/`NIRMANA_DEPLOYED_SHA`, pipeline image, migrations 1040/1041, ownership/isolation and bounded regression proof; migration-barrier decision; campaign events; and release. Preserve all applied migrations, protected ownership/isolation, absence of one-shot Pūrṇa authority, L3/Kāla source/data, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No privileged bootstrap recreation, ruleset bypass, broad IAM, destructive rebuild, feature work, secret disclosure or empirical/product-acceptance claim. Stop on tip drift, failed exact-main CI/deploy/attestation/smoke/promotion, migration ambiguity, rollback ambiguity or unexpected impact. | 2026-09-20 00:31 IST | 2026-09-20 04:30 IST | **RELEASED — EXACT PROTECTED TECHNICAL DELIVERY VERIFIED AT `66b962f2994f0a7500c285025740cd5861534d8c`; DOCS PR #2691 PROTECTED-MERGED AT `cdc701afad40c1ffe8d83468df5127b26e370fba`; PRODUCT ACCEPTANCE REMAINS OPEN** (2026-09-20 01:50 IST) |
| MADHAV-PURNA-DELIVERY-OWNER-USAGE-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Protected-owner runtime-schema successor lease bound only to protected `main` `449dde8305db6d532ddfa939748641ad61e9a7a2` (PR #2689).** Exact-main force-all run `35458376340` under the predecessor lease passed the sidecar, DB inspection, routine migration and strict ownership gates, then the web zero-traffic candidate again failed the non-vacuous Pūrṇa database canary before promotion, now with PostgreSQL `42501`; MCP remained no-traffic and the run was force-cancelled. Direct serving-login reproduction proved the caller now had public-schema USAGE, but both `SECURITY DEFINER` entry points failed because their normalized owner `purna_inquiry_owner` lacked public-schema USAGE. PR #2689 preserves durable owner USAGE while revoking CREATE, grants/attests USAGE-only runtime capability for both owner and serving role through the existing `data_plane_migrator -> data_plane_schema_owner` edge, and makes marked state plus the canary reject owner-visibility regression; local tests passed 12,383/12,383 and all PR/merge-group checks passed. This lease authorized one no-disclosure migrator credential rotation/protected-secret refresh and the exact runtime-USAGE repair. Both completed, but the mandatory global data-plane re-attestation then failed closed because its public-schema ACL allowlist had not yet admitted `purna_inquiry_owner` USAGE. No deploy was dispatched under this lease. The database remains owner/serving USAGE-only with no CREATE; delivery is blocked pending canonical allowlist repair. | 2026-09-19 23:54 IST | 2026-09-20 04:00 IST | **RELEASED — GLOBAL DATA-PLANE SCHEMA ACL ALLOWLIST REJECTED REQUIRED PŪRṆA OWNER USAGE; NO DEPLOY DISPATCH** (2026-09-19 23:57 IST) |
| MADHAV-PURNA-DELIVERY-SERVING-USAGE-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Serving-schema successor lease bound only to protected `main` `ee1b9f22390a086ebb46b38ca6e48b912defbf56` (PR #2688).** Exact force-all run `35458376340` passed sidecar, migration and ownership gates, then the web zero-traffic candidate failed the Pūrṇa canary with PostgreSQL `42501` before promotion because `purna_inquiry_owner` lacked durable public-schema USAGE. MCP remained no-traffic; the run was force-cancelled. No web or MCP promotion occurred. The lease terminated at its stop-on-failure boundary and is superseded only by the exact-`449dde8305db6d532ddfa939748641ad61e9a7a2` owner-USAGE successor above. | 2026-09-19 22:42 IST | 2026-09-20 01:30 IST | **RELEASED — OWNER SCHEMA USAGE MISSING; WEB CANARY FAILED CLOSED WITH PG_42501; NO WEB/MCP PROMOTION** (2026-09-19 23:54 IST) |
| MADHAV-PURNA-DELIVERY-ORCHESTRATION-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Final orchestration successor lease bound only to protected `main` `dd82ef952cd4c8e8f739c6dd68f6ee73b9b5bdc5` (PR #2687).** The predecessor exact-tip force-all run `35446973458` proved strict migration state, marked data-plane/Nirmāṇa/Pūrṇa ownership, successful routine replay, and successful exact-sidecar candidate probes/promotion, but failed its earned-outcome gate because GitHub propagated the intentionally skipped protected-bootstrap ancestor through the successful `migrate` job and skipped web/MCP/pipeline. PR #2687 adds `always()` only to evaluate those downstream jobs, while explicitly requiring successful `changes` and `migrate` results; focused contract tests passed 57/57, PR checks passed, and merge-group CI/TAP/Ekavākyatā passed. The predecessor d1ae-bound lease self-terminated on protected-tip drift. This lease owns only: observe exact-main CI/automatic deploy; one exact force-all dispatch; web/MCP/sidecar/pipeline candidate build, smoke and promotion; actual ready-revision/traffic/`NIRMANA_DEPLOYED_SHA`, pipeline-image, migration 1040/1041, ownership/isolation, bootstrap-absence and bounded regression proof; migration-barrier decision; campaign events; and lease release. Preserve all applied migrations, strict protected ownership/isolation, absence of one-shot Pūrṇa authority, L3/Kāla source/data, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No privileged bootstrap recreation, ruleset bypass, broad IAM, destructive rebuild, feature work, secret disclosure or empirical/product-acceptance claim. Stop on tip drift, exact-main CI/deploy failure, failed smoke/promotion, migration ambiguity, rollback ambiguity or unexpected impact. | 2026-09-19 20:09 IST | 2026-09-19 23:30 IST | **ACTIVE — EXACT-MAIN FORCE-ALL SERVICE PROOF, BARRIER DECISION AND RELEASE** |
| MADHAV-PURNA-DELIVERY-BACKLOG-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Final successor exclusive lease bound only to protected `main` `d1ae1653408431c53b88644090a31472628b902f` (PR #2686).** The prior exact-tip run `35444014454` passed cleaned data-plane/Nirmāṇa/Pūrṇa inspection, strict runtime isolation and routine semantic re-attestation, with the protected bootstrap skipped and sidecar candidate probes/promotion green. It then failed closed before web/MCP/pipeline deployment because new, unapplied migration 1041 used generic `SELECT 1`, whose comment-stripped SQL identity collided with already-applied no-op migration 343. Live tracking confirmed only 1033–1040 present. PR #2686 changes only unapplied 1041 to unique no-op `SELECT 1041`; focused migration/contract tests passed 96/96 and protected/merge-group checks passed. This lease owns exact-main CI and automatic-run observation; routine backlog application from 1041 onward; one exact force-all protected dispatch if required; web/MCP/sidecar/pipeline delivery; actual ready-revision/deployed-SHA and migration/ownership/regression proof; migration-barrier decision, campaign events and lease release. Preserve all applied migrations through 1040, protected ownership/isolation, absence of one-shot Pūrṇa authority, L3/Kāla source/data, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No privileged bootstrap recreation, ruleset bypass, broad IAM, destructive rebuild, feature work, secret disclosure or empirical/product-acceptance claim. Stop on tip drift, exact-main CI/deploy failure, migration/hash/identity ambiguity, rollback ambiguity or unexpected impact. | 2026-09-19 18:57 IST | 2026-09-19 22:30 IST | **ACTIVE — EXACT-IDENTITY BACKLOG DELIVERY, FORCE-ALL SERVICE PROOF, BARRIER DECISION AND RELEASE** |
| MADHAV-PURNA-DELIVERY-ACL-CLEANUP-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Fresh successor exclusive lease bound only to protected `main` `920ae33502f3265dc246a4b3a90bae45adae2429` (PR #2685).** The predecessor lease self-terminated on protected-tip drift after exact dispatch `35441370964` successfully applied migration 1040, revoked CREATE, completed mandatory Pūrṇa postflight and removed the two one-shot secrets plus Cloud SQL bootstrap user. That run then failed closed before routine migrations or web/MCP/pipeline deployment because `purna_inquiry_owner` retained the temporary schema USAGE grant, violating the strict public-schema ACL allowlist; Pūrṇa is live `marked`, and the sidecar alone promoted exact `32e5a9f0ea397a87c359faf09cac264f6bc84453`. PR #2685 changes the protected schema bridge to revoke and attest both USAGE and CREATE. This lease owns only: run that exact merged revoke implementation once through the existing deployment-only `data_plane_migrator` route; re-attest strict data-plane, marked Nirmāṇa and marked Pūrṇa state; observe exact-main CI; dispatch the exact protected workflow without one-shot Pūrṇa authority; apply the routine backlog; deploy source-implied web/MCP/sidecar/pipeline; verify actual ready revisions and deployed SHA, bounded regression smokes, migration state and cleanup; write the migration-barrier decision and release record. Preserve migration 1040 and all earlier migration rows, data-plane ownership, L3/Kāla source/data, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No new bootstrap credential, ruleset bypass, broad IAM, destructive rebuild, secret disclosure, feature work or empirical/product-acceptance claim. Stop on tip drift, cleanup or semantic attestation failure, failed exact-main CI/deploy, rollback ambiguity or unexpected impact. | 2026-09-19 18:03 IST | 2026-09-19 21:30 IST | **ACTIVE — EXACT-MERGED ACL CLEANUP, ROUTINE BACKLOG DELIVERY, SERVICE PROOF AND BARRIER DECISION** |
| MADHAV-PURNA-DELIVERY-SCHEMA-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Fresh successor exclusive lease bound only to protected `main` `32e5a9f0ea397a87c359faf09cac264f6bc84453` (PR #2684).** The prior exact-SHA attempt `35438308690` proved normalized `armed` bootstrap and Pūrṇa preflight, then failed transactionally before migration 1040 at its temporary schema-CREATE guard because `public` is protected by `data_plane_schema_owner` while `amjis_app` has USAGE only. Mandatory postflight succeeded, 1040 remained absent, web/MCP/pipeline/routine migration skipped, only sidecar promoted exact `49eb14a6e6e9a67ab847c069ff874160b28c49e2`, and both one-shot secrets plus the bootstrap user were deleted. PR #2684 adds a bounded deployment-only `data_plane_migrator` schema-owner bridge: grant Pūrṇa USAGE/CREATE only for the protected migration set, always revoke CREATE before existing bootstrap cleanup; migrations 1033–1040 remain unchanged. This lease owns exact-main automatic can-fail observation; a fresh operation-specific backup; fresh exact-topology rearm and `armed` proof; ephemeral protected secrets; exact protected dispatch; protected 1040 plus routine backlog; source-implied services; actual revision/SHA, migration/ownership and bounded regression proof; mandatory cleanup; migration-barrier decision and release record. Preserve marked/strict data-plane state, marked Nirmāṇa state, all L3/Kāla source/data, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No bypass, broad IAM, feature work, destructive rebuild, secret disclosure or empirical/product-acceptance claim. Stop on tip drift, failed exact-main CI/deploy/preflight, backup or topology failure, rollback ambiguity or unexpected impact. | 2026-09-19 17:02 IST | 2026-09-19 21:00 IST | **ACTIVE — EXACT-SHA SCHEMA-BRIDGED PŪRṆA 1040, BACKLOG DELIVERY, CLEANUP AND BARRIER DECISION** |
| MADHAV-PURNA-DELIVERY-REPAIR-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Fresh successor exclusive lease bound only to protected `main` `49eb14a6e6e9a67ab847c069ff874160b28c49e2` (PR #2683).** The predecessor exact-SHA lease self-terminated on protected-tip drift after its first dispatch `35435726103`: migration 1040 did not run; mandatory interrupted-state Pūrṇa cleanup succeeded; web/MCP/pipeline/routine migration skipped; earned-outcome failed; only sidecar independently promoted exact `d4655c9f3c51db019988fe686254aa9d220d7901`; both one-shot secrets and the disabled bootstrap user were deleted. This lease owns only: exact-main automatic can-fail observation; a new operation-specific backup; fresh normalized one-shot `purna_inquiry_bootstrap` with sole `cloudsqlsuperuser` edge and explicit `armed` proof; the two ephemeral protected-environment secrets; exact protected manual dispatch; protected migration 1040 plus routine backlog; source-implied web/MCP/sidecar/pipeline delivery; actual ready-revision, deployed-SHA, migration/ownership and bounded regression proof; mandatory secret/user cleanup; migration-barrier decision and release record. Preserve marked/strict data-plane state, marked Nirmāṇa state, migrations 1033–1039, L3/Kāla source and data, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No ruleset bypass, broad IAM, feature work, destructive rebuild, secret disclosure or empirical/product-acceptance claim. Stop on protected-tip drift, failed exact-main CI/deploy/preflight, backup failure, role-topology mismatch, rollback ambiguity or unexpected production impact. All unrelated older ACTIVE rows are past expiry and not revived. | 2026-09-19 15:57 IST | 2026-09-19 20:15 IST | **ACTIVE — EXACT-SHA PŪRṆA 1040, BACKLOG DELIVERY, VERIFIED CLEANUP AND BARRIER DECISION** |
| MADHAV-PURNA-DELIVERY-REPAIR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Fresh operation-specific exclusive lease bound to protected `main` `d4655c9f3c51db019988fe686254aa9d220d7901`.** Source repair PR #2682 merged through the protected queue; exact protected-main CI passed; automatic deploy `35435197677` correctly failed its earned-outcome detector because migration, web, MCP and pipeline mutations skipped while Pūrṇa state remained `rearm_required`. Owns only: capture current rollback revisions plus one fresh production DB backup; rearm the absent `purna_inquiry_bootstrap` Cloud SQL user with a fresh credential and sole `cloudsqlsuperuser` edge; install the two one-shot protected-environment secrets without revealing material; dispatch the exact protected workflow with existing gates; apply protected migration 1040 and mandatory postflight; apply the already-reviewed routine backlog; deploy web/MCP/sidecar/pipeline as source-implied; verify actual ready revisions, `NIRMANA_DEPLOYED_SHA`, migration/ownership state and bounded regression smokes; then delete the one-shot secrets/user and record release evidence. Preserve data-plane marked/strict state, Nirmāṇa marked state, all L3/Kāla source and data, migrations 1033–1039, protected retired `ka_gochara_sweep`, WriterBase, and L4/L5 holds. No ruleset bypass, broad IAM, destructive rebuild, feature expansion, secret disclosure, or empirical/product-completion claim. Stop on protected-tip drift, backup failure, topology mismatch, any failed gate, rollback ambiguity, or unexpected production impact. All older ACTIVE rows are past their stated expiry and are not revived. | 2026-09-19 15:13 IST | 2026-09-19 19:30 IST | **ACTIVE — BACKUP, ONE-SHOT PŪRṆA 1040 LIFECYCLE, BACKLOG DEPLOY AND VERIFIED CLEANUP** |
| MADHAV-DATA-PLANE-L3-DP047-ROUTINE-SCHEMA-BOUNDARY-20260918 | Execution — Data Plane / Codex | **Terminal protected automatic-release record for exact `main` `a5c7dde5c0f2dc7a53377c9609473ff9ab86671f` (PR #2670).** Main CI `35368302580` and all source checks passed. Exact deployment `35369415078` passed source, production-route, protected ownership, runtime-isolation, Nirmāṇa, and Pūrṇa preflight gates, then failed at the ordered predecessor `platform/migrations/1040_planner_inquiry_successor_lifecycle.sql`: its guarded check requires freshly provisioned direct `purna_inquiry_owner` membership with admin option. That Pūrṇa temporary edge is intentionally closed by migration 1039, and the current marked-state workflow does not reopen the one-shot Pūrṇa preflight. The L3 tracker repair itself was reached but migration 1041 did not run; no L3 data build, service deployment, consumer proof, terminal asset acceptance, L4/L5 work, empirical claim, or change to retired `ka_gochara_sweep` occurred. | 2026-09-18 21:58 IST | 2026-09-18 22:18 IST | **RELEASED — PŪRṆA 1040 PREDECESSOR OWNERSHIP PRECONDITION; DP047 TERMINAL BEFORE 1041 OR SERVICE DEPLOYMENT** |
| MADHAV-DATA-PLANE-L3-DP046-PGCRYPTO-FRESH-CUTOVER-20260918 | Execution — Data Plane / Codex | **Terminal protected-attempt record after source repair #2656 merged.** Bound only to protected `main` `558a61d916a920cbcb0456dfb0fddac11efdc9e4`; exact push CI `35333061975` and ordinary protected deployment preflight `35334140706` passed. Fresh backup `1789727194788`, isolated restore `ceaf6125-917c-4ec8-ba8b-6a160000002f` to `amjis-ri02-validation-c720f1832`, and Cloud Audit request/completion evidence passed. Protected dispatch `35335514577` passed exact-source, production-route, migration-state, ownership, Pūrṇa/Nirmāṇa and pre-transition isolation gates; its atomic ownership cutover and dedicated `brahma-build-pipeline-job` rebind completed. It then failed before re-attestation, routine migrations, or service deployment because `data-plane-secret-isolation-preflight.ts` requires `GOOGLE_CLOUD_REGION`, while the strict post-rebind workflow step exports only `GCP_REGION`. This is a source-wiring failure, not an observed strict-isolation failure. DP046 authorization, lease, and direct-`postgres` ownership route were deleted immediately and the temporary postgres password was rotated out. DP046 backup/restore/authority/lease/run are terminal and cannot be reused. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, established L0-L2/Pūrṇa history, WriterBase, and the L4/L5 hold. | 2026-09-18 15:55 IST | 2026-09-18 16:22 IST | **RELEASED — POST-REBIND STRICT-ISOLATION ENVIRONMENT WIRING FAILED; NO ROUTINE DELIVERY OR SERVICE DEPLOYMENT** |
| MADHAV-DATA-PLANE-L3-DP045-PGCRYPTO-SUCCESSOR-CI-RECLASSIFIED-20260918 | Execution — Data Plane / Codex | **Corrected source-repair record for PR #2656.** After the protected tip advanced independently to `57224e396a8b0079b6148ca106e23bf7d44b694c`, the exact DP044 change was replayed without modification as `238226ea6f7f2d3b15ef6463bf19d003d7fe9b7b`; its focused DP-SD-020 contract again passed 47 tests and no-push deploy build `35328461770` completed successfully after web, sidecar, and pipeline image builds plus packaging/import assertions. Ganga run `35328462090` passed TypeScript, secret, planner, fixture, density, unit, and disposable DB integration work; its Governance Pytest step was cancelled at `2026-09-18T09:21:16Z` after six minutes. The workflow itself documents healthy exact-command variance of 285–422 seconds, so that cancellation was premature and is not source-failure evidence. A clean Python 3.13 environment then executed the exact CI command with its required protected-baseline environment and passed `7047 passed, 93 skipped, 158 deselected, 3 xfailed, 7 subtests` in 315.72 seconds; the narrower pin suite also passed 42 tests. Therefore one replacement Ganga run on the unchanged verified PR head is evidence-backed and permitted; it is not a blind retry. PR auto-merge remains enabled. No protected tip changed from this repair, and no backup/restore, authority, production cutover, data build, deployment, consumer proof, or terminal asset acceptance occurred. | 2026-09-18 14:43 IST | 2026-09-18 15:05 IST | **RELEASED — CANCELLATION RECLASSIFIED; ONE EVIDENCE-BACKED CI REPLACEMENT PERMITTED** |
| MADHAV-DATA-PLANE-L3-DP044-PGCRYPTO-REPAIR-CI-STALL-20260918 | Execution — Data Plane / Codex | **Fail-closed source-repair record for the DP043 prerequisite.** PR #2655 (`8a7bbeca07dcc6c77cf09b47fa34894af9cb2996`) adds idempotent `CREATE EXTENSION IF NOT EXISTS pgcrypto` inside the existing protected ownership transaction before its prerequisite assertion; any later ownership failure rolls the extension back with the transaction. Local focused DP-SD-020 security contract passed 47 tests. On the PR, deploy build check `35327278267` completed successfully after all no-push image builds and packaging assertions. Ganga run `35327278333` passed its completed gates, but its Governance Pytest step remained unchanged from `2026-09-18T09:01:04Z` for more than nine minutes across three dated observations; the run was cancelled at `2026-09-18T09:10:46Z` without a blind rerun. PR auto-merge remains queued, but no protected tip changed, no new backup/restore or authority was created, and no production cutover, data build, deployment, or terminal asset acceptance occurred. Resume only from a fresh protected-tip recheck after a terminal required-check outcome; do not treat the successful no-push build as merge or deployment evidence. | 2026-09-18 14:30 IST | 2026-09-18 14:41 IST | **RELEASED — REQUIRED GANGA RUNNER STALLED; SOURCE DELIVERY HELD** |
| MADHAV-DATA-PLANE-L3-DP043-POST-CI-FRESH-CUTOVER-20260918 | Execution — Data Plane / Codex | **Fail-closed terminal record for DP043.** Exact protected `main` `935a369d4fb074d7311eebc706af534f22c637c1`; fresh production backup `1789720842502`; isolated restore operation `5a41c78e-70d9-48a3-a87a-e0240000002f` to `amjis-ri02-validation-c720f1832`; Cloud Audit `restoreBackup` request/completion evidence; exact-source Ganga CI `35323629858`, ordinary protected deployment `35325137889`, and credential preflight `35325381855` all passed. Run `35326318967` passed dispatch, migration-state inspection, and pre-transition isolation. Inside the lease, `runDataPlaneOwnershipPreflight` stopped at its transactional prerequisite check because production lacks `pgcrypto`; no ownership mutation, migration attestation, named-build rebind, routine delivery, service deployment, or terminal asset acceptance followed. The DP043 authorization, lease, and one-shot direct-`postgres` ownership route were deleted immediately, and that temporary `postgres` password was rotated out of use. DP043 backup, restore, audit, authorization, lease, and run are terminal and must never be reused. Any successor requires a separate authorized recovery design, fresh source/backup/restore/audit evidence, and new one-shot authority. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, established L0-L2/Purna history, WriterBase, and the L4/L5 hold. | 2026-09-18 14:10 IST | 2026-09-18 14:29 IST | **RELEASED — PGCRYPTO PREREQUISITE ABSENT; TRANSACTIONAL STOP BEFORE PRODUCTION TRANSITION** |
| MADHAV-DATA-PLANE-L3-DP042-MIGRATOR-ROUTE-SUCCESSOR-20260918 | Execution — Data Plane / Codex | **Fail-closed terminal record for DP042.** Fresh backup `1789719254747`, isolated restore `3816629a-6e90-4126-a269-14e50000002f` to `amjis-ri02-validation-c720f1832`, and request/completion Cloud Audit evidence passed for protected `main` `02a92632a0f05f46b576ca67d578a61363aabb1d`. Before authorization or dispatch, the mandatory exact-main recheck found the protected tip advanced to `935a369d4fb074d7311eebc706af534f22c637c1`; no DP042 workflow run was created and no production ownership, migration, build, deployment, or terminal acceptance action occurred. The one-shot ownership-admin carrier was deleted and its temporary `postgres` password was rotated out of use. DP042 backup, restore, audit, and lease are terminal and cannot be rebound to the new tip. The repaired direct `data_plane_migrator` carrier remains dedicated to the protected environment for a fresh successor only. | 2026-09-18 13:44 IST | 2026-09-18 13:51 IST | **RELEASED — PROTECTED TIP DRIFT BEFORE AUTHORIZATION OR DISPATCH** |
| MADHAV-DATA-PLANE-L3-DP041-FRESH-CURRENT-MAIN-CUTOVER-20260918 | Execution — Data Plane / Codex | **Fail-closed terminal record for DP041.** Exact protected `main` `02a92632a0f05f46b576ca67d578a61363aabb1d`; fresh backup `1789718208479`; isolated restore `0c46538f-1dd8-4016-8d6a-1a6f0000002f` to `amjis-ri02-validation-c720f1832`; and Cloud Audit request/completion evidence all passed. Run `35322483186` passed the dispatch, production-route, state, and pre-transition isolation gates, then stopped before entering `withDataPlaneCutoverLease` when the pre-existing `DATA_PLANE_MIGRATOR_DATABASE_URL` was not a valid route pinned to the production proxy. No ownership preflight, protected transition, named-build rebind, migration, routine delivery, deployment, or terminal acceptance followed. DP041 authorization, lease, and ownership-admin environment secrets were deleted; the temporary `postgres` password was rotated out of the removed carrier. The stale migrator carrier was replaced with a dedicated direct `data_plane_migrator` route, but DP041 backup, restore, authorization, lease, and run are terminal and must never be reused. Any successor requires fresh backup, isolated restore/audit evidence, authorization, and lease. | 2026-09-18 13:25 IST | 2026-09-18 13:44 IST | **RELEASED — MIGRATOR ROUTE CONTRACT MISMATCH; NO PRODUCTION TRANSITION** |
| MADHAV-DATA-PLANE-L3-DP040-PROTECTED-CI-RUNNER-STALL-20260918 | Execution — Data Plane / Codex | **Fail-closed protected-delivery hold for ownership-admin contract repair.** PR #2637, source commit `bec581d353dec2b83997e179cdc162cd879cc2fc`, separates the isolated validation-proxy credential from a required direct ownership-admin credential and wires only the latter to the direct-`postgres` ownership preflight. Local TypeScript compilation and the focused data-plane security contract suite passed (34 tests). Two independent GitHub Actions run generations for the same PR then stalled without a terminal source failure in three unrelated runner workloads: Governance Gates `pytest — pyjhora_adapter + pipeline`, Unit Tests `npm test -- --reporter=verbose`, and Deploy `Build web image (no push)`. Each was observed unchanged beyond the documented threshold and cancelled; final runs `35271940621` and `35271940763` are `completed/cancelled`. No merge, protected cutover, database action, secret change, deployment, or build/rebuild occurred under DP040. The PR remains open with merge-queue auto-merge enabled; do not bypass required checks or issue a third blind rerun. Resume only after an evidence-backed runner/root-cause outcome and required checks pass; a merged successor will still require fresh, dedicated, time-bounded direct-`postgres` ownership credential provisioning and wholly new cutover receipts. | 2026-09-18 02:18 IST | 2026-09-18 02:18 IST | **RELEASED — TWO-GENERATION GITHUB RUNNER STALL; PROTECTED DELIVERY HELD** |
| MADHAV-DATA-PLANE-L3-DP039-DIRECT-POSTGRES-ADMIN-STOP-20260918 | Execution — Data Plane / Codex | **Fail-closed terminal record for DP039.** Exact protected `main` `a40215cae89af33aa7d7516629c79b33d0c0f3c7`; fresh backup `1789676254086`; isolated restore operation `e1ea7966-e9ce-45e2-ad1e-0b560000002f` (target `amjis-ri02-validation-c720f1832`) and Cloud Audit status `OK`; dispatched run `35270642983`. The repaired validation-reader isolation gate passed, proving DP036 effective. The run then stopped at `runDataPlaneOwnershipPreflight` before ownership transition with `Data-plane ownership preflight requires direct postgres administrative authentication.` The workflow passes its validation-proxy `DATA_PLANE_ADMIN_DATABASE_URL` to that preflight; this credential is deliberately not the direct `postgres` administrator required by the preflight. No subsequent named-build rebind, strict-isolation recheck, migration attestation, production data build/rebuild, routine delivery, deployment, or terminal acceptance step ran. Both one-time authorization and lease secrets were deleted and the environment listing confirmed them absent. DP039 backup, restore, authority, lease, and run are terminal and must never be reused. A successor requires a dedicated one-shot direct-`postgres` ownership-admin route, a source contract that keeps it separate from validation access, protected delivery, then wholly fresh receipts and authority. | 2026-09-18 01:47 IST | 2026-09-18 02:32 IST | **RELEASED — DIRECT POSTGRES ADMIN AUTHENTICATION CONTRACT MISMATCH; NO POST-PREFLIGHT TRANSITION** |
| MADHAV-DATA-PLANE-L3-DP039-FRESH-CURRENT-TIP-20260918 | Execution — Data Plane / Codex | **Fresh, exclusive protected-main cutover after DP038 tip-drift release.** Bound only to current protected `main` `a40215cae89af33aa7d7516629c79b33d0c0f3c7`. The exact-SHA standard CI wave passed; its post-quality protected workflow `35269438827` passed the read-only migration-state inspection and skipped bootstrap, migration, and all deploy jobs. The separate P3-E behavior-smoke red was traced to a skipped no-job workflow and is explicitly non-PR-gating; it is not a data-plane deployment or authorization. Owns only one new backup, isolated restore plus audit receipt, one-time authorization and lease, exact workflow dispatch, and read-only postflight under all existing DP-SD-017/018/019/020 gates. DP038 backup `1789672753008` and restore `41175316-4c48-4daf-b526-3bda0000002f` are terminal and prohibited from reuse. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Pūrṇa history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim, or terminal acceptance. | 2026-09-18 01:47 IST | 2026-09-18 05:47 IST | **ACTIVE — FRESH BACKUP/ISOLATED-RESTORE PRECONDITION** |
| MADHAV-DATA-PLANE-L3-DP038-PROTECTED-TIP-DRIFT-20260918 | Execution — Data Plane / Codex | **Fail-closed exact-SHA stop.** Before installing any DP038 environment secret or dispatching `deploy.yml`, the exact-main recheck found protected `main` advanced from `d759fb467f08d87feefefebf5ef74f8c5f3534d8` to `a40215cae89af33aa7d7516629c79b33d0c0f3c7` (`feat(inquiry): add evidence-bound successor lifecycle (#2636)`). Subsequent environment listing confirms neither `DATA_PLANE_CUTOVER_AUTHORIZATION` nor `DATA_PLANE_CUTOVER_LEASE` exists; no DP038 workflow run was created. DP038 backup `1789672753008` and restore `41175316-4c48-4daf-b526-3bda0000002f` are terminal and may not be reused. A successor must begin from the current protected tip only after its required evidence and use a wholly new backup, isolated restore/audit receipt, authorization, and lease. | 2026-09-18 01:10 IST | 2026-09-18 01:10 IST | **RELEASED — PROTECTED TIP DRIFT BEFORE AUTHORIZATION OR DISPATCH** |
| MADHAV-DATA-PLANE-L3-DP038-FRESH-POST-THROTTLE-20260918 | Execution — Data Plane / Codex | **Fresh, exclusive successor to DP037 after confirmed revocation.** `DATA_PLANE_CUTOVER_AUTHORIZATION` and `DATA_PLANE_CUTOVER_LEASE` both return GitHub HTTP 404 and the environment list excludes both; `deploy.yml` was never dispatched. DP037 backup `1789671140904` and restore `6c27e3a9-989b-455f-bd27-3b810000002f` are terminal and may not be reused. Owns one all-new protected-main cutover for exact `d759fb467f08d87feefefebf5ef74f8c5f3534d8` only after a new backup, distinct isolated restore/audit receipt, new one-time authorization and lease, full existing CI/security/isolation/quiescence/rollback gates, and read-only postflight. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Pūrṇa history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim, or terminal acceptance. Stop on SHA drift, any failed gate, incomplete fresh proof, rollback ambiguity, or unexpected production impact. | 2026-09-18 00:49 IST | 2026-09-18 04:49 IST | **ACTIVE — FRESH BACKUP/ISOLATED-RESTORE PRECONDITION; DP037 SUPERSEDED AND TERMINAL** |
| MADHAV-DATA-PLANE-L3-DP037-GITHUB-THROTTLE-STOP-20260918 | Execution — Data Plane / Codex | **Fail-closed recovery record for DP037 dispatch refusal.** Fresh protected main `d759fb467f08d87feefefebf5ef74f8c5f3534d8`, backup `1789671140904`, and restore `6c27e3a9-989b-455f-bd27-3b810000002f` completed with Cloud Audit `OK` to `amjis-ri02-validation-c720f1832`. A new short-lived environment authorization/lease write completed before GitHub REST throttling; the first `deploy.yml` dispatch was rejected with HTTP 403 rate-limit before a workflow run was created. The required explicit secret-revocation requests are also HTTP 403 throttled; treat both secret names as potentially present until a successful deletion can be verified. No workflow dispatch, production lock, ownership/migration/data/build/deployment action occurred. Do not retry dispatch or replace either secret; revoke the two named values first, then create an entirely new authorization and lease for any successor. | 2026-09-18 00:30 IST | 2026-09-18 04:24 IST | **ACTIVE STOP — GITHUB API THROTTLE; DISPATCH FORBIDDEN PENDING REVOCATION** |
| MADHAV-DATA-PLANE-L3-DP037-FRESH-READER-VALIDATION-20260918 | Execution — Data Plane / Codex | **Fresh, exclusive automated protected cutover under DP-SD-017/018/019/020, after DP036 reader-derived ACL repair.** Owns only the normal protected-main cutover and read-only postflight for exact protected `main` `d759fb467f08d87feefefebf5ef74f8c5f3534d8`, subject to every existing fail-closed gate: fresh production backup; distinct isolated validation restore plus audit receipt; protected environment; exact source/run binding; new authorization and lease; automated CI/security/isolation gates; quiescence; protected ownership/migration atomicity; rollback/canary; routine-delivery barrier; and postflight evidence. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Pūrṇa history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim, or terminal acceptance. Never reuse DP035 receipts, authorization, lease, backup, restore, or run. Stop on SHA drift, failed gate, incomplete fresh proof, rollback ambiguity, or unexpected production impact. | 2026-09-18 00:24 IST | 2026-09-18 04:24 IST | **ACTIVE — FRESH BACKUP/ISOLATED-RESTORE PRECONDITION** |
| MADHAV-DATA-PLANE-L3-DP036-VALIDATION-READER-ACL-20260917 | Execution — Data Plane / Codex | **Bounded source repair after two fail-closed validation-copy ownership probes.** Owns only `platform/scripts/data-plane-cutover-preflight.ts` and its directly required data-plane security contract test, local verification, PR/merge-queue delivery and read-only release observation. The repair derives the actual validation connector's reader role after connector binding, rejects missing/NUL identity, safely quotes the identifier, and grants bootstrap read access only through the isolated validation-copy admin proxy; it neither changes the validation client nor grants production access. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Pūrṇa history, frozen WriterBase and L4/L5 hold. Any future cutover requires fresh protected-main SHA, new backup, distinct isolated restore/audit receipt, new authorization and new lease; all former artifacts are terminal. | 2026-09-17 23:45 IST | 2026-09-18 03:45 IST | **SOURCE DELIVERY AWAITING PROTECTED CI** (PR #2635, commit `bd74e9d3c`; local TypeScript and 34 security-contract tests passed. DP035's merged literal-role grant (`818f022`) did not cover the configured validation-reader role: fresh backup `1789668355112`, restore operation `0f683c70-b8da-465c-8ef4-200a0000002f` to `amjis-ri02-validation-c720f1832`, and run `35257497644` stopped at the read-only ownership preflight with `permission denied for schema public`; it did not acquire the production lock or mutate production ownership/migrations/data/build. Its one-time authorization and lease were deleted and verified absent. Never reuse that backup, restore, run, authority, or lease.) |
| MADHAV-DATA-PLANE-L3-DP034-ADMIN-CREDENTIAL-DIAGNOSTIC-20260917 | Execution — Data Plane / Codex | **Bounded shared-release credential diagnosis and recoverable configuration repair.** Owns only `.github/workflows/data-plane-credential-preflight.yml`, `platform/scripts/data-plane-cutover-preflight.ts`, a directly required diagnostic script and data-plane security-contract tests, isolated verification, PR/merge-queue delivery, protected-environment component diagnosis, and one credential configuration repair only after a verified recoverable route. It exposes only nonsecret component-presence/parse/isolated-proxy-authentication results; it must not print, persist, rotate, or disclose a secret and must not perform a DB write, migration, ownership transition, backup, restore, deployment, build/rebuild, or cutover. The diagnostic is independently dispatchable from the cutover route, retains exact source/run provenance, and fails closed. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. Any later cutover needs a fresh exclusive exact-SHA operation lease and all-new authority/receipt material. | 2026-09-17 15:05 IST | 2026-09-17 19:05 IST | **RELEASED — PROTECTED CONFIGURATION DIAGNOSIS AND REPAIR COMPLETE** (2026-09-17 22:47 IST; PR #2632 merged through the protected queue as `25d692b74e4df283a42b710918a48d7bfb2e95ac` after required CI. Read-only run `35251459070` established that the configured URL was parseable but lacked `user` and `password`; its isolated proxy started successfully and no DB operation occurred. A recoverable configuration route was then verified from the declared canonical `amjis_app` user on both primary and validation instances plus the existing canonical password secret. The environment secret metadata advanced at 2026-09-17T17:16:38Z without value disclosure. Fresh read-only run `35251753930` at exact protected main recorded `parseable=true`, `safe_to_route=true`, no invalid components, and `authenticated=true` through the isolated validation proxy. No production DB write, backup, restore, lease, migration, deployment, L3 build or acceptance occurred; a successor still needs fresh exact-SHA authority and all-new receipts.) |
| MADHAV-DATA-PLANE-L3-DP033-ADMIN-CREDENTIAL-PREFLIGHT-20260917 | Execution — Data Plane / Codex | **Bounded source safety repair following the DP032 fail-closed credential gate.** Owns only `platform/scripts/data-plane-protected-cutover.ts`, directly required `platform/tests/unit/data_plane_security_contract.test.ts`, isolated verification, PR/merge-queue delivery and read-only release observation. It validates the protected administrator connection identity before entering the native cutover lease, preserving the existing authenticated-proxy pinning and all receipt/audit/lock/ownership/migration gates. It does not repair, reveal, rotate or replace any credential; the protected environment owner must supply a password-bearing `DATA_PLANE_ADMIN_DATABASE_URL` before any successor operation. No production deploy/cutover, DB/IAM/secret mutation, build/rebuild, retirement, empirical claim or terminal acceptance is authorized under this source lease. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. Stop on failed check/review, merge ambiguity, source drift or any unexpected impact. | 2026-09-17 14:35 IST | 2026-09-17 18:35 IST | **RELEASED — PROTECTED DELIVERY COMPLETE** (2026-09-17 14:58 IST; PR #2631 merged through the protected queue as `0f2d9cc0259ac5983fda7f5e10d75ce189e7440f` after PR CI and merge-group CI `35204523036` passed. The source now rejects an unusable admin credential before the cutover lease. No live DB/deploy action occurred under this source lease; a successor operation still requires a correctly configured protected environment secret plus fresh exact-SHA authority and receipts.) |
| MADHAV-DATA-PLANE-L3-DP032-PROTECTED-MANUAL-CUTOVER-20260917 | Execution — Data Plane / Codex | **Fresh, exclusive protected-main manual cutover lease under DP-SD-020.** Owns only the normal protected release/deployment/cutover and read-only postflight for exact protected `main` `55fdd35f4f8ca0a42ca0d4c20f9d60e2171f6e37` (PR #2630), after a fresh exact-SHA readback and every existing fail-closed gate: `data-plane-production-cutover`, exact source/run binding, CI/security/isolation checks, dedicated builder identity/secret, least-privilege IAM, newly created production backup plus distinct isolated validation restore/audit receipt, fresh cutover authorization/lease and quiescence, protected ownership/migration atomicity, rollback/canary, routine-delivery barrier, and postflight evidence. The guarded workflow requires a separately deliberate `workflow_dispatch` with `data_plane_cutover=true`; no prior DP026/028/030 receipt, backup, restore, authorization, or run may be reused. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim or terminal acceptance. Stop on a conflicting live lease, main/deploy SHA drift, failed gate, stale/missing new receipt, rollback ambiguity, or unexpected production impact. | 2026-09-17 14:07 IST | 2026-09-17 18:07 IST | **RELEASED — FAIL-CLOSED AT ADMIN-CREDENTIAL SHAPE GATE** (2026-09-17 14:31 IST; exact run `35201792302` passed all prior read-only workflow gates and stopped in `validationAdminProxyConfig` before the production advisory lock, ownership preflight, migration, credential rebind, routine delivery or deployment. The masked `DATA_PLANE_ADMIN_DATABASE_URL` lacks the password required to route it through the authenticated isolated validation proxy. No production database mutation or L3 data build ran. All DP032 authority/receipts are terminal and cannot be reused.) |
| MADHAV-DATA-PLANE-L3-DP031-CUTOVER-TRIGGER-GUARD-20260917 | Execution — Data Plane / Codex | **Bounded source safety repair.** Owns only `.github/workflows/deploy.yml`, directly required workflow/data-plane contract tests, isolated verification, PR/merge-queue delivery and read-only release observation. It repairs the demonstrated unsafe topology: unrelated protected-main commit `0d0d852b5a2dcef42d565be983918f67c4a01eb8` (PR #2629) triggered `One-time Protected DB Bootstrap` in `35195404597`, despite no data-plane source change or fresh exact-SHA cutover lease. Preserve ordinary read-only migration inspection and normal component deployment selection, but make protected data-plane bootstrap/cutover unreachable unless the triggering protected commit actually contains eligible data-plane deployment/cutover inputs or a separately governed explicit dispatch provides the exact authorization. The guard must fail closed, retain source/run provenance and all existing backup/restore/lease/ownership gates, and be regression-tested against the observed unrelated-governance path. No production deploy/cutover, DB/IAM/secret mutation, build/rebuild, retirement, empirical claim or terminal acceptance is authorized under this source lease; any live operation requires a later fresh exact-SHA lease and receipts. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. Stop on conflicting lease, source drift, failed check/review or unexpected scope expansion. | 2026-09-17 13:12 IST | 2026-09-17 17:12 IST | **RELEASED — PROTECTED DELIVERY COMPLETE** (2026-09-17 14:06 IST; PR #2630 merged as protected main `55fdd35f4f8ca0a42ca0d4c20f9d60e2171f6e37` after PR CI and three merge-queue candidate CI suites completed successfully. Automatic PR deploy `35196891494` proved the protected bootstrap skipped without explicit dispatch. No live DB/deploy action occurred under this source lease.) |
| MADHAV-DATA-PLANE-L3-DP030-PROTECTED-CUTOVER-20260917 | Execution — Data Plane / Codex | **Fresh, exclusive protected-main automated cutover lease under DP-SD-020.** Owns only the normal release/deployment/cutover and read-only postflight for exact protected `main` `09d7d75c986bbeb29cb2e9ad0fc62981111829d8` (PR #2628), after rechecking all fail-closed gates: exact source/run binding, automated CI/security/isolation, named `data-plane-production-cutover` environment, dedicated builder identity/secret, least-privilege IAM, newly created backup plus distinct isolated validation restore/audit receipt, new cutover authorization/lease and quiescence, protected ownership/migration atomicity, rollback/canary, routine-delivery barrier and postflight evidence. It supersedes but must not reuse DP028's backup, restore, authorization or secret receipt. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim or terminal acceptance. Stop on conflicting live lease, main/deploy SHA drift, failed gate, stale/missing new receipt, rollback ambiguity or unexpected production impact. | 2026-09-17 12:57 IST | 2026-09-17 16:57 IST | **RELEASED — EXACT-SHA SOURCE DRIFT** (2026-09-17 13:07 IST; exact-main CI `35194491292` for `09d7d75c986bbeb29cb2e9ad0fc62981111829d8` passed, but protected main advanced to `0d0d852b5a2dcef42d565be983918f67c4a01eb8` (PR #2629) before a DP030-bound deployment/cutover run existed. The unrelated-source workflow run `35195404597` then reached bootstrap setup; it was canceled immediately. GitHub records every privileged/mutating step—including protected-state refresh, Pūrṇa migrations, the data-plane cutover, build-job binding and post-cutover attestation—as **skipped**. No DP030 backup, restore, authorization, production database mutation, routine delivery, deployment or L3 data build occurred. A fresh lease and all-new receipts are required for any later operation.) |
| MADHAV-DATA-PLANE-L3-DP029-VALIDATION-ACL-REPAIR-20260917 | Execution — Data Plane / Codex | **Bounded source and protected-delivery correction.** Owns only `platform/scripts/data-plane-cutover-preflight.ts`, its directly required `data_plane_security_contract` tests, isolated local verification, PR/merge-queue delivery and read-only release observation. It repairs the verified pre-cutover ACL bootstrap gap: run `35190881058` reached the designated connector and fresh backup/restore receipt, then the unmarked restored copy denied verifier access to `public._migrations_applied` before the normal locked ownership preflight could grant the terminal ACL. The correction may route the existing admin credential only through the already authenticated isolated validation proxy, after all receipt/audit/instance checks, to grant verifier `USAGE` on `public` and `SELECT` on that migration ledger only on the restored validation copy; it must never route the admin credential to an arbitrary host or mutate production before the normal protected transaction. No production deploy/cutover, production DB mutation, IAM/secret mutation, data build/rebuild, retirement, empirical claim or terminal acceptance is authorized under this source lease; a fresh operation-specific lease and receipt are required after merge. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. Stop on conflicting lease, source drift, failed check/review, merge ambiguity or unexpected impact. | 2026-09-17 12:22 IST | 2026-09-17 16:22 IST | **RELEASED — PROTECTED DELIVERY COMPLETE** (2026-09-17 12:55 IST; PR #2628 merged as protected main `09d7d75c986bbeb29cb2e9ad0fc62981111829d8` after PR and merge-group CI success. No live DB/deploy action occurred under the source lease.) |
| MADHAV-DATA-PLANE-L3-DP028-PROTECTED-CUTOVER-20260917 | Execution — Data Plane / Codex | **Fresh, exclusive protected-main automated cutover lease under DP-SD-020.** Supersedes—but does not revive—the released DP026 operation. It owns only the normal protected release/deployment/cutover and read-only postflight for protected `main` `b202f4fd95a4a049ebb865daa4da3238953b45ed` (PR #2627) after exact-main CI and every existing fail-closed gate: `data-plane-production-cutover`, exact source/run binding, automated CI/security/isolation, dedicated builder identity/secret, least-privilege IAM, fresh backup and distinct isolated validation restore, fresh authorization/lease and quiescence, protected ownership/migration atomicity, rollback/canary, routine-delivery barrier and postflight evidence. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim or terminal acceptance. Stop on conflicting live lease, SHA drift, failed exact-main/deploy/preflight gate, stale/missing backup or restore proof, rollback ambiguity or unexpected production impact. | 2026-09-17 12:02 IST | 2026-09-17 16:02 IST | **RELEASED — FAIL-CLOSED AT VALIDATION ACL GATE** (2026-09-17 12:17 IST; run `35190881058` passed exact-main CI, fresh backup `1789626842561`, isolated restore `7f8cf626-cc11-4ebd-be50-18860000002f`, audit binding, migration/ownership/runtime-isolation and connector identity, then stopped before production lock, ownership preflight, migration, routine delivery or deployment because the unmarked restored copy lacks the verifier's terminal `public` usage/ledger read ACL. No production database mutation or L3 data build ran.) |
| MADHAV-DATA-PLANE-L3-DP027-RESTORE-SECRET-REPAIR-20260917 | Execution — Data Plane / Codex | **Bounded source and protected-delivery correction.** Owns only `.github/workflows/deploy.yml`, its directly required `data_plane_security_contract` assertion, isolated local verification, PR/merge-queue delivery, and read-only release observation. It corrects the demonstrated configuration regression: run `35132360267` reached the protected ownership gate with the designated `DATA_PLANE_RESTORE_VALIDATION_DATABASE_URL`; later source substituted `DATA_PLANE_VERIFIER_DATABASE_URL` and all later runs failed the connector guard before that gate. Restore the dedicated validation-secret binding while retaining the authenticated local-proxy preflight and all exact backup/restore, authorization, lease, isolation, ownership, migration, rollback and routine-delivery gates. No production deploy/cutover, DB mutation, IAM/secret mutation, data build/rebuild, retirement, empirical claim or terminal acceptance is authorized under this source lease; a fresh operation-specific lease and fresh receipt are required after merge. Preserve all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. Stop on any fresh conflicting lease, source drift, failed checks/review, merge ambiguity or unexpected impact. | 2026-09-17 11:40 IST | 2026-09-17 15:40 IST | **RELEASED — PROTECTED DELIVERY COMPLETE** (2026-09-17 12:01 IST; PR #2627 merged as protected main `b202f4fd95a4a049ebb865daa4da3238953b45ed` after PR and merge-group CI success. No live DB/deploy action occurred under the source lease.) |
| MADHAV-DATA-PLANE-L3-DP026-PROTECTED-CUTOVER-20260917 | Execution — Data Plane / Codex | **Fresh, exclusive protected-main automated cutover lease under DP-SD-020.** Supersedes—but does not revive—the expired DP025 rebind lease. It owns only the ordinary protected-main release/deployment/cutover and read-only postflight for protected `main` `3581a10a45cd80416e83b883612d9c9efc04be4d` (PR #2625) after the exact-main CI and every existing fail-closed gate: `data-plane-production-cutover` environment, exact source/run binding, automated CI/security/isolation checks, dedicated builder identity/secret, least-privilege IAM, fresh production backup plus a distinct isolated validation restore receipt, fresh cutover authorization/lease and quiescence, protected ownership/migration atomicity, rollback/canary, routine-delivery barrier and postflight evidence. It preserves all 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. It authorizes no source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim or terminal acceptance. Stop on a conflicting live lease, source/deploy SHA drift, failed exact-main/deploy/preflight gate, missing or stale backup/restore proof, rollback ambiguity or unexpected production impact. | 2026-09-17 11:12 IST | 2026-09-17 15:12 IST | **RELEASED — FAIL-CLOSED AT CONNECTOR GATE** (2026-09-17 11:26 IST; run `35187253238` passed exact-main CI, migration, ownership, runtime-isolation, fresh backup `1789623767433`, isolated restore `f943d720-13bb-4f61-9797-f91e0000002f`, and audit binding, then stopped before any cutover/migration/deploy because the workflow selected the generic verifier credential. No routine migration, service deployment or L3 data build ran.) |
| MADHAV-DATA-PLANE-L3-DP025-PROTECTED-REBIND-20260917 | Execution — Data Plane / Codex | **Fresh, exclusive protected-main release rebind under DP-SD-017/018/019/020.** This row supersedes—not revives—`MADHAV-DATA-PLANE-L3-DP020-RENEWAL-20260917`: that row was bound exclusively to `b62ba029ea8063ce44f2b7f3a7e70922f767c716` and its own source-drift stop condition fired when protected PR #2624 merged as `main` `9f908ddb788cb368a64ac3717d048b3492b5db86`. It owns only the ordinary, protected-main automatic release/deployment/cutover and read-only postflight for that exact SHA after every existing fail-closed gate: the named `data-plane-production-cutover` environment; exact source/run binding; automated CI, security and isolation checks; dedicated builder identity/secret; least-privilege IAM; fresh backup plus distinct isolated-restore receipt; fresh cutover fence and quiescence; protected ownership/migration atomicity; rollback/canary; routine-delivery barrier; and postflight evidence. Preserve all 22 active L3 identities, the retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase and L4/L5 hold. No source expansion, broad IAM, secret disclosure, data build/rebuild, retirement, empirical claim or terminal acceptance is authorized. Stop on a conflicting live lease, any source/deploy SHA drift, failed exact-main/deploy/preflight gate, missing/stale backup or restore proof, rollback ambiguity or unexpected production impact. | 2026-09-17 05:00 IST | 2026-09-17 09:00 IST | **ACTIVE — DP025 EXACT-SHA REBIND; MAIN CI/DEPLOY PENDING** |
| MADHAV-DATA-PLANE-L3-DP020-RENEWAL-20260917 | Execution — Data Plane / Codex | **Fresh, exclusive automated protected-cutover renewal under DP-SD-020.** The predecessor `MADHAV-DATA-PLANE-L3-DP020-AUTOMATED-CUTOVER-20260916` expired at 2026-09-16 22:30 IST and is DEAD under this table's prime rule; this row is its narrow successor, not a revival. It owns only the normal protected-main release, deployment/cutover, and read-only postflight path for exact protected `main` `b62ba029ea8063ce44f2b7f3a7e70922f767c716` (PR #2623), after the exact-main CI and all DP-SD-017/018/019/020 fail-closed gates. The named `data-plane-production-cutover` environment, protected-main-only exact source/run binding, automated CI/security/isolation checks, dedicated builder identity/secret, least-privilege IAM, fresh backup plus distinct isolated restore receipt, fresh cutover fence, quiescence, protected ownership/migration atomicity, rollback/canary, routine-delivery barrier, and postflight evidence all remain mandatory. Preserve all 22 active L3 identities and the retired `ka_gochara_sweep`, accepted L0-L2/Purna history, frozen WriterBase, and the L4/L5 hold. No source expansion, broad IAM, credential disclosure, data build/rebuild, retirement, empirical claim, or terminal acceptance is authorized. Stop on any new conflicting unexpired lease, source/deploy SHA drift, failed exact-main/deploy/preflight gate, missing or stale backup/restore proof, rollback ambiguity, or unexpected production impact. | 2026-09-17 02:54 IST | 2026-09-17 06:54 IST | **ACTIVE — DP-SD-020 RENEWAL; EXACT-MAIN CI IN PROGRESS** |
| MADHAV-DATA-PLANE-L3-DP020-AUTOMATED-CUTOVER-20260916 | Execution — Data Plane / Codex | **Exclusive automated protected-cutover lease under Native authority.** Records and implements DP-SD-020 as a narrow amendment to DP-SD-019: remove only the separate-human GitHub environment reviewer/approval dependency, while preserving the `data-plane-production-cutover` environment, protected-main-only deployment policy, exact source/run binding, complete automated CI/security/isolation gates, dedicated builder identity and secret, least-privilege IAM, backup plus isolated-restore receipt, fresh cutover fence, quiescence, protected ownership/migration atomicity, postflight marker, rollback/canary and routine-deploy barrier. Owns the current PR #2615 branch/worktree, exact cutover authority/state records, directly required workflow/preflight/tests, protected GitHub environment provisioning, already-authorized Data Plane IAM/secret/database-role provisioning, protected merge/deploy/cutover and read-only postflight. Preserves the 22 active L3 identities, retired `ka_gochara_sweep`, accepted L0-L2/Pūrṇa history, frozen WriterBase and L4/L5 hold. No fabricated independent approval, arbitrary-branch environment, broad unrelated IAM, unrelated credential rotation, secret disclosure, chart-data build/rebuild, L4/L5 work or empirical/terminal claim. All older ACTIVE rows are past expiry and DEAD under the prime rule; the latest Data Plane recovery row is RELEASED. Stop on a new conflicting live lease, protected-tip/head drift, failed exact-head/merge-group/main gate, preflight failure, incomplete backup/restore proof, rollback ambiguity or unexpected production impact. | 2026-09-16 16:23 IST | 2026-09-16 22:30 IST | **ACTIVE — AUTHORITY AMENDMENT AND AUTOMATED CUTOVER** |
| MADHAV-DATA-PLANE-L3-DP019-MAIN-CI-RECOVERY-20260916 | Execution — Data Plane / Codex | **Operation-specific protected-release recovery lease.** The prior source candidate merged as protected `main` `fa9857f00e45ab2c2052ffd37eb0910a4d7b6fcd`, but post-merge CI `35074402653` failed because the analysis-pin validator dereferenced pre-squash integration/evidence commits that are not ancestors of GitHub's squash-delivered protected tree; deploy `35074881736` consequently skipped. Owns the narrow recovery in `.github/workflows/ci.yml`, `.github/workflows/deploy.yml`, `platform/scripts/generate/nirmana_analysis_layer_pins.py`, its focused test module, `platform/tests/unit/data_plane_security_contract.test.ts`, and generated `nirmana-analysis-layer-pins.json`: preserve immutable reviewed source/authority identities as metadata, verify protected baseline generations and history without disposable side refs, restore strict `--check` on every event, prove clean single-branch delivery, and remove the exact duplicate legacy `DATABASE_URL=amjis-pipeline-db-url` override that would defeat the already accepted dedicated builder identity/secret cutover. Also owns a fresh-main isolated worktree, protected PR/merge-queue delivery, and—only after a second coordination readback plus exact rollback, secret, migration and protected-owner preflight—the normal deployment of the resulting protected tree and read-only post-deploy verification. Provenance inventory and the topology-independent Python suite remain mandatory everywhere. Preserve the 22-member active L3 denominator, protected retired `ka_gochara_sweep`, accepted L0-L2/Pūrṇa contracts/history, migrations 1035/1036 and their one-time bootstrap/restricted-routine boundary, frozen WriterBase, and the L4/L5 hold. No manual safety-gate bypass, secret disclosure or mutation, IAM/role expansion, chart-data build/rebuild/write, L0/L4/L5 work, retirement, or empirical/terminal acceptance claim. **OVERRIDE:** Pūrṇa validation lease `MADHAV-PURNA-ANVESANA-LIVE-VALIDATION-20260916` expired at 14:12 IST and Data Plane source integration lease `MADHAV-DATA-PLANE-L3-DP019-FRESH-INTEGRATION-20260916` expired at 14:30 IST; under this file's prime rule both are DEAD and neither is revived. Stop on any fresh conflicting live lease, protected-tip drift, failed exact-head/merge-group/main gate, review finding, preflight failure, rollback ambiguity, or unavailable independent environment reviewer. | 2026-09-16 14:57 IST | 2026-09-16 18:30 IST | **RELEASED — SOURCE RECOVERY GREEN; PROTECTED MERGE/CUTOVER HELD** (2026-09-16 15:55 IST; PR #2615 exact head `113ccc37eb93f02416907fee75b52958792eae7a` is CLEAN with every required/advisory PR check green and independent review reporting no findings. Fresh single-branch source/delivery proof passes with discarded side commits absent; the duplicate legacy pipeline-secret override is removed. No merge, deployment, DB, IAM, secret, role, build or chart-data mutation occurred. Live preflight proves the GitHub cutover environment and independent eligible reviewer are absent; the organization/repository currently exposes only `amonty84`, so authenticated `prevent_self_review` approval is impossible. Dedicated builder SA/secret and Data Plane URL secrets also remain absent. Resume requires a newly added independent GitHub actor plus a fresh exclusive cutover lease and the full DP-SD-018 provisioning/backup/restore sequence.)** |
| MADHAV-PURNA-ANVESANA-LIVE-VALIDATION-20260916 | PŪRṆA ANVEṢAṆA LIVE VALIDATION / Codex | **Exclusive bounded live-validation lease.** Owns read-mostly, low-impact production receipts for the already deployed Pūrṇa revision: exact deployed tuple, frozen 34-route replay, controlled managed-job recovery/cross-instance continuity, Portal/managed/raw normalized parity, truthful pagination/continuation/exhaustion, deployed compatibility-consumer census, and bounded health observation. May create only campaign-scoped test inquiry/job records through existing authenticated APIs and read them back; redact all chart, principal and credential material. No deploy, migration, build/rebuild, IAM/secret/key/role mutation, chart/producer/data-plane mutation, retirement, ruleset change, or empirical acceptance signature. Data Plane `MADHAV-DATA-PLANE-L3-DP019-PROTECTED-RELEASE-20260916` remains source/protected-delivery-only and is not touched. Stop on a failed probe, source/deployed tuple drift, any active conflicting live lease, unexpected customer impact, or missing consent/authority; release after recording receipts or an exact external blocker. | 2026-09-16 12:12 IST | 2026-09-16 14:12 IST | **ACTIVE — PŪRṆA G5/G7 LIVE VALIDATION ONLY** |
| MADHAV-DATA-PLANE-L3-DP019-PROTECTED-RELEASE-20260916 | Execution — Data Plane / Codex | Narrow protected-delivery continuation after the Pūrṇa exclusive live-cutover release at coordination `77824b74fd0095181ffaa68de996a5c29ec48ee6`. Supersedes this campaign's earlier source/local rehearsal rows solely for publishing the verified immutable candidate `0a8ea484a` (parent integration `4a426f6ec`, Data Plane head `5863644e6`, protected main `ac3ef565ec98f716da4557891b9c269121fa95e1`) to branch `codex/madhav-data-plane-execution`, updating PR #2607, exact-head CI, independent read-only review, and protected merge-queue delivery. Preserve the protected retired `ka_gochara_sweep`, accepted L0-L2 and Pūrṇa contracts/history, DP-SD-019 bootstrap/restricted-routine design, and L4/L5 hold. This lease authorizes no shared/production DB action, migration apply, build/rebuild, deploy, IAM/secret/role mutation, chart-data write, L0/L4/L5 work, or acceptance claim; a later fresh operation-specific lease is required for any Data Plane live deployment or physical build. Re-fetch protected main before push/merge and stop on any new conflicting active lease, protected-tip movement, failed exact-head gate, review finding, or rollback ambiguity. | 2026-09-16 11:35 IST | 2026-09-16 15:30 IST | **RELEASED — PROTECTED SOURCE DELIVERY COMPLETE** (2026-09-16 14:03 IST; PR #2607 merged at `fa9857f00e45ab2c2052ffd37eb0910a4d7b6fcd` after exact-head 34-check CI and merge-group CI `35073074329`, EKV `35073074385`, and TAP `35073074328` success. No production DB, build, deploy, IAM/secret/role or chart-data mutation occurred.) |
| MADHAV-DATA-PLANE-L3-DP019-FRESH-INTEGRATION-20260916 | Execution — Data Plane / Codex | Fresh source/local/disposable integration against immutable Data Plane head `5863644e6` and protected main `c63cb8048` after Pūrṇa PRs #2609/#2610/#2611 and successful exact-SHA deploy run `35056578950`. Supersedes the self-invalidated earlier rehearsal whose purpose required refresh after any Pūrṇa head change. Owns one new disposable integration worktree; semantic resolution of the known workflow, L3 Kāla query/test and append-only governance conflicts; directly required generated checks; and local release-gate verification. Preserve the protected retired `ka_gochara_sweep`, all accepted L0-L2/Pūrṇa contracts and history, DP-SD-019 one-time bootstrap plus restricted routine-delivery design, and the L4/L5 hold. The Pūrṇa owner retains the exclusive live-operation lease until its durable release; this integration performs no push/PR/protected merge, shared or production DB action, build/rebuild, deploy, IAM/secret/role mutation, chart-data write, L0/L4/L5 work or acceptance claim. Any further protected-main/Pūrṇa head change requires another refresh before release. | 2026-09-16 10:25 IST | 2026-09-16 14:30 IST | **ACTIVE — FRESH SOURCE/LOCAL INTEGRATION; LIVE MUTATION YIELDED TO PŪRṆA UNTIL DURABLE RELEASE** |
| MADHAV-DATA-PLANE-L3-DP019-INTEGRATION-REHEARSAL-20260916 | Execution — Data Plane / Codex | Source/local/disposable integration rehearsal only against immutable Data Plane head `5863644e6`, protected main `979604ed8`, and exact-green Pūrṇa route-repair head `34778a4a7` (PR #2609). Owns one disposable detached rehearsal worktree plus the eight known merge-conflict surfaces: `.github/workflows/deploy.yml`, `00_ARCHITECTURE/{CAPABILITY_MANIFEST.json,CROSS_CUTTING_DECISION_REGISTER_v1_0.md,CURRENT_STATE_v1_0.md,SESSION_LOG.md}`, `00_ARCHITECTURE/briefs/nirmana/MADHAV_DATA_PLANE_STRATEGIC_LEDGER_v1_0.md`, `platform/src/lib/retrieval/registry/layers/L3_kala/query_temporal_activation.ts`, and `platform/tests/unit/nirmana_evidence_ownership_deploy.test.ts`; directly required generated checks/tests and execution ledgers only. Rehearse a semantic union preserving Pūrṇa dual-proxy, conditional one-shot route validation, mandatory cleanup/rearm behavior and marked routine delivery together with DP-SD-019 one-time protected bootstrap, routine credential isolation, Yojaka/Kṣetra/source evidence, and all foreign append-only history. The active Pūrṇa live owner remains exclusive for PR #2609 merge, production migration/deploy, secrets/IAM and lease release; this rehearsal performs no push/PR/merge, shared/production DB action, build/rebuild, deploy, IAM/secret/role mutation, chart-data write, L0/L4/L5 work or acceptance claim. Any Pūrṇa head change invalidates the rehearsal and requires fresh integration. | 2026-09-16 07:31 IST | 2026-09-16 11:30 IST | **ACTIVE — SOURCE/LOCAL REHEARSAL ONLY; LIVE CUTOVER YIELDED TO PŪRṆA** |
| MADHAV-PURNA-ANVESANA-LIVE-CUTOVER-20260916 | PŪRṆA ANVEṢAṆA LIVE CUTOVER / Codex | **EXCLUSIVE live-operation lease.** Owns only the serialized completion of PR #2606 after exact-head CI/review: current-state/rollback capture; IAM trust-boundary repair (exact protected-main github-actions subject; removal of repository-wide/legacy conductor, builder and default-compute deploy/actAs/Editor paths; attached-resource audit/narrowing for legacy Google/App Engine Editors); secret-specific cold-start and deny proof; creation of the two Pūrṇa secrets only after effective-access gates pass; one-shot Cloud SQL bootstrap, migrations 1033/1034/1037/1038/1039 and mandatory postflight; candidate-bound web/MCP deployment and traffic; watchdog Scheduler OIDC convergence and later fallback/secret cleanup; PA-R01–PA-R13 production evidence. The named human project Owner remains the audited break-glass administrator. No unrelated producer/data-plane source, shared rebuild, chart-data rewrite, frozen L4/L5/WriterBase change, ruleset bypass, secret disclosure or invented empirical acceptance. Active DP019 leases are source/local only and explicitly prohibit all live mutation; this lease does not edit their owned paths or migrations 1035/1036. Any newly observed conflicting live lease, failed preflight, stale protected tip, unreviewed candidate, or rollback ambiguity stops mutation. | 2026-09-16 02:45 IST | 2026-09-16 12:00 IST | **RELEASED — live cleanup complete** (2026-09-16 11:33 IST; protected merge `ac3ef565ec98f716da4557891b9c269121fa95e1`, CI `35060226139` and deploy `35061039613` succeeded; `amjis-web-probe-ac3ef565ec98-35061039613-1` is 100% live with exact SHA, no watchdog secret mount/fallback; Scheduler OIDC returned 200 both before and after legacy-secret retirement; anonymous/header probes returned 401/403; `watchdog-secret` v1 is disabled and its web-runtime accessor binding removed. Pūrṇa migrations and protected-owner marker had already passed in the deploy migration job. No production data build or empirical acceptance is claimed.)** |
| MADHAV-DATA-PLANE-L3-DP019-MUHURAT-BREAKDOWN-20260916 | Execution — Data Plane / Codex | Bounded source/local repair of the pre-existing `find_muhurat` explanation regression exposed by the mandatory broad-Python gate. Owns only `platform/python-sidecar/muhurat/finder.py`, `platform/python-sidecar/panchang_engine/tests/test_muhurat_scoring.py`, directly required focused tests and execution ledgers; for those exact paths this packet supersedes the generic test-history lane. Restore the original structured `MuhuratWindow.breakdown` contract required by the accepted `ka_muhurta_seva` brief §3.5: preserve tithi/nakshatra/vara identity, names, raw score, configured weight and contribution; yoga and planet context; native-overlay presence/contribution; inauspicious-window disclosure and knockout. Do not alter the scoring formula, event weights/vocabulary, required location fence, intra-day policy, writer/orchestrator, service routing, migrations, generated writer digest/pins, physical data, or any L4/L5 source. Any source change must prove its exact provenance blast radius and receive an independently accepted exact-tip source packet before root-owned regeneration. Preserve all foreign Pūrṇa ownership, accepted DP-SD-019 history and protected retired sweep. No push/PR/merge, shared/production DB, build/rebuild, deploy or data mutation; full stable checks and independent review required. | 2026-09-16 00:51 IST | 2026-09-16 04:30 IST | **RELEASED — CANDIDATE REJECTED AT PROVENANCE FENCE** (2026-09-16 00:56 IST; restoring the original structured breakdown passed the exact 86-test Muhūrta/service surface but the generated-inventory dry run shifted 29 writer digests across L1-L4, including `ph_muhurta`, rather than only `ka_muhurta_seva`. The shared-source candidate was fully reverted because the active campaign must preserve the L4 hold; checked-in generated artifacts remain untouched and the two applicable explanation tests remain an explicit broad-gate block requiring a separately authorized cross-layer primitive decision.) |
| MADHAV-DATA-PLANE-L3-DP019-PYTEST-HISTORY-20260916 | Execution — Data Plane / Codex | Test-history-only reconciliation for stale expectations surfaced sequentially by the mandatory whole-`platform/` Python gate after collection became green. Owns only Python test files under `platform/python-sidecar/**` and `platform/migrations/__tests__` that fail against already accepted/reviewed current source, plus execution ledgers. Each change must cite and preserve the current source contract and immutable history: update obsolete paths, return shapes, role-specific ACL expectations, fakes or assertions without weakening discovery, converting real failures to skips, deleting still-applicable semantic coverage, changing production source, reopening accepted L0-L2 meaning, or editing L4/L5. Database tests remain governed by the separate disposable-only safety fence. Any actual source defect must leave this lane and receive a separately enumerated packet. No generated artifacts, migrations, push/PR/merge, live DB, build/rebuild, deploy or data mutation; full stable checks and independent exact-tip review required. | 2026-09-16 00:47 IST | 2026-09-16 04:30 IST | **ACTIVE — BROAD PYTEST HISTORY RECONCILIATION** |
| MADHAV-DATA-PLANE-L3-DP019-KSHETRA-GATE-20260916 | Execution — Data Plane / Codex | Disjoint source/local Kṣetra release-gate correction opened after the governed-dependency runtime sweep. Owns only `platform/python-sidecar/services/ka_kshetra/layer1.py`, `platform/python-sidecar/services/ka_kshetra/dhara_null_vec.py`, `platform/python-sidecar/services/ka_kshetra/tests/{test_dhara_null_vectorized.py,test_layer0_projection.py,test_stage1_symbolization.py}` and execution ledgers. Correct Layer 1 to consume the current `(rate, is_synthetic)` baseline contract including `shape_only`; make the obsolete vectorized-null entry point an exact compatibility delegation to the accepted zero-call C/E implementation while retaining helper parity/performance contracts; update the offline stage-1 fake to the current batched cursor protocol. Do not change the Kṣetra writer, resume version, generated writer digest/pins, persistence schema, null hypothesis, R=1024/F-01 resolution, process-wide Swiss boundary, or physical data. Preserve all foreign Pūrṇa and Data Plane migration/source ownership, accepted DP-SD-019 provenance, protected retired sweep and L4/L5 holds. No push/PR/merge, shared/production DB, migration, deploy, build/rebuild or data mutation; exact focused, full Kṣetra, stable checks and independent review are required. | 2026-09-16 00:35 IST | 2026-09-16 04:30 IST | **ACTIVE — KSHETRA RUNTIME GATE REPAIR** |
| MADHAV-DATA-PLANE-L3-DP019-PYTHON-GATE-20260916 | Execution — Data Plane / Codex | Bounded source/local/disposable repair of the mandatory broad-Python release gate after accepted DP-SD-019 source/provenance. Owns only `platform/python-sidecar/pipeline/writers/**`, `platform/python-sidecar/pipeline/__tests__/test_forensic_writer.py`, `platform/python-sidecar/pipeline/orchestrator/writers/__tests__/test_ga_vastu.py`, `platform/python-sidecar/services/ka_kshetra/tests/test_dhara_null.py`, `platform/python-sidecar/test_savepoint_failure_path.py`, `platform/python-sidecar/routers/pyhora.py`, `platform/migrations/__tests__/test_mig_{124,125,126,127}.py`, directly required focused tests/test configuration, execution ledgers and local isolated Python dependency setup. The PyHora addition is the bounded collection-time repair for FastAPI resolving its decorated endpoint's postponed `BirthData` annotation in the decorator module; the generic decorator alternative was rejected because provenance showed it would transitively repin protected L0-L4 writers. The migration-test addition removes their unsafe implicit `127.0.0.1:5433/amjis` target after the exact broad gate found that port accepting connections: they must require a dedicated guarded loopback test database and skip safely when absent. Retire or migrate stale legacy assertions only after reference/teardown proof; do not restore the deleted legacy writer stack, weaken discovery, suppress collection errors, reintroduce native-chart coupling, discard the DHARA semantic contract, or narrow the process-wide Swiss lock. Any database test must require a dedicated guarded disposable local database and skip safely when absent. Preserve all Pūrṇa source/migrations, data-plane migrations 1035/1036, accepted DP-SD-019 provenance/generated artifacts, protected retired `ka_gochara_sweep`, and L0/L1/L2/L4/L5 pins/history. No push/PR/merge, production/shared DB, migration, IAM/secret/role, build/rebuild, deploy or chart-data mutation; release eligibility requires the full stable check sequence and independent exact-tip review. | 2026-09-16 00:24 IST | 2026-09-16 04:30 IST | **ACTIVE — PYTHON RELEASE-GATE REPAIR; UNSAFE IMPLICIT DB TARGET REMOVED** |
| MADHAV-DATA-PLANE-L3-DP019-SOURCE-20260915 | Execution — Data Plane / Codex | DP-SD-019 source/local/disposable continuation only, with one conductor and two disjoint bounded owners. Prerequisite owner may change only `.github/workflows/deploy.yml`, directly required data-plane workflow/state tests, and the existing RI-02 cutover record to separate one-time privileged bootstrap from subsequent restricted routine delivery and prove marked/unmarked/unknown/pending/failed/concurrent paths. Yojaka owner may change only `platform/python-sidecar/pipeline/orchestrator/writers/ka_yojaka.py`, `platform/python-sidecar/services/ka_yojaka/{binder.py,classifier.py}`, and bounded `platform/python-sidecar/tests/l3/test_ka_yojaka*.py` fixtures for complete signed/multidomain transformation and a concrete consumer distinction. Root alone owns generated writer/pin artifacts, execution ledgers, integration and reviews. Preserve foreign `/Users/Dev/nirmana-s/l3` branch `l3-2542-ka-yojaka-output-digest-spec` and its migration 1024, all Pūrṇa live-wrapup source/migrations 1033/1034/1037/1038, data-plane migrations 1035/1036, protected retired `ka_gochara_sweep`, and L4/L5 holds. No push/PR/merge, production deploy, shared/production migration apply, IAM/secret/role mutation, build/rebuild or chart-data write under this lease; those require a later fresh operation-specific exclusive lease after source acceptance. | 2026-09-15 23:35 IST | 2026-09-16 04:00 IST | **RELEASED — SOURCE/PROVENANCE ACCEPTED** (2026-09-16 00:24 IST; routine-delivery and corrected Yojaka packets were independently accepted and integrated, immutable source `64facb976`, reachable authority binding `b9018247d`, successor `dc6ab4cc5` and exact-child review through `85b4bdaca` pass. The mandatory broad Python gate remains red on eight inherited collection issues, so no PR or live mutation occurred; the bounded successor lease above owns that repair.) |
| MADHAV-PURNA-ANVESANA-LIVE-WRAPUP-20260915 | PŪRṆA ANVEṢAṆA LIVE RELEASE + COMPLETE WRAP-UP / Codex | Fresh native/user-authorized live-delivery continuation from immutable Wave 7 source candidate `facccfe4b518b997357c08ff7798b703459326df`. Owns one isolated integration/release branch `codex/purna-anvesana-live-wrapup`, the Pūrṇa campaign brief/evidence/event surfaces, planner/inquiry/web/MCP/job source and tests, campaign migrations 1033/1034/1037/1038 plus any collision-free forward correction demonstrably required, governed protected PR/merge-queue integration, existing-environment deployment, narrowly scoped inquiry signing-key/runtime configuration, staging/production three-channel evidence, empirical evaluation packaging/execution with approved consented data and real independent experts, compatibility disposition, and closure. This supersedes the prior generic source-local authority ceiling only for this campaign's scoped delivery actions; all older verdicts remain historical. It does not authorize ruleset/admin bypass, secret disclosure, destructive shared-data rebuild, broad IAM/infra/model-budget expansion, frozen WriterBase/producer/data-plane architecture mutation, invented consent/reviewer evidence, or unrelated doctrine. Coexists with active Data Plane RI-02 during stocktake/source integration only: it does not edit RI-02-owned L0-L3 files/migrations 1035/1036, deploy, apply shared migrations, rotate keys, or rebuild production while RI-02's exclusive operational scope is active. Before any shared migration/deploy/rebuild, this campaign must re-fetch this surface and hold a fresh operation-specific exclusive lease. | 2026-09-15 23:26 IST | 2026-09-16 12:00 IST | **RELEASED — delivery wrap-up reached its technical authority ceiling** (2026-09-16 11:33 IST; Pūrṇa web is deployed at exact protected SHA `ac3ef565ec98f716da4557891b9c269121fa95e1`; portal/managed/raw G5 dark-state closure was observed, but pagination/data-plane availability and consented empirical acceptance remain explicitly open. Governance evidence is being recorded separately; this release ends the exclusive live-operation lease and permits the Data Plane owner to proceed under its own current authority.)** |
| MADHAV-PURNA-ANVESANA-W7-CORRECTION-20260915 | PŪRṆA ANVEṢAṆA EXACT-HEAD CORRECTION / Codex | Corrective reopening after exact-head CI run `34991468186` and independent review refuted the released Wave 7 candidate. Initial scope adopted the stale `platform/src/generated/capability_estate_census.json` repair. Terminal code review of corrective pre-release head `b80c842c8` then found two source-local defects: weighted background work can starve behind sustained smaller dispatches, and the MCP inquiry route mishandles nested guarded pagination paths. Scope was expanded to own `platform/src/lib/retrieval/qos/dispatch_queue.ts` and its focused tests, `platform/src/app/api/mcp/inquiry/route.ts` and its focused tests, any directly dependent generated artifact, append-only Pūrṇa Wave 7 governance, branch `codex/purna-anvesana-wave7`, PR #2605, exact-head CI and read-only terminal reviews. Repairs were failing-test-first and bounded to those findings. Coexisted with the Data Plane lease and did not edit its files or migrations. No merge, deploy, shared/production migration or mutation, credentials/infrastructure/signing-key action, retirement, doctrine, expert/empirical acceptance, or production-health claim. | 2026-09-15 21:32 IST | 2026-09-15 23:59 IST | **RELEASED — CORRECTIVE PRE-RELEASE APPROVED** (2026-09-15 21:50 IST; exact pre-release head `b32974cbee0ef9bd7a93f811d36a6609ad7095eb` independently approved with no HIGH/MEDIUM findings; only the pre-authorized metadata-only successor, exact-head CI and read-only final reviews remain) |
| MADHAV-DATA-PLANE-L3-RI02-20260915 | Execution — Data Plane / Codex | DP-SD-018 `L3-RI-02-AUTHORIZED-UNBLOCK`: two bounded source/local/disposable lanes only during this lease — P owns L0/L1/L2 plus metadata-only L4 provenance adjudication and versioned superseding receipts; S owns protected-owner/lifecycle API plus exact DBA/release cutover design and role-realistic disposable proof. Root owns serialized generated-file integration, execution ledger, fresh migration collision checks and packet disposition. Preserves the active PŪRṆA Recovery Wave 7 lease, queue, branches and migrations; no mutation of its worktree or governance. No production deploy, shared/production migration apply, role/secret/IAM mutation, build/rebuild or chart-data write under this source-design lease; those require a later fresh exclusive cutover fence after independent acceptance. L4/L5 elevation and protected retired `ka_gochara_sweep` remain excluded. | 2026-09-15 11:55 IST | 2026-09-16 02:00 IST | **RELEASED — SOURCE/DESIGN ACCEPTED** (2026-09-15 23:35 IST; Lane P and corrected Lane S were independently accepted and integrated, the fresh-clone provenance correction passed at `66a85047b`, and no production mutation occurred. DP-SD-019 source packets continue under the separate bounded row above.) |
| MADHAV-L3-UNBLOCK-HANDOFF-20260915 | Strategy — Data Plane / Codex | Native-authorized documentation and dispatch only: prepare and independently review DP-SD-018 to delegate protected provenance reconciliation and narrowly scoped DBA role/credential/deployment cutover, then resume the existing Execution — Data Plane task through the approved full L3 outcome. Owns one L3 amendment plus strategic-ledger changes in /Users/Dev/.codex/worktrees/50a6/Madhav. No application, production, credential or database action by Strategy and no exclusive production lease. Execution must claim its own current exact scope/fence. Preserve concurrent Purna W7 ownership. | 2026-09-15 11:27 IST | 2026-09-15 13:27 IST | **RELEASED — DP-SD-018 DISPATCHED** (2026-09-15 11:45 IST; independently approved amendment content 6b6ce9c06ce54fb7c474d97c70257a505bf2ed84, approval 7f21f27b14a7909424591a530096dc2f5d6e2b13, dispatch record 032fd48394df5164b3ea6de20bd73fa61040337c; message delivered to task 01a0998a-8240-7631-97ce-36c6d4734fde and active resumption turn observed. No repair, production or acceptance claim by Strategy.) |
| MADHAV-PURNA-ANVESANA-W7-20260915 | PŪRṆA ANVEṢAṆA SOURCE-LOCAL RECOVERY / Codex | Recovery Wave 7 only, from immutable W6 head `b48a6997b12ebe591bc5e62b7b60ddb295ee352b`: reopen the prior negative acceptance truthfully; complete every source/design/test and local/disposable portion of PA-R01–PA-R13; add deterministic receipted transit argument derivation/clarification and earn the unchanged 34/34 Beyond-Ācārya route denominator; adversarially audit and repair six-layer SCU semantics, channels, pagination, compatibility, overlay, lifecycle recovery, durable managed jobs, migration rehearsal, runbooks and source-local acceptance; obtain independent reviews and exact-head CI evidence in focused stacked PRs. Owns only the new isolated recovery worktree/branch and append-only Pūrṇa governance. Coexists with active Data Plane leases and does not edit their L3 files or migrations 1035/1036. No merge, deploy, shared/production migration or mutation, credential/infrastructure/signing-key action, retirement, doctrine ratification, expert/empirical acceptance, production-health claim, frozen W6/WriterBase or foreign-campaign mutation. Terminal ceiling is `SOURCE_LOCAL_ACCEPTED`; genuinely external actions remain in one authority packet. | 2026-09-15 11:05 IST | 2026-09-15 23:30 IST | **RELEASED — SOURCE-LOCAL CANDIDATE** (2026-09-15 21:17 IST; technical packets and local/disposable gates complete; immutable-head CI and terminal reviews are non-mutating closure checks) |
| MADHAV-DATA-PLANE-L3-RI01-20260915 | Execution — Data Plane / Codex | Execute only `L3-RI-01-PRECURSOR-GENERATION-INTEGRATION` from accepted W0: reconcile the fresh protected-main and planner/Pūrṇa migration namespace; reserve and author migrations 1035/1036 by relocating the accepted, confirmed-unapplied L1/L2 generation SQL into the active migration directory without semantic reopening; update references; run disposable apply/reapply, migration/security review and protected checks; then merge through normal controls and verify exact deployed source plus automatic migration application. No corpus build/rebuild, L3 data materialization, generation selection, credentials/infrastructure, L4/L5 work, or empirical-performance claim. Any W1 production build requires a later refreshed exclusive purpose after this lease is released. | 2026-09-15 05:07 IST | 2026-09-15 11:30 IST | **EXPIRED — SUPERSEDED BY RI-02** (2026-09-15 11:55 IST; RI-01 source/design review passed, but no PR, shared apply, deploy, build or owner cutover occurred. DP-SD-018 replaces its circular pre-PR cutover condition and requires a fresh operation-specific fence.) |
| MADHAV-DATA-PLANE-L3-W0-20260915 | Execution — Data Plane / Codex | Execute only the approved DP-SD-017 L3 Kāla `L3-W0-FOUNDATION-SAFETY-01` packet from accepted L2 terminal `e5307fadef42cca557a1c0ca3c1831b1296e22b4`: reconcile the 22 active identities and protected retired `ka_gochara_sweep`; freeze the first safe dependency frontier and producer/use/shared-output map; repair and test the Kṣetra planning/resume and Bhaviṣya history hazards; establish bounded baselines, recovery/publication design and consumer sentinels. Owns local source/tests and L3 execution evidence in `/Users/Dev/.codex/worktrees/c9bd/Madhav` plus explicitly isolated helper worktrees. No production build/rebuild, deployment or data mutation under this lease; any later shared/production action requires a refreshed exclusive purpose and conflict check. L4/L5 remain waiting. | 2026-09-15 02:55 IST | 2026-09-15 08:55 IST | **RELEASED — W0 SOURCE ACCEPTED** (2026-09-15 11:55 IST; W0 source/local/disposable packet had already reached independent acceptance. No production build/rebuild, deployment or data mutation occurred under this lease.) |
| MADHAV-L3-KICKOFF-20260915 | Strategy — Data Plane / Codex | Native-approved L3 campaign handoff only: adopt the reviewed L3 strategy, author/review its bounded execution brief, record DP-SD-017 and immutable local content/approval pins, and send the kickoff to the existing Execution — Data Plane task. Owns the three L3 strategy/brief/review documents and concise strategic-ledger changes in `/Users/Dev/.codex/worktrees/50a6/Madhav`; no application edits, production lease or data/build/release action in this planning task. Execution owns its subsequent scope/lease, goal, heartbeat and gated implementation. | 2026-09-15 02:36 IST | 2026-09-15 04:36 IST | **RELEASED — L3 KICKOFF DISPATCHED** (2026-09-15 02:52 IST; independently reviewed content `793972c754b106688097dbc54536c1a9c270a793`, native approval pin `04a9ab33effa23e5e9b4e89772330ae264498a9b`, local dispatch record `f00e73394695d9106fac127c66021714f4b5dcdb`; message delivered to existing execution task `01a0998a-8240-7631-97ce-36c6d4734fde`, active kickoff turn observed. No application/data/build/deploy action or implementation/acceptance claim by Strategy; execution owns subsequent goal, recovery and gated work.) |
| MADHAV-L3-ASTRA-STRATEGY-20260915 | Strategy — Data Plane / Codex | Native-requested second L3 planning pass, with three independent read-only gpt-6-astra / max reviews of build cost, actual data DAG and consumer value. Documentation scope only: L3 proposed strategy and review evidence under `00_ARCHITECTURE/briefs/nirmana/MADHAV_DATA_PLANE_L3_*`, plus its concise strategic-ledger pointer, in `/Users/Dev/.codex/worktrees/50a6/Madhav` on `codex/madhav-data-plane-strategy`. Implementation source is read-only at accepted L2 terminal `e5307fade`. No application/data/build/campaign execution, doctrine adoption, deployment, source branch push/merge or production lease. Planning documents remain uncommitted for review; only this lease row is committed/pushed. | 2026-09-15 01:05 IST | 2026-09-15 05:05 IST | **RELEASED — REVIEWED L3 STRATEGY PROPOSAL** (2026-09-15 01:45 IST; three bounded Astra/max reviews, corrections incorporated; exact 22 active + protected retired identity parity; proposed DAG and document checks PASS; bounded read-only connected-DB metadata/aggregate evidence; no application/data mutation, build, execution dispatch, goal change, deployment or empirical acceptance) |
| MADHAV-PURNA-ANVESANA-W6-20260915 | BEYOND-ACARYA ACCEPTANCE + SOURCE-SCOPE CLOSE / Codex | Wave 6 only, stacked from governed Wave 5 close `4f2d9c97a`: implement versioned source/local Beyond-Acarya acceptance for novel combinations, omission rate, route coverage, semantic-edge coverage, long-inquiry closure and abstention quality; then close the campaign with complete packet evidence and explicit authority-bound residual owners/triggers. Source/local changes, tests, generated acceptance artifacts, governance documentation, commits, push, and focused stacked PR are allowed. No merge, deployment, shared/production migration application or mutation, credentials/infrastructure, retirement, doctrine ratification, frozen WriterBase/foreign campaign mutation, production health, user acceptance, or empirical production-value claim. Acceptance and terminal reviews are independent and read-only. | 2026-09-15 00:44 IST | 2026-09-15 04:44 IST | **RELEASED — WAVE 6 NEGATIVE ACCEPTANCE / SOURCE-SCOPE QUARANTINE APPROVED** (2026-09-15 01:27 IST; branch `codex/purna-anvesana-wave6`, independently approved pre-release head `4e7464700`, stacked PR #2604; governed verdict `NOT_ACCEPTED_SOURCE_LOCAL`, route coverage 30/34, verified continuation 1/1, all original R1–R12 dispositioned and 13 authority-bound residuals quarantined; no merge, deploy, shared/production migration or mutation, credentials/infrastructure, retirement, doctrine, production trace, user acceptance, or production/empirical acceptance claim; immutable final source head requires external exact-head review) |
| MADHAV-PURNA-ANVESANA-W5-20260914 | DISPOSABLE LIFECYCLE + THREE-DOOR PARITY / Codex | Wave 5 only, stacked from governed Wave 4 close `841942b3b`: prove migration 1033 up/down, RLS subject isolation, append-only evidence receipts, lifecycle compare-and-swap/replay and overlay fail-closed behavior in a disposable local PostgreSQL environment, then prove Portal, managed MCP and raw MCP equivalence including principal/token binding, continuation and failure states. Source/local/disposable changes, tests, documentation, commits, push, and focused stacked PRs are allowed. No merge, deployment, shared/production migration application or mutation, credentials/infrastructure, retirement, doctrine ratification, frozen WriterBase/foreign campaign mutation, production health, or empirical acceptance claim. Migration/security and parity reviews are independent and read-only. | 2026-09-14 23:47 IST | 2026-09-15 03:47 IST | **RELEASED — WAVE 5 DISPOSABLE LIFECYCLE / BOUNDED THREE-DOOR PARITY COMPLETE** (2026-09-15 00:39 IST; branch `codex/purna-anvesana-wave5`, independently approved technical head `9bd525bf0`, governance close `493cb1e59`, stacked PR #2603; focused 18 files / 214 tests PASS, disposable migration/security PASS, golden 56/56, lint 0 errors / 590 inherited warnings; full unit qualified by one five-second census timeout whose isolated 2-test rerun passed; shared denominator 6/184 explicitly not full-catalog parity; no merge, deploy, shared/production migration or mutation, credentials/infrastructure, retirement, doctrine, production trace, or production/empirical acceptance claim) |
| MADHAV-PURNA-ANVESANA-W4-20260914 | RESPONSE ACCOUNTABILITY / Codex | Wave 4 only, stacked from governed Wave 3 close `f6b4d1733`: implement a bounded iterative evidence loop, complete fact registration, mandatory fact delivery, continuation closure, and normalized response-coverage receipts across governed managed/raw inquiry paths while preserving R1-R12 and process-local lifecycle honesty. Source/local changes, tests, generated artifacts, documentation, commits, push, and one focused stacked PR are allowed. No merge, deployment, shared/production migration application or mutation, credentials/infrastructure, retirement, doctrine ratification, frozen WriterBase/foreign campaign mutation, durable-lifecycle claim, or production/acceptance claim. Independent response-accountability review is read-only. | 2026-09-14 21:57 IST | 2026-09-15 01:57 IST | **RELEASED — WAVE 4 RESPONSE ACCOUNTABILITY COMPLETE** (2026-09-14 23:45 IST; branch `codex/purna-anvesana-wave4`, independently approved technical head `c7d9b021a`, governance close `072316cb3`, stacked PR #2602; full unit 1,099 files / 11,687 tests PASS, golden 56/56, lint 0 errors / 590 inherited warnings; no merge, deploy, shared/production migration or mutation, credentials/infrastructure, retirement, doctrine, durable-lifecycle, or production/empirical acceptance claim) |
| MADHAV-PURNA-ANVESANA-W3-20260914 | INQUIRY COMPILER / Codex | Wave 3 only, stacked from governed Wave 2 close `99817ef49`: normalize inquiry intent; traverse the reviewed ontology and semantic graph under deterministic floors; implement an independent omission challenger, bounded widening, frontier closure and budget receipts; preserve R1-R12 and fail closed. Source/local changes, tests, generated artifacts, documentation, commits, push, and one focused stacked PR are allowed. No merge, deployment, shared/production migration application or mutation, credentials/infrastructure, retirement, doctrine ratification, frozen WriterBase/foreign campaign mutation, or production/acceptance claim. Independent review is read-only. | 2026-09-14 21:14 IST | 2026-09-15 01:14 IST | **RELEASED — WAVE 3 INQUIRY COMPILER / OMISSION CHALLENGER COMPLETE** (2026-09-14 21:54 IST; branch `codex/purna-anvesana-wave3`, independently approved technical head `b19be1b7a`, governance close `2d12edd13`, stacked PR #2601; full unit 1,098 files / 11,664 tests PASS, golden 56/56, lint 0 errors / 590 inherited warnings; no merge, deploy, shared/production mutation, credentials/infrastructure, retirement, doctrine, or production/empirical acceptance claim) |
| MADHAV-PURNA-ANVESANA-W2-20260914 | SEMANTIC ESTATE / Codex | Wave 2 only, stacked from governed Wave 1 close `0c4a6e341`: replace all 177 descriptor-derived Semantic Capability Unit stubs with meaningful source-linked editorial units; establish the typed concept universe and graph with zero unexplained stubs, semantic gaps, orphan descriptors or unbound concepts. Source/local changes, tests, generated artifacts, documentation, commits, push, and one focused stacked PR are allowed. No merge, deployment, shared/production migration application or mutation, credentials/infrastructure, retirement, doctrine ratification, frozen WriterBase/foreign campaign mutation, or production/acceptance claim. Independent review is read-only. | 2026-09-14 19:36 IST | 2026-09-14 23:36 IST | **RELEASED — WAVE 2 EDITORIAL SCUS / TYPED GRAPH COMPLETE** (2026-09-14 21:10 IST; branch `codex/purna-anvesana-wave2`, technical evidence head `402319736`, stacked PR #2600; W2-P1/W2-P2 and the narrow deterministic receipt recapture independently approved; full unit 1,094 files / 11,640 tests PASS; no merge, deploy, shared/production mutation, credentials/infrastructure, retirement, doctrine, or production/empirical acceptance claim) |
| MADHAV-PURNA-ANVESANA-W1-20260914 | SEMANTIC ESTATE / Codex | Wave 1 only, stacked from Wave 0 commit `00397839e`: establish one explicit, source-linked, testable output-contract disposition for every current-source-active producer and reconcile every runtime descriptor across runtime/public route, provenance, pagination/exhaustion, and dark/unavailable state. Source/local changes, tests, documentation, commits, push, and one focused stacked PR are allowed. No merge, deployment, shared/production migration application or mutation, credentials/infrastructure, retirement, doctrine ratification, frozen WriterBase/foreign campaign mutation, or production/acceptance claim. Independent reviewers are read-only. | 2026-09-14 18:19 IST | 2026-09-14 22:19 IST | **RELEASED — WAVE 1 SOURCE/LOCAL/DISPOSABLE PACKETS COMPLETE** (2026-09-14 19:32 IST; branch `codex/purna-anvesana-wave1`, technical evidence head `8599c6a51`, stacked PR #2599; W1-P1/W1-P2 and migration 1034 independently approved after fixes; no merge, deploy, shared/production migration or mutation, credentials/infrastructure, retirement, doctrine, or deployed-current/production claim) |
| MADHAV-PURNA-ANVESANA-20260914 | SEMANTIC ESTATE / Codex | Native-delegated autonomous six-wave completion campaign. Wave 0 freezes and reconciles Foundation Candidate 0 without enlarging PR #2597; later packets may inspect, design, implement, test, document, commit, push and open focused or stacked PRs in isolated worktrees. Exact ceiling: no merge, deployment, shared/production migration application, production mutation, credential/infrastructure change, retirement, doctrine ratification, frozen WriterBase change, foreign campaign/worktree mutation, or unsupported production/acceptance claim. Portal/managed/raw MCP and migration 1033 proof is restricted to disposable or local environments. Independent reviewers are read-only unless separately assigned an isolated packet. | 2026-09-14 17:47 IST | 2026-09-14 21:47 IST | **RELEASED — WAVE 0 SOURCE PACKETS COMPLETE** (2026-09-14 18:15 IST; FC0 and PR #2597 unchanged; corrected dual-tree census and six-wave source control plane independently reviewed; no merge, deploy, shared/production migration or mutation, credentials/infrastructure, retirement, doctrine, or production claim) |
| MADHAV-PLANNER-KNOWLEDGE-CONTINUATION-20260914 | PLANNER KNOWLEDGE / Codex | Native-directed autonomous continuation of PR #2597 only: reconciled protected-main and review state; closed the required capped-frontier false-completion defect, aligned receipt disclosure, made latest-build overlay selection deterministic, regenerated provenance-bearing snapshot/census artifacts, and completed source, test, documentation and review-readiness evidence on `codex/planner-knowledge-inquiry`. Source checkpoint `daa813f9a5f78a31b791f3e73aa035c2ad7fbbe1`; PR #2597 at that checkpoint: 50 terminal checks, 37 PASS, 13 intentional SKIP, 0 fail/pending, open + mergeable + CLEAN. Local mandatory gate: lint 0 errors/590 inherited warnings, TypeScript PASS, full platform 1,094 files/11,616 tests PASS. Explicit exclusions preserved: no merge, deploy, migration application, production data/build action, credential, infrastructure, frozen WriterBase or foreign campaign/worktree mutation. | 2026-09-14 13:30 IST | 2026-09-14 14:09 IST | **RELEASED** |
| MADHAV-PLANNER-KNOWLEDGE-20260913 | PLANNER KNOWLEDGE / Codex | Owner-authorized implementation of the federated Semantic Capability Unit knowledge graph, immutable capability snapshot and chart/build availability overlay, hybrid Inquiry Compiler and versioned Inquiry Contract, omission/frontier validation, managed Pariprashna and MCP parity, raw-MCP lifecycle enforcement, evidence/coverage receipts, tests, documentation, commits and reviewable PR. Worktree `/Users/Dev/.codex/worktrees/a4d5/Madhav`, base `codex/madhav-data-plane-strategy@66a5ce6f8`. Explicitly excludes merge, deploy, production data/infrastructure, credentials, frozen WriterBase contract changes, and mutation of foreign worktrees/campaign state. | 2026-09-13 22:19 IST | 2026-09-14 06:30 IST | **RELEASED — PR BLOCKED BY BASELINE LINT GATE** (2026-09-13 23:19 IST; four clean local implementation commits through `171c9e57a`; generated drift checks and all branch-changed TypeScript lint passed, but mandatory repository-wide lint reported 199 errors/591 warnings in the inherited tree, so the governed run-checks procedure prohibited push/PR. No merge, deploy, migration application, production, infrastructure or credential action.) |
| MADHAV-DATA-PLANE-V2-20260913 | PRODUCT STRATEGY / Codex | Native-authorized documentation-only data-plane value architecture, asset contribution/disposition register and planning provenance in /private/tmp/madhav-data-plane.pUPGlk/plan on codex/madhav-data-plane-value-v2. Native explicitly extended the actual managed-profile exception: You have full authorization. Drafts remain proposed and uncommitted for review; no application, data, campaign execution, deploy, product push or merge. Independent agents read only. No exclusive production lease. | 2026-09-13 00:54 IST | 2026-09-13 04:54 IST | **RELEASED — DOCUMENTATION CONTENT COMPLETE** (2026-09-13 01:36 IST; master, 129-identity register and source review complete, independent corrections incorporated. Only local validation/evidence and paired close finalization remain; no implementation or canonical adoption.) |
| MADHAV-PRODUCT-V3-20260912 | PRODUCT STRATEGY / Codex | Owner-authorized final product definition v3.0, selected product antecedents, canonical registration and governance-only local commit in /private/tmp/madhav-product-final.FqznR2/publication on codex/madhav-product-definition-v3. Native explicitly approved the one-time documentation-only actual managed-profile exception; no validator changes or invented PASS. No application code, campaign dispatch, DB, migration, production deploy/rebuild, credentials, foreign worktree changes, product-branch push or merge. This documentation-scope record grants no exclusive production lease. | 2026-09-12 23:30 IST | 2026-09-13 03:30 IST | **RELEASED — DOCUMENTATION CONTENT COMPLETE** (2026-09-12 23:58 IST; independent content review incorporated; only local evidence, validated close record and authorized local documentation commit remain. No application/production action or product push/merge.) |
| MADHAV-PRODUCT-MASTER-20260912 | PRODUCT STRATEGY / Codex | Save and locally commit owner-directed master product definition v2.2, companion plan and their strategic history; register canonical references, CCD adoption and session records in owned worktree /Users/Dev/.codex/worktrees/0ee2/Madhav. Documentation only. No application, campaign dispatch, DB, migration, deployment, policy-gate bypass, credentials, foreign-worktree edits or product-branch push/merge. Older ACTIVE rows are past their stated expiry; none is revived. | 2026-09-12 06:43 IST | 2026-09-12 10:43 IST | **RELEASED — HANDSHAKE-BLOCKED** (2026-09-12 06:44 IST; actual desktop session cannot attest madhav-safe/madhav-parity; validator returned handshake_codex_profile_invalid. No product-file, canonical-register or session-log changes made by this save request; no product commit. Drafts preserved; owner asked for a documentation-only exception. Claim commit 1a6e87ab49c115211a08853791e5f548018b6dce.) |
...
| PARISESA-PREFLIGHT-AUTHORITY | PARIŚEṢA / Codex | **Narrow Phase −1 only:** validated session provenance, temporary preflight receipt/tracker canary, and one append-only CCD bootstrap entry. Explicitly excludes application code, data, migrations, deployment, rebuilds, production writes, legacy-process control, or campaign-state takeover. | 2026-08-17 07:05 IST | 2026-08-17 11:05 IST | **RELEASED — PRECHECK-BLOCKED** (2026-08-17 07:07 IST; session-open validator found canonical MACRO_PLAN fingerprint drift; no takeover or other campaign action occurred) |
| PARISESA-PREFLIGHT-AUTHORITY-2 | PARIŚEṢA / Codex | **Narrow governed reconciliation only:** independently review the MACRO_PLAN v2.0→v2.1 provenance; if earned, rotate its canonical record and append CCD evidence through a reviewed PR. Explicitly excludes app/data/migration/deploy/rebuild/production/legacy-process/campaign-takeover actions. | 2026-08-17 07:20 IST | 2026-08-17 11:20 IST | **RELEASED — GOVERNED-RECONCILIATION-COMPLETE** (2026-08-17 08:36 IST; PR #1317 merged through the required queue as `8ee2c7d6774a9599bc5aa0b7c423b8faf5b8b153`; origin/main macro SHA verified `8e98ad46d7f0ba5ee4a9605f17f8ef21ba6da6d126092f7e0c52d318bc9e6c6e`) |
| PARISESA-PREFLIGHT-AUTHORITY-3 | PARIŚEṢA / Codex | **Narrow Phase −1 continuation only:** revalidate the resolved canonical provenance handshake, refresh temporary receipt/session evidence, execute remaining preflight gates, and append the one runbook-authorized bootstrap CCD entry through the governed PR path. Explicitly excludes application code, data, migrations, deployment, rebuilds, production writes, legacy-process control, campaign-state takeover, tracker publication, or worker-pool activation. | 2026-08-17 08:37 IST | 2026-08-17 12:37 IST | **RELEASED — PHASE-MINUS-1-COMPLETE** (2026-08-17 10:08 IST; validated close checklist 0 violations; CCD-006 merged and hash-verified; tracker freshness canary passed; no takeover or production-affecting action occurred) |
| PARISESA-TAKEOVER-PENDING-1 | PARIŚEṢA / Codex | **Phase 1 ACTIVE fenced ownership:** legacy dispatcher identity `ABSENT`; stale inflight records name PIDs 47226, 49262, 46099, 46800, 47153, 47571, 47982, each individually absent in two stable snapshots. State fingerprint `db27b1a4d1a7e1e493eb0a139975d824ca5d236bcae922c1eded2c6ea14b3d27`; run ID `PARISESA-CODEX-20260817`; continuation epoch `1`; fencing generation `1`; owner task `01a00d47-fa78-7252-97d4-3f75d539e392`; heartbeat TTL `15m`; last committed event sequence `2`; scope: audit and control-plane recovery only. Protected deploy/rebuild/DB actions remain blocked by applicable leases. | 2026-08-17 10:22 IST | 2026-08-17 10:52 IST | **ACTIVE** |
| PARISESA-TAKEOVER-PENDING-2 | PARIŚEṢA / Codex | **Phase 1 ACTIVE fenced reacquisition:** generation-1 expired and carries no authority. Two fresh stable snapshots retained state `17cb45fbb7e8ec32e1d064551600e61b05b0582ce9c51e910849d0eacbf64d94`, results `a697470dc6adf9defbaceea3ec373523a4832096bdb5c7687efe134a93b26916`, with legacy dispatcher and all seven recorded worker PIDs absent. Run ID `PARISESA-CODEX-20260817`; continuation epoch `2`; fencing generation `2`; owner task `01a00d47-fa78-7252-97d4-3f75d539e392`; heartbeat TTL `15m`; matching local intent and deterministic bootstrap validated at chained event `6/99f4bef14a93cf5cf6b68a76e60b69463aec0894812ff1ac87fd64166b46561a`. Scope: audit/control-plane recovery and read-only forensic reconciliation only. Protected deploy/rebuild/DB actions remain blocked by applicable leases. Lease renewed by remote CAS before the safety margin; local renewal evidence follows in the fenced event journal. | 2026-08-17 12:20 IST | 2026-08-17 12:40 IST | **ACTIVE** |
| PARISESA-TAKEOVER-PENDING-3 | PARIŚEṢA / Codex | **Phase 1 ACTIVE fenced reacquisition:** generation-2 expired before renewal and carries no authority. Two stable snapshots retain state `17cb45fbb7e8ec32e1d064551600e61b05b0582ce9c51e910849d0eacbf64d94`, results `a697470dc6adf9defbaceea3ec373523a4832096bdb5c7687efe134a93b26916`, and no legacy dispatcher or recorded worker PID. Run ID `PARISESA-CODEX-20260817`; continuation epoch `3`; fencing generation `3`; owner task `01a00d47-fa78-7252-97d4-3f75d539e392`; heartbeat TTL `15m`; local intent revalidated at chained event `11/4fa16312f28bdf1656def12c71a9f1ad69de387219d2463d92251980c777de13`. Scope: audit/control-plane recovery and read-only forensic reconciliation only; protected deploy/rebuild/DB actions remain blocked by applicable leases. Lease renewed by remote CAS before the safety margin; local renewal evidence follows in the fenced event journal. | 2026-08-17 12:50 IST | 2026-08-17 15:13 IST | **ACTIVE** |
...
| NIRMANA-ELEVATION-20260826-1 | NIRMĀṆA ELEVATION / Codex | Autonomous governed continuation of the Nirmāṇa Elevation campaign only: reconcile and correct its frozen asset/DAG definition; land protected PRs through required CI/merge queue; deploy and verify Nirmāṇa migrations, services, tracker, and release provenance; record evidence through the sole governed ingress; and dispatch at most the exact gate-authorized Nirmāṇa build/rebuild wave. Explicitly excludes credential changes, branch-protection or safety-gate bypass, legacy cockpit/Paripraśna surfaces, and unrelated campaigns. | 2026-08-26 09:49 IST | 2026-08-26 13:49 IST | **ACTIVE** |
| NIRMANA-ELEVATION-20260826-2 | NIRMĀṆA ELEVATION / Codex | Renewal of NIRMANA-ELEVATION-20260826-1 with unchanged campaign-only scope after the corrected 128-asset definition, protected release chain, and canonical tracker were verified. Current work is L0 read-only asset analysis and isolated correctness remediation; no accepted rebuild has started. Same exclusions remain: no credential changes, protection bypass, legacy cockpit/Paripraśna surface, or unrelated campaign work. | 2026-08-26 13:12 IST | 2026-08-26 17:12 IST | **ACTIVE** (supersedes the predecessor at renewal; exact scope unchanged) |
| NIRMANA-ELEVATION-20260829-1 | NIRMĀṆA ELEVATION / Codex | Protected integration and release of PR #1656 only: migration 636 plus least-privilege immutable monitor-observation reads, normal merge queue and CI, deployment/migration verification, fresh exact release-sync observation, and one receipt-reconciled supersession attempt. No asset rebuild, L1+ work, credential change, protection bypass, or unrelated campaign mutation. | 2026-08-29 06:50 IST | 2026-08-29 09:50 IST | **RELEASED 2026-08-29 07:38 IST** — PR #1656 merged as `3d4af078277ea4930c754689389787bd522b7582`; migration 636 applied; `amjis-web-01790-mq5` serves 100% at the same SHA; fresh eligible observation `82e64cb7-e0f4-4be4-bdd5-608dfe0b12ee` has zero accepted receipts. Supersession was not attempted because Chrome requires platform-enforced action-time consent; Goal paused pending that exact consent. |
| NIRMANA-ELEVATION-20260829-2 | NIRMĀṆA ELEVATION / Codex | Resume after explicit action-time consent: reconcile current protected main/production and a fresh tracker observation; query accepted receipts; submit at most one exact authenticated definition supersession; verify its definitive server receipt; refresh the accepted definition/labels/stage spine; and dispatch only the first genuinely eligible L0 action through an isolated worktree. No L1+ work, blind retry, credential change, protection bypass, shared-checkout mutation, or unrelated campaign mutation. | 2026-08-29 17:03 IST | 2026-08-29 20:03 IST | **ACTIVE** (fresh fence after `NIRMANA-ELEVATION-20260829-1` was released; release on supersession/first-dispatch checkpoint or any safe pause) |
| NIRMANA-ELEVATION-20260830-1 | NIRMĀṆA ELEVATION / Codex | T0 autonomy-foundation lane only: implement and release, through normal protected controls, a dedicated non-browser Nirmāṇa conductor with new least-privilege workload identities, reviewed IaC, durable lease/fence enforcement, server-side allowlisted action policy, idempotency, audit receipts, expiry/revocation, and unattended-readiness tests. Read-only campaign reconciliation is allowed. No L0 asset lifecycle mutation, F0 acceptance, definition supersession, direct SQL, credential recovery/copying, browser/personal/monitor identity reuse, safety-gate bypass, shared-checkout mutation, or L1+ work until the deployed readiness contract proves every required action path. | 2026-08-30 10:40 IST | 2026-08-30 18:40 IST | **ACTIVE** (new fence; `NIRMANA-ELEVATION-20260829-2` expired 2026-08-29 20:03 IST and is not revived) |
...
| 902–903 | NIRMANA | 902_nirmana_l1_ga_condition_integrity_check_scope.sql, 903_nirmana_evidence_ingress_writer_l1_frontier_grants.sql | MERGED — PR #2390 (L1 W4), landed on `main` ~2026-09-07T21:11:28Z. Collided with this table's stale 902 claim below (both independently claimed 902) — #2390 landed first via merge queue; the sentinel PR renumbered around it (see 905/911 rows). |
| 905–907 | NIRMANA | 905_nirmana_l2_bo_nakshatra_semantic_output_digest_spec.sql, 906_..._bo_special_lagna_output_digest_spec.sql, 907_..._bo_vargottama_dhana_output_digest_spec.sql | CLAIMED — PR #2396 (L2, 3 #1770-extension bo_* MSR co-writer output_digest_specs), renumbered from 902-904 (commit c30d8938, 2026-09-07T21:06:51Z) to escape the #2390 902 collision. Not yet merged (autoMergeRequest armed). |
| 908–910 | NIRMANA | 908_..._bo_nakshatra_semantic_natural_key_partition.sql, 909_..._bo_special_lagna_natural_key_partition.sql, 910_..._bo_vargottama_dhana_natural_key_partition.sql | CLAIMED — PR #2396 (L2), same PR as 905-907, added in a later commit (0ba08084, 2026-09-07T21:10:46Z). Not yet merged. |
| 911 | NIRMANA | 911_bg_sarvatobhadra_grid_freshness_sentinel.sql (asset_freshness sentinel row for bg_sarvatobhadra_grid — empty-by-ruling ADJUDICATION-11 — completes migration 553's throughput sentinel symmetrically) | CLAIMED — Conductor overnight ruling on #2393, unblocks `ka_vedha_gochara` DEP-ASSERT; PR #2395. Renumbered twice: 902→905 (commit 29f65c4a1, 2026-09-07T21:06:16Z, to escape #2390) then 905→911 (commit 846921d3f, cycle 6, to escape a second collision — PR #2396 independently renumbered to the same 905 within 35 seconds of this PR's first renumber, a genuine race between two lanes, not a stale-table error). Not yet merged. |
| 912+ | — | next free; claim here before use — **the "next free" number drifts fast under concurrent campaigns; re-verify live via `git ls-tree -r --name-only origin/main -- platform/supabase/migrations platform/migrations` PLUS open-PR migration file lists (`gh pr diff <n> --name-only \| grep migrations` for any PR with an armed auto-merge) before trusting this table — two independent 902→905 renumbers within the same minute (this table's own history, rows above) show disk+CCD alone missed an in-flight collision** | — |
| 990–997 | NIRMANA | 990/991 (L2, `bo_sangati` — renumbered from a 990/991 collision with L1's `mi_bhavisya` PR #2506), 994/995 (L1, `mi_pramana` PR #2509), 996/997 (L2, `bo_sangati` post-renumber, commit `d69b61e7d`) | MERGED — resolved without this table's help (last entry above this row was still frozen at "912+ next free" throughout). Confirmed clean on issue #1770 (cycle w403: "confirmed #2508 was already renumbered to 996/997 ... No action needed"). **This is the table's SECOND missed in-flight collision** (after 902/905/911 above) — same failure mode, same root cause (nothing updates this table between claims). |
...
**1. `data_plane_builder` cannot complete any W1 dispatch right now — missing `asset_registry` SELECT.**
Dispatched `ga_positions` for canonical chart `482012f1-710e-4a25-994a-93821f5871aa` via the real
`brahma-build-pipeline-job` path (manifest/digest verified correct first). The job's own bound
identity, `data_plane_builder` (its Cloud Run secret binding, not anything this session chose), has
**zero role memberships and no grant of any kind on `asset_registry`** — confirmed directly via
`has_table_privilege` and `pg_auth_members`, not inferred. `runner.py`'s
`_verify_registry_still_matches_manifest` (line ~345) calls `SELECT ... FROM asset_registry` before
any write and raises `psycopg.errors.InsufficientPrivilege`. A second, separate defect compounds
this: the surrounding `try/except ValueError` in `execute_run` (line ~1035) does not catch
`psycopg.Error`, so the exception propagates uncaught and `build_runs` is left at `state='planned'`
indefinitely instead of being terminalized — an orphan watchdog eventually cleaned this one up
(`state='failed'`, `orphan-watchdog: run never dispatched`), so no chart is stuck blocked, but the
underlying gap is real and structural, not transient. **This blocks W1 for any asset, any chart,
until `data_plane_builder` is granted `SELECT` on `asset_registry`** (or membership in a role that
has it, e.g. `role_orchestrator`). Not something this L3 session can or should fix itself (IAM/DB
grants are explicitly forbidden territory for this conductor) — recording it here since it is
shared build infrastructure both campaigns depend on. Native/Codex: this is a one-line GRANT away
from unblocked.

**2. The shared capability-registry content hash couples unrelated campaigns' CI.** Fixing an L3
service-wrapper description default (`call_dasha_eligibility`'s `ayanamsha_id`, PR #2695) shifted
`stableFingerprint()` over the full live capability catalog, which broke two Pūrṇa-owned golden-
baseline tests that pin that fingerprint byte-for-byte:
`tests/pariprashna/route_ports/route_golden_stream.test.ts` and
`src/lib/vidhi/inquiry/beyond_acarya_acceptance.test.ts`. Both are required checks (`Unit Tests` is
in the merge-queue ruleset), so **PR #2695 is currently merge-queue-blocked by this, not by any
defect in it** — verified clean on a bare `origin/main` checkout before the PR's changes, and
verified it is exactly these two files failing and nothing else. This L3 session will not touch
either file (Pūrṇa territory) or weaken the required check. Whoever next regenerates those two
golden baselines on the Pūrṇa side will clear it — and the same coupling will recur for the next
unrelated registry-content change either campaign makes, so it may be worth a `[write-mode
'refresh']`-style pattern or a narrower fingerprint scope on Pūrṇa's side at some point. Auto-merge
is left enabled on #2695 so it lands on its own once this clears.

### 2026-09-12 23:58 IST — PRODUCT STRATEGY v3 documentation release

MADHAV-PRODUCT-V3-20260912 releases its documentation-only scope after completing the integrated master, historical preservation and independent substantive review. Canonical registration is prepared on codex/madhav-product-definition-v3 from protected base 731e311f0b8f5f84db2f152b93951e1d3d50d89a. Local close-evidence finalization and the native-authorized local commit follow this remotely verified release; there is no product push/merge/deploy. No other campaign or coordination entry was changed.

### 2026-09-12 23:30 IST — PRODUCT STRATEGY v3 documentation lease

Native authorized the previously requested one-time documentation-only managed-profile exception ("Yes, authorized."). New scope MADHAV-PRODUCT-V3-20260912 records the final master and narrow governance in an isolated fresh-main worktree. The earlier MADHAV-PRODUCT-MASTER-20260912 lease remains RELEASED — HANDSHAKE-BLOCKED; no retroactive success is asserted. Other campaigns and their production leases are unaffected.
...
- 2026-08-19 13:16Z — **PARIŚEṢA / Codex F90 service proof:** affected MCP revision `amjis-mcp-00574-c72` is 100% traffic and labeled `bc739092`; read-only POST and GET invalid-input canaries both return generic sidecar-unavailable errors with no backend marker. MCP build/deploy/smoke/promotion passed; unrelated Web workflow promotion remains in progress. No write action.

- 2026-08-19 13:19Z — **PARIŚEṢA / Codex Velocity train 1:** F46/PR #1342 is refreshed to independently renewed head `3f4193ca`; F47/PR #1343 is refreshed to independently reviewed corrective head `05f0b482` after a per-window dasha-boundary fix. Both require fresh protected PR contexts; no queue, bypass, deploy, rebuild, or DB action.

- 2026-08-19 13:27Z — **PARIŚEṢA / Codex fallback proof:** normal V4 state writer remains guardian-unavailable, so the authorized own-worktree ledger records read-only current-catalog receipts for F92/F100/F101: repeated `judgment_query` career-v3 calls are stable on verdict/receipt fields; timing is anchored only with served timing rows/activations; empty served D10 evidence is honestly `varga_confirmed:false` with its forced-false disclosure. No session selection, job, adoption, rebuild, or DB action.

- 2026-08-19 13:29Z — **PARIŚEṢA / Codex Ganga admission:** F46 PR #1342 exact head `3f4193ca` is CLEAN on base `bc739092` with all five required contexts SUCCESS, an independently renewed PASS, and a direct rollback. Normal auto-merge admission is authorized; F47 remains outside the queue while its governance context is pending. No bypass, deploy, rebuild, or DB action.
...
- 2026-08-19 14:27Z — **PARIŚEṢA / Codex F50 gate baseline:** normal V4 writer remains guardian-unavailable, so the authorized fallback ledger records exact `45f0ddf567→9b3beb6dc` candidate/parent evidence. F50 review renewal and focused regression are PASS; the local broad TypeScript command has the same four unchanged missing-declaration errors on parent and candidate (identical output hash), so it remains documented known-red rather than waived. Normal protected PR checks remain required; no deploy, rebuild, or DB action.

- 2026-08-19 14:29Z — **PARIŚEṢA / Codex F50 PR:** normal protected PR #1347 opened at immutable reviewed head `9b3beb6dc` against `45f0ddf567`; required contexts are running and no auto-merge, bypass, deploy, rebuild, or DB action has occurred.

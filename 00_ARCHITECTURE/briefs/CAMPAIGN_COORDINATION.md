...
|---|---|---|---|---|---|
| MADHAV-DATA-PLANE-L3-DP047-ROUTINE-SCHEMA-BOUNDARY-20260918 | Execution — Data Plane / Codex | **Exclusive protected automatic-release lease for exact `main` `a5c7dde5c0f2dc7a53377c9609473ff9ab86671f` (PR #2670).** Owns only the already-triggered protected-main CI, its ordinary workflow-run deployment if CI succeeds, and read-only postflight. The source removes an unsafe ordinary-public-schema repair path, makes regular migration tracking reuse the established `_migrations_applied` table without a `CREATE TABLE IF NOT EXISTS`, and converts unapplied migration 1041 to a tracked `SELECT 1` compatibility boundary. No manual dispatch, backup/restore reuse, ownership/role/IAM/secret mutation, data build/rebuild, service-consumer claim, terminal asset acceptance, L4/L5 work, empirical claim, or change to retired `ka_gochara_sweep` is authorized by this lease. Stop and release on any exact-SHA drift, failed CI/deploy/preflight, competing live lease, rollback ambiguity, or unexpected production impact. | 2026-09-18 21:58 IST | 2026-09-18 23:00 IST | **ACTIVE — protected-main CI `35368302580` live; automatic deployment pending its success** |

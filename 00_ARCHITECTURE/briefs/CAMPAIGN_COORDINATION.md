...
|---|---|---|---|---|---|
| MADHAV-PURNA-DELIVERY-OWNER-USAGE-SUCCESSOR-20260919 | PŪRṆA ANVEṢAṆA / Codex | **Protected-owner runtime-schema successor lease bound only to protected `main` `449dde8305db6d532ddfa939748641ad61e9a7a2` (PR #2689).** Exact-main force-all run `35458376340` under the predecessor lease passed the sidecar, DB inspection, routine migration and strict ownership gates, then the web zero-traffic candidate again failed the non-vacuous Pūrṇa database canary before promotion, now with PostgreSQL `42501`; MCP remained no-traffic and the run was force-cancelled. Direct serving-login reproduction proved the caller now had public-schema USAGE, but both `SECURITY DEFINER` entry points failed because their normalized owner `purna_inquiry_owner` lacked public-schema USAGE. PR #2689 preserves durable owner USAGE while revoking CREATE, grants/attests USAGE-only runtime capability for both owner and serving role through the existing `data_plane_migrator -> data_plane_schema_owner` edge, and makes marked state plus the canary reject owner-visibility regression; local tests passed 12,383/12,383 and all PR/merge-group checks passed. This lease owns only: if the protected environment's non-readable migrator URL prevents local use of the existing route, one no-disclosure `data_plane_migrator` password rotation plus immediate refresh of the exact protected environment secret; execute the exact merged runtime-USAGE repair once; re-attest strict data-plane plus marked Nirmāṇa/Pūrṇa state and absent one-shot authority; observe exact-main automatic outcome; one exact force-all dispatch; web/MCP/sidecar/pipeline candidate smoke/promotion; actual ready revision, traffic, deployed SHA, pipeline image, migration 1040/1041 and bounded regression proof; write the migration-barrier decision, campaign events and release. Preserve all migrations and data, strict ownership/isolation, L3/Kāla, retired `ka_gochara_sweep`, WriterBase and L4/L5 holds. No privileged bootstrap recreation, ruleset bypass, broad IAM, CREATE grant, owner membership, destructive rebuild, feature work, secret disclosure or empirical/product-acceptance claim. Stop on tip drift, repair/attestation failure, failed exact-main CI/deploy/smoke, rollback ambiguity or unexpected impact. | 2026-09-19 23:54 IST | 2026-09-20 04:00 IST | **ACTIVE — EXACT-MERGED OWNER/SERVING USAGE-ONLY REPAIR, FORCE-ALL SERVICE PROOF, BARRIER DECISION AND RELEASE** |

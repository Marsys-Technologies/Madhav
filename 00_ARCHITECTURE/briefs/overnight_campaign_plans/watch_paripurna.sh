...
if ! pgrep -f "run_paripurna.sh" >/dev/null 2>&1; then
  echo "⚠ paripurna supervisor NOT running. Start:"
  echo "  nohup caffeinate -i /Users/Dev/shad_overnight/run_paripurna.sh </dev/null >/dev/null 2>&1 & disown"
  exit 1
fi
...
while true; do
  ...
  if [ "$NEWEST" != "$CURRENT" ]; then
    CURRENT="$NEWEST"
    echo
    echo "── [paripurna] attached: $(basename "$CURRENT")  ($(date +%H:%M:%S))  — Ctrl-C to stop watching ──"
  fi

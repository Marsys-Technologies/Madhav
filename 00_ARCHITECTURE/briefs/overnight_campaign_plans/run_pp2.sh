...
# LAUNCH: nohup caffeinate -i /Users/Dev/shad_overnight/run_pp2.sh </dev/null >/dev/null 2>&1 & disown
...
WT="$REPO/.claude/worktrees/pp2-conductor"
BR="paripurna2/audit"
LEDGER_REL="00_ARCHITECTURE/briefs/paripurna2/PP2_STATE.md"
COMMON="/Users/Dev/shad_overnight/sm_common_rails.md"
KICK="/Users/Dev/shad_overnight/pp2_kickoff.md"
...
while [ "$(date +%s)" -lt "$END_TS" ]; do
  ...
  if awk -v t="$TOTAL_COST" 'BEGIN{exit !(t>140 && t<=200)}'; then
  ...
  if awk -v t="$TOTAL_COST" 'BEGIN{exit !(t>200)}'; then

...
`getCatalog()` returned **167** live capabilities at generation time.

By resolved type: **tool**=161, **resource**=5, **prompt**=1.
...
**159** tool registrations (+ 6 mcp-tagged
...
- Only in the generated projection: 146 names (the granular
...
**24 / 167** catalog URIs are referenced somewhere in
`registry_bridge.ts`; **143 / 167** are not referenced
...
a MUCH larger generated MCP surface (near-1:1 with the registry, 159 tools) than
...
`machine_census.generated.json` — **167** entries, every field the registry
...
`live_tool` namespace (30 distinct names across
...
- Vidhi `live_tool` bridge: **12 / 30** resolve to a
  registry URI (before this lane's generated projection, `compiled_floor_adapter.ts`'s
  hand-curated `LIVE_TOOL_TO_RETRIEVAL` mapped only **4 / 30**).
  Unmapped: bodha_chart_digest_get, bodha_remedies_get, bodha_signals_get, ganita_ayurdaya_get, ganita_condition_get, ganita_dasha_lord_capability_get, ganita_nakshatra_get, ganita_sensitive_degrees_get, ganita_strength_get, ganita_structural_get, gochara_activation_get, gochara_election_avoidance_get, gochara_forecast_get, kala_bundle_get, kala_windows_get, ref_doshas_get, standing_predictions_read, synth_tail_divergence_get.
...
Vidhi floor-primitive mappability from 4/30 toward
12/30 without hand-editing that file.
...
**167** entries — one per live capability, no filtering. Each entry
...
- **full**: **151** `mcp_full`-tagged tools, uncapped.

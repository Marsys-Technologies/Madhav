...
ledger.
**Status:** Stage 0 CLOSED. Lane B0 CLOSED + PARĪKṢAKA-VERIFIED (PASS, see below). Lane B1
(Chart Reader) is the **NEXT ACTION** — dispatch a fresh-worktree BUILDER per rails, live-DB
recipe (§DB access below), independent PARĪKṢAKA verdict in ledger BEFORE merge, then Rung P4.

**RULING on file (native+Fable, this session):** all remaining lanes (B1–B7) go through proper
...
see below). Sequencing: one lane at a time through the gates; parallelize only where the master
plan states lanes are independent.

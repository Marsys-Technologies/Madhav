...
| **DD-17** | Receipt/waiver audit — 100% `interpretation_sets` waiver rate, blocked on model-spend policy (added 2026-08-20, PARIPRASHNA-P2-GATE-BATTERY session; RULED 2026-08-20, PARIPRASHNA-DD16-DD17-EXECUTE session) | **Status: RULED, tier upgraded, awaiting live post-deploy confirmation before closure.** Receipts are structurally audit-clean (no fabrication: `evidence_grades`, `facts_consumed`, `derivation_chains[].fact_refs` all honestly empty, `hallucination_count: 0`, consistent with the same disclosed namespace gap in DD-15). Re-queried directly against production for this ruling (broader sample than the original 2-turn finding): **all 7 real `acharya_reading_receipt` rows in the database as of this session** (every one from today, 2026-08-20, all test/harness traffic — zero organic native usage yet) show `interpretation_sets.status = "measured"` with **16/16 detected significant judgments waived**, every one `waiver_reason: "model produced no entry for this judgment_id"` — never the "call failed" or "structural validation failed" waiver strings, which only fire on different, distinct failure paths. This pins the failure to one specific point: the real LLM call (`interpretation/worker.ts`'s `generateInterpretationSets` → `runAdapter`) succeeds and returns parseable JSON, but the returned `sets` array is empty or non-matching. Root-cause hypothesis: `callType: 'worker'` resolved via `DEFAULT_STACK_ID = 'gemini'` to `gemini-2.5-flash-lite`, the stack's cheapest/fastest tier (registry's own hint: "planner + quick answers"), asked — with `reasoning: 'disable'` — to produce ≥3 genuinely distinct interpretive candidates plus a real falsifier per judgment, a materially harder structured-reasoning task than the tier's established use case. Cost estimate for the tier upgrade (reported to the native, not committed to this register): current tier ~$0.0000454/call, `mid` tier ~$0.0002269/call, delta ~$0.0001815/call (5×) — negligible in absolute terms at any realistic single-native volume. **Native ruled: upgrade this call site only.** `interpretation/worker.ts`'s model resolution changed from `getEffectiveModel(DEFAULT_STACK_ID, 'worker', 'primary')` (shared with `summaries/worker.ts`'s title/summarization calls) to a locally-resolved, registry-validated constant pointing at `gemini-2.5-flash` (`mid` tier) — deliberately NOT done via `STACK_ROUTING`/a new `CallType`, so no other call site's model changes. 3 new regression tests added (`worker.test.ts`) pinning the resolved model to `gemini-2.5-flash`/tier=mid and guarding against a silent revert to the `-lite` tier. Full interpretation suite 32/32, full pariprashna suite 1973/1973. **Closure is deliberately deferred to a follow-up entry, per the native's own instruction — "only close DD-17 with live evidence, not on the tier change alone."** This entry will be amended in place once the deploy is confirmed live and a real probe turn shows the waiver rate actually drop. |

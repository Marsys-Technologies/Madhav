...
| **DD-16** | Crash-kill/outbox recovery — untestable in production, blocked on a migration decision (added 2026-08-20, PARIPRASHNA-P2-GATE-BATTERY session) | **Status: OWNED — UNDATED, blocked on native migration ruling.** The "visible incomplete state" half of the gate is already real and covered: `platform/tests/pariprashna/reducer/golden.test.ts` + the `disconnect-mid-block.json` fixture exercise the REAL reducer against a truncated event stream and assert `block.committed === false`/`turn_close === null` — not simulated, not mocked reducer logic. The "outbox recovers" half cannot be honestly marked PASS in production today: `platform/src/lib/pariprashna/store/durable_outbox.ts` / `durable_writer.ts` (P2-D, PPR-10/FD-9) are fully built and have 11 passing unit tests against a fake `OutboxDb` port, but `MARSYS_FLAG_PARIPRASHNA_DURABLE_PERSISTENCE_ENABLED` is off in production and its migration was never opened (confirmed: no `pariprashna_persistence_outbox`-named or -shaped migration exists anywhere in git history). **What the migration would change** — the exact schema is already fully specified in `durable_outbox.ts`'s own module header (not newly designed for this entry): one NEW table, `pariprashna_persistence_outbox` (`id bigserial PK`, `message_id uuid NOT NULL UNIQUE`, `conversation_id uuid NOT NULL`, `chart_id uuid NOT NULL`, `turn_id text NOT NULL`, `payload jsonb NOT NULL`, `status text` CHECK'd to `pending/applied/failed`, `attempts integer`, `enqueued_at/claimed_at/applied_at timestamptz`, `last_error text`, one CHECK constraint tying `applied`⇒no-error) + 2 indexes (`pending`-status partial index, `chart_id` index) + a CREATE-not-ENABLE RLS policy (`chart_id = app_chart_context()`, deferred to the same arm-1 cutover as migration 576's `pariprashna_ledger_outbox` precedent, which this table's shape directly mirrors). **Additive only** — CREATE TABLE + CREATE INDEX, zero `ALTER`/`DROP` on any existing table, zero backfill, zero data migration; nothing existing is touched. **What the currently-untestable half actually verifies once wired:** `replayPendingPersistence(db, applyFn, limit)` claims pending outbox rows (`claimPendingPersistenceEntries`), calls the real persistence function (`writeTurn`) for each, marks `applied`/`failed`, and is idempotent-by-construction (`writeTurn` itself upserts by message id, so replaying the same row twice converges, never duplicates) — this closes the specific window between "the SSE stream told the reader `turn.close`" and "the write actually committed," the one gap a synchronous direct write cannot survive (process death, container eviction, deploy-mid-request). **Blast radius if something goes wrong mid-migration:** low. The migration runner is transactional and loud-failing by this project's own standing doctrine (CLAUDE.md §N.4, DVA Ruling 58 — "migrate.ts... transactional, tracked, loud-failing" is the established, re-confirmed characterization of the deploy-time runner), so a failure mid-DDL rolls back cleanly with no partial state; even in the worst case (an orphaned empty table needing manual cleanup) nothing existing is at risk, since no existing table/column/row is touched. And even after the migration lands, the table sits empty and inert with zero runtime effect until `MARSYS_FLAG_PARIPRASHNA_DURABLE_PERSISTENCE_ENABLED` is separately flipped — landing the migration and flipping the flag are two independent, sequenceable decisions. Awaiting native ruling on whether/when to open the migration; no deadline set by this session. |
| **DD-17** | Receipt/waiver audit — 100% `interpretation_sets` waiver rate, blocked on model-spend policy (added 2026-08-20, PARIPRASHNA-P2-GATE-BATTERY session) | **Status: OWNED — UNDATED, blocked on native model-spend ruling.** Receipts are structurally audit-clean (no fabrication: `evidence_grades`, `facts_consumed`, `derivation_chains[].fact_refs` all honestly empty, `hallucination_count: 0`, consistent with the same disclosed namespace gap in DD-15). Re-queried directly against production for this closure (broader sample than the original 2-turn finding): **all 7 real `acharya_reading_receipt` rows in the database as of this session** (every one from today, 2026-08-20, all test/harness traffic — zero organic native usage yet) show `interpretation_sets.status = "measured"` with **16/16 detected significant judgments waived**, every one `waiver_reason: "model produced no entry for this judgment_id"` — never the "call failed" or "structural validation failed" waiver strings, which only fire on different, distinct failure paths. This pins the failure to one specific point: the real LLM call (`interpretation/worker.ts`'s `generateInterpretationSets` → `runAdapter`) succeeds and returns parseable JSON, but the returned `sets` array is empty or non-matching. **Root-cause hypothesis, well-evidenced but not proven** (no raw model output is logged for this call site — see Step 3 finding below): `callType: 'worker'` resolves via `DEFAULT_STACK_ID = 'gemini'` to `gemini-2.5-flash-lite`, the stack's cheapest/fastest tier (registry's own hint: "planner + quick answers"), asked — with `reasoning: 'disable'` — to produce ≥3 genuinely distinct interpretive candidates plus a real falsifier per judgment, a materially harder structured-reasoning task than the tier's established use case. **New finding this pass:** this call site is not present AT ALL in `llm_usage_events` (the project's own LLM observability/cost table) for the entire session window — `planner`/`title` calls from the same turns ARE logged there, `worker`-tier interpretation-sets calls are not, for reasons not yet investigated (the `QueryRequest` built in `worker.ts` never sets a `pipeline_stage`). This is a second, independent gap (no cost/usage visibility into this call site at all) alongside the waiver-rate symptom, filed together since both point at the same call site. **Cost estimate for the candidate fix (tier upgrade): see Step 3 / a separate cost note, not committed to this register.** No model-tier change made. No flag change made. Awaiting native ruling on the spend tradeoff; no deadline set by this session. |

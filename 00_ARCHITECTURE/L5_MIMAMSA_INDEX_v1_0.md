...
| 5j | `L5_BUILD_READINESS_MANIFEST_v1_0.md` | CURRENT | **THE EXECUTABLE READINESS PLAN** — everything required for press-Build to generate the ENTIRE L5 dataset, as 9 workstreams W0–W9 (preconditions · migrations · registry · WRITERS · deploy · retrieval · serve · cockpit · tests/seal · pre-flight), each a checklist of concrete verifiable items. Execute the boxes; when W9 pre-flight is all-green, build runs clean. The missing writers + every gap appear as explicit line items |
| 5i | `L5_BUILD_READINESS_AUDIT_v1_0.md` | CURRENT | **Code-grounded "will press-Build run clean?" audit** — 2 BLOCKERS (depends_on used table-names not asset-ids → `ph_pramana`/`ph_nimitta`/`ph_phaladesa`; 6 assets missing from seed + live mi_bhavisya bug) + 12 must-fixes (count_sql/scope/services-not-in-spine/clean-imports/migrations-first/pgvector/health-probes…). Verified-acyclic DAG, the seed block to apply (P3.5), and the pre-build pre-flight (P3.7). All P3 work, no orchestrator change |
| 5h | `L5_SPECS/` (spec pack: index + matrix + 11 spec files) | CURRENT | **THE BUILD-READY PER-ASSET SPEC PACK** — 13 assets fully specified (schemas + logic + retrievability + seal gates + frozen-contract), one file per asset, with the traceability matrix + zero-orphan coverage audit. Every G1–G7, R1–R6, decision, gate verified present. The retrieval layer (R1–R6) = `mi_darshana`; insight engines (G1–G7) = `mi_pariksha`+`mi_sambandha`. This is what an Antigravity build session consumes |
| 5g | `L5_SUPERHUMAN_INSIGHT_AND_RETRIEVABILITY_GAPS_v1_0.md` | DRAFT | **The fresh-eyes step-back** — is L5 generating superhuman insight + built for LLM synthesis? 7 insight gaps (G1–G7: emergent-law mining, manifestation grammar, load-bearing maps, contradiction-dominance, temporal rhythms, family-interaction, residual discovery) + 6 retrievability gaps (R1–R6: insight surface, embeddings, query-views, provenance chain, trust-metadata, negative knowledge). Expansion not redesign |
| 5f | `L5_DESIGN_VS_LIVE_INSTRUMENT_CROSSCHECK_v1_0.md` | CURRENT | **Cross-check vs the live L0–L4 instrument** (real sample data) — confirms ph_pramana→L5 contract; instrument already shares L5's staged/honest-ceiling discipline; the degenerate-distribution guard is proven systemic (≥6 assets); upstream-maturity dependencies noted. No design changes required |

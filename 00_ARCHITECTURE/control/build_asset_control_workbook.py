...
def scan_code():
    ...
    for p in sidecar.rglob('*.py'):
        ...
        if not ids:
            continue
        blob, has_plan = txt, 'def plan_substeps' in txt
        ...
        if svc and svc.exists():
            for q in svc.rglob('*.py'):
                try:
                    ...
                    pass
            has_plan = has_plan or 'def plan_substeps' in blob
        resume = 'none'
        ...
        for a in ids:
            ...
            out[a] = {'writer_file': str(p.relative_to(ROOT)),
                      'code_has_plan_substeps': has_plan or prev.get('code_has_plan_substeps', False),

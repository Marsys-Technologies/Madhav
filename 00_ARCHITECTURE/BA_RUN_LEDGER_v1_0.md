...
## BA Phase-4 Runway — W4 THE ONE SHOT (native 482012f1 rebuild) — 2026-07-08

- **NATIVE GO (explicit, ledger-recorded):** the native issued an explicit "Go for W4" in-session 2026-07-08.
  Per brief v1.1 (silence never authorizes), this is the recorded authorization to fire the native rebuild.
- **Pre-rebuild SNAPSHOT (belt; clear-safety allowlist = braces):**
  - Irreplaceable LEL dumped: `w4_snapshot/life_events_native.csv` (57 rows) + `w4_snapshot/eci_native.csv`
    (57 event_chart_state_index rows). **Restore path:** `\copy life_events FROM 'life_events_native.csv' CSV HEADER`
    (and eci) after `DELETE ... WHERE chart_id='482012f1…'` — only needed if a HALT corrupts LEL (it must not:
    life_events is has_writer=false + `lel_events:null` in EXPLICIT_CLEAR_OPS, so no build/clear path touches it).
  - Pre-rebuild row-count manifest of all 61 native chart-scoped tables: `w4_snapshot/manifest_pre.txt`
    (baseline for post-rebuild anomaly detection). Rebuildable L1–L5 data is deterministically regenerated by the
    build (JL-024 principle) on the W3-frozen, Abhinandan-proven config; life_events is the only irreplaceable set.
- **W4 REBUILD — HALTED (build FAILED) 2026-07-08.** Run `69ce321e-5ef0-4ed8-8adf-d2a75d056106` (native, global,
  frozen image 6cd7f509, WORKER_LIMIT=2) → **state=failed** (22:03→22:19 UTC): 23 assets complete, 43 errored,
  ALL 43 `BLOCKED: upstream bo_laksana did not complete`. **Root failure = bo_laksana**:
  `psycopg.errors.QueryCanceled: canceling statement due to statement timeout` on the idempotency MSR-replace
  (`replace_prior_msr_for_chart` → cascade `DELETE FROM kala_darshana`), on the native's large accumulated dataset.
  Same class as the ka_* `SET LOCAL statement_timeout=0` fix (mig 422) — bo_laksana's idempotency delete lacks it.
  Abhinandan (W3, smaller/fresh) never hit it. **NOT an LEL/correctness failure and NOT the W2 work.**
- **HALT state `[verify-against: prod db]`:** native life_events=57 INTACT (no loss); Abhinandan chart_facts=140,214
  UNCHANGED (zero contamination). Native chart left INCONSISTENT: L1 chart_facts rebuilt on new code (138,380→141,261);
  L2 bodha_msr_signals PARTIAL (64,765→66,649, mixed new/old per-ayanamsha as bo_laksana rolled back mid-run);
  L3–L5 STALE (blocked, never ran). **STOPPED per HALT protocol — NO iteration on the native chart pending native decision.**
  The one-shot config has a latent bo_laksana timeout that Abhinandan could not surface; recovery requires the
  statement_timeout=0 fix + re-validation, which un-freezes the config — a native decision, not autonomous.
- **W4 RECOVERY (native-authorized 2026-07-08):** native ruled "fix + re-validate Abhinandan + re-run native."
  - **Fix #1 (HALT #1):** `bo_laksana`/bodha idempotency timeout → `SET LOCAL statement_timeout=0` on the shared
    `_delete` helper + the 7 bo_* writers with inline raw DELETEs (PR #463, image 2e96b1d0). Mirrors ka_* mig-422.
  - **Re-validate:** Abhinandan clean rebuild on 2e96b1d0 (run `87e47acf`, 66/66, 0 errors; gates green:
    structural, 10 floored graha_yuddha/0 winners, SUN=Aquarius, salience 191 distinct). Config re-frozen.
  - **HALT #2:** native re-run reached 65/66 then failed at `mi_darshana` (`TypeError: Decimal - float` on the
    native's CALIBRATED observed_rate; Abhinandan's structural NULL path never hit it). **Fix #2:** coerce DB
    Decimal→float in the insight-unit confidence math + a latent json.dumps(Decimal) sibling (PR #464, image 98a570ac).
  - Pattern: the native's calibrated / 57-LEL data path is a far more thorough test than Abhinandan's structural /
    0-LEL path — it surfaced two latent pre-existing bugs (a timeout + a type error), both tiny pure-correctness
    fixes (no computed-value change). LEL stayed 57 and Abhinandan stayed uncontaminated through every attempt.
- **W4 SUCCESS — native 482012f1 REBUILT + VALIDATED 2026-07-08.** Final run `c86ac468-30d5-4c13-8703-077770fbfd4c`
  (native, global, image 98a570ac, WORKER_LIMIT=2): **state=completed, 66/66 assets, 0 errors, LEL=57 throughout.**
  **Full gate battery — ALL PASS `[verify-against: prod db]`:**
  - **FORENSIC 7/7:** Sun=Capricorn ×5 ayanamshas · Moon=Purva Bhadrapada ×5 · Lagna=Aries ×5 · Tithi=Shukla
    Tritiya · Vara=Ravivara · Yoga=Shiva · Karana=Garaja.
  - **Contamination:** Abhinandan chart_facts=140,214 UNCHANGED (≠ native); native LEL=57 intact.
  - **LEL + calibration:** 57 rows intact; `calibration_state='calibrated'` (load_bearing=true); rectification ran
    LEL-fit (basis=lel_fit) and **STILL validates 10:43** (best candidate 1984-02-05 05:13 UTC = 10:43 IST,
    offset_minutes=0, lagna=Aries).
  - **bhava_arudha 12×5** present (210 rows / 5 ayanamshas / 14 bhavas) → **P3A deferred gate CLOSED → P3A COMPLETE.**
  - **JL-009 v1.1:** 5 spot native lift_vectors carry ontology row-normalized base_rates (career_entry 0.2469,
    spiritual_turn 0.2917, transition 0.2).
  - **JL-027:** native has 0 graha_yuddha rows (no planets within 1° orb — chart-dependent) → zero longitude proxy.
  - **Degeneracy:** min(posterior)=0.02 (<0.2), signature_tier chart_defining=1220 (>0), contradictions=5 (>0),
    salience 153 distinct (non-constant).
  - **Retrieval:** bodha_msr salience ranking-clean (153 distinct, ranked).
  - **R5 punch-list (soft, non-HALT):** `lel_training_matched=0` (LEL-fit corroboration matching to improve);
    `base_rate_source`/`age_band` not stamped in lift_vector jsonb (provenance stamping); posterior cardinality low (6).
- **JL-⟦next⟧ = JL-028:** Phase-4 executed per BA_PHASE4_RUNWAY_PLAN — native 482012f1 rebuilt L1→L5 on the final
  validated config (image 98a570ac), FORENSIC 7/7, LEL-calibrated, 10:43 re-validated, all gates green. RATIFIED
  by native GO + execution. **The BA Phase-4 Runway is CLOSED.**

...
| **Domain correctness** | The astrology is *right*, not merely well-sourced. Verified autonomously and only autonomously, by four checks that can each return false: (a) **source correspondence** — the encoded rule and the passage it cites are compared and must agree, including its prerequisites and exceptions; (b) **cross-witness agreement** — where two admitted sources define the same configuration, the encodings agree, or the disagreement is recorded as school disagreement and never silently resolved; (c) **independent re-derivation** — a classical quantity computed a second way from different inputs must match; (d) **negative cases** — the rule must NOT fire where the tradition says it does not, tested on seeded counterexamples. A component may be structurally immaculate and astrologically wrong; nothing else in this table would catch it. **Discharged above the data plane, in the reasoning layer** (native ruling, 2026-09-25): the data plane is data engineering, mathematical computation and faithful carriage — it carries the cited passage, the witness set with its recorded disagreement and the second derivation, and does not judge them (data plane §13.3 item 1, §12.2 Source carriage and reproduction). The verdict is formed where the reasoning happens; when the reasoning layer is elevated to a governing artefact, the four checks are its obligations to name, including the seeded negative case, which no data-plane layer may author for itself. |

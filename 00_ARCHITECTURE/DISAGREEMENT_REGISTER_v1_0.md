...
## DIS.027 (campaign ref: DR-14) — Timing-System Plurality (Daśā Non-Exclusivity) — RATIFIED

```yaml
finding_entry:
  logged_by: "D-4a conductor, A-6 doctrine registration, 2026-07-19"
  dr_id: DIS.027
  campaign_ref: DR-14
  status: RATIFIED (native, 2026-07-19, Cowork ARC-plan session)
  full_text: "00_ARCHITECTURE/llm_consumption_audit/briefs/doctrine_waves/DR_14_15_16_TEMPORAL_DOCTRINE_v1_0.md#DR-14"
  context: >
    Activation windows must not be served as a single-system (Vimśottarī-only) container; the
    LEL's own Chara-daśā match on EVT.2024.02.16 (Vimśottarī unremarkable there) is the evidence
    anchor motivating plurality.
  ratified_text: >
    Windows are generated by a plurality of timing systems (Vimśottarī default-not-exclusive,
    Jaimini rāśi-daśās, Tājaka/Mudda, Guru-Śani double-transit, aṣṭakavarga-threshold transits,
    Saturn/Jupiter/nodal returns, Sāde Satī). Cross-system confluence raises confidence
    (dvi-pramāṇa generalized). Per-system weights are LEARNED at D-4b; until fitted, weights serve
    as disclosed classical structural priors (`calibration_state`).
  consumed_by: [A-2 (event ontology shapes), A-4 (prospective ledger generator_class), D-5 (engine
    construction)]
```

## DIS.028 (campaign ref: DR-15) — Ensemble First-Class + Proper Scoring — RATIFIED
...
finding_entry:
  ...
  campaign_ref: DR-15
  status: RATIFIED (native, 2026-07-19, Cowork ARC-plan session)
  full_text: "00_ARCHITECTURE/llm_consumption_audit/briefs/doctrine_waves/DR_14_15_16_TEMPORAL_DOCTRINE_v1_0.md#DR-15"
  ratified_text: >
    (a) Confluence ensembles are first-class model contenders, not resolved to a single peak-logic.
    (b) Model comparison uses proper scoring (CRPS/log-score) against mirrored shuffled-birth
    controls; skill = 1 − CRPS_model/CRPS_control; hit-rate (±45d top-decile) retained as legacy
    secondary metric. (c) Every scoring loosening applies identically to controls (inherits DR-13
    mirroring). (d) Pre-registration of thresholds/event-sets/win-criteria to the ledger BEFORE any
    scoring run is mandatory; post-hoc adjustment is gate-gaming.
  consumed_by: [A-3 (controls + scoring harness, structural mirroring enforcement), A-5 (dry-run
    pre-registration requirement)]
```

## DIS.029 (campaign ref: DR-16) — Adverse-Window Disclosure (Honest-Clarity Principle) — RATIFIED
...
finding_entry:
  ...
  dr_id: DIS.029
  campaign_ref: DR-16
  status: RATIFIED (native, 2026-07-19, Cowork ARC-plan session; DR-16 review waived by native with
    honest-clarity refinement folded in)
  full_text: "00_ARCHITECTURE/llm_consumption_audit/briefs/doctrine_waves/DR_14_15_16_TEMPORAL_DOCTRINE_v1_0.md#DR-16"
  ratified_text: >
    Governs every served inauspicious/adverse window from D-5 onward: honest clarity co-equal with
    restraint (no euphemism/withholding); always probabilistic never fatalistic (no death/ruin/
    doom point-claims; uniform birth+100y sweep horizon so no infra encodes a longevity opinion);
    falsifier-bearing (adverse claims enter the prospective ledger and are scored); mitigation-paired
    (suppression + BPHS-cited remedy ship in the same payload); confidence-honest
    (calibration_state/n_observations/control-delta disclosed); tiered per MACRO_PLAN Ethical
...
```yaml
finding_entry:
  logged_by: "D-4b B-1 chunked re-run, wave/D-4b/B1-full-rerun (QUARANTINED), 2026-07-22"
  dr_id: DIS.031
  campaign_ref: DR-20
  question: >
  ...
  ratified_text: >
    ...
    (`QUARANTINE_B1_FULL_RERUN_v1_0.md`) rather than partially salvaged.
  consumed_by: [Every future scoring/bakeoff lane's harness design; CR-123
    (MARSYS_DEFECT_GAP_REGISTER_v2_0.md); the D-4b B-1 re-run superseding the quarantined branch]
```

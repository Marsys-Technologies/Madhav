...
| R-38 | **`judgment_query` receipt claims varga confirmed while serving zero varga rows** — receipt says `varga_confirmed:"D10✓"` (career) and `"D2✓"` (wealth) but `checklist.varga_confirmation.rows=[]` in both calls — the completeness receipt asserts a check the payload shows was not performed | SURF | MED-HIGH | [cowork 2026-07-11] judgment_query career+wealth, 1c826d5a | Receipt value must derive from rows actually fetched (✓ only when rows>0; else "attempted-empty") | REMEDIATED-PENDING-W4 [WP-1.5 / W1; program-wide honest envelope contract — receipts derive from rows actually fetched (trim-lies structurally impossible); deployed 2385fb62/fc84cd0d, 7/7 prod-verified 2026-07-13] [PARIŚODHANA A1 RE-VERIFICATION 2026-07-27: STILL REPRODUCES — the "7/7 prod-verified" claim above does not hold for this field today. Live `judgment_query(chart_id=482012f1-710e-4a25-994a-93821f5871aa, response_format=v3)` reprobed 2026-07-27 on THREE domains: wealth (D2) → `receipt.varga_confirmed:"D2✓"`, `checklist.varga_confirmation.rows:[]`; career (D10) → `"D10✓"`, `rows:[]`; marriage (D9, Venus DEBILITATED, `varga_term:-2.5`, `varga_moved_verdict:true`) → `"D9✓"`, `rows:[]` — the array is unconditionally empty regardless of domain or dignity strength, so this is not a neutral-dignity edge case that a stricter probe would resolve. Root cause: WP-1.5's fix (`reconcileReceiptWithTrimReport` in `platform-mcp/src/tools/registry_bridge.ts`) only downgrades the receipt when the response-budget trim_report shows the rows path was cut to zero post-trim — it does not cover the case (this one) where the capability itself (`register_d9_judgment.ts`) never populates `varga_confirmation.rows` pre-trim. Compare `enforceTimingAnchoredHonesty` (same file, R-39's fix), which DOES have exactly this "genuinely-always-empty, not-a-trim-casualty" counterpart for `timing_anchored` — no equivalent `enforceVargaConfirmedHonesty` exists. Disposition: LIVE-OPEN (reopened). Cross-ref: `POST_REMEDIATION_CONSUMPTION_REGISTER_v1_0.md` CR-2/CR-63 (same defect, same reprobe); this also means R-46's "R-38 varga rows now exist post-WP-1.5" claim below is inaccurate for the `rows` array specifically — what now exists is the separate `verdict.varga_subscores` field, a different path that the verdict formula reads directly without ever populating `checklist.varga_confirmation.rows`.] |
...
| R-41 | **`ganita_yogas_get` v3 envelope contradicts its own content** — verdict says `yogas_fired:0, doshas_fired:0` + judgment_flag `zero_rows_returned` while `content.rows` in the SAME response serves 32 rows (8 yoga_label + 22 dosha_label + 2 flags); pancha_mahapurusha narrative admits "position was not available in this response" — envelope counters read a different source than the served page | SURF | MED-HIGH | [cowork 2026-07-11] ganita_yogas_get(1c826d5a, v3) | v3 verdict counters must count the served rows; PM narrative should join graha positions it already has access to | REMEDIATED-PENDING-W4 [WP-1.5 / W1; envelope counters count the served rows (canonical envelope.ts); deployed 2385fb62/fc84cd0d, 7/7 prod-verified 2026-07-13] [PARIŚODHANA A1 RE-VERIFICATION 2026-07-27: CONFIRMED — live `ganita_yogas_get(chart_id=482012f1-710e-4a25-994a-93821f5871aa, response_format=v3)` reprobed 2026-07-27: `verdict.yogas_fired:7`/`doshas_fired:1` count only confirmed (non-catalog-only) rows; `verdict.catalog_only_rows_in_page:3` plus `content.catalog_only_note` explicitly separate the 3 catalog_only/requires_pass rows from confirmed firings; `judgment_flags` carries `catalog_only_rows_present`; both `verdict.firings_pointer` and `content.firings_pointer` point to `ganita_yoga_firings_get` (`fired_count:13`) as the firings-authoritative surface. Verdict counters now demonstrably match what content actually serves — the envelope-vs-content contradiction this row describes does not reproduce. Disposition: ALREADY-FIXED — upgrade from PENDING-W4 to CONFIRMED for this row.] |

...
**Files:**
- Create: `platform/migrations/595_nirmana_elevation_asset_labels.sql`
...
- [ ] **Step 2: Verify and reserve migration 595**
...
Expected: the guard passes and the second command prints `595`. If it does not, follow the migration-number constraint above before continuing.
...
const sql = readFileSync(join(process.cwd(), 'migrations/595_nirmana_elevation_asset_labels.sql'), 'utf8')

describe('migration 595: Nirmana elevation asset labels', () => {
...
Expected: FAIL because migration 595 does not exist.

- [ ] **Step 5: Create the migration**

Create `platform/migrations/595_nirmana_elevation_asset_labels.sql` with this structure:

```sql
-- Migration 595: immutable bilingual labels for the Nirmana elevation tracker
-- Created: 2026-08-25
...
```bash
git add platform/migrations/595_nirmana_elevation_asset_labels.sql platform/tests/unit/migrations/nirmana_elevation_asset_labels.test.ts

...
const QueryTransitEventInputSchema = z.object({
    planet: z.string().describe('The transiting planet (e.g. "Saturn", "Jupiter", "Rahu").'),
    target: z.string().describe('The transit target — a sign name (e.g. "Aquarius"), house number ("house_10"), ' +
...
export function registerQueryTransitEvent(server, getPrincipal) {
    server.tool('query_transit_event', 'What it does: Searches ephemeris_daily for specific transit events (sign ingresses, ' +
        ...
        'Example: query_transit_event({planet: "Saturn", target: "Aquarius", ' +
        'date_range: {start: "2020-01-01", end: "2025-12-31"}}) → ingress date records.', QueryTransitEventInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('query_transit_event', {
            planet: args.planet,
            target: args.target,
...
//# sourceMappingURL=query_transit_event.js.map
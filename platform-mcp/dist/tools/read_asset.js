...
// ── Tool registration ─────────────────────────────────────────────────────────
export function registerReadAsset(server, getPrincipal) {
    server.tool('read_asset', 
...
"Planetary Positions"}) → returns only the Planetary Positions section.`, 
    // ── Input schema ──────────────────────────────────────────────────────────
    {
        canonical_id: z
            .string()
            .describe(`One of: ${KNOWN_CANONICAL_IDS.join(', ')}. Case-insensitive.`),
        section: z
            .string()
            .optional()
            .describe('Optional section heading filter (e.g., "§A" or "Planetary Positions"). ' +
    ...
    async ({ canonical_id, section }) => {
        ...
        const result = await callPlatformAsset({ canonical_id, section }, principal);
...
//# sourceMappingURL=read_asset.js.map
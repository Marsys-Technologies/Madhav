...
const GetCgmSubgraphInputSchema = z.object({
    node_id: z.string().describe('Seed node for graph traversal. Examples: "SIG.MSR.234", domain node ID, or CGM entity ID.'),
    hops: z.number().int().min(1).max(5).optional().default(2).describe('Traversal depth in graph hops (default 2, max 5).'),
    edge_types: z.array(z.string()).optional().describe('Filter to specific edge types. Examples: "amplifies", "contradicts", ' +
        '"shares_ruler", "temporal_co-activation".'),
});
export function registerGetCgmSubgraph(server, getPrincipal) {
    server.tool('get_cgm_subgraph', 'What it does: Traverses the Cross-Domain Linkage Matrix (CGM) from a seed node ' +
        ...
        'Example: get_cgm_subgraph({node_id: "SIG.MSR.234", hops: 2}) → subgraph of 2-hop neighbors.', GetCgmSubgraphInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('get_cgm_subgraph', {
...
//# sourceMappingURL=get_cgm_subgraph.js.map
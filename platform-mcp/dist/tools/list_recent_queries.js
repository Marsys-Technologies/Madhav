...
// ── Tool registration ─────────────────────────────────────────────────────────
export function registerListRecentQueries(server, getPrincipal) {
    server.tool('list_recent_queries', 
...
list_recent_queries({since: "2026-05-20"}) → all calls since May 20.`, 
    // ── Input schema ──────────────────────────────────────────────────────────
    {
        ...
        since: z
            .string()
            .optional()
            .describe('ISO 8601 date string to filter results from (e.g., "2026-05-01"). ' +
            'Defaults to 7 days ago if not provided.'),
    }, 
    // ── Handler ────────────────────────────────────────────────────────────────
    async ({ limit, since }) => {
        const principal = getPrincipal();
        const result = await callPlatformRecent({ limit, since }, principal);
...
//# sourceMappingURL=list_recent_queries.js.map
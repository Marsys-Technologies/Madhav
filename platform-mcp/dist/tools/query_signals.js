...
const QuerySignalsInputSchema = z.object({
    domain: z.string().optional().describe('Jyotish domain filter. Examples: "career", "health", "relationships", "spiritual".'),
    planet: z.string().optional().describe('Filter to signals involving a specific planet.'),
    dasha_lord: z.string().optional().describe('Filter to signals activated by a specific dasha lord (e.g. "Saturn", "Ketu").'),
    min_confidence: z.number().min(0).max(1).optional().describe('Minimum confidence threshold (0.0–1.0). Returns signals at or above this level.'),
    forward_looking: z.boolean().optional().describe('If true, return only prospective (forward-looking) signals.'),
    limit: z.number().int().min(1).max(500).optional().default(50).describe('Max signals to return (default 50, max 500).'),
});
export function registerQuerySignals(server, getPrincipal) {
    server.tool('query_signals', 'What it does: Queries the MSR signal corpus (499+ astrological signals) with structured ' +
        ...
        'Output shape preview: {ok, result: {signals: MsrSignalRow[]}, trace_id, epistemics: {surgical: true}}. ' +
        'Example: query_signals({domain: "career", forward_looking: true}) → raw MSR signal rows.', QuerySignalsInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('query_signals', {
...
//# sourceMappingURL=query_signals.js.map
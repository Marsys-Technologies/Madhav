...
const LelQueryInputSchema = z.object({
    category: z.string().optional().describe('Event category filter. Examples: "career", "health", "relationship", ' +
    ...
    min_significance: z.number().min(0).max(1).optional().describe('Minimum significance threshold (0.0–1.0). Filters to events at or above this level.'),
});
export function registerLelQuery(server, getPrincipal) {
    server.tool('lel_query', 'What it does: Queries the Life Event Log (LEL) — 36 verified life events, ' +
        ...
        'Example: lel_query({category: "career", min_significance: 0.7}) → documented career events.', LelQueryInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('lel_query', {
...
//# sourceMappingURL=lel_query.js.map
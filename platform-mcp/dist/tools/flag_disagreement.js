...
const FlagDisagreementInputSchema = z.object({
    class: DisagreementClassSchema.describe('Disagreement class from DISAGREEMENT_REGISTER §1. ' +
    ...
    description: z
        .string()
        .describe('2–5 sentences: what the disagreement is, when noticed, and why it cannot be silently resolved. ' +
 ...
 */
export function registerFlagDisagreement(server, getPrincipal) {
    server.tool('flag_disagreement', 'Flag a governance disagreement to the MARSYS-JIS register. Use when you detect ' +
        ...
        'reviews entries. Returns disagreement_id for tracking.', FlagDisagreementInputSchema.shape, async (input) => {
        const principal = getPrincipal();
        const result = await callPlatformWrites('flag_disagreement', {
            entry: {
                class: input.class,
...
//# sourceMappingURL=flag_disagreement.js.map
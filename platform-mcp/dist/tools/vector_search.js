...
const DOC_TYPES = ['l1_fact', 'ucn_section', 'msr_signal', 'cdlm_cell', 'domain_report', 'rm_element'];
const VectorSearchInputSchema = z.object({
    ...
    doc_type: z.array(z.enum(DOC_TYPES)).optional().describe('Filter to specific document types. Options: l1_fact, ucn_section, msr_signal, ' +
        'cdlm_cell, domain_report, rm_element.'),
    top_k: z.number().int().min(1).max(50).optional().default(10).describe('Number of top results to return (default 10, max 50).'),
});
export function registerVectorSearch(server, getPrincipal) {
    server.tool('vector_search', 'What it does: Semantically searches the RAG chunk corpus (MSR signals, UCN, CDLM, ' +
        ...
        '→ top 5 semantically similar RAG chunks with scores.', VectorSearchInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('vector_search', {
            text: args.text,

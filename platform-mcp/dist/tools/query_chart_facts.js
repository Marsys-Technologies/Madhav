...
const QueryChartFactsInputSchema = z.object({
    category: z.string().describe('Fact category to query. Examples: "shadbala", "dignity", "nakshatra", ' +
...
export function registerQueryChartFacts(server, getPrincipal) {
    server.tool('query_chart_facts', 'What it does: Queries the 795-row chart_facts table with structured filters ' +
        ...
        'Output shape preview: {ok, result: {rows: ChartFactRow[]}, trace_id, epistemics: {surgical: true}}. ' +
        'Example: query_chart_facts({category: "shadbala", planet: "Saturn"}) → rows with strength data.', QueryChartFactsInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('query_chart_facts', {
...
//# sourceMappingURL=query_chart_facts.js.map
...
const SCHOOLS = ['parashara', 'jaimini', 'kp', 'tajaka'];
const CrossSchoolLookupInputSchema = z.object({
    claim: z.string().describe('Astrological claim to verify across schools. State as a declarative sentence: ' +
        '"Saturn in 10th house delays career until 36" or ' +
        '"Moon-Ketu conjunction in 4th house indicates maternal separation theme".'),
    schools: z.array(z.enum(SCHOOLS)).optional().describe('Schools to check. Defaults to all four: parashara, jaimini, kp, tajaka.'),
});
export function registerCrossSchoolLookup(server, getPrincipal) {
    server.tool('cross_school_lookup', 'What it does: Checks an astrological claim against Parashara, Jaimini, KP, and Tajaka ' +
        ...
        '→ per-school agree/disagree/silent with convergence score.', CrossSchoolLookupInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('cross_school_lookup', {
            claim: args.claim,
...
//# sourceMappingURL=cross_school_lookup.js.map
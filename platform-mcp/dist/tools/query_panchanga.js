...
const QueryPanchangaInputSchema = z.object({
    ...
    }).optional().describe('Observer location. Defaults to native\'s birth location (Bhubaneswar, lat 20.29, lon 85.82).'),
});
export function registerQueryPanchanga(server, getPrincipal) {
    server.tool('query_panchanga', 'What it does: Returns the 5 limbs of the Vedic day (tithi, vara, nakshatra, yoga, karana) ' +
        ...
        'Example: query_panchanga({date: "2026-05-21"}) → {tithi: "Shukla Chaturdashi", ...}', QueryPanchangaInputSchema.shape, async (args) => {
        const principal = getPrincipal();
        const { status, envelope } = await callPlatformPrimitive('query_panchanga', {
            date: args.date,
...
//# sourceMappingURL=query_panchanga.js.map
...
describe('managed Prashna worker deadline hierarchy', () => {
  it('preserves the worker hierarchy in the reviewed Cloud Run runtime contract', () => {
    ...
    expect(deployWorkflow).toContain(
      'INQUIRY_LIFECYCLE_SIGNING_KEY_CURRENT=inquiry-lifecycle-signing-key:2',
    )
    ...
    expect(deployWorkflow).toContain('DB_INQUIRY_PASSWORD=amjis-inquiry-db-password:2')
    ...
    expect(deployWorkflow).toContain('--remove-secrets=WATCHDOG_SECRET')
    expect(deployWorkflow).toContain('--remove-env-vars="^|^WATCHDOG_LEGACY_FALLBACK_ENABLED')
    expect(deployWorkflow).not.toContain('WATCHDOG_LEGACY_FALLBACK_ENABLED=')
    expect(deployWorkflow).not.toContain('WATCHDOG_SECRET=watchdog-secret:1')
    expect(deployWorkflow).not.toContain('Transitional bridge only')

/**
 * resonance_register.test.ts — Unit tests for the resonance_register MCP tool wrapper.
 *
 * Verifies:
...
import { registerResonanceRegister } from './resonance_register.js'
  ...
  return {
    ...
    envelope: {
      ...
      result: {
        ...
        latency_ms: 10,
        result_hash: 'sha256:resonance-register-test',
        schema_version: '1.0',
      },
      citations: [],
      plan: null,
...
function buildErrorEnvelope() {
  return {
    ...
    envelope: {
      ok: false,
      trace_id: 'trace-resonance-register-err',
      error: { class: 'internal', message: 'Resonance register unavailable' },
    },
  }
}
 ...
 */
async function callResonanceRegister(
  ...
  registerResonanceRegister(stubServer, () => principal)

  if (!capturedHandler) throw new Error('resonance_register handler was not registered')
...
describe('resonance_register — MCP tool wrapper', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('AC.1 — registers without throwing', () => {
    ...
    expect(() => registerResonanceRegister(stubServer, () => mockPrincipal)).not.toThrow()
    ...
    expect(toolName).toBe('resonance_register')
  })
    ...
    vi.mocked(callPlatformPrimitive).mockResolvedValue(buildSuccessEnvelope([
      {
        ...
        claim_text: 'Career-spiritual bridge pattern',
        domains_bridged: ['career', 'spiritual'],
      },
    ]))

    const result = await callResonanceRegister({})
  ...
  it('AC.6 — min_confidence is forwarded to callPlatformPrimitive', async () => {
    ...
    await callResonanceRegister({ min_confidence: 0.8 })

    const params = vi.mocked(callPlatformPrimitive).mock.calls[0][1] as Record<string, unknown>
    expect(params['min_confidence']).toBe(0.8)
  })
  ...
  it('AC.8 — custom limit is forwarded correctly', async () => {
    ...
    await callResonanceRegister({ limit: 75 })
  ...
  it('AC.9 — Zod rejects min_confidence < 0', async () => {
    const result = await callResonanceRegister({ min_confidence: -0.1 })
  ...
  it('AC.10 — Zod rejects min_confidence > 1', async () => {
    const result = await callResonanceRegister({ min_confidence: 1.1 })
    expect((result as { isZodError?: boolean }).isZodError).toBe(true)
  })

  // AC.11: Zod rejects limit > 200
  it('AC.11 — Zod rejects limit > 200', async () => {
    const result = await callResonanceRegister({ limit: 201 })
    expect((result as { isZodError?: boolean }).isZodError).toBe(true)
  })

  // AC.12: Zod rejects limit < 1
  it('AC.12 — Zod rejects limit < 1', async () => {
    const result = await callResonanceRegister({ limit: 0 })
    expect((result as { isZodError?: boolean }).isZodError).toBe(true)
  })

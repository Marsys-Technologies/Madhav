 ...
 */
// F-25: accept principal so the handler closes over the real caller identity
// instead of using a hardcoded system placeholder (audit finding F-25).
export function registerDashaSandhiCalendar(server: McpServer, principal: Principal): void {
  server.tool(
    ...
    async (args) => {
      // F-25: principal is now closed over from the outer registerDashaSandhiCalendar
      // parameter (real caller identity) — the hardcoded system placeholder is removed.
      try {

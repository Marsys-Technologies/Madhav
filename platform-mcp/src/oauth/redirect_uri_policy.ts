/**
 * platform-mcp/src/oauth/redirect_uri_policy.ts
 *
 ...
 *
 * Deliberate duplicate of `platform/src/lib/mcp/oauth/redirect_uri_policy.ts`
 * — the two packages do not share a module boundary, the same reason this
 * package's oauth/types.ts already duplicates the platform package's oauth
 * types (see that file's own header). Keep both copies in lockstep if this
 * policy ever changes.
 *

...
describe('G12 — REGISTERED_TOOL_COUNT is truthful', () => {
  ...
  it('counted tools match REGISTERED_TOOL_COUNT=57', async () => {
    ...
    // kala_upaya_get + kala_ritual_get, +2); the retired registerQueryDashaPeriodsTool (−1)
    // was removed. Measured = 60.
...
describe('V6 — Invariants: tool names snake_case, no hyphens', () => {
  ...
  it('all D7 registry bridge tool names are snake_case', async () => {
    ...
    // added kala_upaya_get + kala_ritual_get, +2). Was 12 (stale pre-B2 count), then 25
    // (pre-W5-L4 count), then 26 (pre-ṢAḌ-DARŚANA count).
